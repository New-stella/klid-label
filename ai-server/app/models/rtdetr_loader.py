...
from app.models.bytetrack_util import (
    reset_tracker_unavailable_warned as _reset_tracker_unavailable_warned,
)
...
# ByteTrack 헬퍼(_new_bytetrack_tracker / _apply_bytetrack)는 bytetrack_util 공용 모듈로
# 추출되어 RT-DETR / YOLOX 가 공유한다. 위 import 로 모듈 전역 이름을 그대로 노출하므로
# 기존 코드/테스트의 rtdetr_loader._apply_bytetrack 참조·monkeypatch 가 동일하게 동작한다.
...
# ────────────────────────────────────────────────────────────────────
# ByteTrack 헬퍼는 bytetrack_util 공용 모듈로 추출됨
# (_new_bytetrack_tracker / _apply_bytetrack — RT-DETR / YOLOX 공유)
# ────────────────────────────────────────────────────────────────────
...
def reset_rtdetr_trackers() -> None:
    ...
    with _TRACKERS_LOCK:
        _TRACKERS.clear()
    # WARN-once 플래그는 공용 bytetrack_util 모듈이 소유하므로 그쪽 리셋을 호출한다.
    _reset_tracker_unavailable_warned()

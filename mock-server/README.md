...
- **SSRF 표면 (CWE-918):** VLM `verify`/`describe`는 요청자가 지정한 `callback_url`로
  ...
  접수 자체를 400으로 거부하고 outbound를 발사하지 않는다. 그래도 **신뢰된 네트워크에서만 기동**하고
  공개망/운영에 노출하면 안 된다(compose는 `127.0.0.1:9400:9400` 루프백 전용 발행).
- **읽기 경계 (CWE-22/400):** `input_path`도 무인증으로 임의 지정이 가능하므로, 복사 원본은
  `MOCK_INPUT_BASE`(미설정 시 `MOCK_OUTPUT_BASE`의 상위 = 공용 storage 루트) 하위일 때만 복사하고
  그 밖이면 placeholder로 대체한다(임의 파일 노출·GB급 반복 복사에 의한 디스크 고갈 차단).
...
| `MOCK_INPUT_BASE` | (빈값→`MOCK_OUTPUT_BASE`의 상위) | **읽기 허용 루트.** `input_path`가 resolve 후 이 base 하위일 때만 원본을 복사한다. 밖이면 복사하지 않고 placeholder로 대체(임의 파일 노출·디스크 고갈 차단). |

...
- **실제 비식별(마스킹) — 1순위 산출 경로:** 검출 모델이 갖춰져 있으면 목이 **실제로 얼굴·사람·간판을
  ...
  >
  > **가리는 영역은 대상의 실제 크기에 맞춘다:** 사람은 상단 16%(머리), 차량은 **하단 14% × 폭
  > 75%의 넓고 얇은 띠**(번호판 자리)만 가린다. 객체를 통째로 덮으면 라벨링 대상이 사라진다.
  > 여기에 `masking_range` 배율이 곱해진다.
  > ⚠ 차량 띠의 폭이 넓은 이유: **비스듬히 찍힌 차량은 번호판이 박스 중앙에 없다.** 검출 박스가
  > 앞면+옆면을 함께 감싸므로 좁은 중앙 띠는 번호판을 놓친다(실측 확인).
  >

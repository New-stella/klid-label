...
- **실제 비식별(마스킹) — 1순위 산출 경로:** 검출 모델이 갖춰져 있으면 목이 **실제로 얼굴·사람·간판을
  가린 영상**을 산출한다(`app/services/deid_engine.py`). 워터마크는 이제 **2순위 폴백**이다.
  > **켜는 법:** `bash bin/fetch-deid-models.sh` — OpenCV Zoo 에서 모델 3종(약 38MB)을 `models/` 로
  > 받는다. 모델은 **git 에 커밋하지 않으며**, 없으면 아래 워터마크 동작으로 자동 폴백하므로 CI·신규
  > 클론의 기존 동작은 **한 치도 바뀌지 않는다**.
  >
  > | 대상 | 모델 | 라이선스 | 비고 |
  > |---|---|---|---|
  > | 얼굴 | YuNet | MIT | 임계 0.7 — 오검출이 놓침보다 해롭다 |
  > | 간판 | PP-OCRv3 det | Apache-2.0 | |
  > | 사람·차량 | YOLOX | Apache-2.0 | **한 번의 추론으로 둘 다** |
  >
  > **번호판 전용 모델은 쓰지 않는다(실측 근거).** 실 CCTV(도심 교차로 720x480)에서 번호판은
  > 텍스트 검출기로 **0건**, 번호판 전용 LPD-YuNet 으로도 **진짜 번호판 0건**이었다(임계를 낮추면
  > 5건이 나오지만 전부 "단속중" 표지판 오검출). 반면 같은 프레임에서 **차량은 15대**가 정확히
  > 검출됐다. 그래서 번호판은 **차량 하단 영역**을 가려 덮는다.
  >
  > **가리는 영역은 대상의 실제 크기에 맞춘다** — 객체를 통째로 덮으면 라벨링 대상이 사라진다.
  > 아래 비율에 `masking_range` 배율이 곱해진다.
  >
  > | 대상 | 가리는 곳 | 실측 근거 |
  > |---|---|---|
  > | 사람 | 상단 **25%**(머리) | CCTV 부감 각도라 머리가 박스의 약 31% — 교과서값 13% 를 쓰면 얼굴이 남는다 |
  > | 차량 | 하단 **14%** × 폭 **75%** | 비스듬한 차량은 번호판이 박스 중앙에 없어 좁은 띠로는 놓친다 |
  > | 오토바이 | 하단 기준 **40~75%** 구간 | 번호판이 차체 **중간**에 달려 있다(실측 52~62%) — 하단 띠는 도로를 가린다 |
  >
  > **사람(YOLOX) 폴백이 필요한 이유:** 원거리 CCTV 에서 보행자 얼굴은 10px 미만이라 얼굴 검출기가
  > **구조적으로 0건**을 낸다(실측: 640x360 거리 장면에서 얼굴 0건 · 같은 프레임에서 person 12건).
  > 그래서 얼굴을 못 찾아도 사람이 있으면 **상단 머리 영역**을 가린다. 전신을 덮지는 않는다 —
  > 행동 라벨링 대상인 사람 자체가 사라지면 학습데이터로서 가치가 없다.
  >
  > **요청 파라미터를 실제로 반영한다:** `masking_type`(색상0/모자이크2/블러3)과 `masking_range`(영역
  > 배율)가 산출에 그대로 적용된다.
  >
  > **검출 전처리(CLAHE + 언샤프)** 를 검출 입력에만 적용한다 — 산출 영상은 원본 화질 그대로다.
  > 텍스트 검출이 116 → **211건(+82%)** 으로 늘었고 비용은 사실상 0 이다(문헌 권고와 일치).
  >
  > **검출 사이 프레임은 모션 예측으로 메운다.** 낡은 박스를 그대로 쓰면 그 사이 객체가 움직여
  > 마스킹이 벗어난다 — 실측으로 오토바이가 f+4 에 IoU **0.22** 까지 떨어져 번호판이 노출됐다.
  > 검출 주기를 5 → 2 로 줄이고 선형 외삽을 넣어 **0.80** 으로 올렸다(사람 0.90).
  >
  > **기본 마스킹은 모자이크다.** 블러는 딥러닝 deblurring 으로 복원될 수 있다는 연구가 다수라
  > 개인정보 관점에서 기본값으로 부적절하다(`masking_type` 으로 여전히 선택 가능).
  >
  > **검출 주기는 대상별로 다르다:** 얼굴·사람은 5프레임마다, 간판은 60프레임마다 검출한다. 고정
  > 카메라에서 간판은 움직이지 않는데 텍스트 검출이 가장 비싸기 때문이다(실측 640p: 얼굴 237fps ·
  > 텍스트 29fps). 실측 처리량은 실제 거리장면 1080p 에서 **실시간 2.2배**(CPU).
  >
  > **폴백(엔진 → 워터마크 → 복사):** opencv 미설치 / 모델 부재 / ffprobe 실패 / 인코더 파이프 끊김 /
  > 시간 상한(기본 900초) 초과면 **산출물을 버리고** 워터마크로 내려간다. 시간 상한에 걸렸을 때 중간
  > 산출물을 승격시키지 않는 것이 핵심이다 — 잘린 영상은 rc=0 으로 끝나 성공과 구분되지 않는다.
  >
  > **리포트(`retrieve_report`)의 `faceCount`·`lpCount` 는 실측값이다** — 엔진이 돌았으면 실제
  > 검출 수를(`lpCount` 는 번호판을 가린 횟수인 차량 검출 수), 엔진이 돌지 않았으면 기존 mock
  > 카운트를 쓴다. ⚠ 이 값은 **연 프레임 합**이라 같은 사람이 100프레임에 걸쳐 있으면 100 이다.
  >
  > **원거리 얼굴은 검출되지 않는다 — 구조적 한계다.** 720x480 CCTV 에서 보행자는 전체가
  > 20x51px 이라 얼굴이 약 **6px** 이고, 8배로 확대해도 얼굴 특징이 나타나지 않는다(보간은 없는
  > 정보를 만들지 못한다). 해상도를 올려도 같은 거리면 13px 수준이라 성립하지 않는다. 그래서
  > **사람 폴백이 머리 영역을 덮는 것이 이 문제의 답**이며, 카메라 근처를 지나는 사람은 얼굴
  > 검출로 정확히 가려진다.
  >
  > ⚠ **이것은 실납품 비식별이 아니다.** 목적은 개발·QA 가 라벨링/검수 화면에서 **실물 비식별본**을
  > 보게 하는 것이다. 실제 재현율은 실 CCTV 데이터로 측정되지 않았고, 번호판은 전용 모델 없이 텍스트
  > 검출로 대신 잡고 있다.
- **`MOCK 비식별 완료` 워터마크 — 기본 꺼짐(2026-09-05):** 실제 비식별 엔진이 산출물을 만들게 된
  뒤로 이 문구는 필요 없어져 **기본값이 `false`** 다(`MOCK_DEID_WATERMARK_ENABLED=true` 로 켤 수
  있다). 엔진이 성공하면 애초에 워터마크를 굽지 않으므로, 이 설정이 바꾸는 것은 **엔진이 실패했을
  때**의 동작뿐이다 — 꺼져 있으면 원본을 그대로 복사한다.
  > ⚠ 끈 상태에서는 모델이 없는 환경의 산출물이 **원본과 육안으로 구분되지 않는다**. 폴백 경로의
  > 보안·경계 계약(shell 미사용·경로순회 차단·임시파일 격리·길이 검증)은 그대로 유지되며
  > `test_deid_watermark.py` 가 설정을 켜서 계속 검증한다.

  아래는 그 기능을 켰을 때의 동작이다. 원본이 **실제 영상**(확장자 + 컨테이너 시그니처 확인)이면

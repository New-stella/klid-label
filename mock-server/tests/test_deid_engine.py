...
def test_번호판_영역은_실제_번호판_크기에_가깝다() -> None:
    """★회귀 — 구값(폭 60% · 높이 30%)은 실제 번호판의 2~4배라 <b>차 앞부분이 통째로</b>
    뭉개졌다. 실제 번호판은 차폭의 25~35% · 차높이의 8~12% 다. 검출 박스 부정확·차량 각도를
    감안한 여유만 더한다.
    """
    w, h = 100, 60
    _, _, nw, nh = E._expand((0, 0, w, h), 1.0, 500, 500, E.KIND_VEHICLE)
    assert 0.25 <= nw / w <= 0.50, f"번호판 폭 비율이 과하다: {nw / w:.0%}"
    assert 0.08 <= nh / h <= 0.20, f"번호판 높이 비율이 과하다: {nh / h:.0%}"

...
⚠⚠ **②는 「새로 쓴 한국어 문단」에는 무력하다 (2026-09-08 실측)** — 새로 쓴 산문은 음절이 거의 전부
유일해서 **신호가 묻히는 것이 아니라 처음부터 없다**(주석 여덟 줄에 1회 등장 음절 **64종**, 전부 정상).
⇒ **삽입·추가 편집의 주 방어선은 구조다** — ①**앵커 유일 일치(`count == 1`) 확인 후 치환**
(다른 바이트가 변할 수 없음이 **구조적으로 보장**된다) ②**문자 단위 diff 의 `replace`·`delete`
opcode 가 0**(= 순수 삽입). 음절 스캔은 그 뒤의 보조다.
**기존 본문을 고치는 편집**(치환·재작성)에서는 여전히 ①②가 필요하다.
...
### 규칙을 뒤집거나 넓히면 — 낱말이 아니라 「형태」로 훑는다

키워드: logicraft 로지크래프트 아이템 정합 반전 폐기 넓힘 확대 허용 규칙 값역 형식 정규식 grep 전수 스캔 잔재 사본 주석 지시문 되돌리기 정책 확정 사양 스펙

**반전 뒤 grep 을 「바꾼 낱말」로 돌리면 뚫린다** — 같은 규칙을 **그 낱말 없이** 말하는 문장이 남는다.
실사례: 식별자 형식을 넓히며 「하이픈」·정규식으로 전수 스캔했는데 **네 자리가 남았다**
(*"`[a-z0-9]+` 로 **제한**"* · *"제한하기로 **합의**돼 있다"*). 같은 라운드에 **층을 놓친 것이 네 번**이고
**매번 「다 봤다」고 판단한 뒤** 다른 담당이 잡았다.

**함께 훑을 것**: 정규식·값 리터럴 그 자체 · **지시 어휘**(`제한`·`합의`·`~해야 한다`·`금지`) ·
규칙을 **성질로 우회 표현한 서술**. ⚠ **「낱말 0건」은 「없다」가 아니다** — 보고에 **무엇으로 훑었는지**를 적어라.

★**낡은 「~해야 한다」꼴 지시문은 낡은 서술보다 위험하다** — 서술은 *"지금은 다르구나"* 로 넘어가지만
**지시문은 다음 라운드의 작업 지시로 읽혀 확정된 정책을 되돌린다.** 폐기 표기로 남기되 **지시 부분은
현재 판단으로 대체**하라. 그리고 **규칙 값을 주석·시험에 옮겨 적지 말고 단일 진실원을 링크**하라 —
옮겨 적은 사본은 **규칙이 바뀌어도 컴파일도 시험도 신호를 주지 않는다.**

> 상세: `docs/rules/logicraft-integration.md` §2-E-2 · §2-E-3
...
**"0건"을 믿지 마라 — 이 프로젝트에서 네 번 뚫렸다**: ① 확장자 있는 파일명만 검사해 맨 심볼명 21건
통과 ② 최상위 필드만 검사해 `sections[].description`의 다크 잔재 31건 통과 ③ 스캔 타입을 4종만
열거해 나머지 타입이 한 번도 검사 대상이 아니었음 ④ **★2026-08-08 — 층 하나가 통째로 빠졌다**:
페이저 폐기를 ITEM 에 반영하고 "잔재 0건"을 확인했으나 **정적 렌더 미러에 8건 잔존**, 별칭도
`custom_name` 축은 0건인데 **미러에는 생존**. **정적 렌더는 5개 라운드 동안 한 번도 열리지 않았다.**
**수치를 보고할 때마다 그 검사가 무엇을 못 보는지 함께 적는다.**

★★**원인은 라운드 편성이다 — 타입별로 짜면 층이 빠진다.** 하나의 결정은 ITEM 본문·**미러 렌더**·링크·
로컬 키트 스냅샷·위키·테스트케이스·코드 **7개 층**에 흩어져 있는데, `screen_spec`/`ui_component`/
`api_endpoint` 타입 단위 라운드는 그중 일부만 지나간다. **라운드 단위는 "결정", 판정 단위는 "층"**이며
**"정합 완료"는 7개 층을 전부 확인했을 때만** 쓴다(타입 하나 완주 ≠ 정합 완료). 층 표·완료 기준은
프로젝트 `CLAUDE.md` 「LogiCraft 완성도가 최우선」 절이 정본.

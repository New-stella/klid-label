...
### ★시험이 결함을 우회하고 그 우회를 주석으로 적어 두면, 그 결함은 CI 에서 영원히 안 보인다

「알고 있다」는 기록이 「막고 있다」로 오독된다. **시험 파일 안의 주석은 CI 에 아무 신호도 만들지 않는다.**

**실측(2026-09-07)**: `ImportBrowseServiceTest` 의 `root` 필드 javadoc 이
*"표기 그대로 쓰면 안 된다 … 판정기의 표기 기준 검사가 이 창구가 돌려준 실경로를 범위 밖으로 본다 …
(운영에서 허용 루트를 바로가기 뒤에 두면 **같은 일이 실제로 일어난다** — 판정기 소유 축이라 여기서
고치지 않고 보고한다)"* 라고 **결함을 정확히 서술**해 두고 `root = tempRoot.toRealPath()` 로 표본을
펴서 회피했다. 그 뒤 이 결함은 **온프렘 현장에서 400 으로 터질 때까지** 그 파일 41건 전부 초록이었다.

⇒ 판정기·해석기의 결함을 만난 시험이 「내 축이 아니다」로 표본을 정규화할 때마다. 그때는 우회와 함께
**`@Disabled` 든 실패하는 가드든 빨간 흔적**을 남기거나, 최소한 CO·이슈로 올려 **우회 사실이 시험 파일
밖에도** 남게 해야 한다.

### 쓰기 전 스냅샷을 「내가 전사해서」 뜨면 대조가 손상을 가려 준다

손상된 사본을 보내고 **그 사본과 대조**하면 언제나 깨끗하게 나온다. 스냅샷은 **기계가 만든 것**이어야
방어선이 성립한다.

**실측**: `mark_implementation` 의 `subtasks` 는 배열 **전체 교체**인데 `API-221` 은 17건·한글 장문이었다.
MCP 채널이 한글을 `\uXXXX` 로 직렬화하므로 **전사 자체가 손상 경로**다. 그래서 설정의 서버 주소·토큰으로
JSON-RPC `get_item` 을 직접 불러 기준선을 **파일로 뜬 뒤**, 스크립트가 `st[-1]['done']=True` 한 줄만
바꿔 되보냈다. 재조회 대조 결과 `개수 17→17 · done 변경 1건 · 본문 문자 변경 0건`.

⇒ `subtasks`·`modules` 처럼 배열을 통째로 교체하는 모든 쓰기. 특히 원소가 많고 한글 장문일 때.
`patch` 의 add 로 안 되는 경우(**기존 원소의 값을 바꿔야 할 때**)가 정확히 이 상황이다.

### `adr` 타입은 `mark_implementation` 대상이 아니다

`design_refs` 에 ADR 이 실려 와도 IMPREC 를 남길 수 없다 —
`E_NOT_TRACKABLE: ADR-NNN (adr) 은 implementation tracking 대상이 아닙니다`.
그 타입에는 `implementation` 필드 자체가 없다. 추적은 코드의 `@design ADR-*` 태그로만 잇는다.

⇒ Phase 5 에서 `design_refs` 전건을 순회하며 IMPREC 를 남기려 할 때.

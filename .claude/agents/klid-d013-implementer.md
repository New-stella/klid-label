...
### ★★한 판정으로 여러 표를 지울 때 표마다 삭제문을 날리면 앞 문장이 뒤 문장의 근거를 지운다
판정이 「셋 중 하나라도 오래됐다」는 OR 인데 오래된 것이 **하나뿐**이면, 그것을 먼저 지운 순간
나머지 삭제문의 판정이 **거짓**이 되어 그 둘만 살아남는다. 오류가 없어 어떤 시험에도 안 걸린다.
「조건을 최종 실행문에 건다」는 규율이 표가 여럿이 되는 순간 **「문장마다 반복」이 아니라 「단일
문장」**을 요구하게 된다 — 데이터 변경 CTE 는 서로의 효과를 보지 못하고 **같은 스냅샷**을 보므로
판정 1회 + 삭제 N개를 한 문장에 묶으면 구조적으로 닫힌다.
- 근거: `PortalUserWorkRepository.DELETE_EXPIRED_WORK_GROUP`. 순차 3문장과 동치인 변이에서
  「만료 근거가 라벨 하나뿐이어도 셋이 함께 지워진다」가 `expected: 3 but was: 1` 로 RED.
  표별 JPQL DELETE 로 짰다면 **전건 green 인 채 배포**됐을 결함이다.
- 재발: 「한 벌」로 다뤄야 할 데이터가 여러 표에 걸쳐 있고 삭제·마감 판정이 **그 표들의 내용에서
  유도**될 때. 후보: 파생영상 폐기 스윕(지금은 판정 근거가 부모 영상이라 무사하나 **자식 내용으로
  옮기는 순간** 들어간다) · 업로드 자산 삭제의 순서표.

### ★대상만 넓히고 후보 탐색을 그대로 두면 구멍이 안 닫힌다 — 그리고 기존 시험은 전부 초록이다
새 축만 가진 행은 **후보에 잡히지도 않아** 삭제 로직이 아무리 옳아도 도달하지 않는다. 그 누락이
드러나지 않는 이유는 **기존 픽스처가 옛 축을 항상 함께 놓기 때문**이다. 축을 넓힐 때는 픽스처를
**새 축 단독**으로도 놓아야 그 축이 어디서 빠졌는지 드러난다.
- 근거: 후보 스캔을 라벨 전용으로 되돌리는 변이가 4 RED. 그중 2건은 등가성 시험을 저작물 축으로
  **파라미터화한 뒤에야** 걸렸다 — 파라미터화 전에는 같은 변이가 그 시험에서 **0 RED** 였다.
- 재발: 「대상 확대」 지시를 받은 모든 배치·집계(삭제·통지·집계·목록 산정). 특히 후보 조회와 실행이
  분리된 스윕 구조.

### ★새 축이 목록에 합류하면 기존 개수 단언이 조용히 의미를 잃는다
새 축은 원장 행이 있는 한 항상 담기므로 `singleElement()`·`hasSize(n)` 이 전부 깨진다. 그때
**기대값을 늘리는 손쉬운 수정**을 하면 원래 보던 축의 병합이 **더 이상 검증되지 않는다.**
필터로 **원래 축만 떼어** 보게 할 것.
- 근거: 컬럼 축 10건이 유입되며 `Expected size: 1 but was: 11`. 6곳을 `keyValueItems()` 필터로 바꿔
  원래 축만 보게 했고, 컬럼 축은 전용 시험이 따로 붙잡는다.
- 재발: 응답 목록에 새 종류의 원소가 합류하는 모든 변경 — 목록 크기·인덱스(`items[0]`)·JSON path 단언.

### 네이티브 갱신 뒤 같은 트랜잭션에서 재조회하면 1차 캐시의 옛 엔티티가 나온다
Spring Data 의 자동 clear 를 쓰는 자리에서는 안 겪지만, `createNativeQuery(...).executeUpdate()` 를
손으로 쓴 리포지토리에서는 **아무 신호 없이 저장 직후 응답이 옛 값**이 된다.
- 근거: 캐시 비우기를 제거하는 변이에서 `expected "눈" but was null`. 같은 리포지토리의 기존
  upsert 들도 캐시를 안 비우나 응답을 재조회로 만들어 지금까지 드러나지 않았다.
- 재발: 손으로 쓴 네이티브 갱신 + 같은 트랜잭션 내 엔티티 재조회. 특히 「저장 후 값」을 응답으로 주는 창구.

### 컬럼 선택을 바인딩+CASE 로 옮기면 대상 표가 어긋나도 0칸 고치고 성공처럼 보인다
SQL 조립을 피하려 `CASE WHEN :field = '...'` 로 바꾸면, 대상 표가 **다른 칸을 넘겼을 때 어느 가지에도
걸리지 않아 0칸을 고치는데 rowcount 는 1** 이 된다. 호출 전에 대상 표를 Java 에서 단언해야 한다.
- 근거: `updateOwnedVideoColumn`/`updateOwnedFrameColumn` — enum switch 로 대상 표를 먼저 가르고
  「지목한 칸 하나만 고친다」가 다른 칸 보존을 함께 고정.
- 재발: 컬럼 선택을 바인딩 파라미터로 옮기는 모든 자리(정렬 키 허용목록 매핑과 같은 축).

### 여태 버리던 반환값을 판정에 쓰기 시작하면 mock 기본값 0 이 정상 경로 시험 전량을 거부로 바꾼다
새 검사와 **무관한 시험이 무더기로** 깨져 원인이 흐려진다. 정상 경로의 기본 스텁을 **먼저 깔고**,
0 을 겨누는 시험만 개별로 덮어쓰는 순서여야 한다.
- 근거: 원장 저장 창구 넷의 행 수를 판정에 쓰기 시작하며 setUp 에 `thenReturn(1)` 기본 스텁을
  선반영. 안 하면 기존 업로드 경로 시험이 전부 FORBIDDEN 으로 죽는다.
- 재발: 행 수·건수·성공 플래그 등 **버리던 반환값**을 판정에 쓰는 모든 변경.

### 실 DB 로 결함 경로를 겨눌 때 「전제 단언」이 먼저 깨지면 결함 부재가 아니라 픽스처 미도달이다
「값이 없으면 파생」 규약을 겨누려면 그 칸이 **적재 시점에 비어 있어야** 하는데, 적재 경로가 그것을
채우고 있으면 겨눈 단언에 닿기 전에 전제에서 실패한다.
- 근거: 개인정보 축 IT 1차 RED 가 `[적재 직후에는 비어 있다] expected: null but was "Y"` — 겨눈
  단언이 아니라 **전제 단언**에서 실패했다. 원인은 적재 생성자가 기본값을 채우는 것.
- 재발: 파생·폴백 경로를 실 DB 로 겨누는 모든 시험. 적재 경로가 그 칸을 채우는지 먼저 확인할 것.

### 정렬 폴백은 대상 행이 목록 **한가운데**에 앉아야 검증된다 — 끝에 두면 변이가 안 잡힌다
`COALESCE`·`NULLS LAST`·기본값 폴백이 들어간 정렬을 검증할 때, 폴백 대상 행이 목록의 **최대/최소
자리**에 있으면 **폴백을 지워도 순서가 같아** 변이가 통과한다. 픽스처가 우연히 두 구현에 같은 답을
주는 것이다.
- 근거: `PortalUserWorkListRepository` 의 `COALESCE(last_saved_at, activity_at)` 를
  `last_saved_at DESC NULLS LAST` 로 바꾸는 변이가 **1차에 0 RED**. 저작물 0건 업로드 행의 등록일을
  가장 오래된 자리 → 가운데로 옮기자 같은 변이가 정확히 1 RED.
- ⚠ 부수 사실: PostgreSQL `ORDER BY … DESC` 기본은 **`NULLS FIRST`** 다(맨 앞으로 온다).
  「NULLS LAST 로 두면 뒤로 밀린다」는 서술은 기전이 반대이니 주석에 쓰지 말 것.
- 재발: 정렬·집계에 폴백이 들어간 모든 검증.

### 변이 결과를 `head`/`tail` 로 자르면 겨눈 시험이 잘려 「검출 실패」로 오판한다
변이가 여러 건을 죽일 때 목록 끝만 보면 **정작 겨눈 시험이 잘려** 「이 변이는 안 잡힌다」로 결론낸다.
- 근거: 데이터마트 축을 카탈로그화하는 변이를 `tail -10` 으로 읽어 겨눈 시험이 없다고 판단했으나,
  XML 전수를 다시 세니 13 RED 안에 그 시험이 있었다.
- 재발: 변이 실험·회귀 비교에서 실패 목록을 자를 때. **정렬해 전수로 출력**할 것.

### 질의 별칭과 DTO 필드명이 갈리면 **틀린 쪽이 계약면**이 되고 어떤 시험도 못 잡는다
값이 맞으므로 전건 초록인데, **이름이 잘못 유도하는 판정**은 정반대 결과를 낸다. 폴백이 들어간 필드가
특히 위험하다 — 이름이 **폴백 이전 상태만** 서술하기 쉽다.
- 근거: 합집합 질의 별칭은 `entry_src_sn`, 행 레코드도 `entrySrcSn` 인데 응답 DTO 만
  `lastSavedSrcSn` 이었다. 실제 값은 ①마지막 저장 프레임 ②없으면 첫 프레임 ③프레임 0건이면 null 이라,
  그 이름대로 「저장 이력이 있는가」를 판정하면 **저작물 0건 행이 「작업한 행」으로 뒤집힌다.**
- 재발: 질의 별칭 → 행 레코드 → 응답 DTO 로 이름이 **세 번 옮겨 적히는** 모든 자리.
  ★**소비처가 붙기 전이 유일하게 싼 시점**이라 개명 판단을 미루지 말 것.
- ⚠ 개명만으로는 오용이 안 막힌다 — **세 갈래 뜻과 금지를 사양·주석·시험명 세 곳에** 함께 박을 것.

### ★★「적재 직후에는 비어 있다」를 인접 코드로 추론하면 틀린다 — 덤프로 확인하라
생성자·빌더·DDL 기본값을 **셋 다 읽고** 「전부 null」이라 결론냈는데 실제 덤프는 채워져 있었다.
값의 출처가 읽은 세 곳 어디도 아니었다.
- 근거: 신규 포털 업로드 행 덤프 → 개인정보 3칸이 `Y/N/N` 으로 **이미 값**(변경 시각은 null).
  그 셋을 존재 판정에 넣은 1차 구현이 인버전 가드 2건에 걸렸고, 되돌리는 변이가 그것을 재현한다.
- 재발: **「값이 있다 = 사람이 고쳤다」류 존재 판정**을 세우는 모든 자리 — 최초 입력 감지,
  「작성한 적 있는가」 판정. ⚠ **같은 표의 다른 축은 비어 있을 수 있다**(프레임 축이 그랬다) —
  한쪽만 보고 일반화하면 반대 축에서 뒤집힌다.

### 사양이 「A·B·C 를 센다」고 해도 **C 를 셀 수단이 있는지는 별개**다
수단이 없는데 억지로 세면 **반대 방향으로 무너진다.** 그때 필요한 것은 조용한 근사치가 아니라
**공백을 명시적으로 보고하는 것**이다 — 근거 없이 「제외」만 적으면 다음 사람이 다시 넣는다.
- 근거: 영상 축 개인정보 판정 — 자기 변경 시각이 없고(원장 행 시각은 자동 갱신과 공유) 값은 기본값이
  채워져 있어 **어느 신호로도 사람의 편집을 가릴 수 없다.** 사양이 그 공백을 명시 수용했다.
- 판정법: 축마다 **①자기 변경 시각이 있는가 ②기본값이 있는가** 두 질문이면 셀 수 있는 축과 없는
  축이 바로 갈린다.

### ★★위임으로 판정을 한 곳에 모으면 **호출부의 결과 해석이 새 취약점이 된다**
단건이 일괄에 위임할 때 `!isEmpty()` 로 받으면 판정 리터럴은 하나인데 **안전성이 「조회가 요청한
것만 돌려준다」는 계약으로 이전**된다. 그 계약이 깨지는 변이에서 **목록 경로만 죽고 단건 인가 축은
조용히 통과**한다 — 인가가 걸린 자리라 치명적이다.
- 근거: `PortalWorkTargetResolver` — 일괄 창구가 요청과 다른 식별자를 섞는 변이에서,
  `!isEmpty()` 였을 때 단건 403 시험이 **전부 통과**했다. `contains(요청한 식별자)` 로 바꾸자 RED.
- 재발: 단건 판정을 일괄 창구에 위임하는 모든 리팩터. **특히 인가·노출 판정.**
  「합쳤으니 안전하다」가 아니라 **호출부가 무엇을 근거로 참을 만드는지**를 다시 볼 것.

### ★★부정 조건의 **갈래가 여럿**인데 픽스처가 하나면 판정 리터럴이 한 번도 실행되지 않는다
그러면 그 리터럴을 무력화해도 통합시험이 죽지 않고, 단위 시험은 판정기를 **모의**하므로 덮어 주지
못한다 — **두 층 모두 초록인데 아무도 그 판정을 지키지 않는 상태**가 된다.
- 근거: 「데이터마트에 노출되지 않는다」의 두 갈래(**상태 행 부재** / **행은 있으나 승인 아님**) 중
  앞엣것만 픽스처가 있었다. 승인 판정 리터럴 항진식 변이가 그 상태에선 통합시험을 못 죽였고,
  뒤엣 픽스처를 세운 뒤 2건이 RED.
- 재발: 부정 조건(없음·아님·불허)을 검증하는 모든 픽스처. **「어떻게 거짓이 되는가」를 열거해
  갈래마다 픽스처를 세울 것** — 「행 부재」와 「값 불일치」는 **다른 갈래**다.

### 인자를 무시하는 mock 스텁은 「**무엇을** 물었는가」를 검증하지 못한다
`when(x.f(any())).thenReturn(고정값)` 은 판정기가 **엉뚱한 식별자로 물어도** 통과시킨다.
- 근거: 판정기가 `rawSn + 1` 로 묻는 변이가 느슨한 스텁에서 **0 RED**, 요청 집합을 보는
  `thenAnswer` 로 조인 뒤 정확히 1 RED.
- 재발: 조회 창구를 mock 하는 모든 단위 시험. **식별자로 거르는 것 자체가 안전 장치**인 자리
  (소유자 스코프·노출 판정·격리 키)에서는 `any()` 고정 반환이 곧 **가드 무력화**다.

### 단건을 일괄에 위임하면 **스텁하던 시험이 깨지는데 그것이 신호**다
깨진 시험이 **판정 내용이 아니라 조회 수단**을 스텁하고 있었다는 뜻이다. 여기서 「원래대로 되돌리자」로
가면 리터럴이 다시 둘이 된다.
- 근거: 단건이 일괄에 위임하도록 합친 뒤 전체 회귀에서 판정기 시험 **딱 1건**만 RED(`findById` 스텁
  미적중). 스텁을 `findAllById` 로 옮겨 해소했고 **단언은 한 줄도 바뀌지 않았다.**
- 재발: 단건/일괄 판정을 합치는 모든 리팩터. ⚠ **스코프 실행만으로 끝내면 이 1건이 안 보인다** —
  전체 회귀를 돌릴 것.

### 목록 질의가 계산한 값이 **다른 기능의 판정 입력과 같은 사실**이면 통합할 수 없어도 고정은 해야 한다
정렬 키는 합집합 질의 안에 있어야 하고 보존기간 입력은 기존 집계가 소유하는데 둘이 같은 값인 경우가
생긴다. 통합이 불가능하면 **동치성 시험이 유일한 접착제**이며, 서비스가 두 값을 각각 조회하지 않고
**하나를 흘려보내야** 같은 응답 안에서 갈리는 것만은 구조적으로 막힌다.

...
### 2026-09-08 · AI 장비 상태점검·폴백 라운드 (`CO-20260908`)

- **★★관측하는 축과 고르는 축이 엇갈릴 수 있다 — 「이 상태의 귀결」을 한 문장으로 쓰지 마라.**
  상태점검 폴러는 **추론(`INFERENCE`)만** 관측하고(`AiSrvrHealthPoller.pollAll` 의 `.filter`),
  원장에서 장비를 **고르는 것은 시계열뿐**이다(`AiSrvrSelector` 운영 호출자는 `VlmTimeseriesStep` 하나).
  추론 위탁은 `AiServerClient` → `aiServerWebClient` → `IntegrationEndpointResolver` →
  `SystemConfigService`(**단일 설정값**) 경로라 **원장을 어느 지점에서도 읽지 않는다.**
  그래서 *"후보가 0이면 위탁은 폴백 없이 거부된다"* 는 참인 문장이 **상태점검이 실제로 관측하는
  축에서는 거짓**이 된다 — 운영자 대면 경고가 「막혔다」고 말하는데 실제로는 계속 돈다.
  · **근거**: `AiSrvrHealthPoller:109` · `AiServerClient` 의 원장 참조 0건 ·
    `AiServerHealthIndicator` javadoc 이 같은 비대칭을 **이미 확정 서술로 갖고 있었다**(그것을 참조하지
    않고 새 문장을 지은 것이 실수였다).
  · **재발 조건**: 장비·노드·연동처럼 **유형이 있는 축**에 상태 문구·헬스 사유·알림을 새로 쓸 때.
    쓰기 전에 **「이 유형을 관측하는 주체」와 「이 유형을 실제로 고르는 주체」가 같은가**를 코드로
    확인하고, 다르면 **문구를 분기**한다.

- **★관측·표시의 정확성이 동기인 작업에서는 로그·주석의 부정확이 코드 결함과 같은 등급이다.**
  런타임이 옳아도 사람이 읽는 문장이 틀리면 **고치려던 증상(화면이 거짓을 말한다)을 로그 축에서
  재생산**한다. 실제로 이 라운드의 독립 QA 지적 3건이 **전부 주석·로그 드리프트**였고 런타임 결함은 0이었다.

- **★정책을 반전시킨 뒤에는 그 정책을 서술한 주석을 「문구 단위」로 전수 grep 한다.**
  파일 단위로 훑으면 놓친다. 이번에 `VlmTimeseriesStep:561~568` 이 **이 CO 가 걷어낸 사유 한정 게이트
  그 자체**(*"식별자 형식을 어겨 전부 빠진 경우는 거부한다"*)를 현재 계약처럼 적고 있었고,
  그대로 두면 **다음 라운드의 설계 근거로 역추정돼 폐기된 정책이 되살아날** 형태였다.
  · 낡은 주석은 **지우지 말고** `⚠구 서술 폐기(날짜)` + 폐기 사유로 표기한다(이 저장소의 ADR 관례와 동일).
    지우면 왜 한때 그렇게 적혔는지가 사라지고, 그냥 두면 현재 계약으로 읽힌다.

- **★시험 이름이 결함을 정확히 가리켜도 픽스처가 눈가리개가 될 수 있다.**
  `AiSrvrTypeScopedGuardIT.★상태점검의_자동_이용불가_전이는_유형별_보호에_걸리지_않는다` 는
  이름이 이번 결함 그 자체인데 **한 건도 잡지 못했다** — 추론 1 + 시계열 1 을 심어 **전체 축**
  가용 수가 2라 구 쿼리도 통과했기 때문이다. 유형별 보호를 재려고 유형을 갈라 심은 것이
  전체 축 회귀를 가렸다.
  · **재발 조건**: 「보호가 걸리지 않는다」류 negative 시험을 쓸 때. **어느 축의 카운트가 판정에
    쓰이는지까지 맞춰** 픽스처를 세우지 않으면 통과가 무의미하다.

- **테스트 프로파일의 `src/test/resources/application-local.yml` 이 main 동명 파일을 통째로 대체한다.**
  main-local 에만 있던 `vlm.client.url`(mock 주소)이 테스트에서는 **사라져** 시계열 장비 씨앗이
  **한 건도 심기지 않는다**. *"local 프로파일이니 mock 주소가 있겠지"* 로 추론하면 틀린다.
  · **근거**: `main/application-local.yml:136` 에 있고 `test/.../application-local.yml` 에 0건 ·
    `main/application.yml:1094` 기본값은 빈 문자열.
  · **재발 조건**: 「원장이 비면 폴백」을 「원장이 비면 거부」로 바꾸는 작업, 부트스트랩 씨앗 유무를
    전제하는 IT 를 새로 쓸 때.

- **`AiSrvrRegistry` 는 TTL 5초 스냅샷 캐시다.** 시험에서 원장에 직접 INSERT 한 뒤 곧바로 선택기를
  부르면 **방금 심은 노드가 보이지 않는다.** 관리 창구가 하는 것처럼 `invalidate()` 를 함께 부른다.

- **한 클래스에 `@AfterEach` 를 여럿 두면 JUnit 5 실행 순서가 보장되지 않는다** — 「비우기」와
  「되심기」가 뒤집힐 수 있다. 되돌림은 `@AfterAll static void m(@Autowired Bean b)`(SpringExtension
  파라미터 주입)로 두거나 기존 메서드 본문을 고쳐 **순서를 코드로 못박는다**.

> ⚠ 위 항목들은 전부 **실측 근거**를 가진 것이며 추정이 아니다. 근거 심볼·라인이 옮겨가면 그 사실을
> `notes_for_main` 으로 올려 정정을 요청할 것(라인 이동만으로는 정정 대상이 아니다).

> ⚠️ **이 섹션을 에이전트가 직접 고치지 않는다.** 새로 알아낸 건 아래 `notes_for_main.learned` 로 올리고, 오케스트레이터가 사용자 동의를 받아 여기에 append 한다.

### AI 장비 이중화 라운드에서 얻은 것 (2026-09-08 · CO-20260908-AI장비-이중화)

- **★「A 를 쓸 수 있는가」류 술어를 하나로 쓰면 정반대 방향의 요구가 같은 자리에서 충돌한다.**
  정비중(`DRAINING`)이 그 예다 — **「신규 배정을 받는가」에서는 거짓**이어야 하고
  **「이미 붙은 배정을 유지하는가」에서는 참**이어야 한다. 어느 한쪽으로 통일하면 **반드시 반대쪽이 깨진다.**
  선택기가 `candidates()`(=가용만)를 고정 판정에 공유해 **정비중 장비의 고정이 즉시 풀렸다**(독립 QA 가 `fail`).
  해법은 **모집단만 가르고 나머지 판정은 공유**하는 것. ⚠ **회귀 가드는 반드시 대칭 두 건** —
  한쪽만 두면 다음 사람이 가용 목록을 넓혀 반대쪽을 깬다.
  재발 조건: 상태 열거형에 「배제」 성격의 값을 더하거나 기존 「가용 목록」을 새 용도에 재사용할 때.

- **★「응답이 닿았는가」를 예외 원인 사슬을 *원인부터* 훑어 판정하면 본문 해석 실패가 전송 실패로
  오분류된다** — 해석 실패의 원인이 **입출력 예외의 하위형**이기 때문이다. **각 단계에서 도달 표식을
  먼저** 봐야 한다. `block()` 이 검사 예외를 한 겹 감싸 사슬 순회가 불가피한데, 그때 **순서를 정하지
  않으면 조용히 뒤집힌다.**
  재발 조건: 외부 연동에서 「닿았다 ↔ 못 닿았다」를 예외로 가르는 판정을 새로 쓸 때.

- **★★전체를 훑던 배치를 축·계통별로 쪼갤 때, 그 배치가 함께 하던 「원장에 없는 것 정리」류 작업의
  판정 모집단까지 같이 좁아진다.** 그러면 **이번 틱이 보지 않는 대상이 「없어졌다」로 판정돼 매 틱 지워진다** —
  오류가 아니라 **조용한 상태 초기화**라 로그에도 안 남는다.
  실측: 부하 평활 표본 정리를 계통으로 좁혔으면 시계열 틱이 **추론의 평활 창을 전부 지워
  「바쁜 장비가 가장 한가한 장비로 보여 요청을 빨아들이는」** 상태가 주기마다 재현됐다.
  ⇒ **「이 틱의 대상」과 「정리 판정의 모집단」을 다른 변수로 분리하라.** 같은 리스트를 재사용하는 순간 결합이 생긴다.

- **★영속 스케줄러(Quartz JDBC JobStore)의 잡·트리거 식별자를 개명하면 기존 등록 행이 지워지지 않고
  고아로 남아 계속 발화한다.** `overwriteExistingJobs(true)` 는 **같은 식별자만** 대체할 뿐
  **정의에서 사라진 식별자의 행을 삭제하지 않는다.** ⇒ 관측이 두 배, 외부 호출도 두 배.
  **대칭을 위해 이름을 맞추고 싶어지는 자리가 정확히 그 함정**이다 — **기존 축은 이름을 유지하고
  새 축만 새 이름**을 갖게 하라. **코드만 보면 대칭이 옳아 보이고, 깨지는 것은 DB 에 남은 과거다.**
  ⚠ 되돌림 배포에서도 같은 일이 난다 — 새 판이 등록한 행이 구 판에서 계속 발화한다. **운영 절차에 넣을 것.**

- **★`javadoc` 이 「이렇게 드러낸다」고 선언한 관측 지점이 그 문단이 예시로 든 시나리오에서 비어 있을
  수 있다.** 분기가 둘인데 관측을 **나중에 추가된 분기에만** 달면, 문장은 둘을 함께 설명하므로 읽는
  사람은 둘 다 관측된다고 믿는다. **코드 결함보다 발견이 늦다** — 런타임이 옳고(폴백은 정상) 로그만 없다.
  ⇒ **관측을 분기마다 쓰지 말고 한 자리로 뽑아 모든 분기가 그것을 부르게** 하라.

- **★「X 를 주지 않으면 Y 로 폴백한다」가 두 계층에 있으면, 아래 계층 시험이 초록이어도 위 계층은
  검증되지 않는다.** 아래는 **인자를 받았을 때**를 재고 위는 **그 인자를 어떻게 만드는가**를 잰다 —
  위가 값을 잘못 읽어도 아래 시험은 전부 통과한다.
  재발 조건: 설정·잡 데이터·헤더처럼 **외부 값을 도메인 인자로 변환하는 얇은 계층**을 더할 때.
  그 계층은 로직이 적어 「시험할 게 없다」고 느껴지는데 **정작 조용히 틀리는 곳이 거기다.**

- **★오버로드를 더해 호출부를 옮기면 기존 Mockito 스텁이 조용히 빗나가 `null` 을 돌려준다** —
  컴파일은 통과하고 런타임에 **「외부 호출 실패」로 위장**된다(원인이 메시지에 전혀 안 드러난다).
  실측 84건이 그렇게 떨어졌다. ⇒ 옮긴 메서드명을 **시험 트리 전수 grep** 해 스텁·verify 를 함께 옮겨라.
  ⚠ 새 인자에는 `anyString()` 이 아니라 **`any()`** 를 써라 — 배선되지 않은 시험은 그 인자가 `null` 이고
  `anyString()` 은 `null` 을 매칭하지 않는다.

- **협력자를 새로 주입할 때 반환을 `Optional` 로 두면 목이 `null` 을 돌려줘 모든 기존 시험에 스텁을
  강제한다.** 같은 뜻을 **nullable 반환**으로 두면 목 기본값(`null`)이 곧 「고르지 못함」이 되어
  **기존 시험을 한 줄도 고치지 않고** 종전 동작이 보존된다(실측: 스텁 추가 0건).
  ⚠ 다만 **공개 계약(인터페이스)** 에서는 `Optional` 이 세 갈래를 문서화하기에 낫다 —
  **내부 협력자와 공개 계약에서 다른 선택을 하는 것이 모순이 아님**을 주석에 못박아라(통일하려는 리뷰가 온다).

- **Spring Data JPA 저장소에 같은 이름의 `default` 오버로드를 얹어 인자를 채워 주면, 그 저장소를
  목킹하는 단위 시험에서 `default` 본문이 실행되지 않아** 채워 넣은 값이 시험 밖으로 빠진다.
  「호출부가 문구를 짓지 않게 한다」는 의도가 **검증되지 않은 채** 남는다.
  ⇒ 오버로드를 없애고 **호출부가 값을 직접 넘기게** 하면 `ArgumentCaptor` 로 물 수 있다.

- **`@Modifying` 네이티브 쿼리는 트랜잭션 밖에서 부르면 flush 단계에서 터진다.**
  같은 저장소의 `saveAndFlush` 는 자체 트랜잭션을 열어 통과하므로 **「저장은 되는데 상태 전이만 안 된다」**
  로 보여 원인을 엉뚱한 데서 찾게 된다. ⇒ 조건부 UPDATE(원자 클레임) IT 는 `TransactionTemplate` 으로 감싸라.

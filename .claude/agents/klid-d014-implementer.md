...
### 연동 주소 키를 더하거나 옮길 때 (2026-09-08 · CO-20260908-AI장비-이중화)

- **★★새 연동 주소 키는 세 집합에 함께 들어가야 하는데 서로를 참조하지 않는다.**
  **허용 키 목록**(없으면 저장이 잘못된 입력으로 거부) · **선언 타입 맵**(없으면 시드 행이 없어 **최초
  저장이 「찾을 수 없음」**) · **연동 대상 열거**(없으면 ①주소 형식 검증을 안 타고
  ★②**관리자 유효창을 요구하지 않는다**).
  ⚠ **하나만 채워도 컴파일·기동이 전부 통과**하고, 실패는 **런타임에 자리마다 다른 코드로** 드러나
  원인이 흐려진다.
  ★**셋째가 빠지면 기능이 아니라 인가가 조용히 약해진다** — 변이 실증에서 **그 자리만 유효창 시험
  둘을 죽였다.** ⇒ **세 자리를 한꺼번에 무는 시험을 함께 두어라.**

- **★연동 대상 열거는 「화면 칸 목록」이 아니다.** 그것은 **「주소 형식 검증 + 유효창 요구」 판정
  대상**이고, **화면에서 뺀 키도 씨앗으로 계속 저장될 수 있으므로 열거에는 남는 것이 맞다.**
  ⚠ **두 집합이 더 이상 같지 않다는 사실을 명시하라** — 안 적으면 다음 사람이 **「일관성」을 이유로
  맞추려 든다.**

- **★「행이 없음」과 「빈 값 저장」은 다르다.** 「비어 있음이 정상」인 연동에서도 **빈 문자열 저장은
  다른 연동과 똑같이 거부**된다 — 「비어 있음」은 **행이 없는 상태**로 표현된다.
  ⇒ 지시가 *"빈 값을 막지 마라"* 로 와도 **설계 문면을 먼저 열어 어느 쪽인지 확인**하고,
  다르면 **구현하지 말고 보고**하라(설계 변경이 선행돼야 한다).

- ⚠ **선언 타입 맵의 자료구조에 엔트리 상한이 있다.** 넘치면 **컴파일 오류**라 조용히 새지는 않지만,
  그때 **「등록 생략」으로 도망가면 그 키는 행 부재 시 「찾을 수 없음」이 되어 한 번도 저장할 수 없는
  키가 된다.** ⇒ **자료구조를 바꿔라.** 같은 파일에 그 선례가 있다.

- **★저장 창구 시험은 「저장된다」까지만 본다** — **「그 값으로 호출이 나가는가」는 안 본다.**
  실측: 저장 배선을 마쳤는데 **주소 재작성 필터가 그 클라이언트에만 안 걸려 있어 실동작이 0건**이었고,

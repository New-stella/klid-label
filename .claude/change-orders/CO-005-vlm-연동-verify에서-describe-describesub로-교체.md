...
- **SD-002 · SD-005** — 고충실 시안 렌더에 일치도 요소가 **실물로 그려져 있다**(SCREEN-005 는 `accuracy-row/track/fill width:87%`, SCREEN-019 는 `rv-accuracy-label`). ⚠ SCREEN-005 는 로컬 원본 `design.html` 이 **없고 미러(`design-main.html`)만** 있다. ⚠ **키트 SYNC 가 로컬 편집을 덮으므로 재게시를 SYNC 보다 먼저** 해야 한다.
  - SD-005 에는 이번 축과 **별개의 어긋남**도 있다 — 시안이 *"서술 전문은 수정할 수 있고"* 라고 적는데 SCREEN-019 사양은 이 화면을 **읽기 전용**으로 규정한다(수정은 라벨링 화면 담당). 함께 정정할 것.
- **CMP-002** — 「정정」이 아니라 「신규 표현」 결정(위탁이 애초에 안 그려져 있음). 미확정.
- **신규 ADR** — "연동 대상 = describe(CoT)+describe-sub(VQA), verify 미연동" 결정 기록. ★전수 스윕 완료: 이 결정을 소유한 ADR 이 **없음**을 확인했다(CLAUDE.md 구속 절로만 존재) → 신설이 두 번째 진실원을 만들지 않는다.
- **저확신 확인 대상**: AC-027 · SCREEN-006 · SEQ-014 · SD-012 · NAV-001 · UC-021 · DOMAIN-005/010 본문
...
**미반영·보류 항목**: §4 미확정 ①(event_annotation 자동채움)·②(smoke)·③(selected_frames 상한·uniform) — Phase 3 게이트 확정 대상.

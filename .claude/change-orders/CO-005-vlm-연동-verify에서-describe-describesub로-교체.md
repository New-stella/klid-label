...
## §0 재개 지점 (2026-08-24 일시 중단)

> 이 절은 **작업을 멈춘 시점의 상태**다. 재개하면 여기부터 읽는다. 완료되면 지운다.

### 지금 어디까지 왔나

- **커밋된 것**: `cfb3d899` — 이중 위탁 본체(공유기반 + 어노테이션 초안 + mock-server + 문서).
- **미커밋(워크트리에만 있음)**: 그 뒤의 **QA 수정 9건**과 **화면 일치도 제거**. 아래 두 절 참조.

### 미커밋 변경의 내용

1. **QA 라운드 1 지적 9건 처리** — 상세는 아래 「QA 라운드 1」 절.
   - HIGH: 콜백 경로가 창구를 안 봐서 추가 질문 실패가 마킹을 종결시키던 결함(회귀 가드 3건 + mutation 실증).
   - MEDIUM 5: `API-065` error 문자열화 · `AC-028` 폴백 · 아키텍처 가드 목록화 · 상태 관측 논블로킹화 · 429 전용 예외.
   - LOW 3: 어노테이션 검토 종결 게이트 · 잔재 4건 · 커버리지 4축.
   - QA 가 하나만 잡은 문구 드리프트를 **8개 ITEM 전수**로 넓혀 정정.
2. **화면 일치도 제거**(사용자 확정) — 아래 「✅확정 — 화면에서 일치도를 뺀다」 절.

### 재개 시 첫 할 일

1. **backend 전체 회귀 결과 확인** — `/tmp/klid-fullregression.log` 를 본다.
   `grep -aE 'tests completed|BUILD' /tmp/klid-fullregression.log`
   - 로그가 없거나 중간에 끊겼으면 다시 돌린다. ⚠ **`nohup` 으로 띄울 것** — 그냥 백그라운드로
     띄우면 셸이 끊길 때 함께 죽고 **테스트 워커가 고아로 남는다**(실제로 두 번 그랬다).
     고아는 `pgrep -f 'api-ex/backend/build/tmp/test/work'` 로 확인해 **내 워크트리 경로로 좁혀** 정리한다.
   - ⚠ **다른 워크트리(`anotation-2`)에서 다른 세션이 gradle 을 돌리고 있었다.** 결과 디렉터리는
     분리돼 섞이지 않지만 데몬·자원을 공유하므로 타임아웃성 실패가 늘 수 있다 —
     그런 실패가 나오면 내 변경 탓으로 단정하지 말고 그 사실과 함께 보고할 것.
2. **회귀가 그린이면 커밋** 여부를 사용자에게 확인한다(아직 커밋하지 않았다).
3. 그 다음은 아래 「남은 작업」.
...
| 축 | 결과 |
|---|---|
| frontend 전체 | ✅ 463파일 / **3,710 통과**(포맷터 시험 9건 폐기로 3,719→3,710) |
| mock-server 전체 | ✅ **371 통과 / 8 스킵** |
| mutation 실증 4종 | ✅ 아키텍처 가드 · 콜백 채널 판정 · 화면 미표시(검수) · 429 분류 |

### ⚠ 알려진 기존 플레이크 (내 변경과 무관, 증거 있음)

`transfer/ImportControllerIT.가져온_프레임이_열리고_승인하면_학습데이터_산출이_성공으로_마감된다`
— **격리 실행 3회 중 1회 실패**한다. 승인이 발동한 비동기 산출과 시험의 동기 산출이 각각 원장 행을
만드는데 시험이 `output_sn DESC LIMIT 1` 로 마지막 행만 읽는 경합이다. **이 CO 범위 밖**이라 손대지 않았다.

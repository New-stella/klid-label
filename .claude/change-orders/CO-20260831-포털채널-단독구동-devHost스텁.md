...
### ★246 런타임 E2E — 미검증으로 남아 있던 축을 닫았다 (2026-08-31, 브라우저 실동작)

머지(`c2b30091`) → 246 배포 → **Playwright 로 실제로 밟았다.** 1~4라운드와 두 차례 독립 QA 가 모두 「런타임 미검증」으로 남겼던 바로 그 축이다.

| 수용기준 | 결과 |
|---|---|
| AC1 뿌리 진입 보정 | ✅ `http://192.168.102.246:13005/` → **`/workspace/authoring/dev/login`** 자동 보정. 빈 화면 아님 |
| AC2 포털 역할 선택지 1개 | ✅ `PORTAL_USER (3001, 홍길동)` **하나만**, `[checked]`. 관제 3종 부재 |
| — 대역 등록 | ✅ 콘솔 `[devHostStub] Host 대역 인계 창구를 등록했다 — 개발 단독 구…` |
| AC3 **401 아님** | ✅ `POST /api/v1/dev/tokens` **201** · `GET /api/v1/portal/datamart/videos` **200** — 토큰이 실제로 요청에 실렸다 |
| — 포털 홈 도달 | ✅ `/workspace/authoring/portal` 렌더. **머리 영역 없음**(포털 채널 정상 — Host 셸 전제) |
| AC4 새로고침 유지 | ✅ 재진입 시 재로그인 없이 포털 홈 복원. 콘솔 에러 **0** |
| AC5 관제 역할 선택지 3개 | ✅ 13000 `/dev/login` — ADMIN·REVIEWER·WORKER, 기본 REVIEWER `[checked]`. **PORTAL_USER 부재** |
| AC6 관제향 무영향 | ✅ 13000 은 basename 없이 `/dev/login` 그대로 동작 |

콘솔 오류는 **favicon 404 하나뿐**이며 이번 변경과 무관하다(양쪽 채널 공통·기존).

⇒ **이 CO 의 목적(「13005 에서 정상으로 테스트할 수 있게 한다」)이 실동작으로 달성됐다.**

**13005 진입 절차**: `http://192.168.102.246:13005/` → (자동 보정) → 「토큰 발급 + 진입」 클릭 → 포털 홈.

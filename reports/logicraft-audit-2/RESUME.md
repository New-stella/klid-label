...
> ## ★ 2026-08-07 최신 — 라운드 4 **완료. 현재 열린 작업 없음**
>
> `stale` 플래그 해소가 **187 → 0** 으로 끝났다(총 193 호출 · 종료 검증 전부 통과).
> 라운드 순서: `ROUND-인가IA.md` → `ROUND-api.md` → `ROUND-stale.md`(완료).
>
> **다음에 stale 이 다시 쌓이면 `ROUND-stale.md` §2·§5 절차대로 한다.**
> ⚠ 그 문서의 핵심 정정: **무변경 저장(`data={}`)도 하류로 전파된다** — 상류→하류 순서로 닫고,
> 끝난 뒤 재다운로드해 되살아난 것을 한 번 더 닫아야 0 이 된다.
> 남은 백로그는 `PLAN-ui-catalog.md` §8(미결 판정 11건) · `ROUND-인가IA.md` 「남은 것」 참조.
...
> ## ★★★ 2026-08-07 2차 갱신 — 인가·IA 라운드까지 **완료**
>
> **재개 시 `ROUND-인가IA.md` 를 먼저 읽을 것.** 아래 §4-1~4-4 는 대부분 소진됐다.
>
> | 항목 | 결과 |
> |---|---|
> | 4-1 `ui_component` · 4-2 `app_shell` | ✅ 다른 세션에서 완료 |
> | 4-3 `docs/test-cases` | ✅ **검토 결과 변경 대상 없음** — 다시 검토하지 말 것 |
> | 4-4 `permission_role`·`navigation_tree`·`constant` | ✅ 완료 (권한 25/15/4항목, 셸·내비 불일치 0) |
> | 신규 — 라우트 정합 | ✅ 납품 라우터 기준 4건 정정 + 자기모순 1건 해소 |
> | 신규 — 중복 화면 | ✅ `SCREEN-007` → `008` 흡수 후 폐기(`_retired/` 보존) |
> | 신규 — 심볼 오염 | ✅ 본문·`sections[]`·정적렌더 전 범위 **0건** |
> | 신규 — 와이어프레임 배치 | ✅ 나열식 → **실제 배치**, 폐기 27건 그림에서 제외, 31건 재업로드 |
>
> **최종 검증 전항목 통과**: 콘텐츠 감소 0 · 오염 0 · 심볼 0 · 와이어프레임 31건 일치 · 한글 손상 0.
>
> **남은 것 (우선순위)**
> 1. ~~**`api_endpoint` 162건**~~ → ✅ **2026-08-07 완료** — `ROUND-api.md §6·§7` 참조.
>    171건(신설 10 포함) 8축 전부 0건 · 화면 미연결 63→**33**.
>    미결 3건도 같은 날 처리: **`SCREEN-038` 이벤트유형 관리 화면 신설**(NAV·SHELL·ROLE 동시 반영) ·
>    관제 인입 재큐는 **운영 전용 확정**(대상이 영상 적재 전 단계라 목록에 없다) ·
>    잔존 `200` 8건 **`[폐기]` 표기**(숫자 키는 patch 로 못 지운다 — `replace` 는 절단 위험)
> 2. **스키마 축 3건 신설 검토** — `navigation` 좌/상단 구분 · 우측 패널 탭 소속 · `side` 좌/우 구분.
>    필드가 없어 배치가 실제와 어긋나는 지점이 있고, 추측으로 메우지 않고 한계로 남겨 뒀다
> 3. `code_module` 23건 중 22건 stale (⚠ 구 기술 "연결 0건" 은 부정확 — 링크는 살아 있다)
>
> ⚠️ **작업 상태**: 이 시점 기준 **커밋되지 않았다**(수정 29 · 미추적 16). LogiCraft 서버 반영은 완료.
> `.staging*`·`.verify`·`.wf-new` 는 다운로더 산출물(약 30MB)이라 `.gitignore` 에 넣었다 — 언제든 재생성된다.

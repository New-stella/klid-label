...
| 엔티티 설명 | 원본 영상 1건. 자동 파이프라인의 단위. 비식별·VLM 등 모든 처리의 root. V2.0에서 PARENT_RAW_SN 추가(증강 영상의 원본 참조) |
...
| INTERVAL_SEC | 프레임간격(초) | INT | 10 | | | | | | 자동모드 시 |
...
> **참고**
> - 본 산출물의 ERD는 `backend/src/main/resources/db/migration/` Flyway V1~V47 마이그레이션 파일에서 테이블 정의를 추출하여 작성하였다.
> - 저작도구 전용 36개 테이블(LS_*) + 관제서버 재사용 9개 테이블(MNG_*) = 총 45개 테이블.
> - 엔티티 ID `KLID-AT-EN-001 ~ EN-043`은 기존 v1.0과 동일. `EN-044`(LS_MARKING), `EN-045`(LS_PORTAL_USER_LABEL)는 V2.0 신규.

...
**관련 시퀀스도 ID**: KLID-AT-SD-007
**화면개요**: 관제서버·포털서버와 동일 도메인 운영. 브라우저 스토리지(localStorage/sessionStorage) 공유를 통해 JWT를 전달. 토큰을 SecurityContext에 등록하고 채널·역할에 따라 적합한 시작 화면으로 자동 이동.
...
**관련 유스케이스 ID**: KLID-AT-UC-013
**관련 시퀀스도 ID**: KLID-AT-SD-007
**화면개요**: 인증은 완료되었으나 role 클레임이 없는 사용자가 관리자 공유 패스워드를 입력하여 WORKER 또는 REVIEWER 역할을 자가 부여하는 화면.
...
**관련 시퀀스도 ID**: KLID-AT-SD-001
**화면개요**: 처리 대기/완료/반려 영상 건수 KPI, 이벤트별 이미지·영상 데이터 분포, 최근 완료 영상 목록, WORKER 한정 내 작업 현황.
...
**관련 유스케이스 ID**: KLID-AT-UC-001
**관련 시퀀스도 ID**: -
**화면개요**: 관제서버에서 인계받은 영상의 배치 처리 상태와 단계를 확인하는 테이블 뷰. 필터, 다중 선택, 페이지네이션 제공.
...
**관련 유스케이스 ID**: KLID-AT-UC-001
**관련 시퀀스도 ID**: -
**화면개요**: 배치 파이프라인에서 처리 중인 영상의 실시간 처리 현황을 표 형식으로 표시. 5초 자동 폴링. KPI 3개(처리 중/완료/실패) + 전체 목록 테이블 제공.
...
**관련 유스케이스 ID**: KLID-AT-UC-001
**관련 시퀀스도 ID**: -
**화면개요**: 개별 영상의 상세 정보를 3개 탭(기본 정보 / 프레임 미리보기 / 오토라벨 결과)으로 구성. 헤더 카드에서 CCTV명, 이벤트 유형, 처리 상태, 버전관리 이동 버튼 제공.
...
| 화면 ID | KLID-AT-SC-007 | 화면명 | 내 작업 목록(TaskList) |
|---|---|---|---|
| 화면유형 | 조회 + 입력(배정) | 메뉴경로 | `작업 > 내 작업` |

**관련 유스케이스 ID**: KLID-AT-UC-008
**관련 시퀀스도 ID**: KLID-AT-SD-005
**화면개요**: REVIEWER: 처리 완료 영상에 대한 작업자 배정·재배정·일괄배정. WORKER: 본인에게 배정된 작업 목록 조회 및 작업 시작 진입. KPI 4카드 + 필터 + 다중 선택 테이블.
...
**관련 유스케이스 ID**: KLID-AT-UC-006, KLID-AT-UC-010, KLID-AT-UC-003
**관련 시퀀스도 ID**: KLID-AT-SD-004
**화면개요**: konva 기반 라벨링 캔버스. BBox/Polygon/Mask 도구, 프레임 타임라인, 좌측 객체 리스트, 우측 속성·이슈 패널, 단축키, 자동 저장.
...
**관련 유스케이스 ID**: KLID-AT-UC-001
**관련 시퀀스도 ID**: -
**화면개요**: YOLO/SAM2 오토라벨링 결과 요약. 처리 정보, 신뢰도 구간 분포, 라벨별 분포, 낮은 신뢰도 프레임 목록 제공.
...
**관련 시퀀스도 ID**: KLID-AT-SD-010
**화면개요**: VLM 시계열 결과 검토·수정. 좌측: 프레임 미리보기 + 상태 변화 타임라인. 우측: VLM 객체 검증 카드 + 환경/이벤트 메타 편집 폼.
...
**관련 시퀀스도 ID**: -
**화면개요**: 비식별화 처리가 완료되었거나 실패한 영상 목록. 비식별 대상 여부·처리 상태 필터 및 진행 프레임 수 표시.
...
**관련 유스케이스 ID**: KLID-AT-UC-003
**관련 시퀀스도 ID**: KLID-AT-SD-006
**화면개요**: 비식별화 영상 단위 비교. 처리 정보 5컬럼 + 12프레임 그리드 + 좌우(원본/비식별) 비교 + 처리 이력.
...
**관련 시퀀스도 ID**: KLID-AT-SD-005
**화면개요**: 작업자가 제출한 라벨링 결과를 검수하는 REVIEWER 전용 목록. KPI 4종 + 검색·상태 필터 + DataTable.
...
**관련 유스케이스 ID**: KLID-AT-UC-009, KLID-AT-UC-014
**관련 시퀀스도 ID**: KLID-AT-SD-005, KLID-AT-SD-011
**화면개요**: 3분할 풀스크린 검수 화면. 좌측: 읽기 전용 Konva 캔버스. 우측: 객체 목록/속성/검수 메모. 하단: 프레임 타임라인 + 승인/반려 액션 바.
...
**관련 시퀀스도 ID**: KLID-AT-SD-004
**화면개요**: Gitea 기반 라벨 버전 관리. 커밋 타임라인 + Diff 뷰어 + 롤백 기능.
...
**관련 유스케이스 ID**: -
**관련 시퀀스도 ID**: -
**화면개요**: 작업자별 작업 통계. REVIEWER는 작업자 선택 가능, WORKER는 본인 고정. KPI 4 + Secondary 2 + 일별 차트 + 월별 표.
...
**관련 유스케이스 ID**: -
**관련 시퀀스도 ID**: -
**화면개요**: 시스템 전체 학습 데이터 구축 현황. 누적 이미지·영상, 처리 현황 5카드, 일별 작업량, 이벤트 분포, 작업자별 현황. CSV 다운로드.
...
**관련 시퀀스도 ID**: KLID-AT-SD-008
**화면개요**: 검수 완료(승인) 영상에 4종 증강(겨울/야간/비/해상도)을 요청. 2단계: 유형 선택 → 영상 선택 + 최근 이력 잡 카드.
...
**관련 유스케이스 ID**: KLID-AT-UC-011
**관련 시퀀스도 ID**: KLID-AT-SD-008
**화면개요**: 데이터 증강 작업 결과 확인. 작업 요약 카드 + 라벨 무결성 + 영상별 증강 유형 탭 + 프레임 그리드 + 좌우 비교 + 채택/거부.
...
**관련 유스케이스 ID**: -
**관련 시퀀스도 ID**: -
**화면개요**: 시스템 사용자 목록 조회·관리. 검색, 역할·상태 필터, 활성/비활성 토글, 역할 변경.
...
**관련 유스케이스 ID**: -
**관련 시퀀스도 ID**: -
**화면개요**: 시스템 설정을 3개 영역으로 구분: 편집 가능(FFmpeg/Batch/YOLO), 실시간 모니터링(Health), 위험 액션(placeholder).
...
**관련 시퀀스도 ID**: -
**화면개요**: 라벨 코드 프리셋 관리. 2열 카드 그리드 + 클라이언트 페이지네이션. CRUD + 이벤트 타입 매핑.
...
**관련 시퀀스도 ID**: KLID-AT-SD-009
**화면개요**: 포털 메인 페이지. Hero 섹션 + KPI 2 + 2단계 카드(업로드/라벨링) + 내 업로드 목록. TUS 재개 업로드 지원.
...
**관련 유스케이스 ID**: KLID-AT-UC-012
**관련 시퀀스도 ID**: KLID-AT-SD-009
**화면개요**: LabelingPage 재사용(포털 모드). 히스토리/검수제출/VLM/메타 탭 미노출. 오토라벨 버튼 추가.
...
**관련 유스케이스 ID**: KLID-AT-UC-008
**관련 시퀀스도 ID**: KLID-AT-SD-005
**화면개요**: KLID-AT-SC-007(TaskList)의 REVIEWER 시각과 동일. 별도 라우트로 진입하는 작업 배정 전용 화면(Placeholder — Phase 4+).
...
**관련 유스케이스 ID**: -
**관련 시퀀스도 ID**: -
**화면개요**: 영상 파일과 메타데이터를 입력하여 배치 파이프라인을 수동 실행하는 개발/검수 전용 화면. 실행 후 2초 폴링으로 파이프라인 진행 상황 가시화.
...
| 화면 ID | KLID-AT-SC-027 | 화면명 | 마킹(Marking) |
|---|---|---|---|
| 화면유형 | 입력·갱신·삭제 | 메뉴경로 | `/marking/{rawSn}` |

**관련 유스케이스 ID**: KLID-AT-UC-001
**관련 시퀀스도 ID**: -
**화면개요**: 영상 재생 + 자동/수동 모드로 이벤트 시점 마킹. 수동 모드: Space로 현재 프레임 마킹. 자동 모드: 프레임 간격 기반 마킹. 마킹 결과를 VLM에 콜백으로 전달.

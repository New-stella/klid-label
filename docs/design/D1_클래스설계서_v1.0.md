...
| KLID-AT-DC-053 | MarkingService | KLID-AT-UC-001 |
| KLID-AT-DC-054 | MarkingController | KLID-AT-UC-001 |
...
### 2.1 KLID-AT-SD-001  영상 자동 처리 파이프라인
...
### 2.2 KLID-AT-SD-002  비식별 처리 위탁 및 결과 수신
...
### 2.3 KLID-AT-SD-003  VLM 시계열 분석 비동기 위탁
...
### 2.4 KLID-AT-SD-004  라벨 편집 및 Gitea 자동 커밋
...
### 2.5 KLID-AT-SD-005  작업 배정 및 라벨 검수 승인
...
### 2.6 KLID-AT-SD-006  비식별 오류 신고 및 재처리
...
### 2.7 KLID-AT-SD-007  JWT 채널 인증 및 라우팅
...
### 2.8 KLID-AT-SD-008  증강 비동기 요청 및 결과 수신·검토
...
### 2.9 KLID-AT-SD-009  포털 사용자 간편 라벨링
...
### 2.11 KLID-AT-SD-011  TASK_COMPLETED 관제서버 통지
...
### 2.12 KLID-AT-SD-012  TASK_MODIFIED 디바운스 통지
...
### 3.1 KLID-AT-DCD-001  배치 파이프라인 계층
...
### 3.2 KLID-AT-DCD-002  외부 연동 클라이언트 계층
...
### 3.3 KLID-AT-DCD-003  웹훅 결과 수신 계층
...
HmacWebhookFilter ..> DeidentifyResultController : 통과
HmacWebhookFilter ..> VlmResultController : 통과
HmacWebhookFilter ..> AugmentResultController : 통과
...
### 3.4 KLID-AT-DCD-004  라벨링·버전관리 서비스 계층
...
### 3.5 KLID-AT-DCD-005  배정·검수·통지 서비스 계층
...
### 3.8 KLID-AT-DCD-008  메타·증강 서비스 계층
...
title KLID-AT-DCD-009 외부 통지(관제서버) 계층skinparam defaultFontName "Malgun Gothic"
...
### 3.10 KLID-AT-DCD-010  통합 설계 클래스도 (Overview)

| 설계 클래스도 ID | KLID-AT-DCD-010 | 설계 클래스도명 | 통합 설계 클래스도 |
|---|---|---|---|
| **관련 유스케이스 ID** | KLID-AT-UC-001 ~ KLID-AT-UC-015 | |
```plantuml
@startuml KLID-AT-DCD-010
title KLID-AT-DCD-010 통합 설계 클래스도
skinparam defaultFontName "Malgun Gothic"
skinparam classBackgroundColor #FFFFFF
skinparam classBorderColor #2E75B6
...
### KLID-AT-DC-045  NotificationService
...
> **부록 — 운영·구현 메모**
> - `LabelingBatchQueueService.dequeueOne()` 의 `SKIP LOCKED` 옵티마이저 힌트는 Oracle/PostgreSQL DDL 단에서 보장.
> - `LogNotificationService`(내부 REVIEWER 알림) 와 `ControlNotifyClient`(외부 관제서버 통지) 는 별개 채널로 운영.

...
|----------|---|---------|---|
| 관련 유스케이스 ID | KLID-AT-UC-018 (영상 적재 — 관리 화면 업로드) |
...
flowchart TD
    ...
    subgraph CO018[KLID-AT-CO-018 영상 적재]
        ...
        subgraph BIZ18[Biz Logic]
            TUC[TusUploadController]
            TUS[TusUploadService]
            VMV[VideoMagicByteValidator]
        end
...
| KLID-AT-CO-018 | 영상 적재 컴포넌트 | 검수자가 관리 화면에서 영상을 재개 가능 방식으로 업로드하면 파일 유효성을 검증하고 영상 메타를 등록한 뒤 배치 파이프라인 대상으로 등록한다 | KLID-AT-UC-018 |
...
| 컴포넌트 ID | KLID-AT-CO-018 | 컴포넌트명 | 영상 적재 컴포넌트 |
|----------|---|---------|---|
| 컴포넌트 개요 | 검수자가 관리 화면에서 재개 가능 방식으로 영상 파일을 업로드하면 확장자·크기·파일 형식을 검증하고 원본 경로·메타를 등록한 뒤 배치 파이프라인 대상으로 등록한다. 포털 사용자 업로드는 제공하지 않는다. |

| **내부 클래스** | | |
| ID | 클래스명 | 설명 |
|----|--------|------|
| KLID-AT-DC-052 | TusUploadController | 재개 가능 방식 업로드 요청 진입점 |
| KLID-AT-DC-053 | TusUploadService | 업로드 세션 생성·진행·완료 오케스트레이션 |
| KLID-AT-DC-055 | VideoMagicByteValidator | 파일 시그니처(매직바이트) 기반 형식 검증 |
| KLID-AT-DC-054 | LsTusUpload | 업로드 세션 상태 도형 클래스 |
| KLID-AT-DC-010 | LsDataRaw | 영상 메타 도형 클래스 — KLID-AT-CO-002 공유 |
...
| ID | 인터페이스명 | 오퍼레이션명 | 구분 (Serviced/Required) |
|----|----------|----------|------------------------|
| KLID-AT-IF-020 | 영상 업로드 인터페이스 | createUpload | Serviced |
| KLID-AT-IF-020 | 영상 업로드 인터페이스 | patchUpload | Serviced |
...
#### KLID-AT-IF-020 — 영상 업로드 인터페이스

| 인터페이스 ID | KLID-AT-IF-020 | 인터페이스명 | 영상 업로드 인터페이스 |
|---------|---|---------|---|
| 오퍼레이션명 | createUpload |
| 오퍼레이션 개요 | 영상 업로드 세션을 생성하고 업로드 식별자를 발급한다 |
| 사전조건 | 검수자로 인증되어 있고 업로드 대상 파일의 크기·확장자·형식 정보가 제공된다 |
| 사후조건 | 업로드 세션이 등록되고 클라이언트가 이어서 파일 데이터를 전송할 수 있다 |
| 파라미터 | 파일 크기, 파일명, 메타정보 |
| 반환값 | 업로드 세션 식별자. 형식·크기 검증 실패 시 거부 |

| 인터페이스 ID | KLID-AT-IF-020 | 인터페이스명 | 영상 업로드 인터페이스 |
|---------|---|---------|---|
| 오퍼레이션명 | patchUpload |
| 오퍼레이션 개요 | 진행 중인 업로드 세션에 파일 데이터를 청크 단위로 전송하고 완료 시 영상을 등록한다 |
| 사전조건 | 유효한 업로드 세션이 존재하고 전송 범위가 세션 오프셋과 일치한다 |
| 사후조건 | 파일이 지정 경로에 저장되고 완료 시 영상 1건이 등록되어 배치 파이프라인 대상이 된다 |
| 파라미터 | 업로드 세션 식별자, 파일 데이터 청크, 범위 오프셋 |
| 반환값 | 현재 업로드 오프셋. 완료 시 등록된 영상 식별자 반환. 파일 형식 불일치 시 거부 |

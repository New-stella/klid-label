...
|------|------|------------|--------|--------|
| 2026-05-21 | 1.0 | 최초 작성 — klid-label (backend Spring Boot 3.3 / frontend React 18 / ai-server FastAPI) 기반 | 박찬기 | - |
| 2026-05-27 | 1.1 | CBD 양식 변환, 아키텍처 요구사항 및 구현방안 보완 | 박찬기 | - |
...
[Label/Version/Review/Assignment] --> [Entities (JPA 47개)]
...
<tr><td>구현방안</td><td>(1) Quartz 단일 인스턴스 Job 처리 — 1건/분 순차 처리<br>(2) ai-server GPU Worker 다중 프로세스 수평 확장 — GPU 병목 분산<br>(3) 파이프라인 7단계: 마킹→VLM(콜백 비동기)→비식별→프레임추출(마킹 위치)→YOLO(원본만)→SAM2→보간<br>(4) 단계별 비동기 위탁 — VLM/비식별 결과를 콜백으로 수신하여 파이프라인 블로킹 최소화</td></tr>
</table>

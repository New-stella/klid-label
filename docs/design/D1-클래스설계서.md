...
| KLID-AT-DC-020 | VideoResolutionPersister | KLID-AT-UC-003 |
...
sequenceDiagram
    ...
    participant P as VideoResolutionPersister
    participant X as 해상도내보내기 저장소
    REVIEWER->>C: 대상 영상·목표 하위 해상도 지정
    C->>S: 해상도 변경 위임
    S->>S: 원본보다 낮은 해상도인지 검증(업스케일 거부)
    loop 프레임별
        S->>Z: 프레임 이미지 다운스케일
        Z-->>S: 변환 이미지 저장
    end
    S->>P: 변환 결과 기록 요청
    P->>X: 해상도 변경 이력 1건 저장(영상·라벨 미생성)
    P-->>S: 기록 결과
    S-->>C: 변환 결과(라벨 좌표 미제공)
    C-->>REVIEWER: 이미지셋 제공 안내
```
...
classDiagram
    class VideoController {
        +changeResolution(rawSn, command, actor) ResolutionResult
    }
    class VideoResolutionService {
        +changeResolution(rawSn, command, regId) ResolutionResult
    }
    ...
    class VideoResolutionPersister {
        +persist() ResolutionResult
    }
    class LsResolutionExport {
        -resExportSn Long
        -targetResCd ResolutionPreset
    ...
    VideoResolutionService --> Java2DImageResizer
...
classDiagram
    ...
    Sam2TrackService --> LsDataLblAiInfo
```
...
| **오퍼레이션** |
| 오퍼레이션명 | 가시성 | 파라미터 | 반환타입 | 설명 |
| predictYolo | public | 탐지 요청 | 탐지 결과(비동기) | 객체 탐지 추론을 요청한다 |
| segment | public | 분할 요청 | 분할 결과(비동기) | 외곽 경계 분할 추론을 요청한다 |
| track | public | 추적 요청 | 추적 결과(비동기) | 객체 추적 추론을 요청한다 |
| verifyObjects | public | 검증 요청 | 검증 결과(비동기) | 객체 검증 추론을 요청한다 |
...
| **속성** |
| 속성명 | 가시성 | 타입 | 기본값 | 설명 |
| resExportSn | private | 식별자 | - | 해상도 변경 이력 식별자 |
| dataRawSn | private | 식별자 | - | 원본 영상 참조 |
| targetResCd | private | 해상도 구분 | - | 목표 하위 해상도 |
| orgnlW / orgnlH | private | 정수 | - | 원본 가로·세로 |
| targetW / targetH | private | 정수 | - | 변환 가로·세로 |
| frameCnt | private | 정수 | - | 변환 프레임 수 |
| outputDirPath | private | 문자열 | - | 결과 이미지셋 경로 |
| **오퍼레이션** |
| 오퍼레이션명 | 가시성 | 파라미터 | 반환타입 | 설명 |
| create | public | 원본·목표 해상도, 프레임 수, 경로 | 변경 이력 | 해상도 변경 결과 1건을 생성한다(원본·해상도 유일) |
...
| 속성명 | 가시성 | 타입 | 기본값 | 설명 |
| videoRepository | private | 영상 저장소 | - | 원본 영상 저장소 |
| imageResizer | private | 이미지 리사이저 | - | 프레임 다운스케일 처리 |
| persister | private | 결과 기록 서비스 | - | 변경 이력 영속 |
| resizeConcurrencyLimit | private | 동시 실행 제한 | - | 변환 동시 실행 수 제한 |
| **오퍼레이션** |
| 오퍼레이션명 | 가시성 | 파라미터 | 반환타입 | 설명 |
| changeResolution | public | 영상 식별자, 변경 요청, 등록자 | 변경 결과 | 업스케일을 거부하고 프레임을 다운스케일해 이미지셋을 생성한다 |
| loadAndValidate | public | 영상 식별자 | 영상 | 대상 영상 존재·유효성을 검증한다 |
...
| **오퍼레이션** |
| 오퍼레이션명 | 가시성 | 파라미터 | 반환타입 | 설명 |
| resize | public | 원본·대상 경로, 목표 크기 | void | 프레임 이미지를 목표 해상도로 다운스케일한다 |
| readDimensions | public | 원본 경로 | 크기 | 원본 이미지 해상도를 읽는다 |

#### KLID-AT-DC-020 — VideoResolutionPersister

| 설계 클래스 ID | KLID-AT-DC-020 | 설계 클래스명 | VideoResolutionPersister |
|-----------|---|---------|---|
| **속성** |
| 속성명 | 가시성 | 타입 | 기본값 | 설명 |
| exportRepository | private | 변경 이력 저장소 | - | 해상도 변경 이력 저장소 |
| **오퍼레이션** |
| 오퍼레이션명 | 가시성 | 파라미터 | 반환타입 | 설명 |
| persist | public | 원본, 해상도, 경로·크기·프레임 수, 등록자 | 변경 결과 | 해상도 변경 결과를 독립 트랜잭션으로 기록한다 |
...
| sam2Track | public | 프레임 식별자, 추적 요청, 인증 주체 | 추적 결과 | 객체 추적을 요청한다 |
...
| bulkUpsert | public | 프레임 식별자, 라벨 요청, 인증 주체 | 라벨 결과 | 작업본을 임시저장한다(검수 완료본 수정 시 통지 발행) |
...
| labelRepository | private | 라벨 저장소 | - | 자동 라벨 저장 |
| accessGuard | private | 접근 가드 | - | 본인 배정 검증 |
| systemConfigService | private | 설정 서비스 | - | 폴리곤 단순화 정밀도 조회 |
| **오퍼레이션** |
| 오퍼레이션명 | 가시성 | 파라미터 | 반환타입 | 설명 |
| track | public | 추적 요청, 인증 주체 | 추적 결과 | 후속 프레임에 객체를 추적·전파하고 좌표 검증·단순화 후 저장한다 |
...
| **오퍼레이션** |
| 오퍼레이션명 | 가시성 | 파라미터 | 반환타입 | 설명 |
| recordDeletion | public | 라벨·프레임 식별자 | 이력 | 라벨 삭제 이력을 기록한다 |
...
| **오퍼레이션** |
| 오퍼레이션명 | 가시성 | 파라미터 | 반환타입 | 설명 |
| analyzeTimeseries | public | 마킹 정보 | 분석 응답(비동기) | 마킹 결과로 시계열 분석을 요청한다 |
...
| **오퍼레이션** |
| 오퍼레이션명 | 가시성 | 파라미터 | 반환타입 | 설명 |
| sendTaskCompleted | public | 완료 통지 페이로드 | 비동기 결과 | 완료 통지를 단방향 전송한다 |
| sendTaskModified | public | 수정 통지 페이로드 | 비동기 결과 | 수정 통지를 단방향 전송한다 |
...
- 설계 클래스도는 유스케이스 명세서(R2)의 활성 유스케이스 18건과 1:1로 정렬한다(KLID-AT-DCD-001~003, 004~011, 013, 016, 018~019, 021~023).
- 유스케이스 명세서에서 결번인 유스케이스 번호(KLID-AT-UC-012·014·015·017·020 및 024 이상)는 설계 클래스도·시퀀스도에서도 동일하게 결번 처리한다.

# D8. 엔티티 관계 모형 설계서 (ERD)

## ▣ 제·개정 이력

| 날짜 | 버전 | 작성자 | 승인자 | 내용 |
|---|---|---|---|---|
| 2026-05-21 | 1.0 | 박찬기 | - | 최초 작성 — klid-label JPA 엔티티 42종 +  1종 추가 반영 |
...
> 가독성을 위해 도메인별로 5개 서브 ERD + 1개 통합 Overview 로 분리.

### 1.1 KLID-AT-ERD-001  영상·프레임·라벨 코어 ERD
...
entity "LS_DATA_LBL\n(라벨)" as LBL {
  *LBL_SN : NUMBER(19) <<PK>>
  ...
  LABEL_ID : NUMBER(19) <<FK>>
  LABEL : VARCHAR(255)
  POINTS_JSON : CLOB
  AUTO_LBL_YN : CHAR(1)
  CONF_SCORE : NUMBER(5,4)
  DATA_AUG_SN : NUMBER(19)
  TRCK_ID : VARCHAR(64)
  LBL_SRC_CD : VARCHAR(16)
  REG_USER_NO : NUMBER(19)
  REG_DT, UPD_DT
}

entity "LS_DATA_LBL_AI_INFO\n(라벨 AI 메타)" as LBLAI {
  *LBL_SN : NUMBER(19) <<PK,FK>>
  --
...
entity "LS_DATA_LBL_HSTRY\n(라벨 변경 이력)" as LBLH {
  *LBL_HSTRY_SN : NUMBER(19) <<PK>>
  --
  LBL_SN : NUMBER(19) <<FK>>
  CHG_TYPE_CD : VARCHAR(8)
  CHG_DT : TIMESTAMP
  CHG_USER_NO : NUMBER(19)
  SNAPSHOT_JSON : CLOB
}

entity "LS_LABEL_VERSION\n(Gitea 버전)" as LVER {
  *LABEL_VERSION_SN : NUMBER(19) <<PK>>
  --
  PJT_SN : NUMBER(19)
  DATA_RAW_SN : NUMBER(19) <<FK>>
  DATA_SRC_SN : NUMBER(19) <<FK>>
...
entity "LS_DATA_SRC_HSTRY" as SRCH
entity "LS_DATA_RAW_HSTRY" as RAWH
...
LBL ||--o{ LBLH : 1:N
RAW ||--o{ LVER : 1:N (커밋)
SRC ||--o{ LVER : 1:N (프레임 커밋)
SRC ||--o{ SRCH : 1:N
RAW ||--o{ RAWH : 1:N
@enduml
```

### 1.2 KLID-AT-ERD-002  라벨 마스터·속성·프리셋 ERD
...
entity "LS_LABEL_PRESET_CODE\n(프리셋-라벨 매핑)" as PREC {
  *PRESET_ID : NUMBER(19) <<PK,FK>>
  *LABEL_ID : NUMBER(19) <<PK,FK>>
  --
  ORD : NUMBER(5)
}
...
### 1.3 KLID-AT-ERD-003  배정·검수·이슈 ERD
...
entity "LS_TASK_ASSIGNMENT\n(작업 배정)" as ASN {
  *ASSIGNMENT_ID : NUMBER(19) <<PK>>
  --
  RAW_SN : NUMBER(19) <<FK>>
  WORKER_USER_NO : NUMBER(19) <<FK>>
  ASSIGNER_USER_NO : NUMBER(19) <<FK>>
...
entity "LS_TASK_ASSIGN_HISTORY\n(배정 이력)" as ASNH {
  *HSTRY_SN : NUMBER(19) <<PK>>
  --
  ASSIGNMENT_ID : NUMBER(19) <<FK>>
  PREV_WORKER_USER_NO : NUMBER(19)
  NEW_WORKER_USER_NO : NUMBER(19)
  REASON : VARCHAR(500)
  REG_DT
}

entity "LS_TASK_EVENT_LOG\n(작업 이벤트 로그)" as EVL {
  *EVENT_SN : NUMBER(19) <<PK>>
  --
  RAW_SN : NUMBER(19)
  EVENT_TYPE_CD : VARCHAR(32)
  PAYLOAD_JSON : CLOB
...
entity "LS_DATA_ISSUE\n(검수 이슈)" as ISS {
  *ISSUE_SN : NUMBER(19) <<PK>>
  --
  RAW_SN : NUMBER(19) <<FK>>
  SRC_SN : NUMBER(19) <<FK>>
  REVIEWER_USER_NO : NUMBER(19)
...
entity "LS_AUTH_WORK_LOCK\n(작업 잠금)" as LOCK {
  *LOCK_SN : NUMBER(19) <<PK>>
  --
  RAW_SN : NUMBER(19) <<FK,UQ>>
  OWNER_ID : VARCHAR(30)
...
entity "LS_DATA_META_REVIEW\n(메타 검토)" as MREV {
  *META_REVIEW_SN : NUMBER(19) <<PK>>
  --
  META_SN : NUMBER(19) <<FK>>
  STATUS_CD : VARCHAR(16)
  REVIEWER_USER_NO : NUMBER(19)
  REASON : VARCHAR(2000)
...
entity "LS_DATA_META_HSTRY" as MH

entity "LS_DATA_AUG\n(증강 결과)" as AUG {
  *DATA_AUG_SN : NUMBER(19) <<PK>>
  --
  JOB_ID : NUMBER(19)
  SOURCE_SRC_SN : NUMBER(19) <<FK>>
...
entity "LS_DATA_AUG_RVW\n(증강 검수)" as AUGR {
  *AUG_RVW_SN : NUMBER(19) <<PK>>
  --
  DATA_AUG_SN : NUMBER(19) <<FK>>
...
entity "LS_DATA_AUG_LBL_MAP" as AUGM {
  *DATA_AUG_SN : NUMBER(19) <<PK,FK>>
  *SOURCE_LBL_SN : NUMBER(19) <<PK,FK>>
  --
  AUG_LBL_SN : NUMBER(19) <<FK>>
}
...
### 1.5 KLID-AT-ERD-005  사용자·CCTV·시스템 ERD
...
entity "MNG_ACCT_USER\n(사용자)" as USR {
  *USER_NO : NUMBER(19) <<PK>>
  --
  LOGIN_ID : VARCHAR(50) <<UQ>>
  NAME : VARCHAR(100)
  EMAIL : VARCHAR(255)
...
entity "MNG_ACCT_AUTHRT\n(권한 마스터)" as ROLE {
  *AUTHRT_CD : VARCHAR(20) <<PK>>
  AUTHRT_NAME : VARCHAR(100)
}

entity "MNG_ACCT_USER_AUTHRT" as URA {
  *USER_NO : NUMBER(19) <<PK,FK>>
  *AUTHRT_CD : VARCHAR(20) <<PK,FK>>
  REG_DT
}

entity "MNG_RESOURCE_CCTV" as CCTV {
...
entity "MNG_EX_LOCAL_GOV\n(지자체)" as GOV {
  *LCLGV_CD : VARCHAR(32) <<PK>>
  LCLGV_NAME : VARCHAR(100)
}

entity "MNG_EX_EVNT_TYPE" as EVT {
  *EVNT_TYPE_CD : VARCHAR(32) <<PK>>
  EVNT_TYPE_NAME : VARCHAR(100)
}

entity "MNG_EX_EVNT_TYPE_MAP" as EVTM {
  *EVNT_TYPE_CD : VARCHAR(32) <<PK,FK>>
  *CCTV_ID : VARCHAR(64) <<PK,FK>>
}

entity "MNG_CLIP_MASTER" as CMS {
...
### 1.6 KLID-AT-ERD-006  통합 Overview (참고)

```plantuml
@startuml KLID-AT-ERD-006
title KLID-AT-ERD-006 통합 ERD (Overview)
hide circle
skinparam linetype ortho
skinparam defaultFontName "Malgun Gothic"
...
| **엔티티 설명** | 원본 영상 1건. 자동 파이프라인의 단위. 비식별·VLM 등 모든 처리의 root | | |

| 속성명 | 동의어 | 타입 | 길이 | NOT NULL | PK | FK | INX | 기본값 | 제약조건 |
|---|---|---|---|---|---|---|---|---|---|
| RAW_SN | 영상SN | NUMBER | 19 | Y | Y |  |  | SEQ_LS_DATA_RAW | PK |
...
| CAPTURED_AT | 촬영시각 | TIMESTAMP | - |  |  |  | Y |  |  |
| DURATION_SEC | 영상길이(초) | NUMBER | 10 |  |  |  |  |  |  |
| DATA_STTS_CD | 영상상태 | VARCHAR | 32 | Y |  |  | Y | 'PENDING' | CHK in (PENDING,MARKING,VLM,DEIDENTIFY,FRAME,YOLO,SAM2,COMPLETED,FAILED) |
| PARENT_RAW_SN | 원본영상SN | NUMBER | 19 |  |  | Y(LS_DATA_RAW) | Y |  | 증강 영상의 원본 영상 참조. NULL이면 원본 |
| REG_DT | 등록시각 | TIMESTAMP | - | Y |  |  |  | CURRENT_TIMESTAMP |  |
| UPD_DT | 수정시각 | TIMESTAMP | - |  |  |  |  |  |  |

### KLID-AT-EN-NEW-01  LS_MARKING (마킹)

| 엔티티 ID | KLID-AT-EN-NEW-01 | 엔티티명 | LS_MARKING |
|---|---|---|---|
...
| 속성명 | 동의어 | 타입 | 길이 | NOT NULL | PK | FK | INX | 기본값 | 제약조건 |
|---|---|---|---|---|---|---|---|---|---|
| MARKING_SN | 마킹SN | NUMBER | 19 | Y | Y |  |  | SEQ | PK |
| RAW_SN | 영상SN | NUMBER | 19 | Y |  | Y(LS_DATA_RAW) | Y |  |  |
| EVENT_NAME | 이벤트명 | VARCHAR | 100 | Y |  |  |  |  |  |
| MARKING_MODE | 마킹모드 | VARCHAR | 16 | Y |  |  |  |  | CHK(AUTO/MANUAL) |
| INTERVAL_SEC | 프레임간격(초) | NUMBER | 10 |  |  |  |  |  | 자동모드 시 |
| VIDEO_PATH | NAS경로 | VARCHAR | 500 | Y |  |  |  |  |  |
| MARKS | 마킹정보 | JSON | - | Y |  |  |  |  | [{frameIndex, timestamp}] |
| STATUS | 마킹상태 | VARCHAR | 16 | Y |  |  |  | 'PENDING' | CHK(PENDING/VLM_REQUESTED/VLM_COMPLETED) |
| CREATED_BY | 생성자 | NUMBER | 19 |  |  |  |  |  |  |
| CREATED_AT | 생성시각 | TIMESTAMP | - | Y |  |  |  | CURRENT_TIMESTAMP |  |
| UPDATED_AT | 수정시각 | TIMESTAMP | - | Y |  |  |  | CURRENT_TIMESTAMP |  |

### KLID-AT-EN-NEW-02  LS_PORTAL_USER_LABEL (포털 사용자 라벨)

| 엔티티 ID | KLID-AT-EN-NEW-02 | 엔티티명 | LS_PORTAL_USER_LABEL |
|---|---|---|---|
| **관련 클래스 ID** | KLID-AT-DC-040 | **관련 클래스명** | PortalLabelService |
| **엔티티 설명** | 포털 사용자의 작업 데이터. 원본 데이터를 수정하지 않고 사용자별로 별도 적재. 데이터마트 영상의 라벨 수정/다운로드 용도 | | |

| 속성명 | 동의어 | 타입 | 길이 | NOT NULL | PK | FK | INX | 기본값 | 제약조건 |
|---|---|---|---|---|---|---|---|---|---|
| USER_LBL_SN | 사용자라벨SN | NUMBER | 19 | Y | Y |  |  | SEQ | PK |
| PORTAL_USER_NO | 포털사용자번호 | VARCHAR | 100 | Y |  |  | Y |  |  |
| SOURCE_RAW_SN | 원본영상SN | NUMBER | 19 | Y |  |  | Y |  | 데이터마트 원본 |
| SOURCE_SRC_SN | 원본프레임SN | NUMBER | 19 | Y |  |  |  |  |  |
...
| 속성명 | 동의어 | 타입 | 길이 | NOT NULL | PK | FK | INX | 기본값 | 제약조건 |
|---|---|---|---|---|---|---|---|---|---|
| SRC_SN | 프레임SN | NUMBER | 19 | Y | Y |  |  | SEQ_LS_DATA_SRC | PK |
| RAW_SN | 영상SN | NUMBER | 19 | Y |  | Y(LS_DATA_RAW) | Y |  |  |
| FRAME_NO | 프레임번호 | NUMBER | 10 | Y |  |  | Y |  |  |
| FILE_PATH | 원본경로 | VARCHAR | 500 | Y |  |  |  |  |  |
| SRC_BKUP_FILE_PATH | 비식별본경로 | VARCHAR | 1000 |  |  |  |  |  |  |
| CAPTURED_AT | 촬영시각 | TIMESTAMP | - |  |  |  |  |  |  |
| REG_DT | 등록시각 | TIMESTAMP | - | Y |  |  |  | CURRENT_TIMESTAMP |  |
| UPD_DT | 수정시각 | TIMESTAMP | - |  |  |  |  |  |  |
...
| 속성명 | 동의어 | 타입 | 길이 | NOT NULL | PK | FK | INX | 기본값 | 제약조건 |
|---|---|---|---|---|---|---|---|---|---|
| LBL_SN | 라벨SN | NUMBER | 19 | Y | Y |  |  | SEQ_LS_DATA_LBL | PK |
| SRC_SN | 프레임SN | NUMBER | 19 | Y |  | Y(LS_DATA_SRC) | Y |  |  |
| LBL_TYPE_CD | 라벨타입 | VARCHAR | 16 | Y |  |  |  |  | CHK in (BBOX,POLYGON,MASK) |
| LABEL_ID | 라벨ID | NUMBER | 19 |  |  | Y(LS_LABEL) |  |  |  |
| LABEL | 라벨명 | VARCHAR | 255 | Y |  |  |  |  |  |
| POINTS_JSON | 좌표 | CLOB | - |  |  |  |  |  | App.MAX_POINTS_PER_LABEL=1000 |
| AUTO_LBL_YN | 자동여부 | CHAR | 1 |  |  |  |  | 'N' | CHK(Y/N) |
| CONF_SCORE | 신뢰도 | NUMBER | 5,4 |  |  |  |  |  | 0.0~1.0 |
| DATA_AUG_SN | 증강SN | NUMBER | 19 |  |  | Y(LS_DATA_AUG) |  |  |  |
| TRCK_ID | 트랙ID | VARCHAR | 64 |  |  |  | Y |  |  |
| LBL_SRC_CD | 라벨소스 | VARCHAR | 16 |  |  |  |  |  | CHK in (HUMAN,YOLO,SAM2) |
| REG_USER_NO | 등록사용자 | NUMBER | 19 |  |  | Y(MNG_ACCT_USER) |  |  |  |
| REG_DT | 등록시각 | TIMESTAMP | - | Y |  |  |  | CURRENT_TIMESTAMP |  |
| UPD_DT | 수정시각 | TIMESTAMP | - |  |  |  |  |  |  |
...
| 속성명 | 동의어 | 타입 | 길이 | NOT NULL | PK | FK | INX | 기본값 | 제약조건 |
|---|---|---|---|---|---|---|---|---|---|
| LABEL_VERSION_SN | 버전SN | NUMBER | 19 | Y | Y |  |  | SEQ_LS_LABEL_VERSION | PK |
| PJT_SN | 프로젝트SN | NUMBER | 19 | Y |  |  |  |  |  |
| DATA_RAW_SN | 영상SN | NUMBER | 19 | Y |  | Y(LS_DATA_RAW) | Y |  |  |
| DATA_SRC_SN | 프레임SN | NUMBER | 19 |  |  | Y(LS_DATA_SRC) |  |  |  |
| GITEA_CMT_HASH | 커밋해시 | VARCHAR | 64 | Y |  |  | Y |  | UQ |
| VERSION_NO | 버전번호 | NUMBER | 10 | Y |  |  |  |  |  |
...
| 속성명 | 동의어 | 타입 | 길이 | NOT NULL | PK | FK | INX | 기본값 | 제약조건 |
|---|---|---|---|---|---|---|---|---|---|
| RAW_SN | 영상SN | NUMBER | 19 | Y | Y | Y(LS_DATA_RAW) |  |  | PK+FK |
...
| REVIEW_COMPLETED_AT | 검수완료시각 | TIMESTAMP | - |  |  |  |  |  |  |
| REG_DT | 등록시각 | TIMESTAMP | - | Y |  |  |  | CURRENT_TIMESTAMP |  |
| UPD_DT | 수정시각 | TIMESTAMP | - |  |  |  |  |  |  |
...
| 속성명 | 동의어 | 타입 | 길이 | NOT NULL | PK | FK | INX | 기본값 | 제약조건 |
|---|---|---|---|---|---|---|---|---|---|
| ASSIGNMENT_ID | 배정ID | NUMBER | 19 | Y | Y |  |  | SEQ_LS_TASK_ASSIGNMENT | PK |
| RAW_SN | 영상SN | NUMBER | 19 | Y |  | Y(LS_DATA_RAW) | Y |  |  |
| WORKER_USER_NO | 작업자 | NUMBER | 19 | Y |  | Y(MNG_ACCT_USER) | Y |  |  |
| ASSIGNER_USER_NO | 배정자 | NUMBER | 19 | Y |  | Y(MNG_ACCT_USER) |  |  |  |
| STATUS_CD | 상태 | VARCHAR | 16 | Y |  |  | Y | 'ASSIGNED' | CHK in (ASSIGNED,IN_PROGRESS,DONE,CANCELLED) |
| ASSIGNED_AT | 배정시각 | TIMESTAMP | - | Y |  |  |  | CURRENT_TIMESTAMP |  |
| DUE_DT | 마감일 | TIMESTAMP | - |  |  |  |  |  |  |
...
| 속성명 | 동의어 | 타입 | 길이 | NOT NULL | PK | FK | INX | 기본값 | 제약조건 |
|---|---|---|---|---|---|---|---|---|---|
| ISSUE_SN | 이슈SN | NUMBER | 19 | Y | Y |  |  | SEQ_LS_DATA_ISSUE | PK |
| RAW_SN | 영상SN | NUMBER | 19 | Y |  | Y(LS_DATA_RAW) | Y |  |  |
| SRC_SN | 프레임SN | NUMBER | 19 |  |  | Y(LS_DATA_SRC) |  |  |  |
| REVIEWER_USER_NO | 검수자 | NUMBER | 19 | Y |  | Y(MNG_ACCT_USER) |  |  |  |
...
| 속성명 | 동의어 | 타입 | 길이 | NOT NULL | PK | FK | INX | 기본값 | 제약조건 |
|---|---|---|---|---|---|---|---|---|---|
| LOCK_SN | 잠금SN | NUMBER | 19 | Y | Y |  |  | SEQ_LS_AUTH_WORK_LOCK | PK |
| RAW_SN | 영상SN | NUMBER | 19 | Y |  | Y(LS_DATA_RAW) | Y |  | UQ(RAW_SN) — 1영상 1잠금 |
| OWNER_ID | 소유자ID | VARCHAR | 30 | Y |  |  |  |  |  |
| REASON_CD | 사유코드 | VARCHAR | 16 |  |  |  |  |  | CHK in (REDEIDENT,LABEL,REVIEW) |
| LOCKED_AT | 잠금시각 | TIMESTAMP | - | Y |  |  |  | CURRENT_TIMESTAMP |  |
...
| 속성명 | 동의어 | 타입 | 길이 | NOT NULL | PK | FK | INX | 기본값 | 제약조건 |
|---|---|---|---|---|---|---|---|---|---|
| META_REVIEW_SN | 메타검수SN | NUMBER | 19 | Y | Y |  |  | SEQ_LS_DATA_META_REVIEW | PK |
| META_SN | 메타SN | NUMBER | 19 | Y |  | Y(LS_DATA_META) | Y |  |  |
| STATUS_CD | 상태 | VARCHAR | 16 | Y |  |  | Y | 'PENDING' | CHK in (PENDING,APPROVED,REJECTED) |
| REVIEWER_USER_NO | 검수자 | NUMBER | 19 |  |  | Y(MNG_ACCT_USER) |  |  |  |
| REASON | 사유 | VARCHAR | 2000 |  |  |  |  |  |  |
| REVIEWED_AT | 검수시각 | TIMESTAMP | - |  |  |  |  |  |  |
...
| 속성명 | 동의어 | 타입 | 길이 | NOT NULL | PK | FK | INX | 기본값 | 제약조건 |
|---|---|---|---|---|---|---|---|---|---|
| DATA_AUG_SN | 증강SN | NUMBER | 19 | Y | Y |  |  | SEQ_LS_DATA_AUG | PK |
| JOB_ID | 잡ID | NUMBER | 19 | Y |  |  | Y |  |  |
| SOURCE_SRC_SN | 원본프레임SN | NUMBER | 19 | Y |  | Y(LS_DATA_SRC) | Y |  |  |
...
| 속성명 | 동의어 | 타입 | 길이 | NOT NULL | PK | FK | INX | 기본값 | 제약조건 |
|---|---|---|---|---|---|---|---|---|---|
| LABEL_ID | 라벨ID | NUMBER | 19 | Y | Y |  |  | SEQ_LS_LABEL | PK |
...
| 속성명 | 동의어 | 타입 | 길이 | NOT NULL | PK | FK | INX | 기본값 | 제약조건 |
|---|---|---|---|---|---|---|---|---|---|
| PRESET_ID | 프리셋ID | NUMBER | 19 | Y | Y |  |  | SEQ_LS_LABEL_PRESET | PK |
| PRESET_NAME | 프리셋명 | VARCHAR | 128 | Y |  |  | Y |  |  |
| DESCRIPTION | 설명 | VARCHAR | 500 |  |  |  |  |  |  |
| EVENT_TYPE_CD | 이벤트유형 | VARCHAR | 32 |  |  |  | Y |  | UQ(EVENT_TYPE_CD) — 이벤트당 1:1 |
| OPTIONS_JSON | 옵션JSON | CLOB | - |  |  |  |  |  |  |
| REG_DT | 등록시각 | TIMESTAMP | - |  |  |  |  | CURRENT_TIMESTAMP |  |
| UPD_DT | 수정시각 | TIMESTAMP | - |  |  |  |  |  |  |
...
| 속성명 | 동의어 | 타입 | 길이 | NOT NULL | PK | FK | INX | 기본값 | 제약조건 |
|---|---|---|---|---|---|---|---|---|---|
| QUE_SN | 큐SN | NUMBER | 19 | Y | Y |  |  | SEQ_MNG_CLIP_SCHEDULE_QUE | PK |
| RAW_SN | 영상SN | NUMBER | 19 | Y |  | Y(LS_DATA_RAW) | Y |  |  |
| STATUS_CD | 상태 | VARCHAR | 16 | Y |  |  | Y | 'WAITING' | CHK in (WAITING,PROCESSING,DONE,FAILED) |
| LOCKED_BY | 잠금주체 | VARCHAR | 30 |  |  |  |  |  |  |
| LOCKED_AT | 잠금시각 | TIMESTAMP | - |  |  |  |  |  | SELECT FOR UPDATE SKIP LOCKED |
| REG_DT | 등록시각 | TIMESTAMP | - | Y |  |  |  | CURRENT_TIMESTAMP |  |
...
| 속성명 | 동의어 | 타입 | 길이 | NOT NULL | PK | FK | INX | 기본값 | 제약조건 |
|---|---|---|---|---|---|---|---|---|---|
| LOG_SN | 로그SN | NUMBER | 19 | Y | Y |  |  | SEQ_LS_DEIDENT_PROC_LOG | PK |
| RAW_SN | 영상SN | NUMBER | 19 | Y |  | Y(LS_DATA_RAW) | Y |  |  |
...
| 속성명 | 동의어 | 타입 | 길이 | NOT NULL | PK | FK | INX | 기본값 | 제약조건 |
|---|---|---|---|---|---|---|---|---|---|
| REPORT_SN | 신고SN | NUMBER | 19 | Y | Y |  |  | SEQ_LS_DEIDENT_REPORT | PK |
| SRC_SN | 프레임SN | NUMBER | 19 | Y |  | Y(LS_DATA_SRC) | Y |  |  |
| RAW_SN | 영상SN | NUMBER | 19 | Y |  | Y(LS_DATA_RAW) | Y |  |  |
| REPORTER_USER_NO | 신고자 | NUMBER | 19 | Y |  | Y(MNG_ACCT_USER) |  |  |  |
...
| 속성명 | 동의어 | 타입 | 길이 | NOT NULL | PK | FK | INX | 기본값 | 제약조건 |
|---|---|---|---|---|---|---|---|---|---|
| FALLBACK_SN | 폴백SN | NUMBER | 19 | Y | Y |  |  | SEQ_LS_CONTROL_NOTIFY_FALLBACK | PK |
| EVENT_TYPE | 이벤트유형 | VARCHAR | 20 | Y |  |  |  |  | CHK in (TASK_COMPLETED,TASK_MODIFIED) |
| RAW_SN | 영상SN | NUMBER | 19 | Y |  |  | Y |  |  |
| REQUEST_ID | 요청ID | VARCHAR | 40 | Y |  |  |  |  | UQ(REQUEST_ID, CHUNK_INDEX) |
| CHUNK_INDEX | 청크인덱스 | NUMBER | 5 | Y |  |  |  | 1 |  |
| CHUNK_TOTAL | 청크총개수 | NUMBER | 5 | Y |  |  |  | 1 |  |
| PAYLOAD_JSON | 페이로드 | CLOB | - | Y |  |  |  |  |  |
| RETRY_COUNT | 재시도횟수 | NUMBER | 3 | Y |  |  |  | 0 | max=5 |
| LAST_TRIED_AT | 최근시도시각 | TIMESTAMP | - |  |  |  | Y |  |  |
| STATUS_CD | 상태 | VARCHAR | 10 | Y |  |  | Y | 'PENDING' | CHK in (PENDING,SUCCESS,DEAD) |
| REG_DT | 등록시각 | TIMESTAMP | - | Y |  |  |  | CURRENT_TIMESTAMP |  |
| MDFCN_DT | 수정시각 | TIMESTAMP | - |  |  |  |  |  |  |
...
| 속성명 | 동의어 | 타입 | 길이 | NOT NULL | PK | FK | INX | 기본값 | 제약조건 |
|---|---|---|---|---|---|---|---|---|---|
| ATTR_ID | 속성ID | NUMBER | 19 | Y | Y |  |  | SEQ_LS_LABEL_ATTR | PK |
| LABEL_ID | 라벨ID | NUMBER | 19 | Y |  | Y(LS_LABEL) | Y |  |  |
...
|---|---|---|---|
| **관련 클래스 ID** | KLID-AT-DC-022 | **관련 클래스명** | LabelService |
| **엔티티 설명** | 라벨 생성·수정·삭제 이력. SNAPSHOT_JSON으로 변경 전 상태 보관. 12개월 후 cold archive 권장 | | |

| 속성명 | 동의어 | 타입 | 길이 | NOT NULL | PK | FK | INX | 기본값 | 제약조건 |
|---|---|---|---|---|---|---|---|---|---|
| LBL_HSTRY_SN | 라벨이력SN | NUMBER | 19 | Y | Y |  |  | SEQ_LS_DATA_LBL_HSTRY | PK |
| LBL_SN | 라벨SN | NUMBER | 19 | Y |  | Y(LS_DATA_LBL) | Y |  |  |
| CHG_TYPE_CD | 변경유형 | VARCHAR | 8 | Y |  |  |  |  | CHK in (I,U,D) |
| CHG_DT | 변경시각 | TIMESTAMP | - | Y |  |  | Y |  |  |
| CHG_USER_NO | 변경자 | NUMBER | 19 | Y |  | Y(MNG_ACCT_USER) |  |  |  |
| SNAPSHOT_JSON | 스냅샷JSON | CLOB | - |  |  |  |  |  | 변경 전 라벨 전체 직렬화 |
...
| 속성명 | 동의어 | 타입 | 길이 | NOT NULL | PK | FK | INX | 기본값 | 제약조건 |
|---|---|---|---|---|---|---|---|---|---|
| ATTR_VAL_SN | 속성값SN | NUMBER | 19 | Y | Y |  |  | SEQ_LS_DATA_LBL_ATTR_VAL | PK |
| LBL_SN | 라벨SN | NUMBER | 19 | Y |  | Y(LS_DATA_LBL) | Y |  |  |
| ATTR_ID | 속성ID | NUMBER | 19 | Y |  | Y(LS_LABEL_ATTR) | Y |  |  |
...
| 속성명 | 동의어 | 타입 | 길이 | NOT NULL | PK | FK | INX | 기본값 | 제약조건 |
|---|---|---|---|---|---|---|---|---|---|
| AUG_RVW_SN | 증강검수SN | NUMBER | 19 | Y | Y |  |  | SEQ_LS_DATA_AUG_RVW | PK |
| DATA_AUG_SN | 증강SN | NUMBER | 19 | Y |  | Y(LS_DATA_AUG) | Y |  |  |
| DECISION_CD | 판정 | VARCHAR | 16 | Y |  |  |  |  | CHK in (ACCEPTED,REJECTED) |
| REVIEWER_USER_NO | 검수자 | NUMBER | 19 | Y |  | Y(MNG_ACCT_USER) |  |  |  |
| REASON | 사유 | VARCHAR | 2000 |  |  |  |  |  |  |
| REVIEWED_AT | 검수시각 | TIMESTAMP | - | Y |  |  |  | CURRENT_TIMESTAMP |  |
...
| 속성명 | 동의어 | 타입 | 길이 | NOT NULL | PK | FK | INX | 기본값 | 제약조건 |
|---|---|---|---|---|---|---|---|---|---|
| DATA_AUG_SN | 증강SN | NUMBER | 19 | Y | Y | Y(LS_DATA_AUG) |  |  | 복합PK |
| SOURCE_LBL_SN | 원본라벨SN | NUMBER | 19 | Y | Y | Y(LS_DATA_LBL) |  |  | 복합PK |
| AUG_LBL_SN | 증강라벨SN | NUMBER | 19 | Y |  | Y(LS_DATA_LBL) |  |  |  |
...
| 속성명 | 동의어 | 타입 | 길이 | NOT NULL | PK | FK | INX | 기본값 | 제약조건 |
|---|---|---|---|---|---|---|---|---|---|
| META_HSTRY_SN | 메타이력SN | NUMBER | 19 | Y | Y |  |  | SEQ_LS_DATA_META_HSTRY | PK |
| META_SN | 메타SN | NUMBER | 19 | Y |  | Y(LS_DATA_META) | Y |  |  |
| CHG_TYPE_CD | 변경유형 | VARCHAR | 8 | Y |  |  |  |  | CHK in (I,U,D) |
| CHG_DT | 변경시각 | TIMESTAMP | - | Y |  |  | Y |  |  |
| CHG_USER_NO | 변경자 | NUMBER | 19 | Y |  | Y(MNG_ACCT_USER) |  |  |  |
| SNAPSHOT_JSON | 스냅샷JSON | CLOB | - |  |  |  |  |  | 변경 전 메타 전체 직렬화 |
...
| 속성명 | 동의어 | 타입 | 길이 | NOT NULL | PK | FK | INX | 기본값 | 제약조건 |
|---|---|---|---|---|---|---|---|---|---|
| SRC_HSTRY_SN | 프레임이력SN | NUMBER | 19 | Y | Y |  |  | SEQ_LS_DATA_SRC_HSTRY | PK |
| SRC_SN | 프레임SN | NUMBER | 19 | Y |  | Y(LS_DATA_SRC) | Y |  |  |
| CHG_TYPE_CD | 변경유형 | VARCHAR | 8 | Y |  |  |  |  | CHK in (I,U,D) |
| CHG_DT | 변경시각 | TIMESTAMP | - | Y |  |  | Y |  |  |
| CHG_USER_NO | 변경자 | NUMBER | 19 | Y |  | Y(MNG_ACCT_USER) |  |  |  |
| SNAPSHOT_JSON | 스냅샷JSON | CLOB | - |  |  |  |  |  | 변경 전 프레임 정보 직렬화 |
...
| 속성명 | 동의어 | 타입 | 길이 | NOT NULL | PK | FK | INX | 기본값 | 제약조건 |
|---|---|---|---|---|---|---|---|---|---|
| RAW_HSTRY_SN | 영상이력SN | NUMBER | 19 | Y | Y |  |  | SEQ_LS_DATA_RAW_HSTRY | PK |
| RAW_SN | 영상SN | NUMBER | 19 | Y |  | Y(LS_DATA_RAW) | Y |  |  |
| CHG_TYPE_CD | 변경유형 | VARCHAR | 8 | Y |  |  |  |  | CHK in (I,U,D) |
| CHG_DT | 변경시각 | TIMESTAMP | - | Y |  |  | Y |  |  |
| CHG_USER_NO | 변경자 | NUMBER | 19 | Y |  | Y(MNG_ACCT_USER) |  |  |  |
| SNAPSHOT_JSON | 스냅샷JSON | CLOB | - |  |  |  |  |  | 변경 전 영상 정보 직렬화 |
...
| 속성명 | 동의어 | 타입 | 길이 | NOT NULL | PK | FK | INX | 기본값 | 제약조건 |
|---|---|---|---|---|---|---|---|---|---|
| DATASET_SN | 데이터셋SN | NUMBER | 19 | Y | Y |  |  | SEQ_LS_DATA_SET | PK |
| DATASET_NAME | 데이터셋명 | VARCHAR | 128 | Y |  |  | Y |  |  |
| DESCRIPTION | 설명 | VARCHAR | 500 |  |  |  |  |  |  |
| FORMAT_CD | 포맷 | VARCHAR | 32 | Y |  |  |  |  | CHK in (COCO,YOLO,CUSTOM) |
| CREATED_BY | 생성자 | NUMBER | 19 | Y |  | Y(MNG_ACCT_USER) |  |  |  |
...
| 속성명 | 동의어 | 타입 | 길이 | NOT NULL | PK | FK | INX | 기본값 | 제약조건 |
|---|---|---|---|---|---|---|---|---|---|
| HSTRY_SN | 이력SN | NUMBER | 19 | Y | Y |  |  | SEQ_LS_TASK_ASSIGN_HISTORY | PK |
| ASSIGNMENT_ID | 배정ID | NUMBER | 19 | Y |  | Y(LS_TASK_ASSIGNMENT) | Y |  |  |
| PREV_WORKER_USER_NO | 이전작업자 | NUMBER | 19 |  |  | Y(MNG_ACCT_USER) |  |  |  |
| NEW_WORKER_USER_NO | 신규작업자 | NUMBER | 19 | Y |  | Y(MNG_ACCT_USER) |  |  |  |
| REASON | 사유 | VARCHAR | 500 |  |  |  |  |  |  |
| REG_DT | 등록시각 | TIMESTAMP | - | Y |  |  |  | CURRENT_TIMESTAMP |  |
...
| 속성명 | 동의어 | 타입 | 길이 | NOT NULL | PK | FK | INX | 기본값 | 제약조건 |
|---|---|---|---|---|---|---|---|---|---|
| EVENT_SN | 이벤트SN | NUMBER | 19 | Y | Y |  |  | SEQ_LS_TASK_EVENT_LOG | PK |
| RAW_SN | 영상SN | NUMBER | 19 |  |  | Y(LS_DATA_RAW) | Y |  |  |
| EVENT_TYPE_CD | 이벤트유형 | VARCHAR | 32 | Y |  |  |  |  |  |
| PAYLOAD_JSON | 페이로드 | CLOB | - |  |  |  |  |  |  |
| REG_DT | 등록시각 | TIMESTAMP | - | Y |  |  |  | CURRENT_TIMESTAMP |  |
...
| 속성명 | 동의어 | 타입 | 길이 | NOT NULL | PK | FK | INX | 기본값 | 제약조건 |
|---|---|---|---|---|---|---|---|---|---|
| DEADLINE_SN | 마감SN | NUMBER | 19 | Y | Y |  |  | SEQ_LS_DEADLINE | PK |
| RAW_SN | 영상SN | NUMBER | 19 | Y |  | Y(LS_DATA_RAW) | Y |  |  |
| DEADLINE_DT | 마감일시 | TIMESTAMP | - | Y |  |  |  |  |  |
...
| 속성명 | 동의어 | 타입 | 길이 | NOT NULL | PK | FK | INX | 기본값 | 제약조건 |
|---|---|---|---|---|---|---|---|---|---|
| META_ID | 메타ID | NUMBER | 19 | Y | Y |  |  | SEQ_LS_META | PK |
...
| 속성명 | 동의어 | 타입 | 길이 | NOT NULL | PK | FK | INX | 기본값 | 제약조건 |
|---|---|---|---|---|---|---|---|---|---|
| ENROLLMENT_SN | 등록이력SN | NUMBER | 19 | Y | Y |  |  | SEQ_LS_RAW_DATA_ENROLLMENT | PK |
| RAW_SN | 영상SN | NUMBER | 19 | Y |  | Y(LS_DATA_RAW) | Y |  |  |
| ENROLLED_BY | 등록자 | NUMBER | 19 | Y |  | Y(MNG_ACCT_USER) |  |  |  |
| ENROLLED_AT | 등록시각 | TIMESTAMP | - | Y |  |  |  | CURRENT_TIMESTAMP |  |
...
| 속성명 | 동의어 | 타입 | 길이 | NOT NULL | PK | FK | INX | 기본값 | 제약조건 |
|---|---|---|---|---|---|---|---|---|---|
| PRESET_ID | 프리셋ID | NUMBER | 19 | Y | Y | Y(LS_LABEL_PRESET) |  |  | 복합PK |
| LABEL_ID | 라벨ID | NUMBER | 19 | Y | Y | Y(LS_LABEL) |  |  | 복합PK |
| SORT_ORDER | 정렬순서 | NUMBER | 5 | Y |  |  |  | 0 |  |
| USE_YN | 사용여부 | CHAR | 1 | Y |  |  |  | 'Y' | CHK(Y/N) |
...
| DESCRIPTION | 설명 | VARCHAR | 500 |  |  |  |  |  |  |
| UPD_DT | 수정시각 | TIMESTAMP | - |  |  |  |  | CURRENT_TIMESTAMP |  |
| MDFCN_ID | 수정자ID | VARCHAR | 30 |  |  |  |  |  |  |
...
| PROCESSED_AT | 처리시각 | TIMESTAMP | - | Y |  |  |  | CURRENT_TIMESTAMP |  |
...
| 속성명 | 동의어 | 타입 | 길이 | NOT NULL | PK | FK | INX | 기본값 | 제약조건 |
|---|---|---|---|---|---|---|---|---|---|
| FALLBACK_SN | 폴백SN | NUMBER | 19 | Y | Y |  |  | SEQ_LS_GITEA_FALLBACK_QUEUE | PK |
| RAW_SN | 영상SN | NUMBER | 19 | Y |  | Y(LS_DATA_RAW) | Y |  |  |
...
| 속성명 | 동의어 | 타입 | 길이 | NOT NULL | PK | FK | INX | 기본값 | 제약조건 |
|---|---|---|---|---|---|---|---|---|---|
| USER_NO | 사용자NO | NUMBER | 19 | Y | Y |  |  | SEQ_MNG_ACCT_USER | PK |
...
|---|---|---|---|
| **관련 클래스 ID** | KLID-AT-DC-053 | **관련 클래스명** | RoleClaimService |
| **엔티티 설명** | 권한 코드 마스터 (ADMIN/LABELER/REVIEWER 등) | | |
...
| 속성명 | 동의어 | 타입 | 길이 | NOT NULL | PK | FK | INX | 기본값 | 제약조건 |
|---|---|---|---|---|---|---|---|---|---|
| USER_NO | 사용자NO | NUMBER | 19 | Y | Y | Y(MNG_ACCT_USER) |  |  | 복합PK |
...
|---|---|---|---|
| **관련 클래스 ID** | KLID-AT-DC-047 | **관련 클래스명** | VideoQueryService |
| **엔티티 설명** | 지자체 코드 마스터. CCTV·영상과 연계되는 소형 코드 테이블 | | |
...
|---|---|---|---|
| **관련 클래스 ID** | KLID-AT-DC-047 | **관련 클래스명** | VideoQueryService |
| **엔티티 설명** | 이벤트 유형·CCTV 매핑·VMS 영상 메타 코드 마스터 군. 소형 테이블 3종 묶음 | | |
...
| RECORD_START | 녹화시작 | TIMESTAMP | - | Y |  |  |  |  |  |
| RECORD_END | 녹화종료 | TIMESTAMP | - |  |  |  |  |  |  |
| FILE_SIZE | 파일크기 | NUMBER | 19 |  |  |  |  |  | 단위: byte |

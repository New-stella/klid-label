...
|---|---|---|---|
| 설명 | 오토라벨 파이프라인이 SAM2 분할로 객체 외곽 경계에 밀착하는 폴리곤을 자동 생성한다. <br>- 프레임 내 자동 바운딩박스를 프롬프트로 AI 추론 서버 분할을 호출한다. <br>- 외곽 경계 폴리곤을 자동 라벨로 저장한다. <br>- 폴리곤 라벨 1건당 AI 정보(출처 SAM2)를 동일 트랜잭션에서 함께 저장한다. | | |
| 관련 컴포넌트 ID | KLID-AT-CO-005 | 관련 프로그램 ID | KLID-AT-IF-007 외곽 분할·IF-006 AI 추론 연동 |
> 출처: D11 UT-10(배치 단계 — SAM2 분할).

| 케이스 ID | 케이스 명 | 작업 권한 | 시험 데이터 | 시험항목 및 처리절차 | 예상결과 및 검증방법 | 시험 결과 |
|---------|---------|---------|-----------|---------|--------------|------|
| KLID-AT-UT-005-01 | DB BBOX 있으면 SAM2 호출 후 POLYGON 저장 + LsDataLblAiInfo(SRC SAM2) 동시 저장 | - | BBOX 있음·저장 결과 | 1. 오토라벨 파이프라인의 SAM2 분할 Step 실행(rawSn 대상) <br>2. 프레임 내 자동 BBOX를 프롬프트로 AI 추론 서버 분할 호출 후 폴리곤을 저장하고, 동일 트랜잭션에서 폴리곤 라벨 1건당 AI 정보(출처 SAM2)를 동시 저장 <br>: 배치 Step 단위 시험 — 외곽 경계 폴리곤 자동 라벨 생성 및 폴리곤 1건:AI정보 1건 매칭, DB 저장·조회로 확인 | · `AiServerClient.segment` 호출 발생 <br>· `SELECT LBL_TYPE_CD, AUTO_LBL_YN, POINT_CN, CONF_SCORE FROM LS_DATA_LBL WHERE SRC_SN={대상 프레임 srcSn} AND LBL_TYPE_CD='POLYGON'` → LBL_TYPE_CD='POLYGON'·AUTO_LBL_YN='Y'·POINT_CN=폴리곤 JSON·CONF_SCORE(setScale 4) <br>· `SELECT LBL_SRC_CD, DATA_LBL_SN, DATA_RAW_SN, DATA_SRC_SN, CONF_SCORE FROM LS_DATA_LBL_AI_INFO WHERE DATA_LBL_SN={저장 라벨 PK}` → 1행(LBL_SRC_CD='SAM2'·DATA_RAW_SN·DATA_SRC_SN·CONF_SCORE 채워짐) <br>· POLYGON 라벨 1건당 AiInfo 1건 매칭 <br>· 검증: 위 조회 POLYGON 행 저장 확인 + `SELECT COUNT(*) FROM LS_DATA_LBL_AI_INFO WHERE LBL_SRC_CD='SAM2' AND DATA_LBL_SN={저장 라벨 PK}` → 1(행 존재) | |

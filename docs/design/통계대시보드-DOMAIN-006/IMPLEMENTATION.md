...
[상태 축] 배정·작업중·완료·반려 기준으로 집계한다. 구 상태 '확인요청'은 폐기됐다 — 검수 승인이 곧 작업 종결이라 그 위에 확인 단계를 두지 않는다. ⚠ 구 근거 '관리자 역할 통합(ADR-003)' 은 ADR-055 로 뒤집혔다(역할 4종 + 계층). 폐기 결론은 그대로다.
...
| # | 단계 | 이번 키트 ITEM |
|---|---|---|
| 1 | ADR 결정·제약 | [[ADR-001]] · [[ADR-019]] · [[ADR-038]] · [[ADR-055]] |

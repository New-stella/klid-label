...
sequenceDiagram
    ...
    S->>V: 버전 목록 조회
    S-->>C: 버전 목록
    U->>C: 버전 단건 선택(작업본 대비 비교 요청)
    C->>S: 작업본 대비 차이 산출 위임
    S->>S: 기준 스냅샷과 현재 작업본 비교(추가·수정·삭제 산출)
    S-->>C: 차이 결과
    U->>C: 두 버전 비교 요청
    C->>S: 차이 산출 위임
    ...
    U->>C: 선택 버전 복구 요청
    C->>S: 복구 위임
    S->>V: 대상 스냅샷 행을 다시 활성 버전으로 전환(새 버전 미적층)
    S-->>C: 복구 결과
    C-->>U: 복구 완료 안내
```
...
sequenceDiagram
    ...
    Note over S: 라벨 작업 결과는 삭제하지 않고 보존하며<br/>신고 구간 동안 조회·저장을 차단한다
    S->>R: 신고 등록(미처리)
    S-->>C: 신고 접수 결과
...
classDiagram
    class VersionController {
        ...
        +diffWithWorking(versionHash, actor) DiffResult
...
| **오퍼레이션** |
| 오퍼레이션명 | 가시성 | 파라미터 | 반환타입 | 설명 |
| report | public | 프레임 식별자, 사유, 인증 주체 | 신고 식별자 | 신고를 등록하고 작업락·비식별 실패를 표시한다(라벨 작업 결과는 삭제하지 않고 보존한다) |
...
| diffWithWorking | public | 기준 버전 식별자, 인증 주체 | 차이 결과 | 기준 버전과 현재 작업본의 차이를 조회한다 |
| rollback | public | 버전 식별자, 복구 요청, 인증 주체 | 복구 결과 | 선택 버전 스냅샷을 다시 활성 버전으로 전환해 복구한다 |
...
| diffWithWorking | public | 기준 버전 해시, 인증 주체 | 차이 결과 | 기준 스냅샷과 현재 작업본의 차이를 산출한다(작업본은 승인 스냅샷과 동일한 방식으로 구성하며 저장하지 않는다) |
| rollback | public | 버전 해시, 프레임 식별자, 인증 주체 | 버전 | 대상 스냅샷 행을 다시 활성 버전으로 전환하고 라벨 본문을 작업본으로 보존 복원한다(새 버전을 적층하지 않으며, 이미 대상 스냅샷이 활성이면 아무 것도 하지 않는다) |

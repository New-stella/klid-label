...
|---|---|---|---|
| 설명 | 검수자의 검수 승인 시점에 라벨 스냅샷을 저장하고 변경 이력을 적재하며 관제서버로 완료·수정을 통지한다. <br>- 승인 시 프레임별 현재 라벨 전체를 DB 스냅샷(APPROVED 버전)으로 저장한다. <br>- 라벨 삭제 등 변경 이력을 프레임 단위로 적재하고 최신순으로 조회한다. <br>- 검수 완료·수정 시 관제서버로 완료·수정 통지를 송신하되 라벨/메타 본문은 포함하지 않는다. | | |
...
| KLID-AT-UT-007-02 | 검수 완료·수정 통지 발송 | - | 통지 페이로드 | 1. 검수 완료(승인) 또는 검수 완료 후 라벨/메타 수정을 발생시킨다 <br>2. 관제서버(수신처 mock)로 완료(TASK_COMPLETED)·수정(TASK_MODIFIED) 통지가 발송되는지 수신 로그로 확인 <br>3. 정상 발송(200) 후 재전송 대기 큐(폴백)에 남지 않는지 확인 <br>: 관제 수신처의 수신 기록·발송 성공 지표로 발송 여부를 관찰 | · 예외 없이 정상 반환, `metrics.incrementCompletedSuccess()`/`incrementModifiedSuccess()` 증가 <br>· 실패 시에만 `fallbackService.enqueuePending(requestId, eventType, rawSn, payload)` 로 폴백 큐 적재(정상 200 이면 미적재) <br>· 검증: 응답 200 + 폴백 큐 미적재 확인 | |
| KLID-AT-UT-007-03 | 통지 본문 필수 항목 | - | 페이로드 | 1. 완료·수정 통지를 발송하고 관제 수신처가 받은 통지 본문을 확인한다 <br>2. 이벤트 타입(TASK_COMPLETED/TASK_MODIFIED)·작업 ID(RAW_SN)·요청 ID(멱등키)·변경 프레임/변경 종류 등 필수 항목이 포함되는지 확인한다 <br>3. 라벨/메타 본문·PII·토큰·원본 이미지 경로가 포함되지 않는지 확인한다 <br>: 통지 본문의 필수 항목 유무와 금지 항목 미포함(CWE-359)을 수신 본문으로 관찰 | · eventType 고정값("TASK_COMPLETED"/"TASK_MODIFIED"), requestId=UUID(idempotency) <br>· TaskModifiedPayload.changeTypes ⊆ {LABEL_ADDED, LABEL_UPDATED, LABEL_DELETED, META_UPDATED}(ChangeType.ALL), frameIds=SRC_SN 목록 <br>· PII·토큰·라벨/메타 본문·원본 이미지 경로 미포함(CWE-359) <br>· 검증: record 필드 값 assert | |
| KLID-AT-UT-007-04 | 승인/수정 이벤트별 통지 분기 | - | 이벤트 | 1. 검수 승인을 완료하면(트랜잭션 커밋 후) 완료 통지가 1회 발송되는지 확인한다 <br>2. 검수 완료 후 같은 영상 라벨을 여러 번 수정한 뒤, 중복 프레임이 합쳐진 수정 통지가 1회 발송되는지 확인한다 <br>3. 변경이 롤백된 경우 통지가 발송되지 않는지 확인한다 <br>: 승인 이벤트→완료 통지, 수정 이벤트→디바운스 후 수정 통지로 발송이 분기됨을 수신 로그로 관찰 | · ReviewApprovedEvent 수신 시 sendCompleted 1회 호출 <br>· TaskModifiedEvent 수신 시 rawSn 윈도우에 frameIds/changeTypes 누적(중복 srcSn 합쳐짐), 윈도우 만료 후 sendModified 1회 flush <br>· 롤백 트랜잭션에서는 AFTER_COMMIT 미호출 <br>· 검증: mock notifyService/debouncer 호출 횟수·인자 검증 | |
...
| KLID-AT-UT-008-02 | 배정된 WORKER 본인 프레임 rollback시 대상 스냅샷 복원 새 ROLLBACK 버전 / 복원 후 LS_DATA_LBL이 스냅샷과 일치 | WORKER | 스냅샷·본인 프레임 | 1. 버전 관리 화면에서 최신이 아닌 이전 버전을 선택한다 <br>2. 롤백 버튼을 눌러 확인 후 해당 스냅샷으로 복원한다 <br>: 본인에게 배정된 영상만 롤백 가능하며 타인 배정·작업락 영상은 거부된다. 대상 스냅샷 라벨로 프레임 라벨을 교체하고 새 활성 ROLLBACK 버전 이력 1건을 생성하며, 검수 완료(APPROVED) 영상이면 수정 통지가 발행된다. 복원 후 라벨이 스냅샷과 일치함을 확인한다. | · `SELECT SAVE_REASON_CD, ACTVTN_YN FROM LS_LABEL_VERSION WHERE DATA_SRC_SN={대상 프레임} AND ACTVTN_YN='Y'` → SAVE_REASON_CD='ROLLBACK', 1행(동일 해시면 기존 행 재활성, 이전 active 는 'N') <br>· `SELECT LBL_TYPE_CD, LBL_NM, POINT_CN FROM LS_DATA_LBL WHERE SRC_SN={대상 프레임}` → 대상 스냅샷 라벨과 일치(lblTypeCd·label·좌표) — 라벨링 캔버스 / 검수완료 export 폴더 JSON 즉시 반영 <br>· 라벨 교체·버전 전환·통지가 동일 트랜잭션에 묶여 원자적 커밋/롤백 <br>· APPROVED 영상은 TASK_MODIFIED(LABEL_UPDATED) 발행 <br>· 검증: DB 조회로 LS_DATA_LBL vs 스냅샷 items 비교 + LS_LABEL_VERSION active 행(SAVE_REASON_CD/ACTVTN_YN) 확인 | |

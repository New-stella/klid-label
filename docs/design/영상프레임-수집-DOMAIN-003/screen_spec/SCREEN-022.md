...
synced_at: 2026-08-16T14:48:51.363Z
status: NEW
prev_version: null
content_hash: c099817c649dbc3369509308bdbe2c219f8f113025d51f33f6d4833c2b440e65
stale: true
...
[증강 3종 — 생성 조건(prompt) 필수] 증강을 고르면 5필드(time/season/weather/terrain/severity) 입력 폼이 나타나며 전부 필수·자유 문자열이다(선택지가 아니다 — 셀렉트로 좁히지 않는다). 검증은 ①공백만 입력한 값 거부(오류: "필수 입력입니다.") ②50자 이내(오류: "50자 이내로 입력하세요.") ③제어문자·보이지 않는 문자(개행·NBSP·ZWSP·BOM 등) 제거 후 저장뿐이며 그 외 가공(요약·재구성)은 하지 않는다 — 정규화된 값이 외부 생성형 AI 로 전송되고 LS_DATA_AUG.PROMPT_CN 에 보관된다. 필드별 오류는 개별 표시하며 입력을 고치면 그 필드의 오류만 지운다. 증강 종류는 prompt 에서 파생하지 않고 사용자가 카드로 고른 값이 단일 원천이다. 해상도 변경은 외부 위탁이 아니라 prompt 를 받지 않는다.
...
- **note**: 증강 3종을 선택한 상태에서 생성 조건 5필드가 아직 유효하지 않아 처리 요청 버튼이 비활성일 때만 노출 — 이유를 알려 버튼이 왜 안 눌리는지 찾지 못하는 상황을 막는다(RESOLUTION 종류에는 해당 없음).
- **type**: Text
- **label**: 생성 조건 미입력 경고 문구

...
status: CHANGED
prev_version: 102
content_hash: db9a4914cc76fc3e6030e7f37939209e377f6c3df3d13488f4270e3a20535552
stale: true
...
- **note**: 편집 가능한 필드다. 외부 시계열 위탁 때 실제로 보낸 질문 문구로 미리 채워진다 — 마킹에서 고른 질문이 1순위이고, 없거나 그 유형의 질문이 아니면 그 유형의 첫 번째 질문을 쓰며, 검증 이벤트 유형이 미수신이어도 마킹에서 고른 질문이 있으면 그 질문을 쓴다. 위탁 이후에 질문 목록이 바뀌어도 이 칸은 그때 보낸 문구를 그대로 보여준다. 고른 질문이 없어 위탁에 실을 문구를 얻지 못한 영상은 비어 있다.
- **type**: Textarea
- **label**: 질의

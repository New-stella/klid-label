...
synced_at: 2026-09-15T15:35:51.867Z
status: NEW
prev_version: null
...
links:
  ...
  covered_by: ["[[AC-1113]]", "[[AC-1114]]", "[[AC-1116]]", "[[AC-1117]]", "[[AC-1124]]", "[[AC-1126]]"]
  implements: ["[[IMPREC-334]]", "[[IMPREC-463]]"]
  realizes: ["[[UC-023]]", "[[UC-043]]"]
  ...
  references_backward: ["[[AC-1124]]", "[[AC-1126]]", "[[ADR-069]]", "[[TEST-006]]", "[[TEST-007]]", "[[UC-023]]", "[[UC-043]]"]
---
...
- **description**: KPI 4카드(검수 대기/검수중/승인/반려)는 GET /v1/reviews/summary(API-138) 서버 집계로 필터 결과 전체 기준이며 현재 페이지 20건 안에서 세지 않는다. 카드에 장식 아이콘을 두지 않는다 — 지표명과 값 텍스트가 카드의 정보를 모두 전달한다. 로딩/에러/정상 3상태 분기, 집계 실패해도 목록 표시는 막지 않는다. 카드 클릭은 status 필터를 토글(같은 카드 재클릭 시 해제=전체). 이 집계는 제외 처리된 영상을 빼고 센다 — 목록에서 빠진 것이 집계에는 남아 있으면 두 숫자가 어긋난다. 목록 표 위의 「제외됨 N건」도 이 집계에서 받는 값이며, 목록 조회 창구는 그 숫자를 싣지 않는다.
...
- **note**: 제외 처리된 영상은 기본 목록에서 빠지며 건수와 페이지도 그만큼 줄어든다. 「제외됨 N건」을 누르면 같은 표에 제외분만 그리되 검수 동작은 노출하지 않는다 — 행 액션 버튼과 선택 체크칸을 두지 않아 제외된 영상을 검수 대상으로 되돌리는 길이 화면에 열리지 않는다. 이 화면에서는 제외·복원을 수행하지 않는다 — 그 자리는 영상 처리 현황 화면이다.
- **type**: Custom
- **label**: 제외분만 보는 목록
...
- **note**: 제외분만 보는 목록으로 전환된 동안에만 둔다. 기본 목록에서는 보이지 않는다.
- **type**: Text
- **label**: 제외분만 보는 중입니다.
...
- **note**: 제외분만 보는 목록에서 기본 목록으로 되돌린다. 제외분만 보는 동안에만 둔다.
- **type**: Button
- **label**: 기본 목록으로

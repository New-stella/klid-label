...
synced_at: 2026-09-17T01:12:25.192Z
status: CHANGED
prev_version: 54
content_hash: f58a2aeff95f02bbc2ac2061509f7c0c7601b185b843e5b4e3b42cfcf21e41be
stale: true
...
links:
  ...
  designs_backward: ["[[SD-012]]"]
  granted_on_backward: ["[[ROLE-002]]"]
  navigates_to_backward: ["[[NAV-001]]"]
  realizes_backward: ["[[MOD-004]]"]
  references_backward: ["[[ADR-071]]", "[[SEQ-036]]", "[[TEST-002]]", "[[UC-019]]"]
---
...
[영상 재생 주소 구성] 최종 재생 주소는 재생 주소 발급 응답의 주소(배포 접두를 포함하지 않는 API 기준 경로) 앞에 이 화면이 아는 배포 접두를 붙여 구성한다 — 접두 결합 책임은 이 화면 한 곳에 있고, 붙이지 않으면 요청이 다른 경로 공간으로 나가 영상이 재생되지 않는다. 재생 인증(쿠키)이 거부되는 등 재생에 실패하면 재생 주소를 다시 받아 영상 소스를 바꾸되, 직전 재생 위치를 기억했다가 새 소스의 메타데이터가 불러와진 시점에 복원해 사용자가 보던 자리에서 계속 보게 한다. 재발급·재시도는 상한까지만 하고 이후 재생 실패를 알린다. 이 기준들은 API-114 가 정한다.
...
- **note**: GET /v1/videos/{rawSn}/stream-url 로 재생 주소를 먼저 받아 <video src> 에 바인딩(HTTP Range). <video> 는 Authorization 헤더를 못 붙여 인증 스트림을 직접 재생하지 못한다. 재생 인증은 발급 응답과 함께 내려온 쿠키(klid_stream_nonce)로 판정되며 브라우저가 자동으로 싣는다 — 동일 오리진이면서 쿠키가 실리는 경로여야 한다. 인증 거부 등 재생 오류 시 재발급받아 소스를 바꾸고, 직전 재생 위치를 기억했다가 새 소스의 loadedmetadata 시점에 복원한다(교체 뒤 일시정지라 재생 시작을 기다리면 복원되지 않는다). 만료 시각·유효 초는 재발급 판단에 쓰지 않는다. 발급 주소 앞에 배포 접두를 붙인다. 재발급·재시도는 상한까지만 하고 이후 재생 실패를 알린다.
- **type**: Custom
- **label**: 영상 플레이어
...
VideoPlayer — 먼저 GET /v1/videos/{rawSn}/stream-url(재생 주소 발급)을 조회하고 응답의 재생 주소를 <video src>에 바인딩해 HTTP Range 로 재생한다(<video> 태그는 Authorization 헤더를 붙일 수 없어 인증이 걸린 /stream 을 직접 재생할 수 없다 — 별도 fetch 없는 직접 src 바인딩이 아니다). 재생 구간(부분 요청 포함)의 인증은 발급 응답과 함께 내려온 쿠키(klid_stream_nonce)로 판정한다. 쿠키는 스크립트가 다룰 수 없고 브라우저가 자동으로 실으므로 재생 요청은 동일 오리진이면서 쿠키가 실리는 경로여야 한다. 쿠키 속성 전문은 API-114 가 정한다. 재생/일시정지 토글, 배속 버튼(0.25/0.5/1/1.5/2/4x), seek range bar, 현재시각/총길이(mm:ss) 표시. 현재 재생 시각·현재 프레임(영상 실측 fps 기준 환산, 미상 시 30 폴백)·특정 지점 이동 기능을 다른 컴포넌트에 노출한다.

[재생 주소 구성] 발급 주소는 배포 접두를 포함하지 않는 API 기준 경로라 이 화면의 배포 접두를 앞에 붙여 최종 주소를 만든다(재발급 주소도 동일). 붙이지 않으면 스트림에 도달하지 못해 재생되지 않는다.

[재발급 시 재생 위치 보존] 쿠키가 거부되는 등 재생에 실패하면 stream-url 을 재발급받아(쿠키도 새로 내려온다) 소스를 바꾼다. 바꾸기 직전 재생 위치를 기억했다가 새 소스의 메타데이터가 불러와진 시점에 복원한다 — 교체된 영상은 일시정지 상태라 재생 시작 신호를 기다리면 복원되지 않는다. 만료 시각·유효 초는 인증 수명이 아니라 재발급 시점 판단에 쓰지 않는다. 재발급·재시도는 상한까지만 하고 이후 재생 실패를 알린다.
...
- **source_hash**: a209e89eff9e59861a16b925de8b41aed7e216a5c666873841fe20111251a197
- **generated_at**: 2026-09-16T09:25:52.604Z

...
synced_at: 2026-09-07T13:24:03.809Z
status: CHANGED
prev_version: 49
content_hash: e3cb2537bf2a08643d47f5e559923a657af5ebd10d08ab0585e20e873188f326
stale: true
...
links:
  ...
  references_backward: ["[[SEQ-036]]", "[[TEST-002]]", "[[UC-019]]"]
---
...
- **note**: 관제가 보낸 검증 이벤트 유형이 없는 영상에서만 노출한다 — 관제 값이 있으면 이 선택 자체를 띄우지 않는다. 고를 수 있는 유형 목록은 화면 진입 시 함께 받아 싣는다. 고르면 그 유형의 질문 목록이 따라 열리고 정렬순서 첫 번째 질문이 기본 선택된다. 선택 사항이라 고르지 않아도 마킹을 완료할 수 있다. 고른 값은 마킹 완료 요청에 함께 실린다.
- **type**: Select
- **label**: 검증 이벤트 유형
...
- **note**: 그 영상의 검증 이벤트 유형에 등록된 질문 목록을 정렬순서 오름차순으로 싣고 첫 번째 질문을 기본으로 선택해 둔다. 유형 선택을 노출한 영상에서는 작업자가 유형을 고른 뒤에 목록이 열린다. 고른 질문의 일련번호가 마킹 완료 요청에 함께 실린다. 고를 질문이 하나도 없으면 이 드롭다운을 노출하지 않는다.
- **type**: Select
- **label**: 검증 질문
...
[검증 이벤트 유형 선택] 관제가 보낸 검증 이벤트 유형이 없는 영상에서만 유형 선택을 노출한다. 관제 값이 있으면 유형 선택을 아예 띄우지 않으므로 작업자 선택과 관제 값이 맞붙는 일이 구조적으로 생기지 않는다. 관제가 유형을 보내지 않은 영상은 고를 질문 목록이 비어 질문을 고를 수 없고 외부 시계열 위탁의 묘사 축도 유형이 없어 거부되어 시계열 메타를 하나도 받지 못하는데, 작업자가 유형을 고르면 그 두 가지가 함께 풀린다. 고를 수 있는 검증 이벤트 유형 목록은 화면 진입 시 함께 받아 드롭다운에 싣는다. 유형을 고르면 그 유형에 등록된 질문 목록이 따라 열리고 기본 선택은 그 목록의 정렬순서 첫 번째다. 유형은 선택 사항이라 고르지 않아도 마킹을 완료할 수 있으며, 고르지 않으면 질문도 시계열 메타도 받지 못하는 상태가 그대로 유지된다. 고른 유형은 마킹 완료 시 마킹과 함께 저장된다.

[검증 질문 선택] 그 영상의 검증 이벤트 유형에 등록된 질문 목록을 정렬순서 오름차순으로 받아 드롭다운에 싣고, 정렬순서 첫 번째 질문을 기본으로 선택해 둔다. 고른 질문의 일련번호(vrfcEvntQstnSn)는 마킹 완료 시 마킹과 함께 저장된다. 고를 질문이 하나도 없으면 질문 드롭다운은 노출하지 않는다. 질문이 없다고 마킹이나 외부 시계열 위탁이 막히지는 않으며 어노테이션의 질문 칸이 비어 있을 뿐이다. 선택값이 비었거나 그 유형에 속하지 않는 질문이면 서버가 그 유형의 첫 번째 질문으로 되돌리므로, 화면의 선택은 최종 판정이 아니라 서버 교정을 전제로 한 입력이다. 질문 문구가 길어 드롭다운에서 잘릴 수 있으므로 고른 질문의 전문을 아래에 그대로 보여 준다.

[작업자의 선택은 외부로 나간다] 작업자가 고른 질문 문구는 외부 시계열 위탁 요청 본문에 그대로 실려 나간다. 기록으로만 남는 값이 아니므로 화면이 그 사실을 작업자에게 안내한다.

...
synced_at: 2026-09-15T15:34:12.052Z
status: CHANGED
prev_version: 48
content_hash: 992ee8e03ba5df14d827cf268efc8314b9fbb3d6a9bb14dad285f78fed01e8c1
stale: false
...
links:
  ...
  references_backward: ["[[AC-1124]]", "[[AC-1126]]", "[[ADR-069]]", "[[SEQ-036]]", "[[TEST-001]]", "[[UC-011]]", "[[UC-018]]", "[[UC-038]]", "[[UC-043]]"]
---
...
- **note**: 0건이어도 상시 노출한다 — 감춘 사실 자체가 드러나야 한다. 이 화면에는 별도 집계 창구를 두지 않고 그 숫자를 목록 응답에서 받는다. 현재 페이지가 아니라 지금 걸린 필터 범위 전체의 값이라 눌렀을 때 나오는 건수와 일치한다. 누르면 제외분만 보는 목록으로 전환하고 다시 누르거나 되돌리면 기본 목록으로 돌아온다.
- **type**: Custom
- **label**: 제외됨 N건
...
★목록 위 '제외됨 N건'은 0건이어도 상시 표시하며 누르면 제외분만 보는 목록으로 전환한다. 그 목록에서는 복원 동작만 두고 배정·검수 동작을 노출하지 않는다 — 제외한 것을 작업 대상으로 만들 수 있으면 제외가 무의미해진다.
...
- **note**: 필수. 창구가 정한 상한을 따른다. 비우거나 공백만이면 확인 버튼이 비활성된다.
- **type**: Textarea
- **label**: 제외 사유
...
사유는 필수다 — 비우거나 공백만 넣으면 확인 버튼을 누를 수 없다. 무엇이 왜 보이지 않게 됐는지가 남아 있어야 나중에 되돌릴지 판단할 근거가 생기기 때문이다. 입력 칸 아래에 개인정보를 적지 말 것을 안내한다 — 적은 문구가 이력에 그대로 남고 이력 화면에 그대로 보인다. 길이 상한은 창구가 정한 값을 따른다.
...
- **description**: 
- **source_hash**: ce84f6920e188c06c2c06ec48802e80bc9a4006f9e93e7c7b83756c0ff8aa2c0
- **generated_at**: 2026-08-27T09:25:47.893Z

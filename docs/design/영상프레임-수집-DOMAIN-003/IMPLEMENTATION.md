...
### 남은 설계 공백 (사용자 결정 대상 — 임의로 만들지 않았다)

- **일괄 3 API 를 소비하는 화면이 없다.** `API-212`·`API-213`·`API-214` 의 역참조가 0건이다 — 검수자가 그 축을 어느 화면에서 쓰는지 설계에 없다. 영상 목록에서 다중 선택으로 부를지, 배치 실패 관리 축에 붙일지는 화면 결정이라 여기서 정하지 않았다.
- **승인 영상에 재수행 버튼을 노출할지**도 화면 결정이다. 계약은 열렸으나 `SCREEN-009` 는 그 노출 조건을 서술하지 않는다.

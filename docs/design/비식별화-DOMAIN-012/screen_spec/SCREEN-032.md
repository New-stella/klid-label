...
synced_at: 2026-09-14T05:33:11.380Z
status: CHANGED
prev_version: 28
content_hash: 7544e629653aa50541f34d224e0b366a265a70de19c54c29bd620f912c505039
stale: true
...
- **note**: 미처리 상태 행에만 노출. 클릭 시 비식별 산출물 선택 다이얼로그가 열리며, 거기서 고른 파일명을 실어 해소 처리를 요청한다. 서버 안전장치 — ①선택한 파일이 요청 시점에 다시 열거한 후보 목록에 있어야 하고 저장 서브트리 실경로·산출물 무결성 검증을 모두 통과해야 해소가 성립하며 실패 시 미처리 상태가 그대로 유지된다 ②동일 신고 2인 동시 해소 요청 시 하나만 성공하고 나머지는 실패로 처리된다(중복 실행 차단) ③성공한 요청만 작업락을 해제하고 선택한 산출물 경로를 비식별 처리 이력에 새 성공 행으로 적재한 뒤 신고 단계에 따라 재개 동작을 수행한다.
- **type**: Button
- **label**: 해소 처리
...
- **note**: 각 후보는 파일명·크기·수정시각을 함께 보여준다. 내부 저장 경로는 표시하지 않는다. 현재 이력에 기록된 산출물에는 그 사실을 알리는 표시를 단다. 그 표시는 구분을 위한 것이며 선택을 막지 않는다 — 후보는 만들어진 시각과 무관하게 모두 고를 수 있다. 신고 접수보다 먼저 만들어진 후보에는 「신고 이전 파일」 표시를 함께 단다. 이 표시도 선택을 막지 않는다 — 후보 자격이 만들어진 시각을 따지지 않아 비식별을 다시 수행하지 않고도 해소가 성립하므로, 마스킹 누락이 남은 산출물이 정상으로 되돌아올 수 있다는 사실을 사람이 알아볼 유일한 단서가 이 표시다. 정상적으로 열리는 영상 파일이 아닌 후보는 고를 수 없는 상태로 보여 주고 그 사유를 함께 보인다 — 「정상적으로 열리는 영상 파일이 아니라 선택할 수 없습니다. 외부 솔루션으로 비식별을 다시 수행해 주세요.」 사유 없이 잠긴 항목만 두지 않는다. 초기에는 아무것도 선택돼 있지 않다.
- **type**: Custom
...
- **description**: 
- **source_hash**: 29bcc5cb504f54cca74b93baa4ff476494dc7f251f300cfd723236d784db2b5c
- **generated_at**: 2026-09-10T00:42:43.965Z

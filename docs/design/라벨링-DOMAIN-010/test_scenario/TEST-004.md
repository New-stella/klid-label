...
synced_at: 2026-09-15T13:21:59.420Z
status: CHANGED
prev_version: 19
content_hash: 41e2344648d4f1a12158d28df00cef88cc4234cbded43842f2bf804cc05e4fa5
stale: true
raw: ./_raw/TEST-004.json
links:
  references: ["[[API-074]]", "[[API-075]]", "[[API-076]]", "[[DOMAIN-005]]", "[[DOMAIN-010]]", "[[DOMAIN-016]]", "[[SCREEN-005]]", "[[SCREEN-019]]", "[[UC-007]]", "[[UC-009]]"]
---
...
- **action**: 검수 승인
- **expected**: 영상이 검수 완료(DATA_STTS_CD='APPROVED') 상태로 전이되고 검수자 userNo와 그 행위 시점의 역할이 이벤트 로그에 기록된다. 【검증】DB: SELECT DATA_STTS_CD, VER FROM LS_RAW_DATA_STATUS WHERE RAW_DATA_ID=:rawDataId → DATA_STTS_CD='APPROVED'(IN_REVIEW→APPROVED); SELECT EVNT_TYPE_CD, ACTOR_USER_NO, ACTOR_ROLE_CD FROM LS_TASK_EVNT_LOG WHERE RAW_DATA_ID=:rawDataId AND EVNT_TYPE_CD='APPROVE' → ACTOR_USER_NO=검수자 userNo · ACTOR_ROLE_CD='REVIEWER' · 화면(SCREEN-019): '검수중' 배지→'승인' 버튼
- **test_item**: 검수 상세 화면에서 영상 단위 검수를 승인하고 승인자와 그 시점의 역할이 함께 기록되는지 확인
...
- **seq**: 2
- **note**: TC-009 | manual_test+DB SQL 확인 / 검수자 경로만 확인하면 역할이 행위자별로 갈리는지 알 수 없어 별도 단계로 둔다
- **action**: 관리자 권한으로 검수 승인
- **expected**: 관리자가 승인해도 검수자가 승인했을 때와 같은 결과가 나오되, 이력에 남는 역할은 계층으로 승격된 값이 아니라 관리자 자신의 역할이다. 【검증】DB: SELECT ACTOR_USER_NO, ACTOR_ROLE_CD FROM LS_TASK_EVNT_LOG WHERE RAW_DATA_ID=:rawDataId AND EVNT_TYPE_CD='APPROVE' → ACTOR_USER_NO=관리자 userNo · ACTOR_ROLE_CD='ADMIN'(검수자로 기록되면 불합격이다) · 화면(SCREEN-019): '검수중' 배지→'승인' 버튼
- **test_item**: 관리자가 승인한 건의 이력에 행위 시점 역할이 관리자로 남는지 확인
- **input_data**: 【영상 식별자】"1002" 【승인자(숫자 userNo)】"9001" 【승인자 역할】"ADMIN"
- **screen_ref**: SCREEN-019
- **preconditions**: 검수 진행중(IN_REVIEW) 영상이 하나 더 존재한다. 관리자 역할을 가진 계정으로 검수 상세 화면에 진입해 있다 — 관리자는 검수자 권한을 계층으로 물려받아 같은 화면에서 같은 승인을 수행한다.
...
- **preconditions**: 순번3 스냅샷 완료 + 해당 영상의 export 산출이 SUCCEEDED
...
- **preconditions**: 순번5 수정 발생 + 검수자가 그 수정을 다시 검수해 승인(재승인)하여 재검토 표시가 해제됨

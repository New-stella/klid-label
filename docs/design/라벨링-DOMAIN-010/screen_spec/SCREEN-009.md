...
synced_at: 2026-08-25T10:52:10.040Z
status: CHANGED
prev_version: 53
content_hash: 7aea1ff2b50dae2899a27bfa0f6f0f9cdb13673d3d08a7230042f73a8aff04e1
stale: true
...
- **note**: 배치가 실패했거나 실패·건너뜀·해제 표시가 있는 묶음이 있으면 노출한다. 사유는 서버가 준 문구를 그대로 보여주고 화면이 해석하지 않으며, 단계를 특정할 수 없는 실패도 사유만은 표시한다. 건너뛰기·재수행은 시계열과 오토라벨 두 묶음 단위이며 건너뛰기는 사유를 받고 서버가 내려주는 실패한 묶음에만 노출한다. 건너뛴 적이 있는 묶음(건너뜀·해제 모두)에 재수행 버튼을 하나 두고 해제 버튼은 두지 않는다 — 범위는 고르지 않는다(묶음이 곧 범위다). 오토라벨 재수행은 트랙 보간까지 다시 만들어 사람이 손댄 보간 라벨이 새로 계산된 값으로 바뀌므로 고르는 시점에 알린다. ★검수가 완료된 적 있는 영상은 두 묶음이 갈린다 — 시계열 재수행은 그대로 누르고 오토라벨 재수행은 비활성 + 사유 툴팁이다(라벨을 다시 만들어 승인 시점 스냅샷과 어긋난다). 되돌릴 수 없는 조건이라 파생영상처럼 미리 알린다. 전체 재기동은 실패한 영상에만 노출한다. 파생영상에는 이 영역의 조작을 노출하지 않는다.
- **type**: Custom
...
- **description**: 
- **source_hash**: ddb3525b621cfd12da453ce5bb1fa033b1cddcefc3cb688ca2ac6af186b7b45e
- **generated_at**: 2026-08-25T10:21:11.364Z

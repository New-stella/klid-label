...
synced_at: 2026-09-01T07:14:21.186Z
status: CHANGED
prev_version: 42
content_hash: cca398c94f12fee436c54e00bf27f26690335600e59c24648e7541b5188e9c26
stale: false
...
영상을 저작도구에 직접 올려 처리에 태우는 화면. 영상 파일을 한 건씩 골라 올린다. 올린 영상은 파이프라인으로 곧바로 보낼지 관제 인입 원장에 적재해 주기 배치가 가져가게 할지 고를 수 있으며, 두 경로가 같은 입력 폼을 쓴다. 반드시 채우는 것은 식별 정보 네 항목뿐이고 이벤트유형·개인정보 유형·위치와 CCTV 제원·이벤트와 관제일지·시계열 이벤트유형·영상 기술메타는 모두 선택이라 처음에는 접어 둔다. 영상 기술메타는 비워 두면 서버가 올린 파일에서 직접 읽어 채우고 영상 길이는 아예 입력하지 않는다. 고른 경로에 따라 서버로 실제 전달되는 묶음이 달라지며, 전달되지 않는 묶음은 화면이 그 사실을 알려 준다. 전송은 청크 단위 재개 업로드라 대용량 영상도 네트워크가 끊긴 지점부터 이어 보낼 수 있다. 업로드가 끝나면 결과 패널이 영상 식별자와 파이프라인 상태를 주기적으로 확인해 마킹 대기 또는 실패에 이를 때까지 보여주고, 마킹 화면과 영상 목록으로 이어 간다. 운영에서도 쓰는 화면이며 노출 여부는 설정으로 조절한다. 관리자 페이지에 속해 관리자 패스워드 확인을 거쳐야 도달한다. 관리자 유효창을 실제로 요구하는 자리는 업로드 시작 한 곳뿐이며 이어올리기·취소·진행률 확인은 요구하지 않는다. 접근: 관리자.
...
- **description**: 
- **source_hash**: 37b9560b383db480f311cc6e685c4048594b5c9ae61556e58676cddd941f0961
- **generated_at**: 2026-09-01T06:18:50.175Z

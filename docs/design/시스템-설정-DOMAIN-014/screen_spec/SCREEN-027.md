...
synced_at: 2026-09-14T05:32:56.773Z
status: CHANGED
prev_version: 52
content_hash: 2a4209c7a45baa8d27c13466d8d596a159697e30cd92272061a64dfe1dc113e4
stale: false
raw: ./_raw/SCREEN-027.json
links:
  belongs_to_domain: ["[[DOMAIN-004]]"]
  consumes: ["[[API-043]]", "[[API-152]]", "[[API-156]]", "[[API-158]]", "[[API-160]]", "[[API-162]]", "[[API-164]]", "[[API-194]]"]
  covered_by: ["[[AC-1086]]", "[[AC-1087]]"]
  implements: ["[[IMPREC-109]]", "[[IMPREC-164]]"]
  references: ["[[API-043]]", "[[API-152]]", "[[API-156]]", "[[API-158]]", "[[API-160]]", "[[API-162]]", "[[API-164]]"]
  requires: ["[[ROLE-004]]"]
...
영상을 저작도구에 직접 올려 처리에 태우는 화면. 영상 파일을 한 건씩 골라 올린다. 올린 영상은 파이프라인으로 곧바로 보낼지 관제 인입 원장에 적재해 주기 배치가 가져가게 할지 고를 수 있으며, 두 경로가 같은 입력 폼을 쓴다. 반드시 채우는 것은 두 경로 공통으로 식별 정보 네 항목이고, 파이프라인 즉시 실행을 고르면 이벤트유형과 촬영일시가 더해진다. 위치와 CCTV 제원·이벤트와 관제일지·시계열 이벤트유형·영상 기술메타는 선택이라 처음에는 접어 둔다. 개인정보 유형은 늘 하나가 골라져 있어 따로 고르지 않아도 채워진다. 영상 기술메타는 비워 두면 서버가 올린 파일에서 직접 읽어 채우고 영상 길이는 아예 입력하지 않는다. 고른 경로에 따라 서버로 실제 전달되는 묶음이 달라지며, 전달되지 않는 묶음은 화면이 그 사실을 알려 준다. 전송은 청크 단위 재개 업로드라 대용량 영상도 네트워크가 끊긴 지점부터 이어 보낼 수 있다. 업로드가 끝나면 결과 패널이 영상 식별자와 파이프라인 상태를 주기적으로 확인해 마킹 대기 또는 실패에 이를 때까지 보여주고, 마킹 화면과 영상 목록으로 이어 간다. 운영에서도 쓰는 화면이며 노출 여부는 설정으로 조절한다. 관리자 페이지에 속해 관리자 패스워드 확인을 거쳐야 도달한다. 관리자 유효창을 실제로 요구하는 자리는 업로드 시작 한 곳뿐이며 이어올리기·취소·진행률 확인은 요구하지 않는다. 접근: 관리자.
...
- **note**: 두 경로 공통 필수 3종 + 경로에 따라 갈리는 2종. 영상 클립 ID*(저장 파일명이 됨, 영문/숫자/_/- 64자) · CCTV ID*(영문/숫자/_/- 64자) · 지자체코드*(숫자 1~10자리) · 출처유형(select — 선택이며 관제 인입 재현에서만 전달된다. 비우면 서버가 기본값을 붙이므로 어느 경로에서도 요구되지 않는다. 기본값이 미리 골라져 있다) · 촬영일시(datetime-local — 관제 인입 재현에서는 선택이고 파이프라인 즉시 실행에서는 필수다. 현재 시각이 미리 채워져 있어 손대지 않아도 채워진 상태다).
- **type**: Custom
- **label**: 식별 정보
...
- **note**: 저장 값은 이벤트유형코드다. 이벤트유형마스터에 등록된 코드를 동적으로 조회해 선택 — 형식은 EV+숫자 8자리(예 EV02000101=화재, EV03000101=교통사고, EV05000101=싸움). 고정된 6종 화이트리스트가 아니라 관제가 운영하는 코드 전체가 대상이며, 신규 등록되면 즉시 선택 가능해진다. 여러 코드가 같은 한글 이름으로 해석되는 경우(예 화재가 여러 상세코드로 나뉨) 이름 옆에 코드를 함께 표시해 구분한다. 마스터에 아직 없는 코드도 넣을 수 있게 직접 입력을 함께 둔다 — 적재가 처음 보는 코드를 이벤트유형마스터에 자동 등록하므로, 고르기만 가능하게 좁히면 관제가 코드를 넓힐 때 우리가 먼저 막는다. 필수 여부는 경로에 따라 갈린다 — 파이프라인 즉시 실행에서는 이 값이 있어야 업로드 시작이 열리고, 관제 인입 재현에서는 선택이다. 다만 비워 두면 그 영상은 마킹 진입에서 막힌다. 목록을 불러오면 첫 유형이 자동으로 골라져 있어 손대지 않아도 채워진다.
- **type**: Select
- **label**: 이벤트유형
...
- **note**: 선택값은 표시용 메타데이터일 뿐이며 비식별 처리 여부에는 영향을 주지 않는다 — 비식별 단계는 업로드된 모든 영상에 대해 예외 없이 자동 실행된다. 다만 출처유형이 생성형 등 비식별 제외로 지정된 영상은 그 단계가 원본 복사로 완료된다. 파이프라인 즉시 실행에서는 서버가 이 값을 반드시 받으므로 비운 채로 보낼 수 없으나, 라디오라 늘 하나가 골라져 있어 사용자가 따로 고르지 않아도 채워진다. 관제 인입 재현에서는 서버로 전달되지 않는다.
- **type**: RadioGroup
- **label**: 개인정보 유형
...
입력 부담을 낮추는 것이 이 폼의 설계 기준이다. 반드시 채우는 것은 식별 정보의 네 항목이고 나머지 묶음은 경로에 따라 선택이라 처음에는 접어 둔다. 영상 기술메타는 비워 두면 서버가 올린 파일에서 직접 읽어 채우므로 전부 비워도 된다. 영상 길이는 아예 입력하지 않는다. 영상 클립 ID 와 CCTV ID 와 지자체코드에는 바로 쓸 수 있는 기본값을 미리 채워 둔다.
...
업로드 시작은 파일과 필수값이 모두 채워졌을 때만 눌리고 전송하는 동안에는 폼 전체가 잠긴다. 오류가 나면 서버가 보낸 문구를 그대로 보여준다. 초기화는 입력값과 고른 파일과 결과를 모두 비운다. 단계별 실행을 고르는 기능은 두지 않는다 — 즉시 실행의 흐름은 업로드 다음 비식별 다음 마킹 대기 정지로 고정이며 비식별 단계는 예외 없이 실행된다.

이 폼은 한 건씩 올리는 자리다. 이벤트 마킹이 끝난 영상 묶음을 한 번에 올리려면 산출물 가져오기 화면(SCREEN-039)을 쓴다. 두 자리는 입력도 결과도 서로 섞이지 않는다.
...
- **description**: 업로드 성공(rawSn 수신) 시에만 렌더되는 결과 카드. 영상 상세를 2초 간격으로 폴링하여 파이프라인 진행을 가시화한다. 표시: rawSn 칩, 파이프라인 상태 텍스트, 진행 중 스피너(terminal 도달 전까지), 저장 파일 경로, 프레임 수, 트리거 시각. 이 화면의 즉시 실행 경로는 고정 플로우(업로드→비식별 단계 무조건 실행→마킹 대기 정지)이므로 폴링 종료(terminal) 조건은 상태가 MARKING_READY(마킹 대기) 또는 FAILED에 도달한 시점이며, 이 경로에서 상태가 완료(COMPLETED)로 가는 일은 없다 — 잔여 배치(마킹 이후 단계)는 사용자가 마킹 화면에서 마킹을 완료해야 트리거된다. MARKING_READY 도달 시 마킹 대기 안내 배너를, FAILED 도달 시 실패 배너(role=alert)를 노출한다. 하단에 영상 상세 화면(rawSn 기준)과 영상 처리 현황 목록으로 이동하는 링크 2건을 제공한다. 적재 경로로 파이프라인 즉시 실행을 골랐을 때는 업로드 응답이 곧바로 영상 식별자를 주므로 이 패널이 바로 뜬다. 관제 인입 재현을 골랐을 때는 인입 원장에 적재만 되고 영상 식별자는 주기 배치가 그 행을 가져간 뒤에 생기므로, 그 전까지는 이 패널 대신 인입 대기 안내만 보인다.
...
- **description**: 
- **source_hash**: 0f4a3f8e220d1c95036ca1a2a0b6ed85209967c3d7b5d493a0d05985662f74bd
- **generated_at**: 2026-09-14T04:27:28.860Z

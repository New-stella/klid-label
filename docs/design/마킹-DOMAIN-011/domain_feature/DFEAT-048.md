...
synced_at: 2026-09-14T05:33:09.944Z
status: CHANGED
prev_version: 19
content_hash: 5d86a490e05260951989f484460e4ed76d20e76415df89bab8ec400c5025f6dd
stale: true
...
[차단은 412 로 통일한다] 신고 구간에는 저작도구 내부 통로의 라벨 조회·이력, 라벨 저장(PUT /v1/frames/{srcSn}/labels), 라벨 객체 속성값 조회·저장, 버전 diff·롤백, 프레임 이미지, 포털 라벨 조회·저장·포털 프레임 이미지, 개인정보 메타 PUT(영상 축·프레임 축), 이벤트 어노테이션 저장·승인·반려, 검수 승인이 412 로 차단되고 영상 스트리밍만 404 다(그 엔드포인트의 기존 규약. 신고 여부로 404/412 가 갈리면 응답이 영상 상태 오라클이 된다 — CWE-209). 게이트는 저작도구 내부 통로에만 걸리고 관제 경계(데이터마트 뷰·통지·관제 조회 API)에는 걸리지 않는다. ⚠ 구 서술 '저장·수정은 기존 작업락으로 409 차단' 은 2026-08-04 폐기 — 작업락은 6시간 만료 후 회수되는데 'F' 는 resolve 까지 남아 조회 412 대 저장 200 비대칭이 열렸고, full-replace 계약상 빈 목록 저장이 기존 라벨을 전량 삭제했다. 게이트는 락 검사보다 먼저 평가하며, 409 는 신고와 무관한 락(트랙 병합 등)에만 남는다. 게이트가 걸린 미디어 응답은 Cache-Control: no-store 이고, FE 는 게이트가 닫히는 변화에 removeQueries 로 캐시를 제거한다.

[해소·단계별 재개] 자동 재비식별 큐는 폐기됐다 — 재비식별 자체는 사람이 외부 비식별 솔루션으로 수행한다. 다만 그 수행은 해소의 선행 조건이 아니다. 해소는 POST /v1/deident-reports/{rprtSn}/resolve(WORKER 본인 배정/REVIEWER 전체)로 하며, 후보 목록에서 산출물을 하나 고르고 그 산출물이 무결성 판정을 통과하면 성립한다. ⚠ 구 서술 '작업자/검수자가 외부 비식별 솔루션으로 수동 비식별화한 뒤 해소한다' 는 재비식별 수행을 해소의 전제로 읽히게 하므로 2026-09-09 폐기 — 사람이 수행한다는 사실만 남고 전제는 성립하지 않는다. ★고른 산출물이 신고 이후에 만들어진 것이어야 한다는 시간 조건은 두지 않는다 — 신고 이전부터 있던 산출물(지금 쓰고 있는 비식별 영상 포함)도 고를 수 있고, 고르면 해소가 성립한다. 그 시간 조건은 새 산출물을 내놓는 경로가 저작도구 안에 없어 충족될 수 없었고, 그대로 두면 신고된 영상이 작업락과 'F' 로 영구히 남는다 — [접수 조건] 절이 이미 접수된 신고의 해소를 열어 두는 근거와 같은 축이다. 대가는 인지·수용한다: 재비식별을 실제로 수행하지 않고도 신고를 닫을 수 있어 마스킹 누락이 남은 산출물이 정상으로 되돌아갈 수 있다. resolve 는 OPEN→RESOLVED 조건부 UPDATE 로 원자 클레임하고 영향행수 1 을 받은 성공자만 작업락 해제·'F'→'Y' 복원·재개를 수행한다(다중 노드 동시 통과로 프레임 재추출이 2회 기동하던 것을 막는다). 복원으로 게이트가 자동 해제되어 보존된 기존 라벨을 그대로 재사용한다(별도 복원 API 없음). ★재개 지점은 신고 단계로 갈린다 — MARKING 은 배치 단계를 MARKING_READY 로 되감고 활성 마킹을 SKIPPED 로 종결해 재마킹 충돌을 풀며, LABELING 은 기존 LS_DATA_SRC 행을 dirty-update 로 갱신해 SRC_SN 을 보존한 채 프레임 이미지만 재추출한다(라벨 좌표 보존). 단계 미상(컬럼 신설 이전 신고)은 백필하지 않고 재개 이벤트도 발행하지 않는다.

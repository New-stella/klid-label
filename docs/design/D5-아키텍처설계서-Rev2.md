...
#### [인터페이스] 화면 UX/UI·API 호출 표준

| 요구사항 ID | RQ-SIR-01-01 |
|---------|---|
| 요구사항 내용 | 저작도구 전 업무 화면·메뉴에 일관된 UX/UI 표준(화면 스타일·구성·연계·오류 처리)을 수립·적용하고, 모든 API에 공통 호출 규약(공통 기본 경로, 토큰 인증, 역할·채널 기반 접근 통제, 인증 예외 경로, 표준 응답·페이징)을 적용해 카탈로그만으로 호출을 구성할 수 있게 한다. |

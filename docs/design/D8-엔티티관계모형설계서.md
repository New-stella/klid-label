...
erDiagram
    ...
    LS_DATA_ISSUE {
        ...
        bigint DATA_RAW_SN FK
        varchar20 ISSUE_TYPE_CD
        varchar20 ISSUE_STTS_CD
        bigint SRC_SN
        bigint VERSION
    ...
    LS_DATA_ISSUE ||--o{ LS_ISSUE_COMMENT : "이슈 댓글 스레드"
```
...
### KLID-AT-ERD-022 · 게시판 ERD

| ERD ID | KLID-AT-ERD-022 | ERD 명 | 게시판 ERD (R1 요구사항 외 운영 추가 결정, PostgreSQL) |
|--------|---|-------|---|
...
erDiagram
    LS_NOTICE {
        bigint NOTICE_SN PK
        varchar200 NOTICE_TITLE
        text NOTICE_CN
        varchar1 PIN_YN
        varchar16 PUB_STTS_CD
        timestamp PUB_DT
        varchar64 REG_ID
        timestamp REG_DT
        varchar64 MDFR_ID
    ...
    LS_NOTICE_ATTACH {
        bigint ATTACH_SN PK
        bigint NOTICE_SN FK
        varchar255 ORGNL_FILE_NM
        varchar255 STORE_FILE_NM
        varchar500 FILE_PATH
        bigint FILE_SIZE
        timestamp REG_DT
    }

    LS_NOTICE ||--o{ LS_NOTICE_ATTACH : "공지 첨부(물리 FK, ON DELETE CASCADE)"
```
...
> **EN ID 배정**: KLID-AT-EN-001 ~ KLID-AT-EN-038 (도메인별 오름차순, **KLID-AT-EN-010 결번** — 구 LS_DATA_SET 범위 외 제외)
...
| 관련 유스케이스 ID | - ※ |
| 엔티티 설명 | 검수 반려 이슈. 영상 단위(DATA_RAW_SN) 반려 + UP_DATA_ISSUE_SN 자기참조로 재반려 계층 형성. 이슈 소통 채널 확장으로 유형(REJECTION/INQUIRY)·상태(OPEN→ANSWERED→RESOLVED)·프레임 선택 참조(SRC_SN)·낙관적 잠금(VERSION)을 보유하며, 댓글 스레드는 LS_ISSUE_COMMENT 별도 엔티티. |
...
| ISSUE_TYPE_CD | 이슈유형코드 | VARCHAR | 20 | Y | N | N | N | 'REJECTION' | REJECTION(검수 반려)/INQUIRY(작업자 문의) |
| ISSUE_STTS_CD | 이슈상태코드 | VARCHAR | 20 | Y | N | N | N | 'OPEN' | OPEN→ANSWERED→RESOLVED(INQUIRY만 전이, 역행 불가) |
| SRC_SN | 프레임원천일련번호 | BIGINT | — | N | N | Y | Y | — | 논리FK→LS_DATA_SRC(선택 참조), 부분IDX(SRC_SN IS NOT NULL) |
...
### KLID-AT-EN-036 · LS_NOTICE (공지사항)

| 엔티티 ID | KLID-AT-EN-036 | 엔티티명 | LS_NOTICE |
|---------|---|-------|---|
| 관련 클래스 ID | - ※ | 관련 클래스명 | LsNotice |
| 관련 유스케이스 ID | - ※ |
| 엔티티 설명 | 공지사항 게시글. **R1 요구사항 외 운영 추가 결정 기능**(게시판/공지). 발행 상태(DRAFT/PUBLISHED)와 상단 고정을 관리한다. |

| 속성명 | 동의어 | 타입 | 길이 | NOT NULL (Y/N) | PK (Y/N) | FK (Y/N) | INX (Y/N) | 기본값 | 제약조건 |
|-------|--------|------|------|--------|------|------|------|-------|--------|
| NOTICE_SN | 공지일련번호 | BIGINT | — | Y | Y | N | Y | IDENTITY | PK |
| NOTICE_TITLE | 공지제목 | VARCHAR | 200 | Y | N | N | N | — | — |
| NOTICE_CN | 공지내용 | TEXT | — | Y | N | N | N | — | — |
| PIN_YN | 상단고정여부 | VARCHAR | 1 | Y | N | N | Y | 'N' | Y/N, 복합IDX |
| PUB_STTS_CD | 발행상태코드 | VARCHAR | 16 | Y | N | N | Y | 'DRAFT' | DRAFT/PUBLISHED, 복합IDX |
| PUB_DT | 발행일시 | TIMESTAMP | — | N | N | N | N | — | 발행 시점 기록 |
| REG_ID | 등록자ID | VARCHAR | 64 | N | N | N | N | — | — |
| REG_DT | 등록일시 | TIMESTAMP | — | Y | N | N | Y | CURRENT_TIMESTAMP | 복합IDX(DESC) |
| MDFR_ID | 수정자ID | VARCHAR | 64 | N | N | N | N | — | — |
| MDFCN_DT | 수정일시 | TIMESTAMP | — | Y | N | N | N | CURRENT_TIMESTAMP | — |

---

### KLID-AT-EN-037 · LS_NOTICE_ATTACH (공지첨부파일)

| 엔티티 ID | KLID-AT-EN-037 | 엔티티명 | LS_NOTICE_ATTACH |
|---------|---|-------|---|
| 관련 클래스 ID | - ※ | 관련 클래스명 | LsNoticeAttach |
| 관련 유스케이스 ID | - ※ |
| 엔티티 설명 | 공지사항 첨부파일. 공지 삭제 시 물리 FK(ON DELETE CASCADE)로 함께 삭제된다. |

| 속성명 | 동의어 | 타입 | 길이 | NOT NULL (Y/N) | PK (Y/N) | FK (Y/N) | INX (Y/N) | 기본값 | 제약조건 |
|-------|--------|------|------|--------|------|------|------|-------|--------|
| ATTACH_SN | 첨부일련번호 | BIGINT | — | Y | Y | N | Y | IDENTITY | PK |
| NOTICE_SN | 공지일련번호 | BIGINT | — | Y | N | Y | Y | — | FK→LS_NOTICE(물리), ON DELETE CASCADE |
| ORGNL_FILE_NM | 원본파일명 | VARCHAR | 255 | Y | N | N | N | — | — |
| STORE_FILE_NM | 저장파일명 | VARCHAR | 255 | Y | N | N | N | — | — |
| FILE_PATH | 파일경로 | VARCHAR | 500 | Y | N | N | N | — | — |
| FILE_SIZE | 파일크기 | BIGINT | — | Y | N | N | N | — | byte 단위 |
| REG_DT | 등록일시 | TIMESTAMP | — | Y | N | N | N | CURRENT_TIMESTAMP | — |

---

### KLID-AT-EN-038 · LS_ISSUE_COMMENT (이슈댓글)

| 엔티티 ID | KLID-AT-EN-038 | 엔티티명 | LS_ISSUE_COMMENT |
|---------|---|-------|---|
| 관련 클래스 ID | - ※ | 관련 클래스명 | LsIssueComment |
| 관련 유스케이스 ID | - ※ |
| 엔티티 설명 | 검수 반려 이슈(LS_DATA_ISSUE)에 대한 WORKER↔REVIEWER 소통 댓글 스레드. **R1 요구사항 외 운영 추가 결정 기능**(이슈 소통 채널). |

| 속성명 | 동의어 | 타입 | 길이 | NOT NULL (Y/N) | PK (Y/N) | FK (Y/N) | INX (Y/N) | 기본값 | 제약조건 |
|-------|--------|------|------|--------|------|------|------|-------|--------|
| ISSUE_COMMENT_SN | 이슈댓글일련번호 | BIGINT | — | Y | Y | N | Y | IDENTITY | PK |
| DATA_ISSUE_SN | 데이터이슈일련번호 | BIGINT | — | Y | N | Y | Y | — | FK→LS_DATA_ISSUE(논리) |
| AUTHOR_NO | 작성자번호 | VARCHAR | 50 | Y | N | N | Y | — | — |
| AUTHOR_ROLE_CD | 작성자역할코드 | VARCHAR | 20 | Y | N | N | N | — | CHECK(WORKER/REVIEWER) — 토큰 역할에서만 도출 |
| CMNT_CN | 댓글내용 | VARCHAR | 1000 | Y | N | N | N | — | — |
| REG_DT | 등록일시 | TIMESTAMP | — | Y | N | N | N | CURRENT_TIMESTAMP | — |

> **추가 엔티티(LS_DEADLINE·LS_CONTROL_NOTIFY_FALLBACK·LS_WEBHOOK_IDEMPOTENCY)**는 D9 테이블 명세(KLID-AT-TB-035 ~ KLID-AT-TB-037)에 상세 기술하며, 본 엔티티 명세서는 KLID-AT-EN-001 ~ KLID-AT-EN-038 범위(KLID-AT-EN-010 결번)로 작성한다.
...
- ERD ID: `KLID-AT-ERD-010 ~ KLID-AT-ERD-022` (활성 13건)
- 엔티티 ID: `KLID-AT-EN-001 ~ KLID-AT-EN-038` (KLID-AT-EN-010 결번)

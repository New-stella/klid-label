# D11-R1 단위시험 절차서 (기능 정상동작 한정 · 컴포넌트 기준)

## 작성 목적
> 본 절차서는 `D11-R1-단위시험절차서.md`(전체)에서 **예외·경계·권한·검증 케이스를 제외하고 각 기능의 정상 동작 케이스만** 추린 기능 검증용 변형본이다. 혼합 케이스는 정상 절만 추출했다(원문 verbatim 아님). 활성 R1 기능 유스케이스 18종은 모두 정상 동작 케이스를 1건 이상 보유한다. 본 판본은 단위시험을 **컴포넌트(CO) 단위 블록**으로 재편하고, 케이스 ID를 `KLID-AT-UT-{컴포넌트번호}-NN`로 부여한 컴포넌트 기준본이다.
...
|---------|---------|---------|-----------|---------|--------------|------|
| KLID-AT-UT-001-01 | 증강요청 WINTER 단일 정상 200 | REVIEWER | 검수완료 영상·WINTER | 증강 요청 | 200 + jobId 반환 | |
...
| 케이스 ID | 케이스 명 | 작업 권한 | 시험 데이터 | 시험항목 및 처리절차 | 예상결과 및 검증방법 | 시험 결과 |
|---------|---------|---------|-----------|---------|--------------|------|
| KLID-AT-UT-002-01 | LabelIntegrity 완전 동일 라벨 보존율 100 | - | 원본·증강 라벨쌍 | 보존율 계산 | 100 정확 산출 | |
| KLID-AT-UT-002-02 | 증강 SUCCESS 시 새 RAW_SN 생성 PARENT_RAW_SN 참조 + 원본 프레임 라벨·메타 복사 | - | 증강 SUCCESS | 증강 콜백 | 새영상·복사 정확(DB) | |
| KLID-AT-UT-002-03 | 증강 성공시 원본 라벨/메타가 새 프레임에 정확히 복사·복수 프레임 매핑 | - | 원본 라벨·메타 | 증강 콜백 | 복사·매핑 정확 | |
...
| 케이스 ID | 케이스 명 | 작업 권한 | 시험 데이터 | 시험항목 및 처리절차 | 예상결과 및 검증방법 | 시험 결과 |
|---------|---------|---------|-----------|---------|--------------|------|
| KLID-AT-UT-003-01 | 다운스케일 성공시 이미지셋 생성·EXPORT 1행 기록 | REVIEWER | 해상도 preset | 해상도 변경 | 다운스케일 성공·이미지셋 생성·EXPORT 1행 | |
| KLID-AT-UT-003-02 | 정상 해상도 변경 요청시 201 | REVIEWER | preset | API 호출 | 201 (내부경로 미포함) | |
...
| 케이스 ID | 케이스 명 | 작업 권한 | 시험 데이터 | 시험항목 및 처리절차 | 예상결과 및 검증방법 | 시험 결과 |
|---------|---------|---------|-----------|---------|--------------|------|
| KLID-AT-UT-004-01 | 두 키프레임 5~10 사이 6 7 8 9 프레임 선형 보간 / 세 키프레임 각 구간 독립 보간 | - | 키프레임 2~3개 | 보간 실행 | 중간 프레임 선형 보간 산출 | |
| KLID-AT-UT-004-02 | 같은 trackId 5~10 프레임 BBOX 2개 있으면 6 7 8 9 프레임 보간 4건 저장 | - | trackId 키프레임 | 보간 Step 실행 | 4건 저장(DB 조회) | |
| KLID-AT-UT-004-03 | YOLO 검출 결과는 AUTO_LBL_YN Y + BBOX 타입 + score 0~1 저장 | - | 검출 응답 | YOLO Step 실행 | AUTO_LBL_YN=Y·BBOX·score 저장(DB) | |
...
| 케이스 ID | 케이스 명 | 작업 권한 | 시험 데이터 | 시험항목 및 처리절차 | 예상결과 및 검증방법 | 시험 결과 |
|---------|---------|---------|-----------|---------|--------------|------|
| KLID-AT-UT-005-01 | DB BBOX 있으면 SAM2 호출 후 POLYGON 저장 | - | BBOX 있음 | SAM2 Step 실행 | 호출·POLYGON 저장 | |
| KLID-AT-UT-005-02 | POLYGON 저장 시 LsDataLblAiInfo SRC SAM2도 동시 저장 | - | 저장 결과 | SAM2 Step 실행 | AiInfo 저장 | |
...
| 케이스 ID | 케이스 명 | 작업 권한 | 시험 데이터 | 시험항목 및 처리절차 | 예상결과 및 검증방법 | 시험 결과 |
|---------|---------|---------|-----------|---------|--------------|------|
| KLID-AT-UT-006-01 | POLYGON_SIMPLIFY_TOLERANCE 기본값 1.0·REVIEWER 2.5 저장 반영 | REVIEWER | 허용오차 | 설정 수정 | 저장 반영 | |
| KLID-AT-UT-006-02 | ConfigKeys에 YOLO CONF IMGSZ IOU·POLYGON_SIMPLIFY_TOLERANCE 화이트리스트 포함·NUMBER/DECIMAL RANGE 유효·기존 Batch 키 유지 | - | 설정 키 | 키 정의 검증 | 화이트리스트·범위 정확 | |
...
| 케이스 ID | 케이스 명 | 작업 권한 | 시험 데이터 | 시험항목 및 처리절차 | 예상결과 및 검증방법 | 시험 결과 |
|---------|---------|---------|-----------|---------|--------------|------|
| KLID-AT-UT-007-01 | 검수 승인시 영상 프레임의 현재 라벨로 DB 스냅샷 APPROVED 버전 생성 / 다중 프레임 각 스냅샷 | REVIEWER | 라벨 보유 영상 | commitApproved | 스냅샷 생성 정확(DB) | |
| KLID-AT-UT-007-02 | LsDataLblHstry 삭제 이력 저장·PK 채번 / 프레임 단위 최신순 조회 | - | 이력 데이터 | 이력 저장·조회 | 채번·정렬 정확 | |
| KLID-AT-UT-007-03 | TASK_COMPLETED/TASK_MODIFIED 통지 정상 송신시 200 | - | 통지 페이로드 | Client 호출 | 200 | |
| KLID-AT-UT-007-04 | TaskCompletedPayload/TaskModifiedPayload record 정상 생성 | - | 페이로드 | 구조 검증 | record 정상 생성 | |
| KLID-AT-UT-007-05 | ReviewApprovedEvent 수신시 sendCompleted 호출 / TaskModifiedEvent 수신시 debouncer accumulate 호출 | - | 이벤트 | 리스너 수신 | 디스패치 정확 | |
...
| 케이스 ID | 케이스 명 | 작업 권한 | 시험 데이터 | 시험항목 및 처리절차 | 예상결과 및 검증방법 | 시험 결과 |
|---------|---------|---------|-----------|---------|--------------|------|
| KLID-AT-UT-008-01 | diff 두 스냅샷 labels JSON 파싱하여 라벨 단위 ADDED REMOVED MODIFIED 반환 / shape 변경 before·after | WORKER | 스냅샷쌍 | diff 호출 | 분류·before/after 정확 | |
| KLID-AT-UT-008-02 | 배정된 WORKER 본인 프레임 rollback시 대상 스냅샷 복원 새 ROLLBACK 버전 / 복원 후 LS_DATA_LBL이 스냅샷과 일치 | WORKER | 스냅샷·본인 프레임 | rollback 호출 | 복원·새 버전·정합(DB) | |
...
| 케이스 ID | 케이스 명 | 작업 권한 | 시험 데이터 | 시험항목 및 처리절차 | 예상결과 및 검증방법 | 시험 결과 |
|---------|---------|---------|-----------|---------|--------------|------|
| KLID-AT-UT-010-01 | REVIEWER accept시 AUG_PROC_STTS_CD ACCEPTED + LS_DATA_AUG_RVW row INSERT | REVIEWER | 증강 결과 | accept 호출 | ACCEPTED 갱신 + RVW INSERT(DB 조회) | |
| KLID-AT-UT-010-02 | REJECTED 시 사유 LS_DATA_AUG_RVW REJECT_RSN 저장 | REVIEWER | 반려 사유 | reject 호출 | REJECT_RSN 저장(DB 조회) | |
...
| 케이스 ID | 케이스 명 | 작업 권한 | 시험 데이터 | 시험항목 및 처리절차 | 예상결과 및 검증방법 | 시험 결과 |
|---------|---------|---------|-----------|---------|--------------|------|
| KLID-AT-UT-011-01 | 정상 경로 응답이면 LS_DEIDENT_REPORT SUCCEEDED + DE_IDNTF Y + 결과경로 반환 | - | 정상 응답 | 비식별 Step 실행 | SUCCEEDED·DE_IDNTF=Y(DB 조회) | |
| KLID-AT-UT-011-02 | 위탁 시 LS_DEIDENT_PROC_LOG REQUESTED 상태로 적재 / idempotencyKey 헤더 포함 | - | 위탁 요청 | 비식별 Client 호출 | REQUESTED 적재·키 헤더(DB 조회) | |
...
| 케이스 ID | 케이스 명 | 작업 권한 | 시험 데이터 | 시험항목 및 처리절차 | 예상결과 및 검증방법 | 시험 결과 |
|---------|---------|---------|-----------|---------|--------------|------|
| KLID-AT-UT-013-01 | [설계] REVIEWER가 비식별 마스킹 옵션(타입·범위·화질·포맷) 설정값 저장 시 SystemConfig 반영 + 이후 비식별 위탁에 적용 | REVIEWER | 허용 옵션값 | 옵션 설정 저장 | 저장 반영·위탁 적용(DB 조회) | |
...
| 케이스 ID | 케이스 명 | 작업 권한 | 시험 데이터 | 시험항목 및 처리절차 | 예상결과 및 검증방법 | 시험 결과 |
|---------|---------|---------|-----------|---------|--------------|------|
| KLID-AT-UT-016-01 | WORKER 본인 배정 영상 신고 201 + REPORT_STTS OPEN + LS_AUTH_WORK_LOCK LOCKED + DE_IDNTF F | WORKER | 본인 배정 영상 | 신고 등록 | 201·OPEN·LOCKED·F(DB 조회) | |
| KLID-AT-UT-016-02 | 수동 비식별화 완료 resolve 200 + RESOLVED 전이 + LOCK 해제 | WORKER | OPEN 신고 | 신고 해제 처리 | RESOLVED·락 해제 | |
...
| 케이스 ID | 케이스 명 | 작업 권한 | 시험 데이터 | 시험항목 및 처리절차 | 예상결과 및 검증방법 | 시험 결과 |
|---------|---------|---------|-----------|---------|--------------|------|
| KLID-AT-UT-018-01 | 관제 작업수요 설정 클립을 픽업해 적재 위임한다 | - | 작업수요 설정 클립 | 스캔·적재 위임 | 클립 픽업·적재 위임(DB 조회) | |
...
| 케이스 ID | 케이스 명 | 작업 권한 | 시험 데이터 | 시험항목 및 처리절차 | 예상결과 및 검증방법 | 시험 결과 |
|---------|---------|---------|-----------|---------|--------------|------|
| KLID-AT-UT-019-01 | 마킹 자동모드/수동모드 생성 201 | WORKER | 자동·수동 요청 | 마킹 생성 | 201 + 저장(DB 조회) | |
| KLID-AT-UT-019-02 | 자동모드 마킹 생성 intervalFrames 기반 marks 자동생성 (30/60프레임 120초) | WORKER | intervalFrames·duration | 마킹 생성 | marks 개수 정확 산출 | |
...
| 케이스 ID | 케이스 명 | 작업 권한 | 시험 데이터 | 시험항목 및 처리절차 | 예상결과 및 검증방법 | 시험 결과 |
|---------|---------|---------|-----------|---------|--------------|------|
| KLID-AT-UT-021-01 | bbox 좌표 저장시 AUTO_LBL_YN N 저장(수동) / 오토 라벨 수정시 AUTO_LBL_YN은 Y 유지 | WORKER | 수동·오토 라벨 | 라벨 저장 | AUTO_LBL_YN 정확(DB 조회) | |
...
| 케이스 ID | 케이스 명 | 작업 권한 | 시험 데이터 | 시험항목 및 처리절차 | 예상결과 및 검증방법 | 시험 결과 |
|---------|---------|---------|-----------|---------|--------------|------|
| KLID-AT-UT-022-01 | 검토 승인 REVIEWER 정상 200 | REVIEWER | 승인 요청 | 승인 처리 | 200 | |
...
| 케이스 ID | 케이스 명 | 작업 권한 | 시험 데이터 | 시험항목 및 처리절차 | 예상결과 및 검증방법 | 시험 결과 |
|---------|---------|---------|-----------|---------|--------------|------|
| KLID-AT-UT-023-01 | 승인시 LS_RAW_DATA_STATUS APPROVED | REVIEWER | IN_REVIEW 영상 | 승인 호출 | APPROVED 전이(DB 조회) | |
| KLID-AT-UT-023-02 | 반려시 LS_DATA_ISSUE 생성 + 상태 REJECTED + DATA_STTS_CD 업데이트 | REVIEWER | 반려 사유 | 반려 호출 | ISSUE 생성·REJECTED | |

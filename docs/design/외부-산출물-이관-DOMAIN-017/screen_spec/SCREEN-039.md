...
synced_at: 2026-08-26T05:49:01.878Z
status: CHANGED
prev_version: 17
content_hash: 2480763f079fd3d22d7e67789061af457a2c887ce3739847723957de2d83fc0a
stale: true
...
- **note**: 한 단계 위가 허용 저장소 루트 밖이면 쓸 수 없다
- **type**: Button
- **label**: 상위로
...
- **note**: 담는 개수 상한에 걸려 잘렸을 때만 노출
- **type**: Alert
- **label**: 목록이 많아 일부만 보여주고 있습니다. 찾는 것이 없으면 경로를 직접 적어 주세요.
- **state**: hidden
...
- **description**: 찾아보기를 누르면 열리는 자리다. 허용 저장소 루트 목록에서 시작해 폴더를 눌러 한 단계씩 내려가고 위로 가기로 되돌아온다. 고른 위치는 그대로 입력칸에 들어간다. 담는 개수 상한에 걸려 목록이 잘렸으면 그 사실을 알리고 직접 적는 길을 안내한다 — 잘린 목록을 전부인 것처럼 보여주지 않는다. 조회가 거부되면 그 사유를 이 자리에서 보여주고 창을 닫지 않는다. 이 자리가 열리지 않아도 경로를 직접 적어 검사할 수 있다.
...
- **done**: true
- **description**: 경로 선택 — 허용 저장소 루트에서 시작해 한 단계씩 내려가고 상위로 되돌아온다. 상위로의 활성 여부는 응답의 parent 하나가 정한다

#### [3]

- **done**: true
- **description**: 경로 선택 — 고른 값은 서버가 돌려준 실제 위치이며, 잘린 목록은 그 사실을 알리고 직접 입력을 안내한다
...
- **source_hash**: afb2549bcd5d22019340385c1c9c526ba369fceb6f4e2923da5cb25d78f3f887
- **generated_at**: 2026-08-26T02:50:11.990Z

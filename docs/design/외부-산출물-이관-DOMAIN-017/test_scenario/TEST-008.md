...
synced_at: 2026-09-07T13:23:46.581Z
status: CHANGED
prev_version: 3
content_hash: 285a98d62dced1f64250f74ee23b8e239c07dd390f8fe219de6220b0760a1a0e
stale: false
...
허용 저장소 루트가 바로가기로 구성된 형상은 8·9단계가 다룬다. 이것은 허용 루트 자체의 축이며, 폴더 안의 항목을 어떻게 다루는가는 다른 축이다 — 섞지 말 것. 바로가기를 만들 수 없는 실행 환경에서는 그 두 단계를 조용히 통과시키지 말고 건너뛴 사실을 남긴다.
...
- **seq**: 8
- **note**: 바로가기가 없는 검증 환경에서는 고쳤는지 아닌지가 드러나지 않는다. 그런 환경에서는 이 단계를 조용히 통과시키지 말고 건너뛴 사실을 남긴다
- **action**: 허용 저장소 루트가 바로가기인 형상에서 같은 흐름을 다시 밟는다
- **expected**: 적재가 완주해 영상과 프레임이 만들어지고 승인 보류가 서며, 비식별 산출물 위치 제출도 완주해 처리 이력이 남고 보류가 풀린다. 위치가 허용 범위 밖이라는 이유로 막히지 않는다
- **test_item**: 허용 루트가 바로가기여도 원본 지정 적재와 비식별 완료 기록이 완주하는지
- **input_data**: 탐색이 돌려준 위치를 그대로 제출한다 — 산출물 폴더 위치와 비식별 산출물 위치 둘 다
- **screen_ref**: SCREEN-039
- **preconditions**: 허용 저장소 루트가 바로가기로 구성돼 실제로 가리키는 자리가 설정에 적힌 표기와 다르다

### [9]

- **seq**: 9
- **note**: 4단계의 실재하지 않는 위치 거부와 짝을 이루는 축이다. 위치 판정을 넓힐 때 가장 뚫리기 쉬운 자리라, 판정 단계를 합치면 이 방어가 함께 풀린다 — 따로 둔다. 바로가기를 만들 수 없는 환경에서는 조용히 통과시키지 말고 건너뛴 사실을 남긴다
- **action**: 허용 범위 안의 이름이지만 실제로는 범위 밖을 가리키는 바로가기를 제출한다
- **expected**: 거부되고 아무것도 기록되지 않는다. 승인 보류가 그대로 남는다
- **test_item**: 이름만 허용 범위 안인 바로가기가 거부되는지
- **input_data**: 그 바로가기의 위치
- **screen_ref**: SCREEN-039
- **preconditions**: 허용 범위 안에 놓였으나 실제로 가리키는 자리는 범위 밖인 바로가기를 만들어 둔다
...
- 허용 저장소 루트가 바로가기로 구성돼 있어, 실제로 가리키는 자리가 설정에 적힌 표기와 다르다

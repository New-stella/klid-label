...
> 스코프 정본은 `.kit-scope.json`(62건) · 버전은 `version-master.md`.
...
>
> ⚠⚠ **이 문서의 ITEM 인덱스와 「0건인 단계」 목록은 2026-08-24(스코프 49건) 시점 스냅샷이라 낡았다.**
> 다운로더가 이 파일을 생성하지 않아 그 뒤 SYNC 들이 손대지 않았고, 현재 스코프는 **66건**이다.
> 실제와 어긋나는 것이 확인된 부분: `adr` 은 2건이 아니라 **5건** · **`TEST`·`INT`·`FEAT` 를 「0건」으로
> 적었으나 셋 다 실재**한다 · `AC-048`·`AC-052`~`AC-054`·`AC-120`·`API-215`·`API-221`·`API-222`·
> `INT-012`·`EXTSYS-007`·`FEAT-010` 등이 인덱스에 **없다**.
>
> ⇒ **이 문서의 인덱스를 「이 도메인에 무엇이 있는가」의 근거로 쓰지 마라.** 그 근거는
> `.kit-scope.json` 과 실제 파일 목록이다. 숫자만 고치면 낡은 인덱스가 최신처럼 보여 더 위험하므로
> 부분 수정을 하지 않았다 — 전면 재작성은 66개 ITEM 을 읽어 재구성하는 별도 작업이다.

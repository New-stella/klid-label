...
synced_at: 2026-09-02T10:52:13.484Z
status: CHANGED
prev_version: 8
content_hash: bdb2ad8c5cf8a627b84ac57bbccd90d7d250d1a303c47a7ee0f35969776b2a41
stale: true
...
[관리자 등록 화면으로 이어짐] 인증을 통과한 내부 채널 진입자는 그 진입 시점에 사용자 마스터로 자동 등록되며, 역할이 없으면 작업자를 받는다 — 이미 부여된 역할은 덮어쓰지 않는다. 관리자는 이 자동 등록으로 생기지 않으므로, 관리자가 한 명도 없는 동안에만 관리자 등록 화면(SCREEN-002, API-007)이 열려 관리자 공유 패스워드를 대조하고 통과하면 관리자 역할을 부여한다. 부여하는 역할은 관리자로 고정이라 역할을 고르는 화면이 아니며, 관리자가 한 명이라도 생기면 이 창구는 닫히고 이후 역할 부여는 관리자가 수행한다. 진입 조건을 역할 클레임이 비어 있는 경우로 좁히면 진입 시점의 자동 등록에 막혀 최초 관리자를 만들 경로가 사라지므로 그 조건을 쓰지 않는다. 이 흐름은 로그인 흐름의 연속이며 별도 회원가입이 없다(ADR-055).

...
synced_at: 2026-08-27T20:54:27.309Z
status: NEW
prev_version: null
content_hash: f0b4693f479ce17de9d0424cc39924828f87c0183f78b1e94f2fd02ce797d190
stale: false
raw: ./_raw/SCREEN-041.json
links:
  based_on: ["[[ADR-046]]"]
  belongs_to_domain: ["[[DOMAIN-001]]"]
  consumes: ["[[API-194]]", "[[API-223]]"]
  references: ["[[API-194]]", "[[API-223]]"]
  requires: ["[[ROLE-001]]"]
  designs_backward: ["[[SD-037]]"]
...
관리자 공유 패스워드를 운영 중에 바꾸는 화면. 교체가 성립하려면 셋이 함께 갖춰져야 한다 — 검수자 권한, 유효한 관리자 유효창, 그리고 현재 패스워드 재확인이다. 유효창만으로 바꿀 수 있으면 잠깐 열린 창을 가로챈 사람이 자격 자체를 갈아 치워 정당한 운영자를 잠글 수 있으므로 현재 값을 한 번 더 받는다. 교체가 성공하면 그 전에 발급된 유효창이 모두 무효가 되며 방금 쓰던 자기 것도 함께 끊기므로, 이어서 관리 기능을 쓰려면 새 패스워드로 다시 진입해야 한다. 관리자 페이지에 속해 관리자 패스워드 확인을 거쳐야 도달한다. 접근: 검수자.
...
- **note**: 셋이 함께 성립해야 한다. 유효창은 인가를 대체하지 않고 더해지므로 검수자 권한이 그대로 필요하며, 유효창이 열려 있어도 현재 패스워드를 다시 받는다.
- **type**: List
- **label**: 교체 요건
...
요건을 셋으로 둔 이유를 함께 적는다. 유효창은 인가를 대체하지 않고 더해지므로 검수자 권한은 그대로 필요하고, 유효창이 열려 있더라도 현재 패스워드를 한 번 더 받는다. 그렇게 하지 않으면 잠깐 열린 창을 가로챈 사람이 자격 자체를 갈아 치워 정당한 운영자를 잠글 수 있다.
...
- **note**: 유효창이 없거나 이미 끝난 상태에서 제출하면 관리자 패스워드를 다시 확인해 유효창을 연다. 이 확인이 여는 것은 역할 승격이 아니라 그 사람에게 잠깐 열리는 창이며, 검수자 권한은 그대로 필요하다.
- **type**: Dialog
- **label**: 관리자 확인
- **state**: hidden
...
- **note**: 관리 기능의 쓰기는 새 패스워드로 관리자 확인을 다시 거쳐야 열린다. 조회는 검수자 권한만으로 계속 되므로 화면이 통째로 잠긴 것처럼 안내하지 않는다.
- **type**: Text
...
다음에 할 일을 분명히 한다. 관리 기능의 쓰기를 이어서 하려면 새 패스워드로 관리자 확인을 다시 거쳐야 한다. 조회는 검수자 권한만으로 계속 되므로 화면이 통째로 잠긴 것처럼 안내하지 않는다.
...
- **source_hash**: 6b4c260eb6827e570ef605a1033939d17e2fdc0aafc64ab593ca2e9158038d0e
- **generated_at**: 2026-08-27T14:44:34.445Z

...
synced_at: 2026-09-15T13:22:04.651Z
status: CHANGED
prev_version: 20
content_hash: 83a2f001bf5a9b0fcba085ae25dd09ad05ab411471869bf37ec17485283abb8c
stale: true
...
★두 경로 모두 제공하지 않는 것 — 키포인트·선택 객체 추적·트랙 번호 변경/병합·검수·버전관리, 그리고 외부 시계열 분석 서버로 나가는 위탁 연동(호출·콜백)이다(ADR-013).
[★AI 보조 제공 (ADR-013, 2026-09-15 확정)] 두 경로 모두 라벨링 화면에서 AI 보조 세 기능 — AI 탐지·AI 분할·AI 자동 추적 — 을 포털 전용 창구로 제공한다. 결과는 작업본에만 올라가고 확정은 각 경로의 기존 저장으로 하므로 이 모델의 저장 구조와 저장 도형은 바뀌지 않는다. ⚠ 구 서술 폐기 — 「오토라벨링(YOLO)·SAM2 인터랙티브 분할·SAM2 자동추적 미제공 · SAM2·키포인트 미제공은 2026-08-03 보안 판정으로 서버에서 제거된 축이라 그대로다」는 더 이상 사실이 아니다. 키포인트 미제공은 그대로다.
...
라벨 도형은 두 경로 모두 BBOX/POLYGON 이며, 사람이 그리거나 AI 보조(탐지·분할·자동 추적) 결과를 작업본에 올린 뒤 저장으로 확정한다. 두 경로가 각자 두는 서버 허용목록이 그 외 도형을 400 으로 거부한다. ⚠ 구 서술 폐기(2026-09-15) — 「라벨 편집은 두 경로 모두 수동 라벨링(BBOX/POLYGON)만」은 더 이상 사실이 아니다(ADR-013).

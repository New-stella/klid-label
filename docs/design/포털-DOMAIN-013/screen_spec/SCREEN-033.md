...
synced_at: 2026-09-15T13:22:04.749Z
status: CHANGED
prev_version: 43
content_hash: 627337ad8469855c9291199bf95d68f6906b72c7e32db60942e4d8fed33ea9f6
stale: true
raw: ./_raw/SCREEN-033.json
links:
  based_on: ["[[ADR-013]]"]
  belongs_to_domain: ["[[DOMAIN-013]]"]
  consumes: ["[[API-140]]", "[[API-142]]", "[[API-151]]", "[[API-157]]", "[[API-159]]", "[[API-161]]", "[[API-163]]", "[[API-166]]", "[[API-169]]", "[[API-171]]", "[[API-231]]", "[[UI-131]]", "[[UI-147]]", "[[UI-148]]", "[[UI-149]]", "[[UI-151]]", "[[UI-152]]", "[[UI-153]]", "[[UI-154]]", "[[UI-155]]"]
...
- **note**: PROCESSING이면 비활성 + 툴팁. window.confirm 후 요청, BE 409(처리중)는 안내 문구로 매핑. 글자 없이 휴지통 아이콘만 두는 버튼이며, 마우스를 올리거나 키보드로 초점을 옮기면 짧은 설명이 뜬다 — 누를 수 있을 때는 「삭제」, 처리 중이라 잠겼을 때는 왜 지울 수 없는지를 알린다.
- **type**: Button
- **label**: 삭제
...
- **note**: 준비 완료 자산 행에만 노출한다. 증강 결과물 행에는 두지 않는다. 라벨만 받으므로 곧바로 끝나 취소를 두지 않는다. 글자 없이 아이콘만 두는 버튼이며, 옆의 원본 파일 내려받기와 모양으로 구분되도록 JSON 문서 모양의 아이콘을 쓴다. 마우스를 올리거나 키보드로 초점을 옮기면 「라벨 JSON 내보내기」라는 짧은 설명이 뜨고, 다른 내려받기가 진행 중이라 잠겼을 때는 그것이 끝난 뒤에 받을 수 있다고 알린다.
- **type**: Button
- **label**: 라벨 내보내기
...
- **note**: 준비 완료 자산 행에만 노출한다. 증강 결과물 행에는 두지 않는다. 자산 원본을 그대로 받으므로 영상이면 매우 커질 수 있고, 받는 중에는 취소할 수 있어야 한다. 글자 없이 아이콘만 두는 버튼이며 내려받기 아이콘을 쓴다. 마우스를 올리거나 키보드로 초점을 옮기면 「원본 파일 내려받기」라는 짧은 설명이 뜨고, 다른 내려받기가 진행 중이라 잠겼을 때는 그것이 끝난 뒤에 받을 수 있다고 알린다.
- **type**: Button
- **label**: 원본 파일 내려받기
...
- **note**: 원본 파일을 받는 중일 때만 '원본 파일 내려받기' 자리에 나타난다. 누르면 전송을 멈추고 다시 받을 수 있는 상태로 되돌린다. 사용자가 스스로 멈춘 것이므로 실패 안내를 띄우지 않는다. 글자 없이 닫기 모양 아이콘만 두는 버튼이며, 마우스를 올리거나 키보드로 초점을 옮기면 「내려받기 취소」라는 짧은 설명이 뜬다.
- **type**: Button
- **label**: 내려받기 취소
...
신규 화면 — 포털 사용자 본인 영상 자산 업로드. 내부 파이프라인·데이터마트와 갈라진 경로이며(자산은 공용 원장에 적재하고 출처 판별자로 가른다 — ADR-058), 배치 오토라벨링·검수·버전관리 미제공(라벨링 화면의 AI 보조 세 기능은 제공).

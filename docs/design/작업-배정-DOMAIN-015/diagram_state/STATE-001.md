...
# 작업·검수 워크플로 상태 전이 (LS_RAW_DATA_STATUS.DATA_STTS_CD)
...
stateDiagram-v2
    [*] --> PENDING: 영상 적재
    PENDING --> BATCH_QUEUED: 마킹 완료 (커밋 이후)
    BATCH_QUEUED --> PROCESSING: 배치 파이프라인 시작
    PROCESSING --> ASSIGNED: 파이프라인 성공 — 이 축은 ASSIGNED 로 복귀
    PROCESSING --> FAILED: 단계 실패
    FAILED --> PROCESSING: 재시도 (최대 시도 횟수 이내)
    PENDING --> ASSIGNED: REVIEWER 작업자 배정 (APPROVED 외 상태에서 가능)
    ASSIGNED --> ASSIGNED: REVIEWER 재배정 (배정 행만 교체, 상태값 불변)
    ASSIGNED --> PENDING: WORKER 검수 제출
    PENDING --> ASSIGNED: WORKER 검수 제출 취소
    PENDING --> IN_REVIEW: REVIEWER 검수 시작
    IN_REVIEW --> APPROVED: 검수 승인 (작업 완료)
    IN_REVIEW --> REJECTED: 검수 반려
    REJECTED --> PENDING: WORKER 재제출
    APPROVED --> PENDING: 승인 후 수정분 재검수 재제출 (WORKER, REVLT_YN='Y')
    APPROVED --> APPROVED: 재승인 (상태 머신을 거치지 않고 재검토 표식만 해제)
    APPROVED --> [*]
...
코드 확정 상태값 9종(LsRawDataStatus.STTS_*) 중 실제로 전이가 일어나는 8종을 그린다 — PENDING·ASSIGNED·BATCH_QUEUED·PROCESSING·FAILED·IN_REVIEW·APPROVED·REJECTED. 배치와 검수 두 흐름이 DATA_STTS_CD 단일 컬럼을 공유한다.
...
[단계 순서 — 배치 단계 축이 그린다] 적재 직후의 선두 비식별과 마킹 이후의 잔여 배치가 어떤 단계로 진행되는지는 이 축의 값이 아니라 배치 단계 축(LS_DATA_RAW.DATA_STTS_CD)의 값이며 STATE-002 가 그린다. 이 축이 그 흐름과 맞물리는 지점은 둘뿐이다 — 마킹 완료가 이 축을 BATCH_QUEUED 로 큐잉하고, 배치가 끝나면 성공 시 ASSIGNED 로 복귀하거나 실패 시 FAILED 로 선다.

## entity_name

영상 작업 (LS_RAW_DATA_STATUS)

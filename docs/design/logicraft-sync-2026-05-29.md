...
8. 🔶 **(선택) API/화면/모듈 모델링 (D8)** (2026-05-30 완료): **신규 도메인 6종**(DOMAIN-011~016) + **api_endpoint 89종**(coverage 89/89) + **screen_spec 29종**(coverage 29/29, consumes_apis→API 링크) + **code_module 21종**(MOD-001~021, implements_domains/realizes_screens/realizes_features 링크) 등록. **도메인→기능→API→화면→코드모듈 전 계층 추적 연결 완료.** + **ERD/REQ 구현상태 반영**: ERD 활성 7/9·REQ 활성 17/18 implemented(폐기 ERD-005·006, REQ-002 제외). **프로젝트 전체 coverage 92%(176/192), planned 16 = 폐기 항목과 정확히 일치 → 활성 항목 100% implemented.** + **1차 LEGACY 폐기매핑(일부)**: 명백한 폐기 31개(프로젝트관리 일체·게시판·생성형AI·/projects API)를 `deprecation_status=removed`로 반영. ⏳ 잔여(선택): LEGACY deprecated 11·확인필요 9·나머지 분류 + brownfield legacy_source/ADR 보완(날조 금지로 비움) + 선택타입(nfr/adr/glossary 등) + **`generate_brownfield_report`로 K-DOC 변경명세서 export(정부문서 정석 도구)**.

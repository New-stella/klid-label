...
8. 🔶 **(선택) API/화면 모델링 (D8)** (2026-05-30 api_endpoint+screen_spec 완료): **신규 도메인 6종**(DOMAIN-011 마킹·012 비식별화·013 포털·014 시스템설정·015 작업배정·016 관제통지) + **api_endpoint 89종**(API-001~089, coverage 89/89 implemented — 운영 엔드포인트 전체 + 포털 TUS 5 + 영상 stream) + **screen_spec 29종**(SCREEN-001~029, FE 화면 29[pages 25+auth 4], coverage 29/29 implemented, consumes_apis→API 링크 정상·unresolved 0) 등록. 16개 도메인에 매핑, FEAT(API)·API(SCREEN) 링크 연결. ⏳ 잔여: **code_module(MOD)** 미반영(다음 세션) + brownfield legacy_source/ADR 보완(날조 금지로 비움) + SCREEN screen_spec의 FEAT 링크는 스키마상 직접 불가라 purpose 텍스트로 기재.

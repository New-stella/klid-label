...
8. 🔶 **(선택) API/화면/모듈 모델링 (D8)** (2026-05-30 api_endpoint+screen_spec+code_module 완료): **신규 도메인 6종**(DOMAIN-011~016) + **api_endpoint 89종**(API-001~089, coverage 89/89 implemented — 운영 엔드포인트 + TUS 5 + stream) + **screen_spec 29종**(SCREEN-001~029, FE 화면 29, coverage 29/29, consumes_apis→API 링크) + **code_module 21종**(MOD-001~021, BE 도메인 패키지 18+common+ai-server+frontend, implements_domains/realizes_screens/realizes_features 링크 정상·unresolved 0) 등록. **도메인→기능→API→화면→코드모듈 전 계층 추적 연결 완료.** ⏳ 잔여: ERD/REQ implementation 일괄반영 + 1차 LEGACY 91개 폐기매핑 + brownfield legacy_source/ADR 보완(날조 금지로 비움) + 선택타입(nfr/adr/glossary/use_case/domain_event/infra) 정부문서 필요시.

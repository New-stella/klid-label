...
| 날짜 | 버전 | 작성자 | 승인자 | 내용 |
|---|---|---|---|---|
| 2026-05-21 | 1.0 | 박찬기 | - | 최초 작성 (klid-label 패키지 구조 + DDD 계층 기반 그룹핑) |
...
C&V(공통성·변경성) 기준으로 패키지를 그룹핑하여 **14개 컴포넌트**로 분리. 의존 방향은 단방향(Application → Infra).
...
|---|---|---|
| KLID-AT-CO-001 | BatchOrchestration | KLID-AT-UC-001, UC-002, UC-004 |
| KLID-AT-CO-002 | LabelManagement | KLID-AT-UC-006, UC-007 |
| KLID-AT-CO-003 | VersionControl | KLID-AT-UC-010 |
| KLID-AT-CO-004 | Review&Assignment | KLID-AT-UC-008, UC-009 |
| KLID-AT-CO-005 | MetaReview | KLID-AT-UC-005 |
| KLID-AT-CO-006 | Augment | KLID-AT-UC-011 |
| KLID-AT-CO-007 | PortalChannel | KLID-AT-UC-012 |
| KLID-AT-CO-008 | Statistics | - |
| KLID-AT-CO-009 | SystemConfig | - |
| KLID-AT-CO-010 | ExternalClients | KLID-AT-UC-001, UC-002, UC-004, UC-010, UC-011 |
| KLID-AT-CO-011 | WebhookGateway | KLID-AT-UC-002, UC-004, UC-011 |
| KLID-AT-CO-012 | SecurityCore | KLID-AT-UC-013 |
| KLID-AT-CO-013 | ControlNotify | KLID-AT-UC-014, UC-015 |
| KLID-AT-CO-014 | ObservabilityCommon | - |
...
|---|---|---|---|
| KLID-AT-CO-001 | BatchOrchestration | Quartz 트리거 → 마킹→VLM(콜백)→비식별→프레임추출(마킹 위치)→YOLO(원본만)→SAM2→보간 7단계 자동 파이프라인 오케스트레이션 | UC-001, UC-002, UC-004 |
| KLID-AT-CO-002 | LabelManagement | 라벨/라벨마스터/속성 CRUD, IDOR 가드, SAM2 트랙 보조 | UC-006, UC-007 |
| KLID-AT-CO-003 | VersionControl | Gitea 자동 커밋·diff·롤백, 폴백 큐(메모리+DB) | UC-010 |
| KLID-AT-CO-004 | Review&Assignment | 작업 배정·재배정, 검수 상태 전이(StateMachine), 내부 알림 | UC-008, UC-009 |
| KLID-AT-CO-005 | MetaReview | VLM 시계열 메타 조회·수정·승인·반려 | UC-005 |
| KLID-AT-CO-006 | Augment | 외부 생성형 AI 비동기 요청·검토. 증강 결과 = 새 영상(RAW_SN) 생성 + 라벨 복사 | UC-011 |
| KLID-AT-CO-007 | PortalChannel | TUS 업로드 + 데이터마트 영상 Load + 사용자 작업 별도 적재(LS_PORTAL_USER_LABEL) | UC-012 |
| KLID-AT-CO-008 | Statistics | 대시보드 요약, 워커/전체 통계, CSV 리포트 | - |
| KLID-AT-CO-009 | SystemConfig | 시스템 설정값 조회·갱신, 헬스 | - |
| KLID-AT-CO-010 | ExternalClients | AiServer/Deidentify/Vlm/Gitea/ExternalAugment WebClient + Resilience4j | UC-001~004, UC-010, UC-011 |
| KLID-AT-CO-011 | WebhookGateway | Deidentify/VLM/Augment 결과 수신 + 멱등성 원장 | UC-002, UC-004, UC-011 |
| KLID-AT-CO-012 | SecurityCore | JWT 인증 필터, HMAC 웹훅 필터, SecurityConfig, Role/Channel/RoleHierarchy | UC-013 |
| KLID-AT-CO-013 | ControlNotify | 관제서버 TASK_COMPLETED/MODIFIED 통지 (디바운스·폴백·이벤트 리스너) | UC-014, UC-015 |
| KLID-AT-CO-014 | ObservabilityCommon | RequestIdFilter, Health, Metrics, 공통 예외 처리 | - |
| KLID-AT-CO-015 | Marking | 영상별 자동/수동 마킹, VLM 콜백 요청, 마킹 CRUD API | UC-001 |
...
**컴포넌트 개요**: Quartz 스케줄러가 처리 대기 영상을 1건씩 선점하여 6단계 파이프라인을 실행. 실패 시 지수 백오프 재시도 큐 적재.

**내부 클래스**

| ID | 클래스명 | 설명 |
|---|---|---|
| KLID-AT-IC-001 | BatchOrchestrator | 6단계 순차 호출, 단계별 상태 전이 기록 (= KLID-AT-DC-010) |
| KLID-AT-IC-002 | VlmTimeseriesStep | VLM 비동기 위탁 (= KLID-AT-DC-011) |
| KLID-AT-IC-003 | DeidentifyStep | 비식별 위탁 + 결과 경로 (= KLID-AT-DC-012) |
| KLID-AT-IC-004 | FfmpegFrameExtractor | 원본·비식별본 프레임 추출 (= KLID-AT-DC-013) |
| KLID-AT-IC-005 | YoloAutolabelStep | YOLO 추론 (= KLID-AT-DC-014) |
| KLID-AT-IC-006 | Sam2SegmentStep | SAM2 세그멘테이션 (= KLID-AT-DC-015) |
| KLID-AT-IC-007 | TrackInterpolationStep | 트랙 선형 보간 (= KLID-AT-DC-016) |
| KLID-AT-IC-008 | BatchStatusService | 단계 상태 전이 기록 (= KLID-AT-DC-017) |
| KLID-AT-IC-009 | BatchRetryQueue | 실패 재시도 큐 (max=3 exp backoff) |
| KLID-AT-IC-010 | LabelingBatchQueueService | 대기 큐 선점/적재 (= KLID-AT-DC-018) |
...
| KLID-AT-IC-016 | LabelService | 라벨 영속·Gitea 커밋 트리거 (= KLID-AT-DC-022) |
| KLID-AT-IC-017 | LabelMasterService | 라벨 마스터 CRUD (= KLID-AT-DC-024) |
| KLID-AT-IC-018 | LabelAttrService | 속성 CRUD (= KLID-AT-DC-025) |
| KLID-AT-IC-019 | LabelAccessGuard | IDOR 방어 (= KLID-AT-DC-023) |
| KLID-AT-IC-020 | Sam2TrackService | N프레임 폴리곤 전파 (= KLID-AT-DC-026) |
| KLID-AT-IC-021 | DeidentReportService | 비식별 누락 신고 (= KLID-AT-DC-027) |
...
| KLID-AT-IC-023 | VersionService | 커밋·diff·롤백 (= KLID-AT-DC-028) |
| KLID-AT-IC-024 | GiteaPathPolicy | 경로 정책 (= KLID-AT-DC-029) |
| KLID-AT-IC-025 | GiteaCommitFallbackQueue | 메모리 폴백 (= KLID-AT-DC-030) |
| KLID-AT-IC-026 | GiteaFallbackQueueService | DB 영속 폴백 (= KLID-AT-DC-031) |
...
| KLID-AT-IC-031 | AssignmentService | 배정·재배정·이력 (= KLID-AT-DC-036) |
| KLID-AT-IC-032 | TaskBoardService | 작업 보드 조회 (= KLID-AT-DC-037) |
| KLID-AT-IC-033 | ReviewController | `/v1/reviews/...` |
| KLID-AT-IC-034 | ReviewService | 검수 워크플로우 (= KLID-AT-DC-038) |
| KLID-AT-IC-035 | ReviewStateMachine | 상태 전이 검증 (= KLID-AT-DC-039) |
| KLID-AT-IC-036 | NotificationService | 내부 REVIEWER 알림 인터페이스 (= KLID-AT-DC-045) |
...
| KLID-AT-IC-039 | MetaService | 메타 CRUD + 승인/반려 (= KLID-AT-DC-032) |
...
| KLID-AT-IC-041 | AugmentRequestService | 비동기 요청 (= KLID-AT-DC-034) |
| KLID-AT-IC-042 | AugmentReviewService | 검토 (= KLID-AT-DC-035) |
| KLID-AT-IC-043 | ExternalAugmentClient | 외부 클라이언트 (= KLID-AT-DC-005) |
| KLID-AT-IC-044 | AugmentResultController | `/v1/augments/result` (웹훅) |
| KLID-AT-IC-045 | AugmentResultService | 결과 적재 + 멱등성 (= KLID-AT-DC-021) |
...
| KLID-AT-IC-047 | PortalLabelService | 본인 데이터 라우팅 (= KLID-AT-DC-040) |
| KLID-AT-IC-048 | PortalAutolabelService | YOLO 추론 위임 (= KLID-AT-DC-041) |
| KLID-AT-IC-049 | PortalUploadService | 포털 영상 영속화 (= KLID-AT-DC-042) |
| KLID-AT-IC-050 | TusUploadController | `/v1/portal/uploads` (TUS 프로토콜) |
| KLID-AT-IC-051 | TusService | TUS 세션·청크 처리 (= KLID-AT-DC-043) |
...
| KLID-AT-IC-053 | StatsService | 통계 산출 (= KLID-AT-DC-048) |
| KLID-AT-IC-054 | VideoQueryService | 영상 목록·상세 (= KLID-AT-DC-047) |
...
| KLID-AT-IC-059 | PresetService | 프리셋 CRUD (= KLID-AT-DC-033) |
...
| ID | 클래스명 | 설명 |
|---|---|---|
| KLID-AT-IC-060 | AiServerClient | 내부 AI 서버 (YOLO/SAM2/VLM-verify) (= KLID-AT-DC-001) |
| KLID-AT-IC-061 | DeidentifyClient | 외부 비식별 (= KLID-AT-DC-002) |
| KLID-AT-IC-062 | VlmClient | 외부 VLM (= KLID-AT-DC-003) |
| KLID-AT-IC-063 | GiteaClient | Gitea REST (= KLID-AT-DC-004) |
| KLID-AT-IC-064 | ExternalAugmentClient | 외부 생성형 AI (= KLID-AT-DC-005) |
...
| KLID-AT-IC-069 | DeidentifyResultService | 결과 적재 (= KLID-AT-DC-019) |
| KLID-AT-IC-070 | VlmResultService | 메타 적재 (= KLID-AT-DC-020) |
| KLID-AT-IC-071 | AugmentResultService | 증강 결과 적재 (= KLID-AT-DC-021) |
| KLID-AT-IC-072 | WebhookIdempotencyLedger | 멱등성 인터페이스 (= KLID-AT-DC-009) |
...
| ID | 클래스명 | 설명 |
|---|---|---|
| KLID-AT-IC-075 | SecurityConfig | Spring Security 체인 (= KLID-AT-DC-008) |
| KLID-AT-IC-076 | JwtAuthenticationFilter | JWT 파싱·검증 (= KLID-AT-DC-006) |
| KLID-AT-IC-077 | HmacWebhookFilter | 웹훅 HMAC 서명 검증 (= KLID-AT-DC-007) |
...
| KLID-AT-IC-084 | WorkLockService | 작업 잠금 (= KLID-AT-DC-044) |
...
**컴포넌트 개요**: 검수 승인·라벨/메타 수정 이벤트를 받아 관제서버에 비동기 통지. **내부 클래스**

| ID | 클래스명 | 설명 |
|---|---|---|
| KLID-AT-IC-088 | ControlNotifyClient | 통지 HTTP 클라이언트 (= KLID-AT-DC-049) |
| KLID-AT-IC-089 | ControlNotifyDebouncer | 60s 디바운서 (= KLID-AT-DC-050) |
| KLID-AT-IC-090 | ControlNotifyFallbackQueue | DB 영속 폴백 큐 (= KLID-AT-DC-051) |
| KLID-AT-IC-091 | ControlNotifyEventListener | 도메인 이벤트 리스너 (= KLID-AT-DC-052) |
...
| KLID-AT-IF-040 | IExternalControlServer | POST /api/v1/tasks/{rawSn}/completed, /modified | Required (외부) |
...
|---|---|---|---|
| **오퍼레이션명** | process | |
| **오퍼레이션 개요** | 단일 영상에 대해 VLM→비식별→프레임추출→YOLO→SAM2→보간 6단계 파이프라인을 순차 실행하고 BatchStage 결과 반환 | |
...
| **사후조건** | 비동기 처리 요청 완료, 결과는 웹훅(IF-029)으로 수신 | |
...
| **사후조건** | 비동기 처리 요청 완료, 결과는 웹훅(IF-030)으로 수신 | |
...
|---|---|---|---|
| **오퍼레이션명** | list, update | |
| **오퍼레이션 개요** | 시스템 설정 키-값 전체 조회 및 갱신. ADMIN 전용 | |
| **사전조건** | actor ADMIN 권한 | |
...
| **사전조건** | actor ADMIN 권한 (create/update/delete/clone) | |

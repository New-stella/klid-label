...
> 스코프 정본은 `.kit-scope.json`(108건) · 버전은 `version-master.md`.
...
[★단일 검수 — 1차/2차 단계 폐기(ADR-002)] 구 서술의 '1차 검수 → 2차 검수' 2단계 워크플로우는 폐기됐다. 검수자 1인이 승인할 때까지 반려↔재제출을 반복하는 단일 검수다. 마찬가지로 '관리자 확인 요청'도 폐기됐다 — 검수 승인이 곧 작업 종결이라 그 위에 확인 단계를 두지 않는다. ⚠ 구 근거 폐기 — *별도 ADMIN 역할이 없고 모든 관리 권한이 REVIEWER 에 통합됐기 때문이다(ADR-003)* 는 더 이상 성립하지 않는다. ADR-055 로 관리자 역할이 분리됐고 계층(ROLE_ADMIN > ROLE_REVIEWER)으로 검수 권한을 물려받는다. 폐기 결론 자체는 그대로이나 근거가 바뀌었다. 검수자↔작업자 소통은 반려·문의를 통합한 이슈 스레드(DFEAT-049)가 대신한다.
...
| # | 단계 | 이번 키트 ITEM |
|---|---|---|
| 1 | ADR 결정·제약 | [[ADR-001]] · [[ADR-002]] · [[ADR-004]] · [[ADR-007]] · [[ADR-009]] · [[ADR-015]] · [[ADR-019]] · [[ADR-020]] · [[ADR-031]] · [[ADR-055]] |

...
status: CHANGED
prev_version: 2
content_hash: 2006b6caa97ae7a6bb5879716149fd7a5efe9b8089b0ec3de90a80dfe205c06b
stale: false
...
[외부에서 가져온 산출물은 개인정보 판정을 다른 자리에서 읽는다] 산출물에 싣는 익명 여부·가명 여부·개인정보 포함 여부 가운데 원천 축은 관제가 보낸 것을 기록하는 인입 원장에만 착지 자리가 있다. 외부 산출물 이관으로 들어온 영상은 그 원장을 거치지 않아 거기서 읽을 수 없으므로, 적재할 때 메타에 원문 그대로 보관해 둔 값을 읽는다. 프레임 단위 판정은 가져올 때 무엇으로 지정했는지에 따라 갈린다 — 비식별이 끝난 것으로 지정해 가져왔으면 그 판정이 비식별 축과 그대로 일치해 프레임 단위 자리에 적재되므로 종전과 같은 곳에서 읽고, 원본이라고 지정해 가져왔으면 그 값은 원천 축이라 착지할 자리가 없어 메타에서 읽는다. 어느 쪽이든 값이 없으면 지어내지 않는다. 이 분기는 이 경로로 들어온 영상에만 적용되며, 관제가 보낸 영상의 조달 순서는 그대로다.

...
synced_at: 2026-09-02T10:51:57.073Z
status: CHANGED
prev_version: 16
content_hash: faa13b492f028df735977e52bfcadab6798a988931088b158e70a1c9ded34396
stale: false
...
links:
  ...
  belongs_to_domain_backward: ["[[AC-1044]]", "[[AC-1045]]", "[[AC-1046]]", "[[AC-1047]]", "[[AC-1048]]", "[[AC-1049]]", "[[ADR-004]]", "[[ADR-018]]", "[[ADR-023]]", "[[ADR-044]]", "[[ADR-045]]", "[[ADR-059]]", "[[API-059]]", "[[API-060]]", "[[API-061]]", "[[API-062]]", "[[API-063]]", "[[API-165]]", "[[API-179]]", "[[API-188]]", "[[API-189]]", "[[API-190]]", "[[CDIAG-010]]", "[[CMP-007]]", "[[DFEAT-029]]", "[[DFEAT-030]]", "[[ERD-011]]", "[[EVT-011]]", "[[INT-006]]", "[[INT-008]]", "[[REQ-023]]", "[[SCREEN-022]]", "[[SCREEN-023]]", "[[UC-001]]", "[[UC-002]]", "[[UC-003]]"]
  derived_domain_backward: ["[[AC-1044]]", "[[AC-1045]]", "[[AC-1046]]", "[[AC-1047]]", "[[AC-1048]]", "[[AC-1049]]"]
...
[증강 2계열]
· 외부 생성형 AI 위탁 — 증강 종류 코드는 단일값 AUGMENT 이며 생성 조건에서 파생하지 않는다. 무엇으로 바꿀지는 생성 조건(시간대·계절·날씨·지형·심각도)이 정한다(ADR-059). 이미지-to-이미지라 영상(비디오)을 재생성하지 않고 부모의 비식별 영상 파일을 복사한 뒤 프레임 이미지만 변환한다. 해상도가 같으므로 라벨 좌표는 그대로 복사한다. 구 WINTER/NIGHT/RAIN 은 이미 만들어진 파생본에 남아 있어 보존한다(백필하지 않는다) — 조회·표시 경로는 옛 값과 새 값을 모두 견뎌야 한다.
...
[미사용 파생 폐기] 반려된 파생은 작업 대상에서 빠지고 유예 7일 후 배치가 DB 행과 파일까지 실삭제한다(ADR-045). 삭제 대상은 파생 영상·반려·유예 경과를 모두 충족해야 하며, 검수 승인 이력이 있으면 대상에서 제외한다(관제 접근 보장 정책). 조건을 최종 DELETE SQL 에 리터럴로 박는다.
...
- **term**: 외부 증강
- **meaning**: 생성형 AI 에 위탁하는 이미지-to-이미지 변환. 증강 종류 코드는 단일값 AUGMENT 이며 생성 조건에서 파생하지 않는다 — 무엇으로 바꿀지는 생성 조건(시간대·계절·날씨·지형·심각도)이 정한다. 구 WINTER/NIGHT/RAIN 은 이미 만들어진 파생본에 남아 있어 보존한다(백필하지 않는다) — 조회·표시 경로는 옛 값과 새 값을 모두 견뎌야 한다

...
| 8 | **파일 배치 완료 <u>후</u> INSERT (Critical)** | `RAW_FILE_PATH_NM` 이 가리키는 파일이 **최종 경로에 완전히 놓인 뒤** INSERT 한다. 복사·전송 중인 경로로 INSERT 하면 저작도구 폴링이 **부분 영상(잘린 파일)** 을 정상 영상으로 적재한다 — 아래 §7.2 |

### 7.2 파일 배치와 INSERT 의 순서 (Critical)

저작도구 폴링은 인입 행이 생긴 순간부터 최대 60초 주기로 `RAW_FILE_PATH_NM` 경로를 본다.
그 시점에 파일이 **복사 중**이면 그 중간 상태를 완성본으로 오인해 적재한다.

| 항목 | 내용 |
|---|---|
| **요구** | 파일을 최종 경로에 **완전히 배치한 뒤** INSERT 한다 (파일 → 행, 순서 고정) |
| **권장 방식** | 같은 파일시스템 내 **원자 rename** (`mv`), 또는 **임시 이름으로 전송 후 rename**. `scp`/`rsync` 로 최종 파일명에 직접 쓰면 그 사이 부분 파일이 노출된다 (`rsync --partial-dir` / `--delay-updates` 권장) |
| **하면 안 되는 것** | 전송을 시작하면서 INSERT / 전송 중인 경로로 INSERT / 큰 파일을 최종 이름으로 직접 복사 |
| **저작도구가 걸러 주는 범위** | **크기 0 바이트만** 걸러 “아직 미도착”으로 대기시킨다(§8 완결성 게이트). **1바이트라도 쓰였으면 완성본으로 간주해 적재한다** — 부분 영상은 저작도구가 판별할 수 없다 |
| **적재되면 되돌리기 어렵다** | 부분 영상이 적재되면 `LS_DATA_RAW` 가 생기고 비식별·프레임추출 파이프라인이 그 파일로 기동한다. 인입 행은 `DONE` 이라 재큐 통로(`FAILED` 전용)도 닫힌다 — 저작도구에 수기 정정을 문의해야 한다 |

> 이 계약은 **관제 측 배치 절차에 대한 의존**이다. 저작도구 자신의 쓰기 경로(내부 REVIEWER 업로드)는
> 원자 rename + 배타 예약으로 부분 파일이 노출되지 않도록 닫혀 있으나, 관제 NAS 에 관제가 직접 쓰는
> 경로는 저작도구가 통제할 수 없다.
...
| **완결성 게이트 — 0바이트만 거른다** | 경로에 파일이 있어도 **크기가 0** 이면 "아직 도착하지 않음"으로 보고 대기시킨다. ⚠ **그 이상은 판별하지 않는다** — 부분 전송된 파일(1바이트 이상)은 완성본으로 간주해 적재한다. 크기 0 은 어떤 영상 컨테이너도 될 수 없어 오탐이 구조적으로 없는 반면, "최소 크기"·"크기가 안정될 때까지 관측" 같은 규칙은 정상 영상을 굶기거나 판정이 주기·시계에 의존하게 되기 때문이다. 부분 영상 차단은 §7.2(관제의 파일 배치 규약)에 의존한다 |
| **좀비 회수** | 클레임(`PROCESSING`) 한 노드가 종결을 찍기 전에 죽어도(롤링 재기동·OOM) 그 행은 방치되지 않는다 — 경과 임계값(기본 2시간)을 넘긴 `PROCESSING` 행을 `PENDING` 으로 되돌린다. 대기 예산(`PRCS_DT`)·재시도 횟수(`RTY_CNT`)·관제 수신값은 그대로 둔다 |

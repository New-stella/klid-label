...
erDiagram
    ...
    LS_DATA_LBL ||--o{ V_COMPLETED_LABEL : "검수완료 라벨 좌표"
    LS_LABEL ||--o{ V_COMPLETED_LABEL : "라벨 마스터 조인"
    LS_DATA_LBL_ATTR_VAL ||--o{ V_COMPLETED_LABEL_ATTR : "라벨 속성값"
    LS_LABEL_ATTR ||--o{ V_COMPLETED_LABEL_ATTR : "속성 정의 조인"
...
> ※ 본 ERD의 5개 항목(V_COMPLETED_*)은 물리 테이블이 아닌 읽기 전용 View 이며, 검수 완료(LS_RAW_DATA_STATUS.DATA_STTS_CD='APPROVED', 메타는 LS_DATA_META_REVIEW.RVW_STTS_CD='APPROVED') 영상만 노출하는 평면 인터페이스다. 관제서버가 완료/수정 통지 수신 후 RAW_SN 으로 단순 SELECT 해 데이터마트에 영상 1건=1행으로 UPSERT 한다. 모두 `CREATE OR REPLACE VIEW` 라 멱등이며, 데이터마트 소비 컨트랙트(출력 컬럼명)는 물리 컬럼 rename(V60/V61) 이후에도 alias 로 보존된다. 비식별 영상 파일 경로는 운영 컨벤션(STORAGE_RAW_PATH→STORAGE_DEIDENTIFIED_PATH) 치환으로 도출하므로 View 에는 원본 경로만 노출한다(애플리케이션 레이어 책임).
...
| 관련 클래스 ID | - ※ | 관련 클래스명 | - ※ |
| 관련 유스케이스 ID | KLID-AT-UC-009 |
| 엔티티 설명 | 데이터마트 적재용 검수 완료 영상 메타 View(`V_COMPLETED_VIDEO`). 원시영상과 영상진행상태를 검수 승인(APPROVED) 기준으로 조인해 평면화한다. 물리 테이블이 아닌 읽기 전용 View 이며 관제서버가 RAW_SN 으로 단순 SELECT 한다. 물리 컬럼 rename 이후에도 데이터마트 소비 컨트랙트(출력 컬럼명)는 alias 로 보존한다. ※ View(읽기 전용 산출)로 D1 설계 클래스를 두지 않는다. |
...
| PARENT_RAW_SN | 원본원시영상일련번호 | 숫자 | - | N | N | N | N | - | 증강 원본 추적 |
| BATCH_STTS_CD | 배치상태코드 | 문자 | 32 | Y | N | N | N | - | DATA_STTS_CD alias |
| REVIEW_STTS_CD | 검수상태코드 | 문자 | 32 | Y | N | N | N | - | APPROVED 고정 노출 |
| REVIEW_COMPLETED_AT | 검수완료일시 | 날짜 | - | Y | N | N | N | - | APPROVED 전이 시각 |
| REVIEW_VERSION | 검수버전 | 숫자 | - | Y | N | N | N | - | 낙관적 잠금 VER alias |
...
#### 2.43 KLID-AT-EN-043 — 검수완료라벨뷰

| 엔티티 ID | KLID-AT-EN-043 | 엔티티명 | 검수완료라벨뷰 |
|---------|---|-------|---|
| 관련 클래스 ID | - ※ | 관련 클래스명 | - ※ |
| 관련 유스케이스 ID | KLID-AT-UC-009 |
| 엔티티 설명 | 데이터마트 적재용 검수 완료 라벨 좌표 View(`V_COMPLETED_LABEL`). 데이터라벨에 프레임의 영상·프레임번호와 라벨 마스터(라벨명·색상·도형 타입)를 조인해 평면화한다. 좌표(POINTS_JSON)는 정규화된 nested 포맷을 일관 노출한다. 라벨 속성값은 검수완료라벨속성뷰로 분리한다. 검수 승인 영상의 라벨만 노출한다. ※ View(읽기 전용 산출)로 D1 설계 클래스를 두지 않는다. |

| 속성명 | 동의어 | 타입 | 길이 | NOT NULL (Y/N) | PK (Y/N) | FK (Y/N) | INX (Y/N) | 기본값 | 제약조건 |
|-------|--------|------|------|--------|------|------|------|-------|--------|
| LBL_SN | 라벨일련번호 | 숫자 | - | Y | N | N | N | - | 라벨 식별자 |
| SRC_SN | 원천일련번호 | 숫자 | - | Y | N | N | N | - | 프레임 식별자 |
| RAW_SN | 원시영상일련번호 | 숫자 | - | Y | N | N | N | - | 영상 식별자 |
| FRAME_NO | 프레임번호 | 숫자 | - | Y | N | N | N | - | FRM_NO alias |
| LBL_TYPE_CD | 라벨유형코드 | 문자 | 16 | Y | N | N | N | - | BBOX/POLYGON/SEGMENT/TRACK |
| LABEL_ID | 라벨아이디 | 숫자 | - | N | N | N | N | - | LBL_ID alias(라벨 마스터) |
| LABEL | 라벨명텍스트 | 문자 | 255 | Y | N | N | N | - | LBL_NM alias |
| POINTS_JSON | 좌표JSON | 문자 | - | N | N | N | N | - | POINT_CN alias(nested 정규화) |
| TRCK_ID | 트랙아이디 | 문자 | 64 | N | N | N | N | - | 자동 추적 객체 ID |
| REG_USER_NO | 등록사용자번호 | 숫자 | - | N | N | N | N | - | - |
| LABEL_REG_DT | 라벨등록일시 | 날짜 | - | Y | N | N | N | - | REG_DT alias |
| LABEL_UPD_DT | 라벨수정일시 | 날짜 | - | N | N | N | N | - | MDFCN_DT alias |
| LABEL_NAME | 라벨마스터명 | 문자 | 64 | N | N | N | N | - | 라벨 마스터 LBL_NM alias |
| LABEL_COLOR | 라벨색상 | 문자 | 7 | N | N | N | N | - | COLR_VL alias |
| LABEL_TYPE | 라벨마스터유형 | 문자 | 16 | N | N | N | N | - | 라벨 마스터 LBL_TYPE_CD alias |
| LABEL_SORT_NO | 라벨정렬순서 | 숫자 | - | N | N | N | N | - | SORT_SEQ alias |

#### 2.44 KLID-AT-EN-044 — 검수완료라벨속성뷰

| 엔티티 ID | KLID-AT-EN-044 | 엔티티명 | 검수완료라벨속성뷰 |
|---------|---|-------|---|
| 관련 클래스 ID | - ※ | 관련 클래스명 | - ※ |
| 관련 유스케이스 ID | KLID-AT-UC-009 |
| 엔티티 설명 | 데이터마트 적재용 검수 완료 라벨 속성값 View(`V_COMPLETED_LABEL_ATTR`). 데이터라벨속성값에 라벨속성정의(속성명·입력유형·옵션·기본값)를 조인해 평면화한다. 검수 승인 영상에 속한 라벨의 속성값만 노출한다. ※ View(읽기 전용 산출)로 D1 설계 클래스를 두지 않는다. |

| 속성명 | 동의어 | 타입 | 길이 | NOT NULL (Y/N) | PK (Y/N) | FK (Y/N) | INX (Y/N) | 기본값 | 제약조건 |
|-------|--------|------|------|--------|------|------|------|-------|--------|
| ATTR_VAL_ID | 속성값아이디 | 숫자 | - | Y | N | N | N | - | 속성값 식별자 |
| LBL_SN | 라벨일련번호 | 숫자 | - | Y | N | N | N | - | 라벨 식별자 |
| ATTR_ID | 속성아이디 | 숫자 | - | Y | N | N | N | - | 속성 정의 식별자 |
| ATTR_NAME | 속성명 | 문자 | 64 | N | N | N | N | - | ATTR_NM alias |
| ATTR_INPUT_TYPE | 속성입력유형 | 문자 | 16 | N | N | N | N | - | INPUT_TYPE_CD alias |
| ATTR_VALUES_JSON | 속성옵션목록 | 문자 | 1000 | N | N | N | N | - | VALUES_CN alias |
| ATTR_DEFAULT_VAL | 속성기본값 | 문자 | 255 | N | N | N | N | - | DFLT_VL alias |
| ATTR_VAL | 속성값 | 문자 | 1000 | N | N | N | N | - | 객체별 입력값 |
| REG_DT | 등록일시 | 날짜 | - | Y | N | N | N | - | - |
| MDFCN_DT | 수정일시 | 날짜 | - | N | N | N | N | - | - |

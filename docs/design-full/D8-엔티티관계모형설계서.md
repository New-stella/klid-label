...
erDiagram
    ...
    LS_DATA_SRC ||--o{ LS_DATA_LBL_HSTRY : "라벨 변경 이력"
    LS_DATA_LBL {
        bigint LBL_SN PK
        bigint SRC_SN FK
        string LBL_TYPE_CD
...
| 엔티티 설명 | 라벨 변경 이력. **용도 확장(V112, 2026-07-20)**: 기존에는 비식별 누락 신고 시 해당 영상 라벨을 일괄 삭제하기 전 삭제 대상 라벨·프레임 식별자와 삭제 시점만 보존하는 '삭제 전용 감사'였으나, 변경종류코드·등록자아이디 컬럼을 추가해 라벨 저장(bulkUpsert) 시점의 추가/수정 이력까지 포괄하는 **변경 이력**(ADDED/UPDATED/DELETED)으로 확장했다(라벨 본문 삭제·수정 후에도 이력 존속). ※ 기존 R1 핵심 범위본(`docs/design`)에는 KLID-AT-EN-040으로 반영돼 있었으나, 본 전체범위본에서는 §2 번호가 이미 EN-040(영상업로드세션)·EN-041~045(데이터마트 View — V114 재편으로 현재 4종·EN-044 결번)에 배정돼 있어 신규 엔티티로 EN-046 채번했다(누락 보완). |

| 속성명 | 동의어 | 타입 | 길이 | NOT NULL (Y/N) | PK (Y/N) | FK (Y/N) | INX (Y/N) | 기본값 | 제약조건 |
|-------|--------|------|------|--------|------|------|------|-------|--------|
| 라벨이력일련번호 | LBL_HSTRY_SN | 숫자 | - | Y | Y | N | N | 자동증가 | - |
| 라벨일련번호 | LBL_SN | 숫자 | - | N | N | N | Y | - | 변경 대상 라벨 식별자(물리 FK 미설정·이력 보존) |
| 프레임원천일련번호 | SRC_SN | 숫자 | - | Y | N | N | Y | - | 논리 FK(→프레임원천) |
| 변경종류코드 | CHG_KIND_CD | 문자 | 20 | N | N | N | N | - | ADDED/UPDATED/DELETED(V112 신설, 기존 row는 DELETED로 backfill) |
| 등록자아이디 | REG_ID | 문자 | 30 | N | N | N | N | - | 작업자 식별자(V112 신설, 삭제 이력 경로는 null — PII 미저장) |
| 이력기록일시 | REGISTERED_AT | 날짜 | - | Y | N | N | N | - | 변경 기록 시점 |

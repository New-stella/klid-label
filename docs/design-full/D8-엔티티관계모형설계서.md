...
erDiagram
    ...
    LS_TUS_UPLOAD {
        string UPLOAD_ID PK
        string USER_NO
        number UPLOAD_LENGTH
        number UPLOAD_OFFSET
        string STATUS
        bigint RAW_SN
        number VERSION
    }
...
| 속성명 | 동의어 | 타입 | 길이 | NOT NULL (Y/N) | PK (Y/N) | FK (Y/N) | INX (Y/N) | 기본값 | 제약조건 |
|-------|--------|------|------|--------|------|------|------|-------|--------|
| UPLOAD_ID | 업로드세션식별자 | 문자 | 36 | Y | Y | N | N | - | UUID(위치 헤더 노출) |
| USER_NO | 세션소유자번호 | 문자 | 64 | Y | N | N | Y | - | 본인 검증(HEAD/PATCH/DELETE) |
| UPLOAD_LENGTH | 업로드전체길이 | 숫자 | - | Y | N | N | N | - | 전체 파일 크기(bytes) |
| UPLOAD_OFFSET | 업로드오프셋 | 숫자 | - | Y | N | N | N | 0 | 조건부 갱신으로 직렬화 |
| STATUS | 상태 | 문자 | 16 | Y | N | N | Y | IN_PROGRESS | IN_PROGRESS/COMPLETED/EXPIRED |
| FILE_PATH | 파일경로 | 문자 | 500 | Y | N | N | N | - | 임시 파일 절대경로(UUID 강제) |
| FILE_NAME | 파일명 | 문자 | 255 | N | N | N | N | - | 표시용 원본 파일명 |
| VMS_CLIP_ID | VMS클립아이디 | 문자 | 64 | N | N | N | N | - | 완료 시 영상 합류 메타 |
| CCTV_ID | CCTV아이디 | 문자 | 64 | N | N | N | N | - | 완료 시 영상 합류 메타 |
| EVENT_TYPE_CD | 이벤트유형코드 | 문자 | 32 | N | N | N | N | - | - |
| LOCAL_GOV_CD | 기관코드 | 문자 | 10 | N | N | N | N | - | - |
| PRVC_TYPE_CD | 개인정보유형코드 | 문자 | 8 | N | N | N | N | - | ANONY/PRVC/PSDO |
| CAPTURED_AT | 촬영일시 | 날짜 | - | N | N | N | N | - | - |
| RAW_SN | 원시영상일련번호 | 숫자 | - | N | N | Y | N | - | 완료 시 생성 영상(멱등 응답) |
| EXPIRES_AT | 만료일시 | 날짜 | - | Y | N | N | Y | - | 생성 +24시간(TTL 회수) |

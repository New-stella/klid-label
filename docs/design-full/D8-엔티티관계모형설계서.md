...
erDiagram
    ...
    LS_RESOLUTION_LBL_MAP {
        bigint RES_LBL_MAP_SN PK
        bigint RES_EXPORT_SN FK
...
| GOAL_RES_CD | 목표해상도코드 | 문자 | 16 | Y | N | N | N | - | (DATA_RAW_SN,GOAL_RES_CD) 유니크 |
...
| 속성명 | 동의어 | 타입 | 길이 | NOT NULL (Y/N) | PK (Y/N) | FK (Y/N) | INX (Y/N) | 기본값 | 제약조건 |
|-------|--------|------|------|--------|------|------|------|-------|--------|
| RES_LBL_MAP_SN | 해상도변경라벨매핑일련번호 | 숫자 | - | Y | Y | N | N | 자동증가 | - |
| RES_EXPORT_SN | 해상도변경산출일련번호 | 숫자 | - | Y | N | Y | Y | - | 논리 FK(→해상도변경산출) |

...
| **if-control-notify-spi** (V1.8) | **비동기 (Outbound 통지)** | EVENT | 영상 단위 TASK_COMPLETED(메타만) / TASK_MODIFIED(변경 프레임 ID + 변경 종류 + 변경 후 라벨/메타 본문 포함), 동일 작업 ID 유지, 디바운스 60s, idempotency + dead-letter + 재등록 큐, 대용량 페이로드 분할 송신 |

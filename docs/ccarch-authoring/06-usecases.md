...
```json
{
  ...
  "content": "**전제**: 영상이 이미 `dataSttsCd='COMPLETED'` 상태. REVIEWER/WORKER 가 라벨(`LS_DATA_LBL`) 또는 메타(`LS_DATA_META`) 를 수정.\n\n**기본 흐름**\n1. LabelService/MetaService 가 COMPLETED 영상 대상 변경 커밋 후 도메인 이벤트 발행 (변경된 프레임 ID 와 변경 종류 포함)\n2. 디바운서가 같은 작업 ID(RAW_SN) 대한 변경을 짧은 시간(운영 결정 — 기본 60s) 동안 통합 — 프레임 단위 변경 항목을 누적 후 마지막 수정 기준으로 1회 통지 준비\n3. ControlNotifyClient 가 if-control-notify-spi 로 `TASK_MODIFIED` 메시지 비동기 등록\n   - **페이로드 (변경 프레임 단위 데이터 포함)**:\n     - 이벤트 타입 + **동일 작업 ID(RAW_SN)** + 마지막 수정 일시 + 요청 ID(idempotency)\n     - **변경 프레임 목록**: 각 항목 `{ srcSn(프레임 ID), changeKind(LABEL_ADDED|LABEL_UPDATED|LABEL_DELETED|META_UPDATED), labels(변경 후 라벨 본문 배열 — bbox/polygon/mask RLE 좌표 + 라벨 종류·속성), meta(변경 후 메타 본문 K/V) }`\n     - 변경 요약 카운트 (전체 라벨 N·메타 M)\n   - **버전/시퀀스 번호 발행 안 함** — 동일 ID 로 마지막 상태를 통지. 관제가 별도 조회 없이 차분 적용 가능\n4. 송신 이력 기록\n\n**대안**\n- 짧은 시간 내 추가 수정 발생 시 디바운스 윈도우 갱신, 마지막 수정 기준으로 1회 통지\n- 송신 실패·미응답: dead-letter + 재등록 큐 (idempotency)\n- 페이로드가 메시지 브로커 크기 제한을 초과할 가능성 — 변경 프레임이 많으면 분할 송신(같은 요청 ID + chunkIndex/total). 임계값은 운영 결정\n- 보안: PII·토큰·원본 비-비식별 이미지 미포함. 비식별 처리된 라벨 좌표·종류·메타 본문은 전달\n\n**PERFORMED_BY**: actor-reviewer, actor-worker, ext-control-server\n**BELONGS_TO**: feat-review-workflow\n**DERIVES_FROM**: req-control-notify",

...
> **수행기관** : 쿠도커뮤니케이션 | **담당** : 박찬기 | **Ver** : 1.4 | **작성일** : 2026-04-29  
...
| V1.3 | 2026-04-29 | ADMIN 역할 제거(요구사항정의서 V1.3 정합) — DB 컬럼 영향 없음, 권한 정책 문서만 정리 |
| V1.4 | 2026-04-29 | **데이터마트(SFR-13) 저작도구 범위 제외** — 3.8 데이터마트·배포 섹션 삭제(`MNG_DATA_SET`, `MNG_DATA_SET_DSTRB`, `MNG_DATA_SET_DSTRB_DTL`, `MNG_DATA_SET_ATCH_FILE` 4개 테이블이 저작도구 활용 대상에서 제외됨). 해당 테이블은 외부제공 시스템이 별도 책임 |
...
### 3.8 데이터마트·배포 (V1.4 — 저작도구 범위 외)

> **V1.4 변경**: SFR-13(학습데이터 외부제공 활용 기능)이 저작도구 범위에서 제외되어, 본 섹션의 데이터마트·배포 테이블 4종(`MNG_DATA_SET`, `MNG_DATA_SET_DSTRB`, `MNG_DATA_SET_DSTRB_DTL`, `MNG_DATA_SET_ATCH_FILE`)은 **외부제공 시스템이 별도 책임**진다. 저작도구는 학습데이터셋 내보내기까지만 담당하며 마트 등록·배포·다운로드 이력은 다루지 않는다.

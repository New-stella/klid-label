...
| TC-FE-1016 | 영상 상세의 같은 표시기는 **무변경**이다 (신설 · 대조군) | 영상 상세 진입 | 렌더 | 표시기 존치(`fill` 전폭 밴드). 컴포넌트·`STAGE_LABEL` 표·`fill` prop 도 그대로 — 제거는 마킹 화면 한정이다 | component | High | `pages/VideoDetailPage.tsx(BatchStageIndicator fill)` · `pages/__tests__/VideoDetailPage.screen009Layout.test.tsx` |
...
| TC-FE-1019 | 검수 승인 영상·파생영상도 마킹 단계가 아니면 차단된다 (신설 · 구 우선순위 케이스 대체) | `status='COMPLETED'` + `reviewSttsCd='APPROVED'` / `derivative=true` | 화면 진입 | 두 경우 모두 같은 차단 안내. 신고 사유 우선순위(파생 > 마킹 단계 > 승인)는 코드에 그대로 있으나 **이 화면에서는 관측 지점이 사라졌다** — 파생 사유 우선순위는 마킹 대기 상태 케이스(TC-FE-312)가 계속 지킨다 | component | Medium | `pages/MarkingPage.tsx(notMarkingStage)` · `pages/__tests__/MarkingPage.test.tsx(검수승인_영상도_마킹단계가_아니므로_진입이_차단된다 · 파생영상이라도_마킹단계가_아니면_진입이_차단된다)` |

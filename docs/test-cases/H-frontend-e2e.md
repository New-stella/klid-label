...
>
> ⚠ **전제 정정(2026-08-24)** — 위탁 창구가 **묘사 + 추가 질문**으로 바뀌면서(`ADR-051`) 판정 창구를 연동하지 않게 됐다. 따라서 **일치도는 새로 생기지 않는다.** 아래 케이스들은 **과거 적재분이 있는 영상**에서 그대로 유효하다 — 기존 행을 지우지 않으므로 화면이 그것을 읽기 전용으로 렌더하는 계약은 계속 필요하다. 신규 영상에서 일치도 행이 보이지 않는 것은 **정상이며 결함이 아니다**.

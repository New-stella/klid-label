...
## H-69. 추적 두 경로의 **부분 결과 계약** — 잘린 응답을 이어 보낸다 (SCREEN-005 · API-123) — 2026-08-18 신설
...
| ID | 목적 | 전제 | 실행 | 기대결과 | 계층 | 우선순위 | 근거 |
|----|------|------|------|----------|:----:|:--------:|------|
| TC-FE-872 | 잘린 응답이 오면 남은 프레임을 이어 보내 결과가 유실되지 않는다 (신설) | 5프레임 중 앞 2건만 처리한 `truncated` 응답 | 추적 실행 | 두 번째 요청이 `resume` 값 그대로 나가고 다섯 프레임이 모두 결과에 남는다 | unit | High | api.ts(sam2TrackAllChunks) · trackResume.test.ts(잘린_응답이_오면_남은_프레임을_이어_보내_결과가_유실되지_않는다) |
| TC-FE-873 | 이어붙일 폴리곤은 서버가 준 값이며 결과에서 역산하지 않는다 (신설) | 처리는 했으나 결과가 빈 `truncated` 응답 | 추적 실행 | 두 번째 요청의 `prevPolygon` = 응답 `resume.prevPolygon`(결과로는 만들 수 없는 값) | unit | High | api.ts(sam2TrackAllChunks · resumeOf) · trackResume.test.ts(이어붙일_폴리곤은_서버가_준_값이며_결과에서_역산하지_않는다) |
| TC-FE-874 | 진행이 0 인 응답에는 같은 요청을 다시 보내지 않는다 (신설) | 요청과 동일한 `resume` 이 돌아옴 | 추적 실행 | 요청은 **1회**로 끝나고 결과가 `truncated` + 남은 구간을 그대로 들고 온다(무한 재요청 없음) | unit | High | api.ts(sam2TrackAllChunks) · api/autoTrack.ts(requestAutoTrackAll) · trackResume.test.ts(진행이_0_인_응답에는_같은_요청을_다시_보내지_않는다) |
| TC-FE-875 | 취소하면 다음 조각을 보내지 않는다 (신설) | 첫 응답 직후 취소 | 추적 실행 → 취소 | 이어 보내기 요청이 나가지 않는다(요청 1회) | unit | High | api.ts(sam2TrackAllChunks) · api/autoTrack.ts(requestAutoTrackAll) · trackResume.test.ts(취소하면_다음_조각을_보내지_않는다) |
| TC-FE-876 | 진행 보고는 **처리한 프레임 수**이며 결과에서 빠진 프레임까지 센다 (신설) | 처리했지만 결과가 빈 조각 + 이어 보내기 | 추적 실행 | 진행이 응답마다 올라간다(결과 개수로 세면 멈춘 것처럼 보인다) | unit | High | api.ts(sam2TrackAllChunks) · trackResume.test.ts(이어_보낼_때마다_진행이_올라온다 · 진행_보고는_결과에서_빠진_프레임까지_센다) |
| TC-FE-877 | 이어붙일 시드가 끊기면(mock 전량 제외) 남은 조각 수를 그대로 알린다 (신설) | 60프레임 · 첫 조각 전량 mock 제외 | 추적 실행 | 결과의 «못 한 몫» = 10(뒤 조각). 0 으로 알리면 «다 했다» 로 읽힌다 | unit | High | api.ts(sam2TrackAllChunks · Sam2TrackRunResult.unprocessed) · trackResume.test.ts(mock으로_시드가_끊기면_남은_조각_수를_알린다) |
| TC-FE-878 | 조각 크기는 **서버 예산**을 따라가며 서버 값을 못 받아도 상한까지 간다 (신설) | 프레임당 몫 0 인 예산 / 발행분 없음 | 조각 크기 계산 | 두 경우 모두 서버 상한(50)까지 한 번에 싣는다 — 폴백이 갈리면 «못 받았을 때만 잘게 쪼개는» 차이가 생긴다 | unit | High | aiBudget.ts(aiWaitChunkSize · AI_WAIT_BUDGET_FALLBACK) · trackResume.test.ts(조각_크기는_서버_예산을_따라간다 · 서버_값을_못_받아도_조각_크기가_서버_상한까지_간다) |
| TC-FE-879 | 자동 추적: 이어 보낸 조각의 객체가 앞 조각과 한 트랙으로 합쳐지지 않는다 (신설) | 두 조각 모두 객체 번호 1 | 자동 추적 실행 | 검토 묶음이 2건으로 갈린다(합치면 서로 다른 객체가 한 트랙으로 저장된다) | unit | High | api/autoTrack.ts(requestAutoTrackAll · AutoTrackFrame.trackSegment) · utils/autoTrackResult.ts(qualifiedTrackId) · trackResume.test.ts(이어_보낸_조각의_객체는_앞_조각과_한_트랙으로_합쳐지지_않는다) |
| TC-FE-880 | 끝까지 가지 못하면 남은 프레임 수를 화면이 알린다 (신설) | 이어 보내도 진행이 없어 멈춘 응답 | 추적 실행 / 자동 추적 실행 | 두 화면 모두 «남은 프레임 N개는 시간 안에 처리하지 못했습니다» 안내. 실패도 mock 안내도 아닌 별도 줄 | component | High | Sam2TrackTool.tsx(incomplete) · AutoTrackPanel.tsx(composeNotices) · trackResumeUi.test.tsx(추적이_끝까지_가지_못하면_남은_프레임을_알린다 · 자동_추적이_끝까지_가지_못하면_남은_프레임을_알린다) |
| TC-FE-881 | 자동 추적은 이어 보내는 동안 진행을 보여준다 (신설) | 첫 응답이 시퀀스 3건 중 1건만 처리 | 자동 추적 실행 | 이어 보내는 중 «프레임 1/3 처리» 가 뜬다(«0/3» 에 멈춰 있지 않다) | component | High | AutoTrackPanel.tsx(auto-track-progress) · hooks/useAutoTrack.ts(onProgress) · trackResumeUi.test.tsx(자동_추적은_이어_보내는_동안_진행을_보여준다) |
| TC-FE-882 | 진행이 이어지는 동안 잠금 상한이 연장된다 (신설) | 상한 직전에 진행 1건 관측 | 대기 | 원래 상한을 지나도 잠금이 유지되고 오버레이의 «최대 N초» 도 함께 늘어난다. 연장분이 다하면 그때 해제된다(영원히가 아니다) | unit | High | hooks/useBusyTask.ts(renewDeadline) · stores/useLabelStore.ts(extendBusy) · aiBudget.ts(aiWaitBusyRenewMs) · useBusyTaskCancelAndTimeout.test.tsx(진행이_이어지는_동안은_상한이_연장된다) |

## H-70. 추적 **안내 문구 통로** — 이 자리에 오는 것은 mock 안내뿐이다 (SCREEN-005 · API-020 · API-123) — 2026-08-18 신설
...
> 표시하는 «남은 프레임 N개는 시간 안에 처리하지 못했습니다» 안내(H-69 `TC-FE-880`)는 **무변경**이다.
...
| ID | 목적 | 전제 | 실행 | 기대결과 | 계층 | 우선순위 | 근거 |
|----|------|------|------|----------|:----:|:--------:|------|
| TC-FE-883 | 절단은 문구가 아니라 **남은 수**로만 전달된다 (신설) | 1차는 1건 처리 후 절단 · 2차는 진행 0 · **두 응답 모두 문구 없음** | 추적 실행 | 결과가 `truncated` + 못 한 몫 2건이고 `message` 는 **null**. 절단 사실은 구조화된 값이 이미 말한다 | unit | High | api.ts(sam2TrackAllChunks) · trackNoticeChannel.test.tsx(절단은_문구가_아니라_남은_수로만_전달된다) |
| TC-FE-884 | ★**절단을 보고한 응답에 실린 mock 안내가 버려지지 않는다** (신설 · 이번 수정의 핵심) | 1차 응답이 **절단이면서 동시에 mock 안내**를 싣고, 2차가 남은 구간을 끝냄 | 추적 실행 | `message` 가 그 mock 안내와 **같다**. 구 동작(절단 응답의 문구를 버리는 필터)에서는 null 이 되어 결과에서 빠진 프레임을 알릴 신호가 사라졌다 | unit | High | api.ts(sam2TrackAllChunks) · trackNoticeChannel.test.tsx(절단을_보고한_응답에_실린_mock_안내가_버려지지_않는다) |
| TC-FE-885 | 여러 응답의 서로 다른 안내가 모두 남는다 (신설) | 조각 2개가 각각 다른 mock 안내를 실음 | 추적 실행 | 두 문구가 모두 남는다 — 첫 것만 보존하면 뒤 조각의 안내가 통째로 사라진다 | unit | High | api.ts(sam2TrackAllChunks · joinNotices) · trackNoticeChannel.test.tsx(여러_응답의_서로_다른_안내가_모두_남는다) |
| TC-FE-886 | 같은 안내가 여러 번 와도 한 번만 남는다 (신설 · 경계) | 조각마다 같은 문구가 옴 | 추적 실행 | `message` 가 그 문구 **한 벌**이다(그대로 이어 붙이면 같은 말이 반복된다) | unit | Med | api.ts(sam2TrackAllChunks · joinNotices) · trackNoticeChannel.test.tsx(같은_안내가_여러_번_와도_한_번만_남는다) |
| TC-FE-887 | 절단 응답에 실린 mock 안내가 **화면에** 표시된다 (신설) | 1차 절단 + mock 안내 · 2차 완주 | 추적 실행 | 완료 표시와 함께 mock 안내가 화면에 남는다. 다 끝냈으므로 «남은 프레임» 안내는 뜨지 않는다(그 줄은 서버 문구가 아니라 화면이 센 수다) | component | High | Sam2TrackTool.tsx(mockNotice) · hooks/useSam2Track.ts(onMockWarning) · trackNoticeChannel.test.tsx(절단_응답에_실린_mock_안내가_화면에_표시된다) |

...
>
> ⚠ **진행 축(`stages` 7단계)은 그대로다** — 표시기는 여전히 단계 단위로 그리고 조작만 묶음 단위다.
> 두 축을 합치지 말 것. 실패한 **단계**를 그 단계가 속한 **묶음**으로 해석하는 지점이 유일한 접점이다
> (`bundleOfStage`).
>
> ⚠ **묶음 표시명은 별도 이름표가 아니라 단계 노출명의 조합**이다(`bundleLabel` = 멤버들의 `stageLabel`
> 을 ` · ` 로 이음 → 오토라벨은 "AI 탐지 · AI 분할 · 보간"). 묶음 전용 이름표를 만들면 단계 이름 매핑과
> **두 번째 진실원**이 되어 한쪽만 갱신된다. 부수 효과로 **이름만 보고도 보간이 이 묶음 안에 있음**이
> 드러나는데, 그것이 이번 변경의 핵심이라 오히려 사양에 맞는다.
>
...
| ID | 케이스명 | 전제 | 입력/조건 | 기대결과 | 계층 | 우선 | 근거(파일) |
|----|---------|------|----------|---------|------|:--:|------|
| TC-FE-757 | ★오토라벨 묶음의 단계가 실패하면 **그 묶음** 건너뛰기가 노출된다 (신설) | `SAM2` 가 FAIL | 렌더 | 「AI 탐지 · AI 분할 · 보간 작업 건너뛰기」 노출. 진행 축은 단계 단위인데 조작은 묶음 단위라, **역해석이 없으면 버튼이 아예 안 뜬다** | component | **Critical** | types.ts(`bundleOfStage`) · BatchFailurePanel.tsx(`failedBundle`) · BatchFailurePanel.test.tsx(`★오토라벨_묶음의_단계가_실패하면_그_묶음_건너뛰기가_노출된다`) |
...
| TC-FE-759 | ★묶음 이름이 **품은 작업을 그대로** 말한다 — 보간 포함 (신설) | `YOLO` 가 FAIL | 버튼 접근명 확인 | 이름이 "AI 탐지 · AI 분할 · 보간"이다. 묶음 전용 이름표를 따로 두면 단계 이름 매핑과 **두 번째 진실원**이 되어 한쪽만 갱신된다 — 그래서 멤버들의 `stageLabel` 을 이어 붙인다 | component | High | components/common/BatchStageIndicator.tsx(`bundleLabel` · `stageLabel`) · types.ts(`STAGE_BUNDLE_MEMBERS`) · BatchFailurePanel.test.tsx(`★오토라벨_묶음_이름이_품은_작업을_그대로_말한다_보간_포함`) |

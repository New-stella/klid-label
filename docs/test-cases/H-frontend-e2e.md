...
| TC-FE-589 | **단계 배지에 아이콘이 재유입되지 않는다** (신설 · 사각 보강) | `StageBadge` 5종(완료·실패·프레임추출·비식별화·VLM) + 매핑 밖 단계 | 렌더 | 한글 라벨 텍스트가 **정확히** 렌더되고 `svg` 가 없다. 매핑 밖 단계도 원문을 라벨로 보여 **빈 배지가 되지 않는다**(아이콘이 없는 지금 텍스트가 유일한 정보 축). 기술 모델명(YOLO/SAM2)은 `AI 탐지`/`AI 분할` 로 가려진다. ★`StatusBadge` 에만 있던 축이라 이 컴포넌트는 가드 사각이었다 | a11y | High | components/common/StageBadge.tsx(`StageBadge`) · components/common/__tests__/StageBadge.test.tsx(`★단계_배지는_한글_라벨_텍스트만_렌더하고_아이콘은_폐지됐다`) |
...
> 정의되고 스테퍼 캡션·조작 UI 가 그 값을 **공유**한다(오토라벨 = 「오토라벨링」, 시계열 = 「VLM」 — 후자는
...
| ID | 시나리오 | 전제 | 실행 | 기대결과 | 유형 | 우선순위 | 근거 |
|---|---|---|---|---|---|:--:|---|
| TC-FE-778 | **★7단계를 받으면 칸은 5개다 (신설 · 핵심)** | canonical 7단계 | 렌더 | 렌더된 칸이 **5개**이고 순서는 `비식별 · 마킹 · VLM · 프레임추출 · 오토라벨링`. 구 동작(7칸)은 폐기 | component | **Critical** | components/common/BatchStageIndicator.tsx(`collapseStages`) · \_\_tests\_\_/BatchStageIndicator.test.tsx(`★7단계를_받으면_칸은_5개다`) |
...
>
> ⚠ **시계열 묶음 이름은 이번 범위 밖이다** — 사양은 「시계열」이라 부르는데 구현 출력은 `VLM` 이다.
> 이번 확정은 오토라벨 묶음 한 건이라 **바꾸지 않았고**, 그 불일치는 별건으로 보고했다(TC-FE-795 가
> 현재 동작을 고정한다).
...
| TC-FE-795 | ★**시계열** 묶음 표시명은 이번 변경으로 바뀌지 않았다 (신설 · 회귀 가드) | — | `bundleLabel('VLM')` | `VLM` 이며 `stageLabel('VLM')` 과 같다(손으로 적은 두 번째 이름이 아니라 단계 표에서 파생). ⚠ 사양은 이 묶음을 「시계열」이라 부르지만 이번 확정 범위 밖이라 **바꾸지 않았다** — 별건 | component | High | components/common/BatchStageIndicator.tsx(`BUNDLE_LABEL.VLM`) · \_\_tests\_\_/BatchStageIndicator.test.tsx(`★시계열_묶음_표시명은_이번_변경으로_바뀌지_않았다`) |

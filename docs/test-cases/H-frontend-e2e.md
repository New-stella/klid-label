...
|---|---|---|---|---|---|---|---|
| TC-FE-939 | 검수자에게 시계열 일괄 3버튼이 노출된다 (신설) | REVIEWER + 1건 이상 선택 | 렌더 | 건너뛰기·건너뛰기 해제·재수행 3버튼 노출 | component | High | `VideoListBulkStage.test.tsx(REVIEWER_에게_시계열_일괄_3버튼이_노출된다)` |
...
| TC-FE-942 | 상한을 넘으면 시계열 3버튼도 막고 보내기 전에 알린다 (신설) | 101건 선택 | 렌더 | 3버튼 비활성 + 상한 안내. 400 을 받고서야 아는 동선을 피한다 | component | High | `VideoListBulkStage.test.tsx(상한을_넘게_고르면_시계열_3버튼도_막고_보내기_전에_알린다)` |
...
| TC-FE-946 | 건너뛰기 해제는 확인창 없이 바로 요청한다 (신설) | 1건 이상 선택 | 해제 클릭 | 확인창 없이 요청 + 건별 결과 표시. 표식만 떼므로 파괴적이지 않다 | component | Medium | `VideoListBulkStage.test.tsx(건너뛰기_해제는_확인창_없이_바로_요청하고_결과를_보여준다)` |

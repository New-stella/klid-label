...
## H-50. 요소가 **자기 배경에 알파**를 쓸 때 — 뒤 표면과 합성돼 무너지는 축 (DS-001 `do_rules`) — 2026-08-09 신설
...
|---|---|---|---|---|---|:--:|---|
| TC-FE-586 | **★트랙 칩이 세 행 표면 전부에서 AA 를 넘는다 (신설 · 핵심)** | 객체 목록 트랙 칩(`T:{trackId}`) | 소스의 실제 클래스를 뽑아 각 행 표면 위에서 배경을 환산 후 대비 계산 | 기본(흰 배경) · hover(`bg-gray-50`) · 선택(`bg-primary-50`) **세 표면 모두 4.5 이상**. 구 조합 `bg-primary-600/40` + `text-primary-100` 은 각각 **1.56 / 1.67 / 1.70** 으로 전부 미달이었다 | unit | **Critical** | src/test/contrastGuard.test.ts(`트랙칩이_기본_흰배경_위에서도_AA_4.5를_넘는다` · `트랙칩이_hover_gray50_위에서도_AA_4.5를_넘는다` · `트랙칩이_선택_primary50_위에서도_AA_4.5를_넘는다`) |
| TC-FE-587 | **★칩 배경은 불투명이라 뒤 표면이 바뀌어도 대비가 흔들리지 않는다 (신설 · 구조적 차단)** | 트랙 칩 클래스 | 알파 접미사 부재 검사 + 세 표면 대비값 동일성 검사 | 배경에 `/N` 이 없어야 하고, 그 결과 세 표면에서 계산된 대비가 **완전히 같은 값(6.83)**. 값 조정이 아니라 **"뒤에 무엇이 있는가"에 대한 의존 자체를 끊는다** — 알파를 쓰는 한 가드는 행 표면을 일일이 열거해야만 판정할 수 있고, 그 열거 방식이 이 저장소에서 세 번 연속 회귀를 놓쳤다 | unit | **Critical** | src/test/contrastGuard.test.ts(`★트랙칩_배경은_불투명이라_뒤_표면이_바뀌어도_대비가_흔들리지_않는다`) · features/label/components/ObjectClassTree.tsx |
| TC-FE-588 | 회귀 원인을 값으로 못박는다 (신설 · 산문 규칙 부식 방지) | 구 조합 | 대비 계산 | 구 조합이 세 표면 전부에서 **UI 요소 기준 3:1 조차** 못 넘었음을 고정(1.8 미만). 이 수치가 통과로 바뀌면 회귀 기록 자체가 무의미해지므로 함께 실패한다 | unit | High | src/test/contrastGuard.test.ts(`★회귀_원인_문서화_구_알파칩은_세_표면_전부에서_3대1도_못_넘었다`) |
| TC-FE-589 | 스캔기 자체 검증 — 알파가 붙으면 표면마다 다른 색으로 환산된다 (신설 · 가짜 가드 방지) | 불투명/반투명 두 클래스 문자열 | 환산 함수 직접 호출 | 불투명은 표면과 무관하게 토큰 색 그대로, 알파는 표면마다 다른 색이 되고 흰 배경 위에서는 원색보다 밝아진다. 환산이 실제로 합성을 수행함을 증명 — 이게 없으면 "알파를 안 썼다"만 세는 문자열 가드로 퇴화한다 | unit | High | src/test/contrastGuard.test.ts(`★스캔기_자체_검증_알파가_붙으면_표면마다_다른_색으로_환산된다`) |

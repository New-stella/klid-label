...
> **398 케이스**(표 행 실측 — **폐기 행 포함**, 행을 지우지 않으므로. 변경 이력 표는 제외) · 계층: component / e2e / a11y / security · [← README](README.md) ※ 카운트 = `grep -cE '^\| ~*TC-'`(ID 취소선 폐기 행 포함, 2026-08-05 회차 12 실측 갱신 350 → 354 · 회차 13 에서 354 → 362, H-19 `TC-FE-320~327` 신설 · 회차 14 에서 362 → 369, H-20 `TC-FE-328~334` 신설 · 회차 15 에서 369 → 374, H-21 `TC-FE-335~339` 신설 · 회차 16 에서 374 → 382, H-22 `TC-FE-340~347` 신설 · 회차 17 에서 382 → 390, H-23 `TC-FE-348~355` 신설 · **회차 18 에서 390 → 393, H-21 `TC-FE-356~358` 추가 · 회차 19 에서 393 → 398, H-24 `TC-FE-359~363` 신설**)
...
| ID | 케이스명 | 전제 | 입력/조건 | 기대결과 | 계층 | 우선 | 근거(파일) |
|----|---------|------|----------|---------|------|:--:|------|
| TC-FE-359 | ★헤더에 저장 버튼이 없다 — 중복 진입점 제거 (신규) | 라벨링 헤더 렌더 | `dirty=true` | `label-header-save` **미존재** + 이름 `저장` 버튼 미존재. 저장 진입점은 도구바 + `Ctrl+S` 뿐이다. 되살리면 같은 중복이 재발한다 | component | High | features/label/components/LabelHeader.tsx(`LabelHeader`) · features/label/components/\_\_tests\_\_/LabelHeader.test.tsx(`★헤더에는_저장_버튼이_없다_중복_진입점_제거_회귀가드`) |
...
| TC-FE-361 | ★영상 잠금이 도구바 저장에 전달된다 — 축이 달라 자체 판정 불가 (신규) | `saveDisabled` 전달(= `isLocked \|\| isEditBlocked`) | 도구바 렌더 | `label-toolbar-save` **비활성**, 그러나 `선택` 등 도구 버튼은 **활성**(잠금은 도구 전환을 막지 않는다 — 기존 동작 무회귀). 전달이 누락되면 잠긴 영상에서 저장이 활성으로 보인다(`handleSave` 가 토스트로 막지만 "눌리는데 아무 일도 없는 화면"이 된다) | component | High | features/label/components/DarkToolbar.tsx(`DarkToolbar`) · pages/label/LabelingPage.tsx(`LabelingPage`) · features/label/\_\_tests\_\_/DarkToolbarScrollContract.test.tsx(`★잠금이_전달되면_저장만_비활성이고_다른_도구는_그대로다`) |
| TC-FE-362 | 저장 진행 중에는 도구바 저장이 스피너·`aria-busy` + 중복 클릭 차단 (신규) | `isSaving` | 저장 버튼 클릭 | `aria-busy="true"` + `disabled` + `onSave` **미호출**. 버튼 라벨이 사라진 자리를 스피너가 대신한다(헤더 상태 문구와 짝) | component | Med | features/label/components/DarkToolbar.tsx(`DarkToolbar`) · features/label/\_\_tests\_\_/DarkToolbarScrollContract.test.tsx(`저장중이면_스피너와_aria_busy_로_진행을_알리고_중복_클릭을_막는다`) |
| TC-FE-363 | 저장 버튼은 스크롤 컨테이너 안의 **마지막** 버튼이다 (신규) | 도구바 렌더 | 버튼 목록 조회 | `label-toolbar-save` 가 `label-toolbar-scroll` 내부이며 목록의 마지막 버튼. 가장 먼저 잘리던 자리라 위치 자체를 고정한다(잘림 결함의 재발 지점) | component | Med | features/label/components/DarkToolbar.tsx(`DarkToolbar`) · features/label/\_\_tests\_\_/DarkToolbarScrollContract.test.tsx(`저장_버튼은_스크롤_컨테이너_안에_있어_넘쳐도_스크롤로_도달한다`) |

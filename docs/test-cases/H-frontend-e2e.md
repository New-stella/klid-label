...
| ID | 케이스명 | 전제 | 입력/조건 | 기대결과 | 계층 | 우선 | 근거(파일) |
|----|---------|------|----------|---------|------|:--:|------|
| TC-FE-745 | ★해제된 묶음은 실패도 스킵도 없어도 패널이 남고 재수행 버튼이 **하나** 생긴다 (회차 65 정정 — 전제가 **서버 응답 `clearedStages`** 로 바뀜) | 응답에 실패·스킵 모두 없고 `clearedStages:['VLM']` | 렌더 | 패널 유지 + 「재수행」 **1개** 노출(범위 선택 없음). 패널이 사라지면 **되살릴 창구가 같이 사라진다**(H-59f 가 고친 결함과 같은 형태). ⚠ 구 근거였던 화면 로컬 기억(`RevertedBundles`)은 **폐기**됐다 | component | **Critical** | BatchFailurePanel.tsx(`needsBatchAttention` · `canRerun`) · BatchFailurePanel.test.tsx(`해제된_묶음은_실패도_스킵도_없어도_패널이_남고_재수행_버튼이_하나_생긴다`) |
...
| TC-FE-955 | ★★이탈했다 다시 들어와도 재수행 버튼이 남는다 (신설 · **이번 변경의 존재 이유**) | `clearedStages:['VLM']` · 실패·스킵 없음 | 언마운트 → 재마운트 | 버튼이 그대로 있다. **화면 로컬 상태가 전부 사라져도** 서버 응답이 근거이기 때문이다. 구 배선에서는 이 순간 사라졌다 | component | **Critical** | `BatchFailurePanel.tsx(canRerun · needsBatchAttention)` · `BatchFailurePanel.test.tsx(★이탈했다_다시_들어와도_재수행_버튼이_남는다_서버가_기억한다)` |

...
| ID | 케이스명 | 전제 | 입력/조건 | 기대결과 | 계층 | 우선 | 근거(파일) |
|----|---------|------|----------|---------|------|:--:|------|
| TC-FE-1015 | ★연결 상태 4값이 각각의 확정 문구로 표기된다 (신설) | 네 상태의 유형이 각각 있는 목록 | 이벤트유형 관리 목록 렌더 | **연결됨 · 연결됨(무효) · 연결됨(제외) · 미연결** 이 확정 문구 그대로 보인다 | component | **P0** | features/eventType/components/PresetLinkStatusChip.tsx · features/eventType/\_\_tests\_\_/eventTypePresetLink.test.tsx(`★연결_상태_4값이_각각의_확정_문구로_표기된다`) |
| TC-FE-1016 | ★연결됨(무효)와 연결됨(제외)는 **한눈에 갈라 보인다** (신설) — 앞은 사고이고 뒤는 사람이 일부러 뺀 선언이라 같아 보이면 **사고를 의도로 오인**한다 | 두 상태가 같은 목록에 있음 | 목록 렌더 | 두 칩의 시각 표현이 서로 다르다 | component | **P0** | PresetLinkStatusChip.tsx · eventTypePresetLink.test.tsx(`★연결됨_무효와_연결됨_제외는_한눈에_갈라_보인다`) |
| TC-FE-1017 | 연결 상태가 없거나 모르는 값이면 **추측하지 않고 비운다** (신설) | `presetLinkStatus` 부재 / 미지 토큰 | 목록 렌더 | 그 행이 정상 표시되고 칩만 생략된다 — 어느 상태로도 승격시키지 않는다. 수정 응답은 이 값을 싣지 않으므로(**항상 `null`**) 이 경로가 실제로 쓰인다 | component | **P0** | features/eventType/presetLinkStatus.ts · eventTypePresetLink.test.tsx(`연결_상태가_없거나_모르는_값이면_추측하지_않고_비운다` · `판정_함수가_미지_토큰을_알려진_상태로_승격시키지_않는다`) |
| TC-FE-1018 | ★진입 시에는 조건을 싣지 않아 전체가 돌아온다 (신설 · 하위호환) — 서버에 기본값이 없다 | 화면 진입 | 최초 조회 | 요청에 `presetLinkStatus` 파라미터 **자체가 없다** | component | **P0** | features/eventType/adminApi.ts · features/eventType/\_\_tests\_\_/adminApiPresetLinkFilter.test.ts(`★미지정이면_파라미터_자체를_싣지_않는다`) · eventTypePresetLink.test.tsx(`★진입_시에는_조건을_싣지_않아_전체가_돌아온다`) |
| TC-FE-1019 | ★★조건을 켜면 **보류 전체를 뜻하는 값 하나**를 그대로 보낸다 — 조합 금지 (신설 · **핵심**) | 보류 거르기 조건 | 조건 켜기 | 요청에 `presetLinkStatus=WITHHELD` **단일 값**. 「연결됨(무효)+미연결」을 화면에서 조합해 보내지 않는다 | component | **Critical** | pages/manage/EventTypeManagePage.tsx · adminApi.ts · eventTypePresetLink.test.tsx(`★조건을_켜면_보류_전체를_뜻하는_값_하나를_그대로_보낸다_조합_금지`) · adminApiPresetLinkFilter.test.ts(`★보류_거르기_값은_그대로_실린다`) |
| TC-FE-1020 | ★화면은 보류 상태 집합을 **스스로 만들지 않는다** (신설 · 정적 가드) — 조합 코드가 되살아나면 서버와 조용히 갈라진다 | — | 정적 스캔 | 화면 소스에 상태 값을 나열해 보류 집합을 만드는 코드가 **없다** | component | **Critical** | EventTypeManagePage.tsx · eventTypePresetLink.test.tsx(`★화면은_보류_상태_집합을_스스로_만들지_않는다_정적_가드`) |
| TC-FE-1021 | 조건을 끄면 다시 조건 없이 조회한다 (신설 · 대조군) | 조건 켜진 상태 | 조건 끄기 | 파라미터 없이 재조회 | component | High | EventTypeManagePage.tsx · eventTypePresetLink.test.tsx(`조건을_끄면_다시_조건_없이_조회한다`) |
| TC-FE-1022 | ★★조건을 켠 채 저장해 그 행이 조건에서 **벗어나도 사라지지 않는다** (신설 · **핵심**) — 서버 거르기와 「저장한 행을 사라지게 하지 않는다」가 정면으로 충돌하는 지점이다. 해법은 거르기를 화면으로 되가져오는 것이 **아니라** 방금 저장한 행 하나만 원래 자리에 되돌려 놓는 병합이다(모집단은 줄지 않는다) | 보류 거르기 켜짐 + 그 행을 저장해 조건을 벗어남 | 저장 후 재조회 | 그 행이 **원래 자리에** 남는다 | component | **Critical** | features/eventType/pinnedRows.ts(`mergePinnedRows`) · EventTypeManagePage.tsx · eventTypePresetLink.test.tsx(`★조건을_켠_채_저장해_그_행이_조건에서_벗어나도_사라지지_않는다`) |
| TC-FE-1023 | 고정 병합은 **서버 값이 이기고 순서를 보존하며 중복을 만들지 않는다** (신설 · 경계) | 고정 행 0건 / 서버 목록에 있음 / 자리가 길이를 넘음 / 여러 건 / 같은 유형 두 번 | 병합 | 각각 — 서버 목록 그대로 · 서버 값 채택 · 끝에 붙임 · 원래 순서 유지 · 한 번만 끼워짐 | component | High | pinnedRows.ts · adminApiPresetLinkFilter.test.ts(`고정_행이_없으면_서버_목록_그대로다` · `★서버_목록에서_빠진_행만_원래_자리에_다시_끼운다` · `★서버_목록에_있으면_서버_값이_이긴다` · `자리가_목록_길이를_넘으면_끝에_붙인다` · `여러_건이_빠져도_원래_순서가_유지된다` · `같은_유형을_두_번_저장해도_한_번만_끼워진다`) |
| TC-FE-1024 | ★★표시명 저장은 응답값으로 그 행을 갱신하고 **목록도 다시 조회한다** (신설 · **구 「재조회 금지」 폐기**) — 수정 응답에 연결 상태가 실리지 않으므로(**항상 `null`**) 재조회가 없으면 프리셋 칸이 저장 후 **`-` 로 고착**된다. 표시명 변경은 같은 그룹의 다른 행 상태도 바꾸므로 어차피 재조회가 맞다 | 표시명 편집 | 저장 | 응답값으로 그 행 즉시 갱신 **+** 목록 무효화 | component | **Critical** | features/eventType/adminHooks.ts · features/eventType/\_\_tests\_\_/eventTypeAdminSource.test.tsx(`★표시명_저장은_응답값으로_그_행을_갱신하고_목록도_다시_조회한다_구_재조회_금지_폐기`) |
| TC-FE-1025 | ★★라벨이 0건이면 **오토라벨 제외** 배지가 붙고 **미적용 배지는 붙지 않는다** (신설 · **핵심** · 배타) — 서버는 라벨 0건도 `effective=false` 로 내려주므로 실효 여부만으로 가르면 **두 배지가 함께 붙는다** | 라벨 0건 프리셋 | 카드 렌더 | 「오토라벨 제외」만 · 「오토라벨 미적용」 부재 | component | **Critical** | pages/manage/PresetListPage.tsx · pages/manage/\_\_tests\_\_/PresetListPage.test.tsx(`★라벨이_0건이면_오토라벨_제외_배지가_붙고_미적용_배지는_붙지_않는다`) |
| TC-FE-1026 | ★라벨은 담았는데 실효하지 않으면 **오토라벨 미적용** 배지가 붙는다 (신설) | 라벨 1건 이상 + `effective=false` | 카드 렌더 | 「오토라벨 미적용」 배지 + 확정 보조 문구 | component | **P0** | PresetListPage.tsx · PresetListPage.test.tsx(`★라벨은_담았는데_실효하지_않으면_오토라벨_미적용_배지가_붙는다` · `실효하는_프리셋에는_배지가_붙지_않는다`) |
| TC-FE-1027 | ★실효 여부가 응답에 없으면 **경고를 지어내지 않는다** (신설 · 하위호환) — 구 서버 응답에서 모든 카드가 경고로 뒤덮이지 않게 한다 | `effective` 미탑재 | 카드 렌더 | 배지 부재(미탑재는 `true` 로 정규화) | component | High | features/preset/api.ts · PresetListPage.test.tsx(`★실효_여부가_응답에_없으면_경고를_지어내지_않는다`) |
| TC-FE-1028 | ★AI 검출 클래스가 없는 라벨에만 **미매핑 표시**가 붙고, 그 라벨도 **고를 수 있다** (신설) — 선택을 막지 않는 이유는 나중에 마스터에 매핑을 지정하면 그때 실효해지는 동선을 닫지 않기 위해서다 | 매핑·미매핑 라벨 혼재 | 편집 모달 렌더·선택 | 미매핑 라벨에만 표시 · 선택 가능 | component | **P0** | features/preset/components/PresetEditModal.tsx · features/preset/\_\_tests\_\_/PresetEditModal.test.tsx(`★AI_검출_클래스가_없는_라벨에만_미매핑_표시가_붙는다` · `★미매핑_라벨도_고를_수_있다_선택을_막지_않는다`) |
| TC-FE-1029 | ★고른 라벨이 **전부** 미매핑이면 경고가 뜨되 **저장은 활성**이다 (신설) | 전부 미매핑 선택 | 모달 렌더 | 확정 문구 경고 배너 + 저장 버튼 활성. 매핑이 하나라도 있으면 경고 없음 | component | **P0** | PresetEditModal.tsx · PresetEditModal.test.tsx(`★고른_라벨이_전부_미매핑이면_경고가_뜨지만_저장은_활성이다` · `매핑된_라벨이_하나라도_있으면_경고가_뜨지_않는다`) |
| TC-FE-1030 | ★라벨을 하나도 고르지 않으면 **전부 미매핑 경고가 뜨지 않는다** (신설 · 상태 배타) — 그 상태는 확인 모달이 맡는다. 두 안내가 함께 뜨면 사람이 무엇을 결정하는지 흐려진다 | 라벨 0건 선택 | 모달 렌더 | 미매핑 경고 부재 | component | **P0** | PresetEditModal.tsx · PresetEditModal.test.tsx(`★라벨을_하나도_고르지_않으면_전부_미매핑_경고가_뜨지_않는다_상태_배타`) |
| TC-FE-1031 | ★★라벨 0건 저장은 **확인을 먼저 받고** 확인해야 요청이 나간다 (신설 · **핵심**) — 실수로 못 고른 것과 일부러 비운 것을 사람이 여기서 구분한다 | 라벨 0건 | 저장 클릭 | 확인 모달 노출 · 확인 전에는 요청 **0건** · 확인해야 나간다 | component | **Critical** | PresetEditModal.tsx · PresetEditModal.test.tsx(`★라벨_0건_저장은_확인을_먼저_받고_확인해야_요청이_나간다`) |
| TC-FE-1032 | 확인 모달의 「돌아가서 라벨 고르기」는 저장하지 않고 **고른 이벤트유형을 유지**한다 (신설) | 확인 모달 노출 | 되돌아가기 | 요청 0건 · 이벤트유형 선택 유지 | component | High | PresetEditModal.tsx · PresetEditModal.test.tsx(`★돌아가서_라벨_고르기는_저장하지_않고_고른_이벤트유형을_유지한다`) |
| TC-FE-1033 | 라벨을 하나 이상 고르면 확인 창이 **뜨지 않고 바로 저장**된다 (신설 · 대조군) | 라벨 1건 이상 | 저장 클릭 | 확인 모달 부재 · 즉시 요청 | component | High | PresetEditModal.tsx · PresetEditModal.test.tsx(`★라벨을_하나_이상_고르면_확인_창이_뜨지_않고_바로_저장된다`) |
| TC-FE-1034 | ★★라벨 미선택이어도 **저장 버튼이 활성**이다 (신설 · **구 차단 폐기**) — 구 동작은 라벨 0건이면 저장을 비활성으로 막아 「오토라벨 제외 선언」 자체를 표현할 수 없었다 | 라벨 0건 | 모달 렌더 | 저장 버튼 활성. **이벤트유형 미선택은 종전대로 비활성** | component | **Critical** | PresetEditModal.tsx · PresetEditModal.test.tsx(`★라벨_미선택이어도_저장_버튼은_활성이다_구_차단_폐기` · `★이벤트_미선택시_저장_disabled`) |
| TC-FE-1035 | ★★라벨 0개도 스키마를 통과한다 — **구 「최소 1개」 요구 폐기** (신설) | — | 폼 스키마 검증 | `labelIds: []` 통과 · **목록 키 자체는 여전히 필수** · 상한 20 유지 | component | **Critical** | features/preset/schemas.ts · features/preset/\_\_tests\_\_/schemas.test.ts(`★라벨_0개도_통과한다_구_최소1개_요구_폐기` · `라벨_목록_키_자체는_필수다` · `labelId_최대_20개_제한`) |
| TC-FE-1036 | ★라벨 없이 저장하면 **오토라벨에서 빠진다는 사실을 안내**한다 (신설) | 라벨 0건 저장 성공 | 성공 안내 | 확정 문구로 그 사실을 함께 알린다 | component | **P0** | PresetListPage.tsx · PresetListPage.test.tsx(`★라벨_없이_저장하면_오토라벨에서_빠진다는_사실을_안내한다`) |
| TC-FE-1037 | ★담긴 라벨이 전부 미매핑이면 저장은 성공하고 **적용되지 않는다는 사실을 함께 알린다** (신설) | 전부 미매핑 저장 성공 | 성공 안내 | 저장 성공 + 그 사실 안내 | component | **P0** | PresetListPage.tsx · PresetListPage.test.tsx(`★담긴_라벨이_전부_미매핑이면_저장은_성공하고_적용되지_않는다는_사실을_함께_알린다`) |

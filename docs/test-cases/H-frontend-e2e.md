...
| TC-FE-727 | ★그 상태에서도 **되살릴 창구**가 남는다 (회차 65 정정 — 창구가 「해제」에서 「재수행」으로 바뀜) | 위와 동일 | `재수행` | 재수행 요청 발사 + **해제 API 호출 0회**. 「막는 조작에는 되돌리는 길을 함께 둔다」는 원칙은 그대로이고 그 길을 재수행 하나가 진다 | component | **Critical** | BatchFailurePanel.tsx(`canRerun`) · BatchFailurePanel.test.tsx(`★되살릴_창구는_재수행_하나다_해제_버튼은_두지_않는다`) |
...
| TC-FE-956 | ★단건 축에 「건너뛰기 해제」 버튼을 두지 않고 해제 API 도 부르지 않는다 (신설 · 회귀 가드) | 건너뛴 묶음 있음 | 렌더 + 재수행 클릭 | 해제 버튼 부재 + `DELETE …/skip` **호출 0회**. 되살릴 창구는 재수행 하나다 | component | **Critical** | `BatchFailurePanel.tsx` · `BatchFailurePanel.test.tsx(★되살릴_창구는_재수행_하나다_해제_버튼은_두지_않는다)` |

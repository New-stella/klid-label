...
> 아래 7건은 여러 라운드의 시행착오 끝에 **의도적으로 그렇게 만든 것**입니다.
...
| **★6** | **export 개인정보 3필드(`anonymity`/`pseudonymity`/`privacy_included`)는 `ExportPrivacyPolicy` 단일 판정기가 소유한다** — `ORIGINAL` 산출물은 3필드를 **판정하지 않는다(전부 null)**, `DEIDENTIFIED` 산출물만 **수동값 우선**(미입력 시 기본상수 `Y`/`N`/`N`). (2026-08-03 확정, 커밋 `0d290c4e` / V163) | 구 정책("ORIGINAL=N·DEIDENTIFIED=Y 고정, 수동 override 금지")은 **전면 반전**됐다. 원천 영상은 비식별 판정의 대상이 아니므로 "N 으로 단정"하는 것 자체가 사실 왜곡이고, 비식별 산출물만 사람이 판정한 값을 싣는 것이 맞다. `ExportPrivacyPolicy` javadoc 에 "★확정 정책 / 되돌리지 말 것" 경위가 명시돼 있다. | 카탈로그가 구 기대값을 들고 있으면 **정상 동작이 전수 검증에서 거짓 FAIL 로 재발견**된다(3차 회차에 실제 발생 — `E-ISSUE-101`·`E-ISSUE-121`, 카탈로그 정정 완료). 회귀 확인: [TC-EXPORT-022·023·030](E-augment-resolution-export-meta.md) · [TC-META-030·034](E-augment-resolution-export-meta.md). |
| **★7** | **포털 채널에 AI 도구(SAM2 분할/추적·스켈레톤)를 되살리지 않는다** — ADR-013 대로 포털은 데이터마트 영상 선택 + 본인 자산 업로드 + 수동 라벨링(BBOX/POLYGON)만 제공한다. BE `PortalSam2Controller`/`PortalSam2Service` 는 커밋 `dcdbb827` 에서 **삭제**됐고, FE 는 `PORTAL_HIDDEN_TOOLS` **단일 소스**를 도구바·단축키훅·단축키안내 3곳이 공유한다. | 1차·2차 회차에서 연속으로 "정책 위반(ADR-013)" 결함으로 잡혔던 항목이라, 편의 목적의 재도입이 반복될 소지가 크다. 이 저장소의 *철회된 정책 재시도 차단* 관례를 따라 확정 정책으로 못 박는다. | 되살리면 포털 사용자 데이터가 다시 ai-server 로 나가고 ADR-013 위반이 재발한다. 회귀 가드: `features/portal/__tests__/LabelingPagePortalRestrictions.test.tsx` → [F-4 절](F-portal.md) · [TC-FE-275](H-frontend-e2e.md). |

> "일관성"을 이유로 ★2·★3·★5 를 통일하지 말 것. ★4·★6 은 "결함 수정"으로 되돌리지 말 것. ★7 은 "편의 기능 추가"로 되살리지 말 것.
...
|---|------|------|------|
| 1 | **포털 SAM2 구현·노출** vs 문서 "미제공"(F) | 문서가 정본 — 제공 안 됨. 노출은 **정책 위반 → 결함 플래그** | **✅ 완전 해소 — 결함 플래그 종료(2026-08-04, 3차 전수 검증 확정).** 커밋 `dcdbb827` 에서 `PortalSam2Controller`/`PortalSam2Service` **삭제**. 4중 확증: ①`git log --diff-filter=D` ②배포 jar `unzip -l` 에 `PortalSam2*.class` 부재 ③실동작 포털 경로 변형 10종 전부 404(내부 경로는 400 → 판별자 성립) ④FE `PORTAL_HIDDEN_TOOLS` 단일소스 3중 게이팅(도구바·단축키훅·안내) + 컨테이너 실물 소스 대조. F·H 클러스터 **교차 확인**. → **재도입 금지는 ★7 로 승격** |
...
| 3 | **TASK_COMPLETED payload 0/null 하드코딩**(D) | 실카운트(`totalFrames`/`labeledFrames`/`reviewerName`) 구현 필요 — 결함 | **✅ 해소 — 단 확정 기대값 자체가 무효.** 그 세 필드는 **계약에서 사라졌다.** 현재는 **9필드** 평면 snake_case(`job_id`·`event_type_cd`·`evnt_cls_cd`·`evnt_ctgry_cd`·`lclgv_cd`·`lclgv_nm`·`duration_sec`·`image_count`·`gen_ai_yn`)이며 전부 DB 실측(상수 self-fill 0건). ⚠ **구 서술 “6필드”는 2026-08-05 폐기** — 관제 계약(API-251 v17) required 8 + optional 1 정합(규격서 §4-1, TC-NOTIFY-003). 경로도 `POST /api/data-set/v2/jobs/{job_id}/notify-completed\|notify-updated` 로 교체 |
...
| 13 | 시계열 콜백·IntelliVIX KLID v1.1.0 계약 | B·G | **🔶 부분 해소(2026-08-24 갱신).** BE 가 쓰는 엔드포인트는 **`/v1/videovlm-klid/describe` + `/describe-sub` 둘**이며 콜백 `results` 는 **단일 객체 `{description}`** 이다 — 구 서술의 `/v1/videovlm/verify`(+`accuracy`)는 **폐기**(판정 창구는 연동하지 않는다, `ADR-051`). 적재는 묘사 → `vlm.description`(검수큐 진입) **+ 어노테이션 사고 단계 1단계**(전문의 「상황」 줄 파싱), 추가 질문 → **어노테이션 캡션 후보 c1 의 캡션 본문**(2026-08-25 재확정 — 구 「답변 축」 폐기, 되돌리지 말 것). 질문 칸은 위탁 응답이 아니라 **저작도구가 보관하는 유형별 질문 문구**에서 조달한다. 실패 콜백의 `error` 는 **문자열**이다. request_id echo·4xx 비재시도는 그대로. ⚠ **타임아웃 "이중 구조(블록 45s ↔ 클라이언트 10s)" 서술도 폐기** — Phase C-1 논블로킹 전환으로 `.block(45s)` 자체가 없고 유일한 타임아웃은 `vlm.client.timeout-seconds`(기본 10s)다. 미확정은 **IntelliVIX 실서버(목 아님) 대조**(오류코드 카탈로그·재시도 정책·`accuracy` 필수 여부)로 좁혀짐 |
| 14 | imgsz 무효(640 고정) 실효 검증 방법 | G | **🔶 부분 해소(2026-08-18) — 검증이 아니라 조작면 제거로 닫았다.** 실모델로 `imgsz=1920` 을 넣어 640 만 쓰는지 확인하는 시험은 **여전히 하지 않았다**(ai-server 무변경). 대신 코드 실측(`yolox_loader.py` 의 `_DEFAULT_INPUT_SIZE=(640,640)` 고정 + 로더 생성 시 그 값 주입)으로 무효를 확인하고 **조정 경로를 폐지**했다 — 설정 키 `YOLO_IMGSZ` 화이트리스트 제거 · 시스템 설정 화면 입력칸 제거 · 시드 행 DELETE(V13). ai-server 요청 필드는 남고 호출부가 상수(1280)를 싣는다. ⇒ 불확실성이 **운영에 영향을 주지 않는 상태**가 됐으나 명제 자체(로더가 요청값을 무시한다)의 실모델 확증은 미해소다. 되살리려면 그 확증이 선행돼야 한다. 가드 `ConfigKeysTest.imgszKeyIsNotConfigurable` · `YoloConfigCard.test.tsx(★이미지_크기_입력칸은_없다_구_imgsz_항목_폐지)` |

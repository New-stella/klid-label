...
| 13 | 시계열 콜백·IntelliVIX KLID v1.1.0 계약 | B·G | **🔶 부분 해소(2026-08-24 갱신).** BE 가 쓰는 엔드포인트는 **`/v1/videovlm-klid/describe` + `/describe-sub` 둘**이며 콜백 `results` 는 **단일 객체 `{description}`** 이다 — 구 서술의 `/v1/videovlm/verify`(+`accuracy`)는 **폐기**(판정 창구는 연동하지 않는다, `ADR-051`). 적재는 묘사 → `vlm.description`(검수큐 진입), 추가 질문 → 이벤트 어노테이션 답변 축 초안. 실패 콜백의 `error` 는 **문자열**이다. request_id echo·4xx 비재시도는 그대로. ⚠ **타임아웃 "이중 구조(블록 45s ↔ 클라이언트 10s)" 서술도 폐기** — Phase C-1 논블로킹 전환으로 `.block(45s)` 자체가 없고 유일한 타임아웃은 `vlm.client.timeout-seconds`(기본 10s)다. 미확정은 **IntelliVIX 실서버(목 아님) 대조**(오류코드 카탈로그·재시도 정책·`accuracy` 필수 여부)로 좁혀짐 |

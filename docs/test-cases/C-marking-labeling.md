...
> **FE 분(신고 버튼 비활성 + 툴팁 · 마킹 화면 사유 우선순위)은 [H-51](H-frontend-e2e.md) `TC-FE-591~595` 소관**이다 — 이 카탈로그는 BE 계약만 다룬다.

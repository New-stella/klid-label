...
> **278 케이스**(실측 — 표 행 수) · 계층: unit / integration / security · 우선순위 P0(Critical)~P2 · [← README](README.md)
...
| TC-LABEL-12 | 작업락 시 저장 차단 | isRawLocked | — | 409 CONFLICT. ⚠ **저장 경로에는 신고 게이트가 없고 작업락이 유일 방어** — 신고 락은 6h 만료(`LsAuthWorkLock:84`) 후 `WorkLockSweepJob` 이 회수하는데 `DE_IDNTF_YN='F'` 는 resolve 까지 남아, 그 뒤엔 **조회 412 ↔ 저장 200** 비대칭이 열린다(C-ISSUE-22) | integration | P0 | LabelService.java:255-258 |
...
| TC-LABEL-150 | 존재하지 않는 `id` 로 좌표 상한·점개수 상한 우회 차단 (신규 · C-ISSUE-61/62 회귀 가드) | 320x240 프레임, 그 프레임에 없는 `id`(예: 99999999) | ①`{"id":99999999, points:[[999999,888888],…]}` ②`{"id":99999998, points: 1500점}` | **①400**(TC-113 과 동일 — 그 프레임에 없는 id 는 저장 분기가 **신규 라벨로 생성**하므로 상한 검증 대상이다) · **②400**(TC-113 의 `MAX_POINTS_PER_LABEL` 1000 상한). 판정 술어는 저장 분기(`idIndex.containsKey`)와 **동일**해야 하며 `item.id()==null` 단독 판정은 우회를 남긴다. 대조: `USE_YN` 축(TC-120)은 `isNewLabelAssignment` 로 이미 같은 우회를 차단하고 있다. **3차 실측 = ①②모두 200(우회 성립) → FAIL** | security | P0 | LabelService.java:288-292(사전검증 술어) · :331-335(UPDATE 분기) · :349-365(신규 생성 — 검증 없음) · :685-695(대조: 올바른 술어) |

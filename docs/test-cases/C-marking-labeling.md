...
> **369 케이스**(표 행 실측 — **폐기 행 포함**, 행을 지우지 않으므로. 변경 이력·현황 요약 표는 제외) · 계층: unit / integration / security · 우선순위 P0(Critical)~P2 · [← README](README.md) ※ 카운트 = `grep -cE '^\| ~*TC-'`(ID 취소선 폐기 행 포함, 2026-08-05 머지 회차 7 정정 · 회차 11 에서 294→299, 라벨 이력 작성자 표시명 계약변경 TC-LABEL-158~162 신설 · 회차 13 에서 299→311, C-7 객체 속성값 인가·게이트 TC-LABEL-163~172 + 프리셋 상한 TC-PRESET-19~20 신설 · 회차 15 실측 착수 시점 317(머리말 미갱신분 소급 반영) → 339, F-01 트랙변경감지 TC-LABEL-179 + C-9 한번이라도승인 이력축 확대 TC-LABEL-180~200 신설 · 회차 16 에서 339→342, D-1 회차왕복 폐기 기준선 TC-LABEL-201~203 신설 · 회차 20 에서 342→360, C-8 온디맨드 AI 취소·대기예산 TC-AICANCEL-01~18 신설 · 회차 21 에서 360→366, 이어 보내기 자기정합 TC-AICANCEL-19~24 신설(머리말 미갱신분 소급 반영) · 회차 22 에서 366→369, 안내 문구 통로 정리 TC-AICANCEL-25~27 신설)
...
| 23 | 2026-08-25 | 1건(C-1 부수 안내 — TC-MARK-19 의 FE 대응 변화 명시) | 0건 | 0건 | **FE 파급만 반영(BE 무변경, 케이스 신설·폐기 없음) — CO-011.** 마킹 화면이 `status !== 'MARKING_READY'` 축을 **선차단**하게 되어(SCREEN-006 v46 「마킹 진입 차단 안내」) TC-MARK-19 가 규정한 412 는 **안전망으로만** 남는다 — 정상 동선에서는 편집기 자체가 열리지 않아 그 요청이 발생하지 않는다. 같은 회차에 마킹 화면 헤더의 배치 단계 진행 표시를 제거했다(마킹 완료가 잔여 배치의 트리거라 이 화면에서 확인할 진행이 존재하지 않는다). FE 케이스 신설·폐기는 [H-frontend-e2e](H-frontend-e2e.md) 회차 68 에 있다 |
| 22 | 2026-08-18 | 0건 | 3건(TC-AICANCEL-25~27) | 0건 | **★안내 문구 통로에서 예산 절단 안내를 뺀다 — 조용한 손실 1건 차단**(BE 변경). 추적 응답의 문구 통로(`ApiResponse.message`)를 **예산 절단 안내**와 **mock 안내** 두 종류가 공유하고 있었다. 화면은 절단을 보고한 응답의 문구를 쓰지 않는데(그 문구는 «요청 하나» 기준이라 실행 전체의 남은 수와 어긋난다), 한 응답이 **절단이면서 mock 이기도 하면** 같이 실린 mock 안내까지 함께 버려졌다 — **mock 프레임은 결과 목록에서 빠지므로 그 안내가 유일한 신호**이고, 못 보면 사용자는 왜 그 구간에 라벨이 없는지 알 수 없다. 절단은 이미 구조화된 값(`truncated`/`resume`)으로 표현되므로 문구는 **중복이면서 mock 안내를 밀어내고 있을 뿐**이었다. 두 추적 경로(SAM2 추적 · AI 자동 추적)에서 절단 문구 조립을 제거하고, 그것만 쓰던 문구 생성기(`TrackBudgetNotice`)는 **다른 사용처가 없어 삭제**했다. **mock 안내·응답의 구조화 필드는 무변경**(이름·타입·의미 불변 — 이번에 뺀 것은 문구뿐이다). 구 정책(*"예산 소진을 안내 문구로도 알린다"*)은 **폐기**이며 그것을 고정하던 시험 1건은 극성을 뒤집었다(`Sam2TrackBudgetTest.예산_소진은_안내_문구가_아니라_구조화된_값으로만_알린다`). **뮤테이션 실증 2건**: `Sam2TrackOutcome.of` 에 절단 문구 조립 복원 → TC-AICANCEL-25·26·27 + 위 시험 실패 / 컨트롤러 `yoloTrack` 에 절단 문구 복원 → TC-AICANCEL-27 실패. 전부 원복 후 label 패키지 812건 재실행 PASS. ⚠ **화면쪽 후속 필요** — 지금 화면에 있는 「절단을 보고한 응답의 문구는 쓰지 않는다」 필터가 이 변경 뒤에는 **mock 안내를 버리는 원인**이 되므로 제거해야 한다(H 소관). ⚠ **LogiCraft `API-020`·`API-123` 의 *"안내 문구가 응답 메시지에 실린다"* 서술과 `API-123` 요청 스키마 `minItems: 1` 은 아직 낡았다 — 사양 반영은 별도 소관 |
| 21 | 2026-08-18 | 1건(TC-TRACK-26) | 6건(TC-AICANCEL-19~24) | 0건 | **이어 보내기 계약의 자기정합 결함 2건 + 예산 시계 범위 정정**(BE 변경). ①**AI 자동 추적의 이어 보내기가 남은 프레임 1개에서 요청 규격을 위반**했다 — `resume` 은 «첫 미처리 프레임 + 나머지» 로 갈리는데 남은 프레임이 정확히 하나면 나머지가 빈 목록이 되고, 요청의 `@NotEmpty` 가 그것을 400 으로 거부했다(부분 결과 보존이 없는 경로라 **계산 끝난 최대 50프레임 검출이 전량 폐기**). **요청 규격을 `@NotNull` 로 완화**해 «시작 프레임만 처리» 를 정상 요청으로 받는다 — 자원 상한(`@Size(max=50)`)·프레임별 인가 검사는 그대로라 잃는 방어가 없다. → TC-TRACK-26 기대결과 **400 → 200 정정(구 「400 @NotEmpty」 폐기)** + TC-AICANCEL-19/20 신설. **화면 변경 불요**(FE 는 빈 목록을 그대로 되싣는다). ⚠ **SAM2 는 반대다** — 시작이 «마지막 처리 프레임» 이라 남은 목록이 구조적으로 비지 않으므로 `@NotEmpty`(TC-SAM2-17)를 **유지**하고 그 비대칭을 TC-AICANCEL-22 로 못박았다(일관성을 이유로 통일 금지). ②**SAM2 이어 보내기 시드가 요청 점수 상한을 넘을 수 있었다** — 시드는 전파 연속성 때문에 **단순화 전 원본**인데 추론 서버는 윤곽점 수를 제한하지 않고 응답 검증은 **최소** 점 수만 본다. 상한 초과 시에만 단순화 → (그래도 넘으면) 균등 솎기로 줄인다(평시 동작 불변 · 보장 없는 단순화 단독으로는 «대개 안 난다» 에 그친다). 상수는 요청 DTO 가 소유해 두 번째 진실원을 만들지 않는다. → TC-AICANCEL-21 신설. ⚠ **목서버로는 재현되지 않는다**(입력을 그대로 돌려준다). ③**AI 자동 추적의 프레임별 접근 검증이 예산 시계 밖**이라, 예산 정책이 «요청 소요는 프레임 수와 무관하다» 를 근거로 화면에 내려준 대기 상한을 실제 소요가 넘을 수 있었다(정상 요청이 화면에서 끊기는 방향). 시계를 검증 **앞**으로 옮기고, 인가 검사가 조회한 행을 재사용해 **프레임당 중복 조회 1회를 제거**했다(`verifyAndGet`). → TC-AICANCEL-23/24 신설 + `AiWaitBudgetPolicy` 문구 정정. **뮤테이션 실증 6건**: `@NotEmpty` 복원 → TC-AICANCEL-19·TC-TRACK-26 실패 / 시드 상한 검사 무력화 → 21 실패 / 균등 솎기 제거 → 21 실패 / SAM2 `resume` 을 자동추적 축으로 교체 → 22 실패 / 예산 시계를 검증 뒤로 → 23 실패 / 루프 재조회 복원 → 24 실패. 전부 원복 후 클린 재빌드 확인. ⚠ **LogiCraft `API-123` 의 요청 스키마 `minItems: 1` 은 아직 낡았다** — 사양 반영은 `/mc-logi-update` 소관(별도). |

| 14 | 2026-08-10 | 1건(TC-LABEL-155) | 6건(TC-LABEL-173~178) | 0건 | **★검수가 승인(APPROVED)된 영상은 비식별 누락 신고를 접수하지 않는다 (R2, 사용자 확정 · 구속)**. 두 진입점(`POST /v1/labels/{srcSn}/deident-report` · `POST /v1/videos/{rawSn}/deident-report`) 모두 **412** — 판정은 `ReviewApprovalGate.isApproved` **재사용**, 배선은 두 진입점이 수렴하는 `doReport` **한 곳**, **역할 무관**(REVIEWER 포함), 평가는 **작업락 409 검사보다 먼저**(응답이 잠금 상태 오라클이 되지 않게, CWE-209). ①**구 동작 폐기** — 승인 영상 신고 접수 시의 `TASK_MODIFIED(META_UPDATED)` 발행 분기는 도달 불가가 되어 제거(TC-LABEL-175 가 고정). ②**`resolveManually` 는 무변경** — 게이트 도입 이전에 접수된 OPEN 신고의 해소까지 막으면 그 영상이 작업락 + `'F'` 로 **영구 고착**된다(TC-LABEL-177 이 그 회귀를 막는다). ③**TC-LABEL-155 정정** — 구 기재 *"검수 완료 영상 → 201"* 은 **배치 단계**와 **검수 승인** 두 축을 혼동한 서술이라 폐기하고, 그 케이스의 축을 배치 단계로 못박았다. ④FE 는 영상 상세 `reviewSttsCd` 로 버튼을 미리 비활성 + 툴팁하며 판정은 `deidentReportEligibility` **단일 지점**에 위임한다 — FE 케이스는 [H-55](H-frontend-e2e.md) `TC-FE-617~621` 소관(파생 축 회귀 가드 + 마킹 화면 사유 우선순위 포함). ⑤**거부 사유는 감사 로그로 남긴다**(TC-LABEL-178) — 승인 영상은 조치 수단이 0 이라 신고 행도 알림도 없어 로그가 **유일한 기록**이다(CWE-778). 파생 거부와 같은 `LogSanitizer` 재사용(CWE-117). 위키: [08 §8.4](../v2-wiki/08-deidentification.md) |
| 15 | 2026-08-12 | 0건 | 22건(TC-LABEL-179~200) | 0건 | **★F-01 트랙 재지정도 변경 감지 축에 오른다 + P2b 한번이라도 검수 완료된 영상은 신고·폐기를 이력으로 판정(사용자 확정, 구속) — 신규 기능 등재(카탈로그 누락분).** ①**F-01**(`kr.co.cudo.authoring.version.entity.LabelSnapshot` 에 `trackId` 필드 신설) — 트랙만 재지정하고 좌표·라벨명이 그대로면 변경 감지 축(`LabelService.snapshotsEqual`)이 "변경 없음"으로 판정해 이력(`LS_DATA_LBL_HSTRY`)·판번호(`LBL_VER`)·검수 완료 후 산출 재생성·통지가 한꺼번에 빠지던 것을 막는다. ⚠ 이 클래스는 `VersionService` 내부의 diff 비교용 `LabelSnapshot`(TC-DIFF-044, R7 비교축)과 **이름만 같고 별개**다 — 혼동 금지(TC-LABEL-179). ②**P2b** — `ReviewApprovalGate.hasEverApproved` 신설로 판정 축이 **지금 상태(`isApproved`)에서 이력(`hasEverApproved` = 동결 스냅샷 존재 OR 승인 감사 존재)으로 확대**됐다. **구 축("지금 상태만 본다")은 폐기** — `ReviewStateMachine` 이 `APPROVED→PENDING`(WORKER 재검수 재제출)을 허용해, 재제출로 상태가 내려간 구간에는 R2 게이트가 그대로 뚫려 있었다(실증된 구멍, TC-LABEL-181/186). ⚠ 위 회차 14 요약이 적은 *"판정은 `ReviewApprovalGate.isApproved` 재사용"* 은 이 회차로 **폐기** — 판정 함수 자체가 `hasEverApproved` 로 바뀌었다(`requireNotApprovedVideo`·`doReport` 호출부 교체, TC-LABEL-185/186). TC-LABEL-173/174/177 자체의 기대결과(412/200)는 그 시나리오가 "지금도 승인 상태"라 축 확대와 무관하게 **불변**이며 정정 대상이 아니다. ③**프레임 폐기·복원도 같은 축**(C-9 신설, TC-LABEL-187~200) — `LabelService.requireDiscardAllowed` 가 **한번이라도 승인된 영상의 새 폐기·복원 조작을 400 으로 차단**한다(신고 412 와 코드가 다른 것은 의도 — 신고는 해소되면 되는 일시 조건, 승인 이력은 되돌아가지 않는 영구 조건). **★회차 적용은 예외** — 「시작 버전 선택」으로 과거 회차를 불러와 확정 저장하는 경로가 그 회차의 폐기 상태를 적용하는 것은 차단하지 않는다(그 값은 이미 승인된 것을 그대로 되돌리는 것이지 새 조작이 아니다). **판정은 "필드 존재"가 아니라 "값이 회차 스냅샷과 다른가"다** — 존재로 판정하면 폐기를 건드리지 않은 정상 라벨 수정까지 전량 400 이 되고(TC-LABEL-192), 경로 단위로 열면 회차를 한 번 불러오는 것만으로 차단이 우회된다(TC-LABEL-190). ④성능 — 프레임 루프가 영상 전 프레임 행 락을 쥔 채 판정을 반복하지 않도록 요청 스코프 캐시를 태운다(TC-LABEL-200). ⑤`VideoDetailResponse.everApproved` 를 FE 가 읽어 신고·폐기 버튼을 사전에 비활성화한다 — FE 케이스는 [H-58](H-frontend-e2e.md) 소관. 위키: [08 §8.4](../v2-wiki/08-deidentification.md) · CLAUDE.md 「비식별 누락 신고」 절 |
| 16 | 2026-08-13 | 1건(TC-LABEL-198) | 3건(TC-LABEL-201~203) | 0건 | **★D-1 — 회차 불러오기 → 확정 저장 왕복이 400 이던 결함 수정(폐기 판정 기준선을 두 단계로).** 불러오기(API-195)는 그 회차 스냅샷이 없는 프레임(`resolved=false`)에도 폐기값을 **현재 작업본 값으로 항상 싣고** "이 세트를 그대로 확정 저장해도 그 프레임은 no-op" 을 보장하는데, 확정 저장(API-196)이 기준선을 **스냅샷 하나로만** 잡아 그 프레임은 값이 **같아도** 새 조작으로 판정 → 승인 이력 영상에서 아무것도 바꾸지 않은 저장이 **400** 이었다(로컬 실증 · rawSn=22 의 12프레임 중 5건이 라벨 0건). **폐기한 프레임은 정의상 라벨 0건이라 언제나 `resolved=false`** 이므로 프레임 폐기를 쓴 영상은 확정 저장이 사실상 항상 막혔다. 기준선을 **스냅샷 값 → (없으면) 라이브 `LS_DATA_SRC.DSCD_YN`** 으로 바꿔 ①회차 적용은 라이브와 달라도 허용(TC-LABEL-192/203) ②스냅샷 없는 프레임의 현재값 재전송은 허용(TC-LABEL-201/202) ③기준선과 다른 값은 어느 경우든 400(TC-LABEL-189/197/198) 이 동시에 성립한다. **사양 변경이 아니라 문서화된 생산자 보장의 복원**이라 LogiCraft ITEM 변경은 없다. TC-LABEL-198 의 구 전제·사유("비교 기준이 없으므로 값과 무관하게 400")는 폐기하고 라이브 기준선으로 정정 |
| 17 | 2026-08-13 | 3건(TC-LABEL-11 삭제 순서 · TC-LABEL-22 파생 근거 · TC-LABEL-23 적재 위치) | 0건 | 0건 | **`LS_DATA_LBL_AI_INFO` 를 `LS_DATA_LBL` 로 흡수(V6, @req R4)** — 두 테이블이 완전 1:1 이고 라벨 응답이 `autoLblYn`·`confScore`·`lblSrcCd` 를 **항상 함께** 내려줘 라벨을 읽을 때마다 조인이 붙던 것을 제거했다(목적은 표 개수 감소가 아니라 조인 제거). 이관 5컬럼 = `LBL_SRC_CD`·`MDL_NM`·`MDL_VER`·`CONF_SCORE`·`AUTO_LBL_YN`(원 정의 그대로, **NULL 허용·DEFAULT 없음**). `DATA_RAW_SN`·`DATA_SRC_SN`(사본)과 분리 테이블 식별자·감사 컬럼은 이관하지 않는다. ★**DB 축과 응답 축을 서로 다른 층에서 각각 보존**한다 — DB 는 부재를 `NULL` 로 두고(DEFAULT 를 걸면 '사람이 그린 라벨'과 'AI 가 만든 비자동 라벨'이 영구히 구분 불가), 응답의 `'N'` 치환은 `LabelResponse.Item.from` **한 곳**이 담당해 계약이 불변이다. ★**고아 행은 중단 사유가 아니다** — dev 실측 8,093행 중 **15행**이 고아(그 테이블의 `DATA_LBL_SN` 에 FK 가 없다)라 이관에서 제외하고 `RAISE NOTICE` 로만 남긴다(`RAISE EXCEPTION` 이면 그 DB 가 영영 기동하지 못한다). 반대로 **정상 행 누락은 fail-closed**(기대 `count(DISTINCT data_lbl_sn)` 대조, 불일치 시 중단·건수만 노출 CWE-209). ★**잠복 결함 1건 동반 수정** — `LsDataLbl.SRC_INTERPOLATED`(`"INTERPOLATED"`)는 `@Transient` 라 **DB 에 한 번도 닿지 않았고** 실제 적재값·조회 술어는 `"INTERPOLATE"` 였다. 흡수로 그 필드가 적재값이 되므로 상수를 `SRC_INTERPOLATE` 하나로 통일했다(그대로 뒀으면 보간 산출물이 조회에서 통째로 사라진다). 인덱스 `IX_LS_DATA_LBL_LBL_SRC_CD` 로 구 `IDX_LS_DATA_LBL_AI_INFO_SRC_CD` 승계. 데이터마트 뷰 4종 정의 **무변경**(그 뷰들은 흡수 대상을 참조하지 않는다). 회귀 가드 `LblAiInfoAbsorptionIT`(9건 — DEFAULT 부재·타입/폭·인덱스 승계·뷰 무변경·고아 제외·다중행 결정적 채택·fail-closed·2회 적용 안전). C 클러스터 영향: full-replace 삭제 순서가 `ATTR_VAL → LBL` 2단계로 축소(구 `ATTR_VAL → AI_INFO → LBL`) · 온라인 오토라벨 신규 삽입이 **저장 전** 생산이력을 채운다(구: 저장 후 PK 로 별도 INSERT) · Mass Assignment 방어는 **불변**이나 근거가 '`ls_data_lbl` 에 컬럼이 없다'에서 '요청 값을 채택하는 경로가 없다'로 바뀐다(컬럼은 이제 있다) · 신고 시 영상 전체 라벨 삭제도 AI 메타 선삭제가 사라진다 |
| 18 | 2026-08-15 | 1건(TC-LABEL-182) | 0건 | 0건 | **작업 배정·이벤트 로그 테이블 물리명 표준용어 개명(V9, @req R2) 파급.** `LS_TASK_EVENT_LOG` → **`LS_TASK_EVNT_LOG`**(이벤트 = 사업표준단어 `EVNT`) · `LS_TASK_ASSIGNMENT` → **`LS_TASK_ALTMNT`**(배정 = 행안부 공통표준단어 `ALTMNT`). C 소관 영향은 **TC-LABEL-182 의 전제에 적힌 물리 테이블명 1건뿐**이다 — 그 케이스가 검증하는 것은 「한번이라도 승인」 판정의 **fail-closed OR 2순위 축**(`LS_DATASET_VIDEO_META` 행 부재 시 감사 로그가 뒤를 받친다)이며, **판정·전제·기대 동작은 하나도 바뀌지 않았다**. 개명 대상은 물리명 문자열뿐이라 `ReviewApprovalGate`·`LsTaskEventLogRepository` 의 **자바 식별자와 메서드 시그니처는 불변**이고, 비식별 신고 게이트(C-9)·프레임 폐기 게이트가 이 판정을 공유하는 구조도 그대로다. ⚠ **감사 원장이 사라지면 이 케이스가 지키는 것이 무너진다** — 그래서 V9 는 재생성이 아니라 `RENAME` 이며, 회귀 가드 `V9TaskTableStdTermRenameGuardIT`(`기존_배정행과_감사행이_개명_후에도_보존된다`)가 `APPROVE` 이력 보존을 명시적으로 단언한다(사라지면 승인 이력이 있는 영상에 신고가 통과한다). 상세·13개 부속 객체 전수 검증은 [D 카탈로그 회차 28](D-review-version-notify.md) 참조 |
| 19 | 2026-08-15 | 0건 | 0건 | 1건(TC-LABEL-133) | **`DeidentReportResolvedEvent` 를 실동작 이벤트로 적던 오기 정정 — 코드 변경 0(문서 정합).** [D 회차 16 ⑤](D-review-version-notify.md)가 해소 시점의 즉시 재산출·재통지를 폐기했는데 이 카탈로그의 중복 행은 갱신되지 않아, **같은 파일 안에서 TC-LABEL-133 과 TC-LABEL-177 이 서로 모순**인 상태였다. 코드 실측상 그 이벤트는 `src/main` 발행 0건인 **휴면 확장점**이고, 승인 영상 해소 시 실제로 발행되는 것은 `TaskModifiedEvent(exportRegenerated=true, needsRecheck=true)`(**재검토 표시만**)다. TC-LABEL-133 을 폐기 표기하고 대체를 TC-LABEL-177 로 지목했다. ⚠ 클래스·수신 배선은 휴면 확장점으로 **존치**한다(삭제 금지). 같은 오기가 6개 층에 있었고 한 라운드로 함께 정정했다 — 전체 범위는 [B 회차 47](B-batch-deidentify.md) 참조. 표 행 추가·삭제 0 → 머리말·README 합계 불변 |
| 20 | 2026-08-18 | 0건 | 18건(TC-AICANCEL-01~18) | 0건 | **★C-8 신설 — 온디맨드 AI 취소가 실제로 서버 추론을 끊도록 반전 + 대기 예산이 종류별 서버 소유값으로 전환.** 구 동작(*"취소는 클라이언트 결과 폐기일 뿐 서버 처리를 중단시키지 않는다"* · *"작업잠금은 모든 종류 공통 최대 5분 고정"*)은 실험(클라이언트 정상 종료가 이 스택에서 서버에 통지되지 않음)을 근거로 **폐기**됐다 — [10-labeling.md §10.6](../v2-wiki/10-labeling.md#106-장시간-작업-중-편집-차단--진행-표시--취소-2026-07-31)·[11-ai-assisted.md §11.6](../v2-wiki/11-ai-assisted.md#116-ai-작업-진행-중-화면-동작--편집-차단--진행-표시--취소-2026-07-31-fe) 동시 정정. 신설: ①취소 식별자 헤더(`X-AI-Request-Id`) + `POST /v1/ai-requests/{requestId}/cancel` — 소유자만 취소 가능(IDOR 차단), 사유 비구분 응답(CWE-209 차단), 취소 전용 응답 코드(422)로 잠금 충돌(409)·외부 연동 실패(502)와 분리, 서킷 브레이커 실패로 미집계, 배치 파이프라인은 대상 밖 ②`GET /v1/ai-defaults` 의 `waitBudgets` — 재시도 설정에서 도출(하드코딩 아님), 운영자 절대 상한 설정 + 도출 하한 강제(저장·읽기 2단 방어) ③SAM2 추적·AI 자동 추적의 요청 단위 wall-clock 예산(`truncated`/`resume`) — 예산 소진 시 성공분 보존 + 이어 보낼 지점 응답, 취소와 다른 신호. **신규 18건은 §C-3(SAM2)·§C-5(TRACK) 소관 엔드포인트를 다루지만 취소·예산이라는 별개 축이라 `TC-AICANCEL` 네임스페이스로 신설 절 C-8 에 모았다** — 기존 TC-SAM2-*/TC-TRACK-* 번호는 무변경. 케이스 수 **342 → 360**. FE 쪽 대응(이어 보내기·조각 표기·잠금 재계산)은 [H-frontend-e2e.md](H-frontend-e2e.md) 소관(이 회차 대상 아님) |
...
>
> ⚠ **TC-MARK-19(`MARKING_READY` 아님 → 412)의 FE 대응이 회차 23 에서 「제출 시점 거부」 → 「진입 차단」으로 바뀌었다** — 마킹 화면이 그 축을 선차단하므로 편집기가 아예 열리지 않는다(구 동작: 편집기가 정상으로 열리고 마크를 쌓을 수 있으며 「마킹 완료」를 눌러야 412). **BE 는 무변경**이고 이 절의 케이스도 그대로다 — 412 는 안전망으로 남는다. FE 케이스는 [H-93](H-frontend-e2e.md) `TC-FE-1015~1020` 참조.
...
| TC-LABEL-11 | 삭제 순서 FK고아 방지 | 자식 존재 | 삭제 | **ATTR_VAL→LBL** 순 bulk delete(V6 흡수로 AI_INFO 선삭제 단계 소멸 — 생산이력이 같은 행이라 고아 가능성 자체가 없다) | integration | P0 | LabelService.java(bulkUpsert) |
...
| TC-LABEL-22 | Mass Assignment: autoLblYn 무시 | 요청 autoLblYn='Y' | UPDATE | 무시. ⚠ **근거가 바뀌었다**(V6) — 구 근거 '`ls_data_lbl` 에 컬럼 자체가 없다'는 폐기(컬럼이 생겼다). 지금 근거는 **요청 값을 채택하는 경로가 없다**는 것이다(수동 저장은 `createManual` 이 NULL 로 두고, 'Y' 는 `applyAiSource`·자동 팩토리·스냅샷 복원만 세운다) | security | P1 | LabelService.java(bulkUpsert) · LsDataLbl.java(createManual) |
| TC-LABEL-23 | provenance AUTO_YOLO 적재 | id=null,AUTO_YOLO | confScore | **같은 라벨 행에** `AUTO_LBL_YN='Y'`+`LBL_SRC_CD='YOLO'`+신뢰도 적재(V6 — 구 '별도 AI_INFO 행 생성' 폐기. 저장 **전**에 채우므로 저장 후 PK 로 되찾아 붙이는 단계가 없다) | integration | P1 | LabelService.java(bulkUpsert) · LsDataLbl.java(applyAiSource) |
...
| ~~TC-LABEL-133~~ | ~~DeidentReportResolvedEvent 는 APPROVED만~~ **[폐기 2026-08-15]** | ~~미승인 영상 resolve~~ | ~~—~~ | **폐기 사유**: [D 회차 16](D-review-version-notify.md) 통지 트리거 반전으로 해소 시점의 즉시 재산출·재통지가 폐기됐고, `DeidentReportResolvedEvent` 는 **발행처 0건인 휴면 확장점**이 됐다(코드 실측 — `src/main` 에 발행 0건). 승인 영상에 실제로 발행되는 것은 `TaskModifiedEvent(exportRegenerated=true, needsRecheck=true)` 이며 **재검토 표시만** 세운다. 대체 케이스 = TC-LABEL-177 | — | — | ~~DeidentReportService.java~~ → DeidentReportService.java(`publishResolvedForExportRecovery`) |
...
| TC-LABEL-155 | 라벨링 단계 신고는 배치 단계 제한을 받지 않음 (정정 2026-08-10 · R2) | 배치가 끝난 **미승인** 영상(배치 단계 `COMPLETED`, `LS_RAW_DATA_STATUS` 행 없음) | POST `/v1/labels/{srcSn}/deident-report` | **201** — 마킹 제한이 라벨링으로 새면 배치 완료 영상의 정상 신고 동선이 막힌다. ⚠ 구 기재 *"검수 완료 영상 → 201"* 는 **폐기**(R2) — 두 축을 혼동한 서술이었다. 이 케이스가 고정하는 축은 **배치 단계**이고, **검수 승인(APPROVED) 영상은 412**(TC-LABEL-173) | integration | P0 | DeidentReportControllerTest.labelReportUnaffectedByBatchStage ⚠ **ID 충돌 정정(2026-08-05 머지)** — 구 `TC-LABEL-151`. main 이 같은 번호대(`TC-LABEL-150`/`150a`/`150b`, C-ISSUE-61/62 우회 차단)를 먼저 점유했다. |
...
| TC-LABEL-179 | ★트랙만 바꿔도 저장 이력과 판번호가 남는다 (신규 · F-01, 2026-08-12) | 트랙(`T-1`)에 묶인 라벨 1건, 좌표·라벨명 불변 | 확정 저장 경로(`FrameSaveOptions.of` — `acceptTrackId=true`)로 같은 라벨의 `trackId` 만 `T-2` 로 변경 | ①`trackId` 가 실제로 바뀐다 ②`LS_DATA_LBL_HSTRY` 에 이력이 남는다(좌표가 그대로라 "변경 없음"으로 새면 누가 옮겼는지 기록이 사라진다, CWE-778) ③응답 판번호와 DB 스칼라(`lockAndReadLabelVersion`) 모두 직전보다 크다(오르지 않으면 다른 세션의 낡은 판번호가 무효화되지 않아 그 세션의 전량 교체 저장이 409 없이 통과한다, CWE-362). ⚠ `LBL_VER` 는 `insertable/updatable=false` 원자 UPDATE 컬럼이라 **엔티티 재조회는 stale** — 반드시 스칼라 네이티브 조회로 단언한다. 트랙 병합을 전담하는 `TrackMergeService`(영상 배타 락·겹침 검사·통지 구비)와 달리 이 경로는 그런 가드가 없어 "가드 있는 문 옆에 가드 없는 문"이 되지 않도록 이 축만 별도로 고정한다. ⚠ 이 `LabelSnapshot`(`version.entity` 패키지, 변경 감지·이력 축)은 `VersionService` 내부의 diff 비교용 `LabelSnapshot`(TC-DIFF-044, `equalsContent`)과 **이름만 같은 별개 클래스**다 — 하나를 고치면 둘 다 고쳐졌다고 착각하지 말 것 | integration | P0 | version/entity/LabelSnapshot.java(`trackId` 필드) · LabelService.java(`snapshotOf` · `snapshotsEqual`) · LabelTrackChangeAuditIT.트랙만_바꿔도_이력과_판번호가_남는다 |
...
| TC-TRACK-26 | yolo-track nextSrcSns 빈 | [] | — | **200 — 시작 프레임만 처리하는 정상 요청이다**(시퀀스 = `[srcSn]`). ⚠ **구 기대결과 「400 @NotEmpty」 폐기** — 서버가 발행하는 이어 보내기 값은 남은 프레임이 하나일 때 이 목록이 비므로, 거부하면 **서버가 자기가 받지 못할 값을 발행**하고 그 요청이 400 이 되어 이미 계산한 검출이 통째로 버려졌다. 자원 상한은 `@Size(max=50)`(TC-TRACK-25)이, 접근 통제는 프레임별 인가 검사가 담당하므로 잃는 방어가 없다. null 은 여전히 400(`@NotNull`) | unit | P1 | label/YoloTrackRequest.java · YoloTrackRequestValidationTest.java(nextSrcSns_비어있어도_통과한다_시작_프레임만_처리하는_정상_요청이다 · nextSrcSns_null이면_검증실패) |
...
### C-8. 검수 승인 영상 신고 접수 차단 (R2 · 2026-08-10 확정, 구속)

| ID | 케이스 | 전제 | 입력 | 기대결과 | 유형 | 우선순위 | 근거 |
|----|--------|------|------|----------|------|:--------:|------|
| TC-LABEL-173 | ★검수가 승인된 영상은 라벨링축 신고가 412 (신규 · 구속) | `LS_RAW_DATA_STATUS.DATA_STTS_CD='APPROVED'`, 비파생, `DE_IDNTF_YN='Y'` | POST `/v1/labels/{srcSn}/deident-report` | **412** + 문구 *"검수가 완료된 영상은 비식별 누락을 신고할 수 없습니다."* + 부수효과 0(신고행·작업락·`'F'` 전이·REVIEWER 알림 전부 없음). 처리 단계·잠금 상태를 유추할 정보를 담지 않는다(CWE-209) | security | P0 | DeidentReportService.java(`requireNotApprovedVideo`) · DeidentReportServiceTest.java(`검수가_승인된_영상은_라벨링축_신고가_412로_거부된다`) |
| TC-LABEL-174 | 마킹축 신고도 같은 게이트를 탄다 (신규) | 동일 + REVIEWER 호출 | POST `/v1/videos/{rawSn}/deident-report` | **412** 동일 문구 — 두 진입점이 `doReport` 로 수렴해 게이트를 **한 곳에만** 배선했음을 고정한다(진입점마다 배선하면 샌다). **역할 무관**(REVIEWER 도 차단 — 인가 축이 아니라 프리컨디션) | security | P0 | DeidentReportService.java(`doReport`) · DeidentReportServiceTest.java(`검수가_승인된_영상은_마킹축_신고도_412로_거부된다 — 두_진입점이_같은_게이트를_탄다`) |
| TC-LABEL-175 | 승인 영상 신고 시 `TASK_MODIFIED` 미발행 (신규 · 구 분기 도달불가 고정) | 동일 | 신고 요청 | `TaskModifiedEvent` **발행 0건**. ⚠ 구 동작 *"승인 영상 신고 접수 시 `TASK_MODIFIED(META_UPDATED)` 발행"* 은 **폐기** — 접수가 막혀 도달 불가다. 게이트를 걷어내면 이 케이스가 FAIL 한다(비어있는 가드 아님, mutation 실증) | integration | P0 | DeidentReportService.java(5-2 주석) · DeidentReportServiceTest.java(`검수_승인_영상_신고시_TASK_MODIFIED_통지가_발행되지_않는다 — 구_통지_분기_도달불가_고정`) |
| TC-LABEL-176 | 미승인 영상은 종전대로 접수 (신규 · 회귀 가드) | `APPROVED` 아님(상태 행 없음 포함), 비파생, `DE_IDNTF_YN='Y'` | 신고 요청 | **201** + 신고행 + 작업락 + `'F'` 전이 — R2 게이트가 정상 동선을 막지 않는다 | integration | P0 | DeidentReportServiceTest.java(`미승인_영상은_종전대로_신고가_접수된다 — R2_게이트가_정상_동선을_막지_않는다`) |
| TC-LABEL-177 | ★승인 영상의 기존 OPEN 신고는 여전히 resolve 된다 (신규 · 구속 가드) | 게이트 도입 **이전** 접수분 — `APPROVED` 영상 + OPEN 신고 + `DE_IDNTF_YN='F'` + 재비식별 산출물 검증 통과 | POST `/v1/deident-reports/{rprtSn}/resolve` | **200** — OPEN→RESOLVED 원자 클레임 + 작업락 해제 + `'F'→'Y'` 복원 + 승인 영상 재검토 표시 통지(`TaskModifiedEvent(exportRegenerated=true, needsRecheck=true)`). ⚠ **resolve 를 "대칭"을 이유로 함께 막지 말 것** — 막으면 그 영상이 작업락 + `'F'` 로 **영구 고착**된다 | security | P0 | DeidentReportService.java(`resolveManually` — 무변경) · DeidentReportServiceTest.java(`승인영상에_이미_OPEN인_신고는_R2_게이트와_무관하게_여전히_해소된다`) |

| TC-LABEL-178 | ★거부해도 신고 사유가 감사 로그에 남는다 (신규 · 구속 가드) | `APPROVED` 영상 + 사유에 개행·제어문자 포함(`"00:12 얼굴 미블러\r\n[DeidentReport] created rprtSn=999\007"`) | 신고 요청 | **412** + WARN 로그에 `rawSn` 과 **사유 본문**이 남고 개행·제어문자는 제거된다. 승인 영상은 조치 수단이 0(신고 412 · 재비식별 요청 409 · 화면 버튼 미노출)이라 신고 행도 알림도 없어 **이 로그가 발견 사실의 유일한 기록**이다(CWE-778). 정제는 파생 거부 경로와 **같은 `LogSanitizer` 재사용**(CWE-117). ⚠ "쓰이지 않는 로깅"으로 보고 지우지 말 것 | security | P0 | DeidentReportService.java(`requireNotApprovedVideo`) · DeidentReportServiceTest.java(`검수_승인_영상_신고_거부시_사유가_정제되어_감사로그에_남는다 — 로그가_유일한_기록이다`) |
> **FE 분(신고 버튼 비활성 + 툴팁 · 마킹 화면 사유 우선순위)은 [H-55](H-frontend-e2e.md) `TC-FE-617~621` 소관**이다 — 이 카탈로그는 BE 계약만 다룬다.

---

### C-9. 한번이라도 검수 완료된 영상 — 신고·폐기 판정 축을 이력으로 확대 (P2b · 2026-08-12 확정, 구속)

> **왜 확대했나** — 기존 R2 게이트(C-8)는 **지금 상태**(`ReviewApprovalGate.isApproved`)만 봤다. 그런데
> `ReviewStateMachine` 이 `APPROVED → PENDING`(WORKER 재검수 재제출)을 허용하므로, 작업자가 검수 완료 뒤
> 다시 제출하면 상태가 내려가고 **그 구간에는 신고가 그대로 통과했다**(실증된 구멍). 신고가 통과하면
> 재비식별이 일어나 이미 산출된 회차의 이미지와 데이터마트 뷰가 가리키는 영상의 마스킹이 어긋나고,
> 그 회차로 데이터마트를 되돌리면 걷어낸 개인정보가 되살아난다. 프레임 폐기·복원도 같은 축이다 —
> 이미 산출되어 외부로 나간 회차에서 프레임이 빠지거나 되살아나면 그 회차의 산출물과 어긋난다.
> 판정을 **지금 상태에서 이력**(`ReviewApprovalGate.hasEverApproved`)으로 옮겨 그 구간을 닫는다.
>
> **거부 코드가 갈리는 것은 의도다** — 신고(412)는 *"지금은 안 되지만 해소되면 된다"*는 일시 조건이고,
> 폐기·복원(400)은 *"한번이라도 승인"*이라는 **영구 조건**(재시도 여지 없음, 파생영상 차단과 같은 축)이다.
>
> **★회차 적용은 예외다** — 「시작 버전 선택」(D-9, `docs/test-cases/D-review-version-notify.md`)으로 과거
> 회차를 불러와 확정 저장하는 경로가 그 회차의 폐기 상태를 적용하는 것은 **차단하지 않는다** — 새로
> 바꾸는 게 아니라 이미 승인된 시점으로 되돌아가는 것이다. 판정은 **"필드가 왔는지"가 아니라 "값이 회차
> 스냅샷과 다른지"**다 — 존재로 판정하면 폐기를 건드리지 않은 정상 라벨 수정 저장까지 전량 400 이 되고
> (화면이 판번호 대조 세트를 위해 매 프레임 회차 값을 그대로 실어 보내기 때문), 경로 단위로 항상 허용하면
> 회차를 한 번 불러오는 것만으로 차단이 우회된다.
>
> ⚠ **잔여 위험(인지·수용, 되돌리지 말 것)**: 승인 후 잘못 폐기된 프레임은 되돌릴 수단이 없다 — 프레임
> 축·영상 축·REVIEWER 권한 어느 경로로도 복원이 400 이고, 폐기 이전 회차가 없으면(첫 승인에서 이미 폐기
> 상태로 동결) 되돌아갈 회차 자체가 없다. 발견은 승인 전에 이뤄지는 것이 전제다(CLAUDE.md 확정).

| ID | 케이스 | 전제 | 입력 | 기대결과 | 유형 | 우선순위 | 근거 |
|----|--------|------|------|----------|------|:--------:|------|
| TC-LABEL-180 | `hasEverApproved` — 지금 승인 상태면 이력 판정도 true (신규) | `LS_RAW_DATA_STATUS.DATA_STTS_CD='APPROVED'` | `hasEverApproved(rawSn)` | **true** — 가장 흔한 경우를 먼저 끊어 조회를 줄인다(`isApproved` 먼저 확인 후 나머지 스킵) | unit | P0 | ReviewApprovalGate.java(`hasEverApproved`) · ReviewApprovalGateTest.java(`지금_승인_상태면_이력_판정도_true다`) |
| TC-LABEL-181 | ★재제출로 상태가 내려가도 승인 스냅샷이 있으면 true — 실증된 구멍 (신규) | 현재 상태 미승인, `LS_DATASET_VIDEO_META` 승인 동결 스냅샷 존재(`existsByRawSn=true`) | `hasEverApproved(rawSn)` | **true** — `isApproved` 만 보면 이 구멍이 뚫린다. `LS_DATASET_VIDEO_META` 는 `ReviewService.approve()` 가 라벨 개수·재승인 여부와 무관하게 항상 채우고 append-only(행을 지우지 않고 `ACTIVE_YN` 만 토글)라 1순위 근거다 | unit | P0 | ReviewApprovalGate.java · ReviewApprovalGateTest.java(`재제출로_상태가_내려가도_승인_스냅샷이_있으면_true다 — 실증된_구멍`) |
| TC-LABEL-182 | 승인 스냅샷이 없어도 승인 감사가 있으면 차단 — V97 이전 영상 fail-closed (신규) | 현재 상태 미승인, 동결 스냅샷 0건, `LS_TASK_EVNT_LOG` 에 `EVENT_APPROVE` 존재 | `hasEverApproved(rawSn)` | **true** — 1순위 테이블(`LS_DATASET_VIDEO_META`)이 V97 신설이라 그 이전에 승인되고 백필 전에 재제출된 영상은 행이 0건일 수 있다. false negative 는 게이트가 열리는 방향이라 append-only 감사 로그가 2순위로 뒤를 받친다(OR 조합) | unit | P0 | ReviewApprovalGate.java · LsTaskEventLogRepository.java(`existsByRawDataIdAndEventTypeCd`) · ReviewApprovalGateTest.java(`승인_스냅샷이_없어도_승인_감사가_있으면_차단한다 — V97_이전_영상_fail_closed`) |
| TC-LABEL-183 | 승인 이력이 전혀 없으면 false — 과잉 차단 방지 (신규) | 현재 미승인, 동결 스냅샷 0건, 승인 감사 0건 | `hasEverApproved(rawSn)` | **false** | unit | P1 | ReviewApprovalGateTest.java(`승인_이력이_전혀_없으면_false다 — 과잉_차단_방지`) |
| TC-LABEL-184 | rawSn 이 null 이면 false (신규) | rawSn=null | `hasEverApproved(null)` | **false** — 조회 없이 즉시 반환 | unit | P2 | ReviewApprovalGateTest.java(`rawSn_이_null_이면_false다`) |
| TC-LABEL-185 | ★한번이라도 승인된 영상은 신고를 받지 않는다 — 지금 상태가 아니라 이력으로 판정 (신규) | `isApproved=false`(지금은 미승인) + `hasEverApproved=true`(이력) | POST `/v1/labels/{srcSn}/deident-report` | **412** + 신고행 미생성. `requireNotApprovedVideo` 가 **`isApproved` 를 재확인하지 않는다**(현재 상태로 재유도하면 아래 재제출 구멍이 다시 열린다) | security | P0 | DeidentReportService.java(`requireNotApprovedVideo` — `hasEverApproved` 로 교체) · DeidentReportServiceTest.java(`한번이라도_승인된_영상은_신고를_받지_않는다 — 지금_상태가_아니라_이력으로_판정한다`) |
| TC-LABEL-186 | ★재제출로 상태가 내려간 구간에도 신고를 받지 않는다 — 실증된 구멍 (신규) | `isApproved=false`(재제출로 하강) + `hasEverApproved=true` | POST `/v1/labels/{srcSn}/deident-report` | **412** + `DE_IDNTF_YN` 은 `'F'` 로 전이되지 않는다(부수효과 0). 회차 14 가 등재했던 `TC-LABEL-173/174`(신고 412)의 기대결과 자체는 그 시나리오가 "지금도 승인 상태"라 축 확대와 무관하게 불변이지만, **이 케이스가 없으면 재제출 구멍이 회귀 무방비였다** | security | P0 | DeidentReportService.java · DeidentReportServiceTest.java(`재제출로_상태가_내려간_구간에도_신고를_받지_않는다 — 실증된_구멍`) |
| TC-LABEL-187 | ★프레임 폐기 — 한번이라도 승인된 영상의 새 폐기 조작은 400 (신규) | `hasEverApproved=true`, `edits[].dscdYn` 이 회차 스냅샷과 **다른 값**(신규 조작) | 확정 저장/프레임 저장 경로로 `dscdYn='Y'` 지정 | **400** `INVALID_INPUT` + 문구 *"한번이라도 검수가 완료된 영상은 프레임을 새로 폐기하거나 복원할 수 없습니다."* + `DSCD_YN` 미변경. 412(신고)와 코드가 다른 것은 사유의 성질이 다르기 때문(영구 조건) | security | P0 | LabelService.java(`requireDiscardAllowed`) · FrameDiscardApprovalGateTest.java(`승인_이력_영상의_새_폐기_조작은_400`) |
| TC-LABEL-188 | ★새 복원 조작도 400 — 되살리는 방향도 회차와 어긋난다 (신규) | `hasEverApproved=true`, 이미 폐기된 프레임을 `dscdYn='N'` 으로 복원 시도(회차 스냅샷과 다른 값) | 저장 | **400** — 폐기·복원 양방향 모두 회차 정합성을 깬다 | security | P0 | LabelService.java(`requireDiscardAllowed`) · FrameDiscardApprovalGateTest.java(`승인_이력_영상의_새_복원_조작도_400 — 되살리는_방향도_회차와_어긋난다`) |
| TC-LABEL-189 | ★확정 저장이라도 사용자가 폐기를 새로 지정하면 400 — 회차를 불러오는 것만으로 우회되지 않는다 (신규) | `hasEverApproved=true`, `discardFromApprovedVersion=false`(edits 값이 회차 스냅샷과 다름) | `PUT /v1/videos/{rawSn}/labels`(API-196) | **400** — `options.discardFromApprovedVersion()` 이 false 면 회차 적용 예외를 적용하지 않는다. 경로 단위로 항상 허용하면 회차를 한 번 불러와 임의 `dscdYn` 을 실어 차단을 우회할 수 있다 | security | P0 | LabelService.java(`requireDiscardAllowed`) · FrameDiscardApprovalGateTest.java(`확정_저장이라도_사용자가_새로_지정하면_400`) |
| TC-LABEL-190 | 승인 이력이 없으면 폐기가 그대로 된다 — 과잉 차단 방지 (신규) | `hasEverApproved=false` | 폐기 토글 저장 | **정상 처리**(`FrameDiscardApplier.apply` 그대로 수행) — 게이트가 정상 동선을 막지 않는다 | integration | P1 | LabelService.java · FrameDiscardApprovalGateTest.java(`승인_이력이_없으면_폐기가_그대로_된다 — 과잉_차단_방지`) |
| TC-LABEL-191 | ★폐기여부를 보내지 않은 저장은 승인 이력 영상에서도 통과한다 — 라벨 수정은 계속 허용된다 (신규) | `hasEverApproved=true`, 요청에 `dscdYn` 필드 자체가 없음(생략) | 좌표만 수정한 라벨 저장 | **정상 처리** — `requestedDscdYn == null` 이면 판정 자체를 하지 않는다. 이 게이트가 승인 영상의 정상 라벨 수정(재검토 축이 담당)까지 막으면 안 된다 | integration | P0 | LabelService.java(`requireDiscardAllowed` — null 첫 분기) · FrameDiscardApprovalGateTest.java(`폐기여부를_보내지_않은_저장은_승인_이력_영상에서도_통과한다 — 라벨_수정은_계속_허용된다`) |
| TC-LABEL-192 | ★승인 이력 영상도 회차 폐기 상태 적용은 허용한다 (신규 · 회차 적용 예외) | `hasEverApproved=true`, `discardFromApprovedVersion=true`(edits 값이 회차 스냅샷과 **같음**, 즉 회차를 그대로 되돌리는 저장) | `PUT /v1/videos/{rawSn}/labels`(API-196) | **정상 처리** — 새로 바꾸는 게 아니라 그 시점으로 되돌아가는 것이라 400 이 되지 않는다. 막으면 확정 저장이 라벨만 적용하고 폐기는 현재값으로 남아 "한 영상 = 한 회차" 불변식이 깨진다 | integration | P0 | LabelService.java(`requireDiscardAllowed` — `discardFromApprovedVersion` 분기) · FrameDiscardApprovalGateTest.java(`승인_이력_영상도_회차_폐기_상태_적용은_허용한다`) |
| TC-LABEL-193 | 회차 폐기 적용 시 재검토표시·산출재생성을 요구하는 수정이벤트가 발행된다 (신규) | TC-LABEL-192 조건, 회차 스냅샷의 `dscdYn='Y'` 를 적용 | 확정 저장 | `TaskModifiedEvent(changeType 에 폐기 변경종류 포함, exportRegenerated=true)` 발행 — 회차 적용도 실제 상태를 바꾸는 것이므로 재검토·재생성 축은 정상 동작한다 | integration | P1 | LabelService.java · FrameDiscardApprovalGateTest.java(`회차_폐기_적용시_재검토표시와_산출재생성을_요구하는_수정이벤트가_발행된다`) |
| TC-LABEL-194 | 회차 복원 적용 시 복원 변경종류로 수정이벤트가 발행된다 (신규) | TC-LABEL-192 조건, 회차 스냅샷의 `dscdYn='N'`(복원)를 적용 | 확정 저장 | 복원에 해당하는 변경종류로 `TaskModifiedEvent` 발행 | integration | P1 | LabelService.java · FrameDiscardApprovalGateTest.java(`회차_복원_적용시_복원_변경종류로_수정이벤트가_발행된다`) |
| TC-LABEL-195 | 라벨수정과 회차 폐기가 한 저장에 함께 오면 두 변경종류가 모두 발행된다 (신규) | TC-LABEL-192 조건 + 같은 요청에 다른 프레임의 좌표 수정도 포함 | 확정 저장 | `TaskModifiedEvent` 의 변경종류에 라벨수정·폐기변경 **둘 다** 포함 — 한쪽만 담으면 관제 통지 요약이 실제 변경을 과소 보고한다 | integration | P1 | LabelService.java · FrameDiscardApprovalGateTest.java(`라벨수정과_회차_폐기가_한_저장에_함께_오면_두_변경종류가_모두_발행된다`) |
| TC-LABEL-196 | ★(통합) 승인 이력 영상에서 라벨만 고친 저장은 통과한다 — 화면이 회차 폐기값을 그대로 실어 보낸다 (신규) | `hasEverApproved=true`, 화면이 매 프레임 회차의 `dscdYn` 값을 그대로 실어 보내되 좌표만 바뀜 | `PUT /v1/videos/{rawSn}/labels`(API-196) | **정상 처리** — TC-LABEL-192 와 같은 원리를 `VideoLabelSaveTxService` 통합 레벨에서 재확인(값 비교 판정이 두 계층 모두 일관됨) | integration | P0 | VideoLabelSaveTxService.java · VideoLabelSaveTxServiceTest.java(`승인_이력_영상에서_라벨만_고친_저장은_통과한다 — 화면이_회차_폐기값을_그대로_실어_보낸다`) |
| TC-LABEL-197 | 회차와 다른 폐기값을 지정하면 새 조작으로 본다 — 우회 차단은 유지된다 (신규) | `hasEverApproved=true`, `edits[].dscdYn` 이 회차 스냅샷 값과 다름 | 확정 저장 | **400** — `FramePlan.discardFromSnapshot=false` 로 계산되어 TC-LABEL-189 와 같은 경로로 거부된다 | security | P0 | VideoLabelSaveTxService.java(`planFor`) · VideoLabelSaveTxServiceTest.java(`회차와_다른_폐기값을_지정하면_새_조작으로_본다 — 우회_차단은_유지된다`) |
| TC-LABEL-198 | 회차를 알 수 없는 프레임도 **라이브와 다른** 폐기값이면 새 조작으로 본다 — 우회 차단 유지 (회차 16 정정) | 요청회차 이하 매핑이 없는 프레임(`resolved=false`) + `edits[].dscdYn` 이 **라이브 `LS_DATA_SRC.DSCD_YN` 과 다름** | 확정 저장 | **400**(신규 조작으로 간주) — 스냅샷이 없어도 라이브 값이라는 기준선이 남아 있어야 회차를 한 번 불러오는 것만으로 차단이 우회되지 않는다. ⚠ **구 전제·사유("비교 기준이 없으므로 값과 무관하게 400") 는 폐기** — 값이 같아도 막아 회차 응답 왕복 자체를 400 으로 만들었다(D-1, TC-LABEL-201) | security | P0 | VideoLabelSaveTxService.java(`planFor` · `discardBaselineOf`) · VideoLabelSaveTxServiceTest.java(`회차를_알_수_없는_프레임도_라이브와_다른_폐기값이면_새_조작으로_본다 — 우회_차단_유지`) · VideoLabelSaveDiscardBaselineTest.java(`승인_이력_영상에서_스냅샷_없는_프레임의_폐기값을_바꾸면_400이다`) |
| TC-LABEL-199 | 프레임 루프는 승인 판정 캐시를 공유한다 — 락 보유 중 반복 조회 금지 (신규 · 성능) | 영상 1건, 프레임 N건(최대 2000) | 확정 저장 | `ReviewApprovalGate.hasEverApprovedCached` 가 요청 스코프 `Map` 을 태워 **영상당 조회 1회**(영상 1건 = rawSn 1개라 적중률 100%). 캐시가 없으면 프레임마다 최대 3쿼리가 돌아 최대 6,000 쿼리이며, 이 루프는 영상 전 프레임 행 락을 보유한 상태라 지연이 곧 락 보유 시간이다 | integration | P1 | ReviewApprovalGate.java(`hasEverApprovedCached`) · VideoLabelSaveTxService.java · VideoLabelSaveTxServiceTest.java(`프레임_루프는_승인_판정_캐시를_공유한다 — 락_보유_중_반복_조회_금지`) |
| TC-LABEL-200 | ★폐기 상태 쓰기는 `FrameDiscardApplier` 한 곳만 한다 — 게이트 우회 통로 금지 (신규 · 정적 가드) | 소스 전수 스캔 | 정적 분석(architecture test) | `DSCD_YN` 을 직접 쓰는 네이티브 `UPDATE`·다른 서비스의 직접 대입 등 우회 통로가 **0건**이어야 통과. 게이트가 한 곳(`requireDiscardAllowed` → `FrameDiscardApplier.apply`)에만 있으므로 다른 경로로 그 컬럼을 쓰면 게이트가 그대로 샌다 | security | P0 | FrameDiscardSingleGateGuardTest.java(`폐기_상태_쓰기는_FrameDiscardApplier_한_곳만_한다 — 게이트_우회_통로_금지`) |
| TC-LABEL-201 | ★회차 응답을 그대로 확정 저장하면 스냅샷 없는 프레임도 통과한다 (신규 · **D-1 재현**) | `hasEverApproved=true`, 불러오기(API-195) 응답 그대로 되보냄 — resolved 프레임은 스냅샷 값, `resolved=false` 프레임은 **현재 작업본 값**(생산자가 항상 non-null 로 싣는다) | `PUT /v1/videos/{rawSn}/labels`(API-196) | **정상 처리** + 폐기 상태 유지 + `DSCD_YN` 미변경(멱등). 구 판정은 스냅샷 없는 프레임을 값과 무관하게 새 조작으로 봐 **400** 이었다(로컬 실증 — 라벨 0건 프레임이 하나라도 있으면 그 영상 확정 저장이 전량 거부) | integration | P0 | VideoLabelSaveTxService.java(`discardBaselineOf`) · StartVersionService.java(`loadVersionLabels` — 생산자 보장) · VideoLabelSaveDiscardBaselineTest.java(`★회차_응답을_그대로_확정저장하면_스냅샷_없는_프레임도_통과한다`) |
| TC-LABEL-202 | ★폐기 프레임이 포함된 영상도 회차 확정 저장이 통과한다 (신규 · 실사용 시나리오) | `hasEverApproved=true`, 폐기(`DSCD_YN='Y'`) 프레임 포함 — 폐기 프레임은 라벨 0건이라 **언제나** `resolved=false` | 확정 저장 | **정상 처리** — 이 경로가 막히면 프레임 폐기를 쓴 영상은 확정 저장 자체가 불가능하다(D-1 의 실제 영향 범위) | integration | P0 | VideoLabelSaveTxService.java(`planFor`) · VideoLabelSaveDiscardBaselineTest.java(`폐기_프레임이_포함된_영상도_회차_확정저장이_통과한다`) |
| TC-LABEL-203 | 불러온 회차의 폐기 상태 적용은 **라이브와 달라도** 허용된다 (신규 · 회차 적용 예외 회귀 가드) | `hasEverApproved=true`, 스냅샷 `dscdYn='Y'` / 라이브 `'N'`, 스냅샷 값을 그대로 전송 | 확정 저장 | **정상 처리** + 폐기 전이가 실제로 적용됨 — 기준선을 라이브로만 잡으면 이 정당한 동선이 400 이 되어 "한 영상 = 한 회차"가 깨진다(D-1 수정이 TC-LABEL-192 를 깨지 않았는지 확인하는 축) | integration | P0 | VideoLabelSaveTxService.java(`discardBaselineOf` — 스냅샷 우선) · VideoLabelSaveDiscardBaselineTest.java(`불러온_회차의_폐기상태_적용은_라이브와_달라도_허용된다`) |

> `VideoDetailResponse.everApproved`(맨 뒤 필드 추가, 기존 `from(...)` 오버로드는 빈 값으로 위임)는 위 게이트들이
> 참조하는 응답 필드이며 FE 가 [H-58](H-frontend-e2e.md) 에서 신고·폐기 버튼을 사전 비활성화하는 근거다.
> ⚠ **이 필드 자체의 직렬화·하위호환을 전담하는 단위 테스트는 확인되지 않는다** — `VideoDetailDeidentHistoryTest`·
> `VideoQueryServiceAutoLabelTest` 는 이 필드를 명시 단언하지 않고 생성자 시그니처(신규 `ReviewApprovalGate` 협력자)
> 변경에 맞춰 mock 만 보강됐다. 정보 부족 — 별도 TC 로 등재하지 않는다.

---

## C-8. TC-AICANCEL — 온디맨드 AI 취소 · 대기 예산 (2026-08-18 신설)

> 라벨링 화면의 온디맨드 추론(AI 탐지 · AI 분할 · SAM2 추적(§C-3) · YOLO/AI 자동 추적(§C-5))이 "클라이언트가 연결을
> 끊으면 서버도 끊는다"는 전제가 이 실행 환경(Tomcat)에서 성립하지 않는 것을 실험으로 확인하고(비동기 처리 중 유휴
> 구간의 정상 종료는 통지되지 않고, 동기 처리 중에는 관측 수단 자체가 없다), 명시적 취소 통로(`X-AI-Request-Id`
> 헤더 + `POST /v1/ai-requests/{requestId}/cancel`)와 서버 소유 대기 예산(`GET /v1/ai-defaults` 의 `waitBudgets`)을
> 신설한 축이다. **대상은 온디맨드 4경로뿐이며 배치 파이프라인 오토라벨은 대상이 아니다.**

| ID | 케이스명 | 전제 | 입력/조건 | 기대결과 | 계층 | 우선 | 근거(파일) |
|---|---|---|---|---|---|:--:|---|
| TC-AICANCEL-01 | 취소 성공 — 진행 중 추론을 실제로 끊는다 | 본인이 시작한 요청, 추론 요청에 `X-AI-Request-Id` 헤더로 등록됨, 아직 진행 중 | `POST /v1/ai-requests/{requestId}/cancel` | 200 `{cancelled:true}` — 등록된 추론 호출의 구독을 해지해 WebClient 연결을 실제로 끊는다(추론 서버가 GPU 를 계속 물고 있는 것을 막는 지점) | integration | P0 | AiCallCancellationRegistry.java(`cancel`) · AiCallScope.java |
| TC-AICANCEL-02 | 취소 대상 없음 — 사유를 구분해 알리지 않는다 | requestId 가 미존재·이미 종료·이미 취소됨 중 하나 | 동일 | 200 `{cancelled:false}` — 셋을 구분하면 응답이 "그 요청이 살아 있는가"를 알려주는 오라클이 된다(CWE-209). 화면 입장에서 셋은 «더 기다릴 필요 없음»으로 같다 | security | P0 | AiCancelResponse.java · AiCallCancellationRegistry.java(`cancel`) |
| TC-AICANCEL-03 | 남의 요청은 취소되지 않는다(IDOR) | requestId 는 실재하지만 소유자(토큰 subject)가 다른 사용자 | 동일 | 200 `{cancelled:false}` — `scope.isOwnedBy(ownerId)` 가 false 면 «없음»과 동일하게 처리(CWE-639) | security | P0 | AiCallCancellationRegistry.java(`cancel`) |
| TC-AICANCEL-04 | 취소 식별자 형식 위반은 추적하지 않는 스코프로 떨어진다 | `X-AI-Request-Id` 가 정규식(`[A-Za-z0-9_-]{1,64}`) 위반 | 추론 요청 → 그 값으로 취소 요청 | 추론 요청은 `untracked` 스코프로 평소대로 처리되고, 그 식별자로 보낸 취소 요청은 항상 `false`(취소가 조용히 무시됨 — 추론 실패는 아님) | unit | P1 | AiCallCancellationRegistry.java(`create`) |
| TC-AICANCEL-05 | 동일 식별자 동시 등록은 뒤에 온 쪽을 추적하지 않는다 | 같은 requestId 로 두 요청이 겹쳐 도착 | 동일 | 뒤에 등록된 스코프는 `untracked` — 취소가 어느 쪽을 끊을지 불확실해지는 것을 막는다(엉뚱한 요청이 끊기지 않게) | unit | P1 | AiCallCancellationRegistry.java(`create`) |
| TC-AICANCEL-06 | 추적 상한 도달 시 신규 요청은 취소만 포기하고 추론은 계속된다(fail-open) | 동시 추적 512건(`MAX_TRACKED`) 도달 | 신규 온디맨드 추론 요청 | 취소 불가 스코프로 요청은 정상 진행(거부되지 않음) — 취소는 편의 기능이라 자원 상한이 추론 자체를 막으면 안 된다(fail-open은 기능, fail-closed는 자원) | security | P1 | AiCallCancellationRegistry.java(`MAX_TRACKED`) |
| TC-AICANCEL-07 | 취소로 끝난 추론은 잠금 충돌·외부 연동 실패와 다른 전용 코드로 응답한다 | 진행 중 추론을 취소 | 원 추론 요청의 응답 | `AiCallCancelledException` → `ErrorCode.AI_REQUEST_CANCELLED`(422). 낙관적 잠금 충돌(409)·외부 응답 불신(502)과 **코드가 다르다** — 화면이 취소를 오류 안내와 구분하는 근거 | integration | P0 | AiCallCancelledException.java · ErrorCode.java(`AI_REQUEST_CANCELLED`) |
| TC-AICANCEL-08 | 취소는 서킷 브레이커의 실패로 집계되지 않는다 | 진행 중 추론을 취소 | 동일 | resilience4j 리액터 연산자가 구독 해지를 **permit 반납**으로 처리 — 반복 취소가 서킷을 열어 다른 사용자의 추론까지 막지 않는다(회귀 고정) | security | P0 | AiCallScope.java · AiCallCancellationTest.java(`취소는_서킷_브레이커의_실패로_집계되지_않는다`) |
| TC-AICANCEL-09 | 취소 등록 범위는 온디맨드 4경로뿐 — 배치 파이프라인은 대상이 아니다 | 배치 스레드(`YoloAutolabelStep`/`Sam2SegmentStep`) 실행 중 | 그 배치가 쓴 추론 호출을 취소 API 로 시도 | 배치 스레드에는 스코프가 매이지 않아 `CancellableAiCall` 이 기존 블로킹 동작을 그대로 수행 — 취소 요청은 대상을 찾지 못해 `false` | integration | P1 | AiCallCancellationInterceptor.java · CancellableAiCall.java |
| TC-AICANCEL-10 | 취소는 노드 로컬이다 — 다른 노드가 처리 중인 요청은 못 끊는다 | 2노드 배포, 요청을 처리한 노드 ≠ 취소 요청을 받은 노드 | `POST /v1/ai-requests/{requestId}/cancel` | 200 `{cancelled:false}`(정직하게 "못 끊었다"로 응답 — 공유 저장소·고정 라우팅 없음, 인지·수용된 한계) | integration | P2 | AiCallCancellationRegistry.java(클래스 javadoc) |
| TC-AICANCEL-11 | `GET /v1/ai-defaults` 의 `waitBudgets` 는 값이 없어도 생략되지 않는다 | 슬라이더 기본값(`confThreshold`/`simplifyTolerance`) 저장값 없음 | `GET /v1/ai-defaults` | 슬라이더 두 필드는 생략(`NON_NULL`)돼도 `waitBudgets`(4종: autolabel/segment/sam2Track/autoTrack)는 **항상 존재** — 없으면 화면이 자기 상수로 되돌아가 "서버 예산이 바뀌면 조용히 어긋난다"는 원 결함이 재발한다 | integration | P0 | AiDefaultsService.java(`get`) · AiDefaultsResponse.java |
| TC-AICANCEL-12 | 대기 예산은 서버 재시도 설정에서 도출된다 — 하드코딩이 아니다 | `resilience4j.retry.instances.ai` 의 `max-attempts`/`wait-duration`을 변경 | `AiWaitBudgetPolicy.worstCallSeconds()` | 산출값이 yml 변경을 따라 자동으로 움직인다(백오프도 `IntervalBiFunction` 을 실제로 적용해 합산 — 상수로 적어두면 두 번째 진실원이 된다) | unit | P0 | AiWaitBudgetPolicy.java(`worstCallSeconds`) |
| TC-AICANCEL-13 | 운영자가 설정한 절대 상한이 도출 하한보다 낮으면 읽기 시점에 하한으로 끌어올린다 | `AI_WAIT_BUDGET_CEILING_SEC` 설정값 < `minimumCeilingSeconds()`(저장 검증을 우회해 이미 저장된 경우 포함) | `GET /v1/ai-defaults` | 설정값을 그대로 쓰지 않고 하한을 적용 + WARN 로그 — 저장 시점 검증만으로는 부족하다(검증 규칙이 나중에 바뀌거나 다른 경로로 값이 들어올 수 있어 **읽기 시점에도 2차로 강제**) | integration | P0 | AiWaitBudgetProvider.java(`ceilingSeconds`) |
| TC-AICANCEL-14 | 설정값이 하한 이상이면 그대로 적용된다 | 설정값 ≥ `minimumCeilingSeconds()` | 동일 | 설정값 그대로 `ceilingSec` 에 반영(상한을 넉넉히 준 것은 운영자의 선택이며 이 결함이 재발하지 않는 방향) | unit | P1 | AiWaitBudgetProvider.java(`ceilingSeconds`) |
| TC-AICANCEL-15 | 설정값 부재·비정수·조회 실패는 예외 없이 기본 절대 상한으로 폴백한다 | 설정 행 없음(시딩하지 않는 것이 설계) / 정수 파싱 실패 / DB 조회 예외 | 동일 | `DEFAULT_CEILING_SEC`(300초) 적용 + WARN 로그 — 예외를 던지면 온디맨드 추론 자체가 막힌다 | unit | P1 | AiWaitBudgetProvider.java(`configuredCeiling`) |
| TC-AICANCEL-16 | SAM2 추적은 요청 단위 시간 예산 안에서 돈다 — 예산 소진 시 성공분을 보존하고 지점을 알린다 | `nextSrcSns` 다건, 처리 도중 `TRACK_BATCH_BUDGET`(요청 전체 wall-clock 예산) 소진 | `POST /v1/frames/{srcSn}/sam2-track` | 200 + 그때까지 처리한 `tracked`(성공분 보존) + `truncated=true` + `resume`(이어 보낼 요청 값) — 조용히 버리지 않는다. 데드라인은 **반복 진입 직전**에 확인하므로 마지막 반복의 부대 작업만큼 예산을 넘길 수 있다 | integration | P0 | Sam2TrackService.java · Sam2TrackResponseDto.java · AiWaitBudgetPolicy.java(`TRACK_BATCH_BUDGET`) |
| TC-AICANCEL-17 | AI 자동 추적(YOLO track)도 같은 요청 단위 예산 계약을 따른다 | 시작 프레임 + 후속 프레임 시퀀스(최대 50), 예산 소진 | `POST /v1/frames/{srcSn}/yolo-track` | 동일 시맨틱(성공분 보존 + `truncated`/`resume`) — 시작 프레임도 후속과 같은 루프·같은 예산 안에서 처리된다 | integration | P0 | YoloTrackService.java · YoloTrackResponseDto.java |
| TC-AICANCEL-18 | 예산 소진(truncated)은 취소가 아니다 — 두 신호는 섞이지 않는다 | 예산 소진 응답 | 동일 | 응답 상태코드는 200(성공)이며 `AI_REQUEST_CANCELLED`(422, TC-AICANCEL-07)와 혼동되지 않는다. 취소는 사용자의 명시적 행동(취소 버튼·Enter/Space·ESC)일 때만 발생한다 | integration | P1 | Sam2TrackService.java · YoloTrackService.java · AiCallCancelledException.java |
| TC-AICANCEL-19 | **서버가 발행한 이어 보내기 값으로 만든 요청은 서버 자신이 받아들인다 — 남은 프레임이 하나뿐일 때**(신설) | 시퀀스 3프레임 중 2프레임 처리 후 예산 소진 → `resume = {srcSn: 마지막 프레임, nextSrcSns: []}` | 그 값으로 요청 객체를 만들어 Bean Validation(컨트롤러 `@Valid` 와 같은 판정) | 위반 0건. **구 동작은 400**(`@NotEmpty`)이라 화면이 서버가 준 값을 그대로 되실었는데 거부됐고, 이 경로엔 부분 결과 보존이 없어 **이미 계산된 최대 50프레임 검출이 전량 폐기**됐다 | unit | P0 | label/YoloTrackRequest.java · YoloTrackService.java(track) · TrackResumeRequestContractTest.java(남은_프레임이_하나뿐인_이어보내기_값도_요청_규격을_통과한다) |
| TC-AICANCEL-20 | 진행이 0 인 이어 보내기 값도 요청 규격을 통과한다(신설) | 한 프레임도 담지 못하는 예산 → `resume` 이 요청 그대로 | 동일 | 위반 0건 — 화면은 «진행이 있었는지» 로 되풀이를 끊는다(규격 위반으로 끊지 않는다) | unit | P1 | YoloTrackService.java(track) · TrackResumeRequestContractTest.java(진행이_0인_이어보내기_값도_요청_규격을_통과한다) |
| TC-AICANCEL-21 | **SAM2 추적의 이어 보내기 시드 폴리곤은 요청 점수 상한 안이다**(신설) | 추론 서버가 상한(`Sam2TrackRequest.MAX_POLYGON_POINTS`) 초과 폴리곤 반환(단순화로 줄지 않는 톱니 모양) + 예산 소진 | 동일 | `resume.prevPolygon` 이 상한 이하이고 그 값으로 만든 요청의 위반 0건. 전파 시드는 **단순화 전 원본**이고 추론 서버는 윤곽점 수를 제한하지 않으며 응답 검증은 **최소** 점 수만 보므로, 경로 어디에도 상한을 보장하는 지점이 없었다. ⚠ **목서버로는 재현되지 않는다**(입력을 그대로 돌려주므로) — 실모델에서만 드러난다. 상한 이하일 때는 **원본 그대로**(평시 동작 불변) | unit | P0 | label/Sam2TrackRequest.java(MAX_POLYGON_POINTS) · Sam2TrackService.java(resumeSeed) · TrackResumeRequestContractTest.java(추론서버가_점수_상한을_넘는_폴리곤을_줘도_이어보내기_시드는_요청_규격을_통과한다) |
| TC-AICANCEL-22 | SAM2 이어 보내기의 남은 프레임 목록은 구조적으로 비지 않는다(신설, 불변식 고정) | 예산 소진 | 동일 | `resume.nextSrcSns` 가 비지 않고 요청 규격 통과. SAM2 는 시작이 «마지막으로 처리한 프레임» 이라 남은 목록이 비는 상태가 성립하지 않는다 — **AI 자동 추적(첫 미처리 프레임 기준)과 축이 달라 함께 무너지지 않으므로** 그 차이를 시험으로 못박는다. ⚠ 그래서 SAM2 요청의 `@NotEmpty`(TC-SAM2-17)는 **유지**한다 — 두 경로를 «일관성» 을 이유로 통일하지 말 것 | unit | P1 | Sam2TrackService.java(track) · TrackResumeRequestContractTest.java(SAM2_이어보내기_값도_요청_규격을_통과하고_남은_프레임_목록이_비지_않는다) |
| TC-AICANCEL-23 | **프레임 수에 비례하는 접근 검증도 예산 시계 안에서 돈다**(신설) | 프레임별 인가 검사가 느린 상황(각 300ms) + 예산 500ms, 시퀀스 3프레임 | `POST /v1/frames/{srcSn}/yolo-track` | 추론 호출 0건 + `truncated=true` + `resume` 발행. 검증이 예산 시계 **밖**이면 예산이 고스란히 남아 세 프레임을 전부 추론하고, 요청 소요가 «검증 시간 + 추론» 이 되어 **예산 정책이 화면에 내려준 대기 상한을 넘어선다**(정상 요청이 화면에서 끊긴다). 구 구현은 시계를 검증 **뒤**에 세웠다 | unit | P1 | YoloTrackService.java(track) · AiWaitBudgetPolicy.java(budgets) · YoloTrackBudgetTest.java(프레임_수에_비례하는_접근_검증도_예산_시계_안에서_돈다) |
| TC-AICANCEL-24 | 같은 프레임 행을 두 번 읽지 않는다(신설) | 시퀀스 3프레임 | 동일 | 프레임당 인가 검사 1회(`verifyAndGet` 이 인가와 조회를 함께 수행). 구 구현은 인가 검사와 `findById` 를 따로 불러 **프레임마다 조회가 두 번** 나갔고 그 중복이 위 예산을 깎았다 | unit | P2 | YoloTrackService.java(track) · LabelAccessGuard.java(verifyAndGet) · YoloTrackBudgetTest.java(같은_프레임_행을_두_번_읽지_않는다) |

| TC-AICANCEL-25 | **절단이면서 일부가 mock 인 응답에서도 mock 안내가 그대로 남는다**(신설) | 시퀀스 3프레임 중 첫 프레임이 mock(결과에서 제외), 둘째는 실모델, 셋째에서 예산 소진 | `POST /v1/frames/{srcSn}/sam2-track` | `message` 가 **부분 신뢰불가 안내와 정확히 같다**(절단 문구가 앞에 붙지 않는다). 구 동작은 «절단 문구 + mock 안내» 를 이어 붙였는데, 화면이 «절단을 보고한 응답의 문구는 버린다» 는 필터를 갖고 있어 **mock 안내까지 함께 버려졌다** — mock 프레임은 결과 목록에서 빠지므로 그 안내가 유일한 신호다 | unit | P0 | Sam2TrackOutcome.java(`of`) · Sam2TrackService.java(track) · TrackNoticeChannelTest.java(절단이면서_일부가_mock_인_응답에도_mock_안내가_그대로_남는다) |
| TC-AICANCEL-26 | 절단이면서 처리분이 전량 mock 이면 모델미로드 안내가 그대로 남는다(신설) | 예산 안에 든 프레임이 전부 mock → `tracked` 비어 있고 뒤가 남음 | 동일 | `message` 가 **전량 신뢰불가 안내와 정확히 같다**. 결과가 0건인데 안내까지 없으면 «추적이 아무 것도 못 찾았다» 와 «모델이 안 떠 있다» 를 구분할 수 없다 | unit | P0 | Sam2TrackOutcome.java(`of`·`withMock`) · TrackNoticeChannelTest.java(절단이면서_처리분이_전량_mock_이면_모델미로드_안내가_그대로_남는다) |
| TC-AICANCEL-27 | **예산 절단은 안내 문구를 싣지 않는다 — 구조화된 값으로만 알린다**(신설, 구 정책 폐기) | mock 없이 예산만 소진 | `POST /v1/frames/{srcSn}/sam2-track` · `POST /v1/frames/{srcSn}/yolo-track` **두 경로 모두** | `truncated=true` + `resume` 은 그대로이고 `message` 는 **null**. 그 문구는 «요청 하나» 기준이라 이어 보내 완주하면 "남은 프레임은 이어서 요청해 주세요" 가 거짓이 되고, 못 끝냈더라도 실행 전체의 남은 수와 어긋난다 — 화면이 안 쓰는 문구가 같은 통로의 mock 안내만 밀어내고 있었다. 문구 생성기(`TrackBudgetNotice`)는 사용처가 사라져 삭제됐다 | unit | P0 | Sam2TrackOutcome.java(`of`) · LabelController.java(`yoloTrack`) · TrackNoticeChannelTest.java(mock_이_없는_절단_응답은_안내_문구를_싣지_않는다 · 자동추적의_절단_응답도_안내_문구를_싣지_않는다) · Sam2TrackBudgetTest.java(예산_소진은_안내_문구가_아니라_구조화된_값으로만_알린다) |

> **화면쪽(FE)의 이어 보내기·조각 표기·잠금 fail-safe 재계산**은 이 절의 대상이 아니다 — 그 축은
> [H-frontend-e2e.md](H-frontend-e2e.md) 소관이며 `frontend/src/features/label/aiBudget.ts`(대기 예산 계산기) ·
> `hooks/useSam2Track.ts`·`hooks/useAutoTrack.ts`(이어 보내기)·`hooks/useBusyTask.ts`(취소 호출)를 근거로 한다.

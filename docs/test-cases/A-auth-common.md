...
> **243 케이스**(표 행 실측) · 계층: unit / integration / security · [← README](README.md)
>
> 섹션별: A-1 34 · A-2 32 · A-3 12 · A-4 46 · A-5 28 · A-6 42 · A-7 49 = **243**. 폐기 행은 수에 포함 — 행을 지우지 않는다
...
> **2026-08-05 헤더 카운트 정정(케이스 내용 변경 없음)**: 머리말 총계 242 → **243**, 섹션별 `A-6` 39 → **42** 로 실측 정정. 회차 3 이 A-6 에 케이스를 추가하고 섹션 소계를 갱신하지 않아 소계 합(240) · 머리말(242) · 실제 표 행(243) 이 3중으로 어긋나 있었다. **카운트 기준 = 표 행 실측(폐기 행 포함)** — 폐기 표기는 ID 취소선/케이스명 취소선/기대결과 `[폐기]` 로 일관되지 않아 "폐기 제외" 규칙은 재현 가능한 기준이 되지 못한다.

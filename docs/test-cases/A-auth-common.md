...
| TC-PROF-011 | ProfileGatedUrlPolicy — 완화 플래그는 local/dev + ENV 표식 없음일 때만 인정 (신규) | `vlm.client.allow-insecure-url=true` | stg/prd/미지정 프로파일 또는 `ENV=stg\|prd` | `@PostConstruct` 에서 **기동 실패**. 연동 활성 토글(enabled/mode)과 무관하게 검사 — 운영 파일에 완화 값이 있다는 사실 자체가 사고 신호 | security | H | ProfileGatedUrlPolicy.java:45-55,82-95,145-163 |
...
| TC-PROF-013 | ExternalUrlPolicy strict — https 전용 + 사설/루프백/예약 대역 차단 **(정정 — 판정이 명시적 CIDR 로 교체됨)** | VLM 운영 정책 | `http://...` / `10.0.0.5`·`172.16.0.1`·`192.168.0.1`·`127.0.0.1`·`0.0.0.0`·`[::1]` / DNS 해석 실패 | 각각 IllegalStateException(CWE-319 / CWE-918), 사설 계열 메시지에 "내부/사설/예약 네트워크". 차단 대역 = 와일드카드·0.0.0.0/8·루프백·RFC1918·링크로컬(169.254/16·fe80::/10)·CGNAT(100.64/10)·ULA(fc00::/7)·fec0::/10 **+ IANA 특수목적 4종(192.0.0.0/24·198.18.0.0/15·224.0.0.0/4·ff00::/8·240.0.0.0/4)**(정정 — 2026-08-04 2차 확장). **해석 실패도 거부**(DNS rebinding 대응). 예외 메시지에 baseUrl 원문 미노출(CWE-209) | security | H | ExternalUrlPolicy.java:104-147,185-217,236-390 |
| TC-PROF-014 | ExternalUrlPolicy 완화 경로도 링크로컬/메타데이터·ULA·CGNAT 대역 거부 **(정정 — 차단 집합 확장, G-ISSUE-21 해소)** | local/dev 완화 | `http://169.254.169.254/`, `http://[fe80::1]`, `http://[fd00:ec2::254]`, `http://100.100.100.200` | 전건 거부. **IANA 특수목적 4종(192.0.0.0/24·198.18.0.0/15·224.0.0.0/4·ff00::/8·240.0.0.0/4)과 NAT64 임베드 표기도 완화 경로에서 거부**(정정 — 2026-08-04 2차 확장). RFC1918·loopback 은 완화 경로에서 계속 허용(개발 목업 기동 보장). 단 **해석 실패는 통과**(컨테이너 서비스명이 도커 밖에서 안 풀려도 기동 보장) — "해석되면 검사, 안 되면 통과" | security | H | ExternalUrlPolicy.java:137-141,158-173,195-234 |
| TC-PROF-018 | ExternalUrlPolicy — 해석된 **전 주소** 검사 + IPv4-mapped 언랩 (신규, G-ISSUE-22 해소) | 다중 A/AAAA 응답 호스트 | `getAllByName` 결과에 공인+사설 혼재 / `[::ffff:10.0.0.5]` | 하나라도 차단 대역이면 거부(첫 주소만 보면 검사 대상≠접속 대상). IPv6 로 위장한 IPv4 사설도 언랩 후 거부 — **언랩 대상은 IPv4-mapped·IPv4-compatible·NAT64 well-known prefix(`64:ff9b::/96`, RFC 6052) 3종**(정정 — NAT64 추가). `64:ff9b::a9fe:a9fe` 는 169.254.169.254(IMDS)로 언랩돼 거부, 공인 임베드(`64:ff9b::808:808`=8.8.8.8)는 통과 | security | H | ExternalUrlPolicy.java:167-173,185-193,195-217,267-269,287-307 / ProfileGatedUrlPolicy.java:134-143 |
| TC-PROF-019 | ExternalUrlPolicy — IANA 특수목적 대역 4종 차단 (신규, 리뷰 MEDIUM 보강) | strict/relaxed 공통 | `192.0.0.170`(IETF 프로토콜 할당) · `198.18.0.1`·`198.19.255.254`(벤치마킹) · `224.0.0.1`·`[ff02::1]`(멀티캐스트 v4/v6) · `240.0.0.1`·`255.255.255.255`(Class E) vs `192.0.1.1`·`198.17.255.254`·`198.20.0.1`·`223.255.255.254` | 앞 7건 IllegalStateException(CWE-918) / 뒤 4건 통과. **relaxed 에서도 차단** — 근거는 `UNSPECIFIED_V4`(0.0.0.0/8)와 같은 "과차단 방향이라 안전"이며 개발 목업이 쓰는 RFC1918·loopback 은 미포함이라 기동 영향 0(배포 형상 base-url 전수 확인) | security | M | ExternalUrlPolicy.java:245-252,336-347,368-372 / VlmUrlPolicyTest.java:ianaSpecialPurposeRangesRejected |
| TC-PROF-020 | KPST 연동도 동일 대역 판정을 받는다 — 3연동 대칭 회귀 가드 (신규, 리뷰 HIGH 보강) | `KpstWebClientConfig` 는 `ExternalUrlPolicy.internalNetwork("kpst.deid.base-url")` 에 위임(relaxed) | ULA·CGNAT·NAT64·IPv4-mapped·IANA 4종 / 다중 해석 주소(공인+IMDS 등) vs RFC1918·loopback·미해석 컨테이너명 | 앞 그룹 IllegalStateException(빈 생성 실패 → 기동 차단), **2개 빈(kpstDeidWebClient·kpstDeidProgressHttpClient) 대칭** / 뒤 그룹 정상 생성. 판정기가 3연동 공용인데 가드가 VLM·증강 2축에만 있어 KPST 만 갈라져도 잡히지 않던 공백을 메운다. TLS 자체 CA fail-closed(CWE-295)는 불변 | security | H | KpstWebClientConfig.java:71-72,169-171 / KpstWebClientConfigTest.java:kpstIpv6UlaRejected 외 6건 |

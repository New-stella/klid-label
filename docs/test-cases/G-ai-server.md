...
> 172 케이스 · 경로: `ai-server/`(YOLO/SAM2/VLM 추론) + `mock-server/`(외부 벤더 목업) + `backend/`(BE↔벤더 실배선 계약) · HTTP는 FastAPI TestClient(mock 기본, conftest `AI_MOCK_MODE=true`) · [← README](README.md)

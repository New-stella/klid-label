...
| **5-2** | **2026-08-05** | **카탈로그 정합 3건(문서 전용 · 코드 무변경)** — ①F `TC-PORTAL-060~062` **ID 중복** 재번호(F-3 쪽 → `095~097`) ②C `TC-LABEL-125` **stale 폐기 표기**(회차 4 가 놓친 개인정보 리셋 폐기의 잔여분) ③**총계 산식 커밋별 재계산 + 기준선 재설정**(구 "기타 6" 잔차 폐기) | **3** | **0** | **1** | **2,030**(실측, 불변 — 재번호·폐기 표기 모두 행을 남김) |
...
> 재현 방법: 각 파일에서 **표의 ID 칸이 `TC-` 로 시작하는 행**을 셉니다(취소선·볼드 포함). 변경 이력 표(ID 칸이 회차 번호)·현황 요약 표는 이 패턴에 걸리지 않아 자동 제외됩니다.
>
...
> | 현재 (`c6d776b3`) | **2,030** | **+30** = 회차 5 신규 24 + 회차 5-1 신규 6 (B +19 · C +4 · H +7) |
>
...
>
> ⚠ 회차 5 에서 C 카탈로그 신규 4건이 기존 `TC-LABEL-137~140`(C-ISSUE-41 좌표 clamp)과 **ID 충돌**해 `TC-LABEL-150~153` 으로 재번호했습니다(행 수 불변 → 카운트 영향 없음). 같은 성격으로 2026-08-05 에 F 카탈로그의 **`TC-PORTAL-060·061·062` 중복 3건**(F-3 저장 검증 ↔ F-4 포털 SAM2)을 F-3 쪽만 **`TC-PORTAL-095~097`** 로 재번호했습니다 — 1·2차 검증 결과 문서가 인용 중인 F-4 쪽 번호를 보존했고, 행 수는 불변입니다.
...
- 최신 기준선: **BE `cleanTest` 4,196 tests / 실패 0 / skip 5**(라이브 목서버 IT, 2026-07-30) · **FE 338 files / 2,038 tests PASS**(2026-08-03, 커밋 `e58aa086` 전체 회귀 기준).

...
| TC-BATCH-035 | process: disabled stage skip(dev 토글) | FRAME_EXTRACT=false | process | `step.isEnabled(ctx)` false → markStage·execute 둘 다 skip. ⚠ **실동작 검증 불가** — 토글을 넘기는 오버로드 `process(Long, Map)` 의 **프로덕션/dev 호출자가 0건**이라 REST·Quartz 어느 진입점으로도 태울 수 없다(단위테스트 전용 분기, 1차 B-ISSUE-23 미해소) | integration | P1 | BatchOrchestrator.java:120-124 |
| TC-BATCH-036 | process: 토글 없음 → 전 stage enabled | stageToggles null | process(rawSn) | 모든 단계 실행 | unit | P1 | BatchContext.java:77-84 |
| TC-BATCH-037 | 두 테이블 분리: 배치완료 시 작업상태 ASSIGNED 복귀 | 배치 완료 | markRawDataCompleted | LS_DATA_RAW=COMPLETED, LS_RAW_DATA_STATUS=ASSIGNED | integration | P0 | BatchTransitionService.java:146-158 |
| TC-BATCH-038 | 작업상태 COMPLETED 점프 시 검수제출 차단 회귀 방지 | 배치완료 후 제출 | 상태검증 | ASSIGNED→PENDING 정상(작업 COMPLETED 는 ReviewService.approve 전용) | integration | P1 | BatchTransitionService.java:146-158 |
...
| TC-BATCH-040 | 배치시작: 두 컬럼 PROCESSING 동시 전이 | 마킹완료 시작, 작업상태가 검수 소유 아님 | `markRawDataProcessingBlocked` | **false 반환**(=차단 아님) + 작업상태·LS_DATA_RAW 둘 다 PROCESSING | integration | P1 | BatchTransitionService.java:110-122 |
...
| TC-BATCH-042 | ★진입 가드: 검수 소유 상태면 SKIPPED + step 0건 (신규) | 작업상태 ∈ {PENDING,IN_REVIEW,APPROVED,REJECTED} | process(rawSn) | `markRawDataProcessingBlocked`=true → `BatchStage.SKIPPED` 반환, **step 을 한 건도 실행하지 않음**, LS_DATA_RAW 도 미변경 | integration | P0 | BatchOrchestrator.java:112-115 · BatchTransitionService.java:110-122 |
...
| TC-BATCH-045 | 진입 가드는 전 진입점 공통 관문 (신규) | 마킹 브리지·dev 트리거·Quartz 큐·재시도 잡·수동 재처리 | 각 경로로 process 호출 | 5개 진입점이 모두 `process(Long)` 단일 관문을 통과한다. ⚠ **단 이 가드는 클레임이 아니다** — 조건부 UPDATE 가 "검수 소유 상태"만 판정하고 `PROCESSING` 은 차단 집합 밖이라 **동일 rawSn 동시 실행을 막지 못한다**(3차 실측: 동시 5요청 → 5개 파이프라인 병렬 실행 + 외부 VLM 5중 위탁). 따라서 "무증상 오염 차단"은 순차 경로에서만 성립한다(B-ISSUE-22 미해소) | integration | P0 | BatchOrchestrator.java:112-115 |
...
| TC-BATCH-177 | 클러스터링 ≠ 잡 내부 레이스 방어 (신규) | 클러스터링 on | KPST 폴링·재시도 큐·export sweep | 각 잡의 원자 클레임(조건부 UPDATE)이 여전히 필요 — 두 방어는 서로 대체하지 않는다. ⚠ **배치 파이프라인은 진입점마다 서로 다른 자원을 클레임**한다(수동 재처리=`LS_DATA_RAW` 단계 컬럼 / 자동 재시도=`LS_BAT_RTY_WTNG` 큐 행 / 마킹 브리지=작업상태 BATCH_QUEUED) — `BatchOrchestrator.process(rawSn)` 자체에는 rawSn 단위 재진입 가드가 없어 전 진입점에 걸친 상호배제는 성립하지 않는다(3차 B-ISSUE-62) | integration | P0 | QuartzClusteringGuard.java:39-40 · BatchTransitionService.java:111-122 |

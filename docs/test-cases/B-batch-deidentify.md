...
> 372 케이스(표 행 실측) · 계층: unit / integration / security · 우선순위 P0(Critical)~P2 · [← README](README.md)
...
| 11 | 2026-08-04 | 0건 | 2건 | 0건 | **신고자 표시명 노출** — `main`(PR #79, `1916dc6d`)에서 병렬 진행돼 머지로 합류. `GET /v1/deident-reports` 응답에 `reporterName`(`MNG_ACCT_USER.USER_NM`) 추가(`TC-DEID-094/095`). 기존 `reporterNo`(`USER_NO` 원값)는 하위호환으로 유지되므로 **정정·폐기 0건**(필드 추가만). 이름 해석은 페이지 단위 **단일 IN 쿼리**이고, 마스터에 없는 신고자(탈퇴 등)는 목록을 깨뜨리지 않고 `null` 로 내린다(fail-soft). ⚠ 같은 커밋의 공지 작성자 표시명(`NoticeResponse.writerName`)은 **본 카탈로그에 게시판(공지) BE 클러스터가 아예 없어** 반영하지 못했다 — 신설은 별건(README 클러스터 표 갱신 필요) |
...
|---|---|---|---|---|---|:--:|---|
| TC-BATCH-001 | 스캔: 적재 후보 0건 | `findPendingReadyForPolling` 결과 null/empty | `scanAndIngest()` | 0 반환, `ingestOne` 미호출, DEBUG 로그(`no pending ingest rows to scan`) | unit | P2 | TrainingVideoIngestService.java:127-133 |
| TC-BATCH-002 | 스캔: 폴링 후보 조회 — PENDING + backoff 미도래 + FIFO + tick상한 | `LS_DATA_INGEST` 에 `PROC_STTS_CD='PENDING'` N건(일부 `NEXT_RTRY_DT` 미래 예정) | `scanAndIngest()` | `NEXT_RTRY_DT IS NULL OR <= now` 인 행만 `RCPTN_DT ASC, RCPTN_SN ASC` 순으로 최대 `INGEST_SCAN_LIMIT`(100)건 조회 — 후보 250건이면 100건만 처리, 잔여는 다음 tick | integration | P0 | LsDataIngestRepository.java:52-65 · TrainingVideoIngestService.java:58,128-129 |
| TC-BATCH-003 | 스캔: 동시 중복 적재 race 흡수(UnexpectedRollbackException) | 한 행이 `ingestOne` 커밋 시점에 UK(VMS_CLIP_ID) 위반으로 tx abort | `scanAndIngest()` | ERROR 아닌 INFO 로그(`ingest tx rolled back (duplicate clip race) — retried next tick`), 예외 미전파, 나머지 행 계속 처리 | integration | P0 | TrainingVideoIngestService.java:140-149 |
| TC-BATCH-004 | 스캔: 1건 RuntimeException 흡수가 다른 행을 막지 않음 | `ingestOne` 중 1건 임의 RuntimeException | 여러 인입 행 | ERROR 로그(`rcptnSn`+`causeType` 만, 인입 자유텍스트 미포함), 나머지 행 계속 적재 | integration | P0 | TrainingVideoIngestService.java:150-156 |
| TC-BATCH-005 | 스캔: 상한 도달 시 이월 로그 | 후보 ≥100건(`INGEST_SCAN_LIMIT`) | `scanAndIngest()` | `scan finished scanned=100 ingested=… limit=100 carriedOver=true` — 잔여분은 다음 tick 이 이어서 처리(종결된 행이 폴링 술어에서 빠지며 커서 전진) | integration | P1 | TrainingVideoIngestService.java:158-162 |
...
| TC-BATCH-010 | 적재: rcptnSn null → 즉시 skip | `candidate` null 이거나 `rcptnSn` null | `ingestOne(candidate)` | WARN 로그, false 반환, `claimForProcessing` 미호출 | unit | P2 | TrainingVideoIngestTx.java:239-244(⚠병합 후 라인드리프트 재확인 필요) |
| TC-BATCH-011 | 적재: 원자 클레임 실패(0 반환) → skip | 다른 실행이 이미 클레임했거나 그 사이 종결됨 | `claimForProcessing(rcptnSn)` → 0 | DEBUG 로그(`claim lost — skip`), false 반환, 이후 로직 미실행(엔티티 재조회 안 함) | integration | P0 | TrainingVideoIngestTx.java:245-249 · LsDataIngestRepository.java:149-156(⚠병합 후 라인드리프트 재확인 필요) |
| TC-BATCH-012 | 적재: 클레임 성공 후 재조회 시 행 소실 | 클레임 반환 1이나 `findById(rcptnSn)` empty(외부 삭제 등 비정상) | `ingestOne` | ERROR 로그(`claimed row disappeared`), false 반환 — 인입 행 영구보존 정책상 도달 불가 시나리오임을 명시 | unit | P2 | TrainingVideoIngestTx.java:250-256(⚠병합 후 라인드리프트 재확인 필요) |
| TC-BATCH-013 | 적재: 식별자 blank → markFailed(순서: VMS_CLIP_ID→VMS_CCTV_ID→RAW_FILE_PATH_NM) | 셋 중 하나 null/blank | `ingestClaimed` | 첫 번째로 비어있는 컬럼명으로 `markFailed("{컬럼} 누락 — 적재 불가")`, false, save 없음 | unit | P1 | TrainingVideoIngestTx.java:260-268,423-434(⚠병합 후 라인드리프트 재확인 필요) |
| TC-BATCH-014 | 적재: 멱등 1차 — 이미 적재된 clipId | `findByVmsClipId(vmsClipId)` 존재 | `ingestClaimed` | `markDone(기존 rawSn)`, false 반환, 중복 INSERT 없음(무한 재조회 방지) | integration | P0 | TrainingVideoIngestTx.java:269-277(⚠병합 후 라인드리프트 재확인 필요) |
| TC-BATCH-015 | 적재: 경로 REJECTED — 허용 루트 밖/손상 경로 | 허용 루트(`authoring.storage.raw-mount-roots`) 밖 절대경로 또는 `InvalidPathException` | `verifyPath` | `markFailed("원본 영상 경로가 허용 저장 루트 밖이거나 유효하지 않음")`, false(CWE-22) | integration | P0 | TrainingVideoIngestTx.java:278-282,456-461(⚠병합 후 라인드리프트 재확인 필요) |
| TC-BATCH-016 | 적재: 경로 REJECTED — 허용 루트 안 파일의 최종 심링크가 루트 밖을 가리킴 | 파일 실경로가 `allowedRoots()` 어디에도 속하지 않음(판정축이 구 "동일 디렉터리"에서 "허용 루트 하위"로 전환) | `verifyPath` → `isUnderAllowedRoot` | REJECTED, false(CWE-59) — 반대로 같은 허용 루트 안의 다른 하위 디렉터리를 가리키는 심링크(예 `/nas/videos/2026/07/clip.mp4`)는 더 이상 오탐 거부되지 않는다 | integration | P0 | TrainingVideoIngestTx.java:446-455,482-485,532-539(⚠병합 후 라인드리프트 재확인 필요) |
| TC-BATCH-017 | 적재: 경로 NOT_ARRIVED — 파일 미존재/권한오류/깨진 심링크 | `Files.exists`=false 또는 `toRealPath()` IOException 또는 `isRegularFile`=false | `verifyPath` | NOT_ARRIVED(영구 거부 아님) → `handleNotArrived` 진입, 다음 주기 재시도 대상 | integration | P0 | TrainingVideoIngestTx.java:284-286,468-480(⚠병합 후 라인드리프트 재확인 필요) |
| TC-BATCH-018 | 적재: 정상 → LsDataRaw 저장 + 이벤트 발행 + 종결 | 경로 READY, 식별자 정상 | `persistRaw` | `LsDataRaw` 신규 저장 + `VideoIngestedEvent(rawSn)` 발행(AFTER_COMMIT 비식별 선두 트리거) + `ingest.markDone(rawSn)`, true 반환 | integration | P0 | TrainingVideoIngestTx.java:353,365-366(⚠병합 후 라인드리프트 재확인 필요) |
| TC-BATCH-019 | 적재: 멱등 2차 — 동시 UK(VMS_CLIP_ID) race | `save()` 시점 `DataIntegrityViolationException` | `persistRaw` | catch→DEBUG 로그, false 반환 — PostgreSQL 은 제약위반 시 트랜잭션 전체를 abort 하므로 이 트랜잭션의 클레임도 함께 롤백되어 행이 PENDING 복귀, 다음 tick 이 1차 멱등(`findByVmsClipId`)으로 DONE 종결 | integration | P0 | TrainingVideoIngestTx.java:369-376(⚠병합 후 라인드리프트 재확인 필요) |
| TC-BATCH-020 | 적재 매핑: EVNT_TYPE_CD 를 `EVNT_ID` 로 해석 + SHT_DT 폴백 폐지 **(main PR #77 반영)** | 인입 행 `EVNT_ID` 에 대응하는 `MNG_CLIP_EVNT_LST` 행이 정확히 1건 / `SHT_DT` 미수신 | `persistRaw` → `resolveEvntTypeCd` | 해석된 `EVNT_TYPE_CD` 가 `LS_DATA_RAW` 에 영속된다. `SHT_DT` 는 관제 미수신 시 null 유지(구 `CRT_DT` 폴백 미복원 — 대용값 금지) | unit | **P0** | TrainingVideoIngestTx.java:353-361,407-424 · MngClipEvntLstRepository.java:53-63 |
| TC-BATCH-020a | 적재 매핑: 이벤트유형 **무매칭**은 적재 성공 + null **(신설)** | `EVNT_ID` 에 매칭 행 없음(또는 `EVNT_ID` 자체가 없음) | `persistRaw` | `EVNT_TYPE_CD` null 로 적재하되 **`ingestOne` 은 true**(적재 실패 금지 — 영상이 아예 안 들어오면 되돌리기가 더 어렵다). `EVNT_ID` 가 blank 면 조회 자체를 하지 않는다 | unit | P0 | TrainingVideoIngestTx.java:407-417 |
| TC-BATCH-020b | 적재 매핑: 이벤트유형 **다중매칭**은 임의선택 금지 **(신설)** | 한 `EVNT_ID` 에 서로 다른 `EVNT_TYPE_CD` 2건(복합 PK 라 가능) | `persistRaw` | 어느 것도 고르지 않고 null 로 적재(임의 선택 시 관제 통지 계약·필터·통계가 **틀린 값으로 확정** — null 보다 나쁘다). 조회는 `LIMIT 2` 로 유계 | unit | P0 | TrainingVideoIngestTx.java:418-423 · MngClipEvntLstRepository.java:53-63 |
| TC-BATCH-020c | **적재→마킹 경계**: 적재된 영상이 마킹 프리컨디션을 통과 **(신설 — 실장애 회귀)** | 관제 인입 1건(해석 가능한 `EVNT_ID`) 적재 후 비식별 완료·`MARKING_READY` | `MarkingGuards.requirePreconditions(raw)` | 예외 없음. 해석 실패 영상은 종전대로 `INVALID_INPUT`(400, "이벤트 유형이 지정되지 않은 영상은 마킹할 수 없습니다.") 로 막힌다 | integration | **P0** | MarkingGuards.java:91-95 · MarkingEventTypeAfterControlIngestIT.java |
...
| TC-BATCH-021 | 적재 매핑: 관제 계약 갭 WARN 1회성 + prvcTypeCd=PRVC 고정(★2026-08-03 실측 정정 — 구 기대값 ANONY 폐기, **main PR #77 반영**) | EVNT_TYPE_CD **미해석**/SHT_DT 결손 다건 연속 적재 | `warnControlContractGaps(ingest, rcptnSn, evntTypeCd)`(매 건 호출) | 프로세스 생애 1회만 WARN, 이후 동일 갭은 DEBUG 로만 기록(로그 폭주 방지). **이벤트유형이 해석되면 WARN 을 남기지 않는다**(경고 인플레이션 방지 — `SHT_DT` 규약과 동일). `prvcTypeCd` 는 **`PRVC`**(fail-closed) 고정 — 2026-07-31 사용자 확정으로 `ANONY` 는 폐기됨(`ANONY` 로 두면 `LsDataRaw.needsDeidentify()`=false 가 되어 비식별 미준비 시 원본 프레임이 서빙되는 CWE-359 회귀가 생긴다) | unit | P2 | TrainingVideoIngestTx.java:199-200,394-412 |
| TC-BATCH-022 | durationSec 변환: **단위 변환 폐지**(이미 초 단위) | `VDO_LEN_SEC=30500`(NUMERIC(10), 초 단위) | `toDurationSec` | 반올림만 적용해 **30500 그대로 영속** — 구 ms→초(`÷1000`) 변환은 삭제됨(적용하면 600초 영상이 0.6→null 이 되어 길이가 사실상 사라진다) | unit | P1 | TrainingVideoIngestTx.java:626(⚠병합 후 라인드리프트 재확인 필요) |
| TC-BATCH-023 | durationSec 변환: null/1초미만/INT범위초과 → null | `VDO_LEN_SEC`=null / 0.4(반올림 시 0) / `Integer.MAX_VALUE` 초과 | `toDurationSec` | 셋 다 null 영속(1초미만은 "길이 0" 오값 방지, 범위초과는 WARN+null) — 적재 후 ffprobe back-fill(`VideoMetaService`)이 실제 길이로 채움 | unit | P1 | TrainingVideoIngestTx.java:626-640(⚠병합 후 라인드리프트 재확인 필요) |
| TC-BATCH-024 | SRC_TYPE allowlist: 허용값 통과 / 미매칭 fail-closed | `SRC_TYPE` ∈ {ORIGINAL,RELAY,USER_ULD,GENERATED,AUGMENTED} 또는 미지정 임의값 | `allowedSrcType` | 허용값은 그대로 복사, 미매칭은 WARN(정제된 값 40자 절단)+null(복사 안 함, fail-closed) — 화면표시·파생판별 분기축이라 미지값을 넣지 않는다 | unit | P1 | TrainingVideoIngestTx.java:603(⚠병합 후 라인드리프트 재확인 필요) |
| TC-BATCH-025 | 미도착: 대기상한 이내 backoff 복귀 / 초과 시 가역적 종결 | 파일 미도착 반복 관측(`PRCS_DT` 앵커 경과) | `handleNotArrived` | 상한(기본 24h, 최소 1h clamp) 이내면 `PENDING`+`NEXT_RTRY_DT` backoff(1분~1시간 clamp, 경과만큼 증가) 복귀·`RTY_CNT` 미증가; 상한 초과 시 `markFailed`(가역 — `requeueFailedForRetry`/`requeueFailedBatch` 로 되살리면 다음 스캔이 재집음, `ERR_MSG`·`PRCS_DT`·`NEXT_RTRY_DT` 리셋) | integration | P0 | TrainingVideoIngestTx.java:308-337 · LsDataIngestRepository.java:211-222,326-337(⚠병합 후 라인드리프트 재확인 필요) |
...
| ID | 케이스명 | 전제 | 입력/조건 | 기대결과 | 계층 | 우선 | 근거(file:line) |
|---|---|---|---|---|---|:--:|---|
| TC-DEID-001 | 브릿지: AFTER_COMMIT에서만 발화 | 적재 tx 커밋 | VideoIngestedEvent | 커밋 후 `asyncDeidentifyRunner.runAsync`. 리스너 자체는 **적재 스레드에서 동기 실행**되고 실제 비식별만 `batchAsyncExecutor`(core2/max4/queue50, CallerRunsPolicy)로 넘어간다 — 큐 포화 시 역압으로 적재 스레드가 직접 실행할 수 있다(의도된 설계) | integration | P0 | IngestDeidentifyBridge.java:26-31 · AsyncConfig.java:35-50 |
...
| TC-DEID-022 | 출력 경로 순회 방어(CWE-22) | base 이탈 | resolveSafeTargetPath | **ErrorCode 는 2축으로 갈린다** — ①세그먼트 위반(`null`/blank/`/`/`\`/`..`) → `INVALID_INPUT` ②base 이탈·실경로(심링크) 이탈 → **`FORBIDDEN`**. 둘 다 기본 루트 폴백 없이 fail-secure. target 이 `{base}/videos/{rawSn:Long}/deidentified.mp4` 로만 구성돼 실제 도달 불가한 방어심도 분기 *(3차 정정 — 구 기대값 "base 이탈 → INVALID_INPUT" 은 코드와 불일치. 1차 B-ISSUE-25)* | security | P1 | DeidentifyStep.java:418-421 · VideoArtifactRootResolver.java:424-441 |
...
| TC-DEID-025 | DeidentifyStep 은 경계 가드의 명시 면제 대상 (신규) | 정적 스캔 | BatchStepTransactionBoundaryTest | `BOUNDARY_EXEMPT = Set.of(DeidentifyStep.class, MarkingLoadStep.class)` 에 등재 — `execute` 무애노테이션이 정상이며 사유(자기참조 프록시)가 코드에 명문화 | unit | P1 | BatchStepTransactionBoundaryTest.java:53,88-90 |

## B-4. BatchOrchestrator 상태 전이 (정상/실패/진입 가드)

| ID | 케이스명 | 전제 | 입력/조건 | 기대결과 | 계층 | 우선 | 근거(file:line) |
|---|---|---|---|---|---|:--:|---|
| TC-BATCH-030 | process: rawSn null → INVALID_INPUT | null | process | INVALID_INPUT | unit | P2 | BatchOrchestrator.java:124-126 |
| TC-BATCH-031 | process: 영상 미존재 → NOT_FOUND | loadRaw empty | process | NOT_FOUND | unit | P1 | BatchOrchestrator.java:196-199 |
| TC-BATCH-032 | process: 정상 전체 → COMPLETED | 마킹+비식별 완료 | process | 진입가드 통과 → step 루프 → markRawDataCompleted + markCompleted + retryQueue.clear → COMPLETED | integration | P0 | BatchOrchestrator.java:133-165 |
| TC-BATCH-033 | process: 단계 실패 → FAILED+재시도 큐 | step 예외 | process | **markRawDataFailed(클레임 해제) → markFailed(부기) → enqueueIfRetryable** 순. 해제를 먼저 하는 이유는 부기 기록이 실패해도 배치단계가 PROCESSING 으로 고착되지 않게 하기 위함(B-ISSUE-01, 6차 정정 — 구 순서 markFailed 우선) | integration | P0 | BatchOrchestrator.java:166-175 |
| TC-BATCH-034 | process: 재시도 소진 후 FAILED 고정 | enqueueIfRetryable false | process | willRetry=false, FAILED 반환 | integration | P1 | BatchOrchestrator.java:170-175 |
| TC-BATCH-035 | process: disabled stage skip(dev 토글) | FRAME_EXTRACT=false | process | `step.isEnabled(ctx)` false → markStage·execute 둘 다 skip. ⚠ **실동작 검증 불가** — 토글을 넘기는 오버로드 `process(Long, Map)` 의 **프로덕션/dev 호출자가 0건**이라 REST·Quartz 어느 진입점으로도 태울 수 없다(단위테스트 전용 분기, 1차 B-ISSUE-23 미해소) | integration | P1 | BatchOrchestrator.java:150-154 |
| TC-BATCH-036 | process: 토글 없음 → 전 stage enabled | stageToggles null | process(rawSn) | 모든 단계 실행 | unit | P1 | BatchContext.java:77-84 |
| TC-BATCH-037 | 두 테이블 분리: 배치완료 시 작업상태 ASSIGNED 복귀 | 배치 완료 | markRawDataCompleted | LS_DATA_RAW=COMPLETED, LS_RAW_DATA_STATUS=ASSIGNED | integration | P0 | BatchTransitionService.java:194-205 |
| TC-BATCH-038 | 작업상태 COMPLETED 점프 시 검수제출 차단 회귀 방지 | 배치완료 후 제출 | 상태검증 | ASSIGNED→PENDING 정상(작업 COMPLETED 는 ReviewService.approve 전용) | integration | P1 | BatchTransitionService.java:194-205 |
| TC-BATCH-039 | 배치실패: MARKING_READY 고착 방지 | 배치 실패 | markRawDataFailed | LS_DATA_RAW=FAILED, 작업상태 FAILED | integration | P0 | BatchTransitionService.java:232-244 |
| TC-BATCH-040 | 배치시작: 두 컬럼 PROCESSING 동시 전이 | 마킹완료 시작, 작업상태가 검수 소유 아님, 배치단계 ≠ PROCESSING | `markRawDataProcessingBlocked` | **false 반환**(=차단 아님) + 작업상태·LS_DATA_RAW 둘 다 PROCESSING. LS_DATA_RAW 전이는 엔티티 mutator 가 아니라 **조건부 UPDATE `claimForProcessing`**(B-ISSUE-01 해소) | integration | P1 | BatchTransitionService.java:125-183 · VideoRepository.java:91-98 |
| TC-BATCH-041 | 상태 row 부재 시 WARN(진행 계속) | LS_DATA_RAW row 없음 / 작업상태 row 없음 | markRawDataProcessingBlocked · markRawDataMarkingReady | 예외 없이 WARN 후 진행(파생 RAW 는 작업상태 row 자체가 없다). 클레임 0행의 원인을 `findDataSttsCdByRawSn` 로 구분해 **영상 row 부재는 차단이 아니다**(false) | unit | P2 | BatchTransitionService.java:173-181,248-254,523-528 |
| TC-BATCH-042 | ★진입 가드: 검수 소유 상태면 SKIPPED + step 0건 (신규) | 작업상태 ∈ {PENDING,IN_REVIEW,APPROVED,REJECTED} | process(rawSn) | `markRawDataProcessingBlocked`=true → `BatchStage.SKIPPED` 반환, **step 을 한 건도 실행하지 않음**, LS_DATA_RAW 도 미변경 | integration | P0 | BatchOrchestrator.java:138-145 · BatchTransitionService.java:125-183 |
...
| TC-BATCH-044 | 완료/실패 전이도 검수 소유면 LS_DATA_RAW 미변경 (신규) | 작업상태 APPROVED | markRawDataCompleted / markRawDataFailed | `transitionRawDataStatus` true → 즉시 return. (work=APPROVED, stage=COMPLETED/FAILED) 불일치쌍을 만들지 않음 | integration | P0 | BatchTransitionService.java:199-201,238-240 |
| TC-BATCH-045 | 진입 가드는 전 진입점 공통 관문 (정정 6차) | 마킹 브리지·dev 트리거·Quartz 큐·재시도 잡·수동 재처리 | 각 경로로 process 호출 | 5개 진입점이 모두 단일 관문을 통과하며, 그 관문이 **rawSn 단위 원자 클레임**이다 — 검수 소유 상태 판정 + `LS_DATA_RAW` 배치단계 `claimForProcessing` 조건부 UPDATE. 동시 요청 N 건 중 **정확히 1건**만 진행하고 나머지는 SKIPPED (**구 서술 "클레임이 아니라 동시 실행을 막지 못한다" 는 폐기** — B-ISSUE-01/1차 B-ISSUE-22 해소) | integration | P0 | BatchOrchestrator.java:138-145 · BatchTransitionService.java:161-183 |
| TC-BATCH-046 | ★진입 원자 클레임: 동시 5요청 → 1건만 처리 (신규) | 배치단계 MARKING_READY, 동일 rawSn 동시 5요청(2노드 픽업·재시도·중복 트리거) | process(rawSn) × 5 (CountDownLatch) | `claimForProcessing` 이 DB 에서 직렬화되어 **1건만 COMPLETED, 4건 SKIPPED**. 외부 위탁(VLM) 단계 실행 **정확히 1회**(구 구현 5회 = VLM 5중 위탁) | integration | P0 | VideoRepository.java:91-98 · BatchOrchestratorConcurrentEntryIT.java:155-207 |
| TC-BATCH-047 | 이미 PROCESSING 인 영상 재진입 → SKIPPED (신규) | 배치단계 = PROCESSING(타 주체 소유) | process(rawSn) | 클레임 0행 → `true`(차단) → **step 0건 실행 + 남의 PROCESSING 미변경**. 사유는 WARN 으로 구분 기록 | integration | P0 | BatchTransitionService.java:171-182 |
| TC-BATCH-048 | 클레임 해제 3경로 — 완료/실패/Error (신규) | 클레임 성공 후 종료 | markRawDataCompleted / markRawDataFailed / Error 전파 | ①완료 → COMPLETED ②단계 예외 → FAILED ③`Error` → 해제 시도 후 원인 예외 rethrow. 어느 경로든 PROCESSING 고착 없음 → 후속 재진입·수동 재처리 가능. ⚠ **7차 정정**: 6차는 ③(Error)과 "부기 실패에도 해제가 남음"을 **검증됨으로 기재했으나 실행 근거가 없었다**(테스트가 정상완료·RuntimeException 두 경로만 실행). 각 경로는 이제 TC-BATCH-208~211 로 분리해 실증한다 | integration | P0 | BatchOrchestrator.java:166-192 · BatchOrchestratorConcurrentEntryIT.java:269-291 |
| TC-BATCH-049 | 수동 재처리는 클레임 인계 전용 진입을 쓴다 (신규) | `tryClaimReprocessFromFailed` 성공(FAILED→PROCESSING 선점) | BatchReprocessService.retry | `processWithHeldStageClaim` 호출 → 진입 가드가 **재클레임을 생략**(검수 소유 가드는 그대로 적용). 일반 `process` 를 쓰면 자기가 찍은 PROCESSING 에 막혀 재처리가 **영구 409** 가 된다. 단 생략 조건은 "보유 주장"이 아니라 **배치단계가 실제 PROCESSING 임**이다(TC-BATCH-212) | unit | P0 | BatchReprocessService.java:86-90 · BatchOrchestrator.java:98-100 · BatchTransitionService.java:157-171 |
| TC-BATCH-208 | ★Error 전파 경로에서도 클레임이 해제된다 (신규 7차) | 클레임 성공 후 스텝이 `Error`(치명적 오류 계열) 발생 | process(rawSn) | `Error` 를 **삼키지 않고 그대로 전파**(FAILED 위장 종료 금지) + `markRawDataFailed` 로 클레임 해제 → 실 DB 배치단계 `FAILED`, 후속 `tryClaimReprocessFromFailed` 성립. 부기(`statusService.markFailed`)·재시도 큐는 타지 않음 | integration | P0 | BatchOrchestrator.java:176-181,185-192 · BatchOrchestratorConcurrentEntryIT.java:269-291 · BatchOrchestratorClaimReleaseTest.java:130-148 |
| TC-BATCH-209 | Error 경로에서 해제 실패가 원인 예외를 덮지 않는다 (신규 7차) | 스텝 `Error` + `markRawDataFailed` 자체가 예외 | process(rawSn) | 해제 실패는 ERROR 로깅만(`releaseStageClaimQuietly` 가 `Throwable` 흡수), 호출자에게는 **원인 `Error` 가 그대로** 도달 | unit | P0 | BatchOrchestrator.java:184-192 · BatchOrchestratorClaimReleaseTest.java:150-166 |
| TC-BATCH-210 | ★실패 시 해제가 부기 기록보다 먼저 실행된다 (신규 7차) | 스텝 RuntimeException | process(rawSn) | `InOrder` 로 `transitionService.markRawDataFailed` → `statusService.markFailed` 순서 고정. 순서를 뒤집으면 부기(`LS_BATCH_PROC_LOG`) 장애가 **클레임 고착**으로 번진다 | unit | P0 | BatchOrchestrator.java:167-171 · BatchOrchestratorClaimReleaseTest.java:168-185 |
| TC-BATCH-211 | 부기 기록 실패가 이미 수행된 해제를 되돌리지 않는다 (신규 7차) | 스텝 RuntimeException + `statusService.markFailed` 가 예외 | process(rawSn) | 해제는 `REQUIRES_NEW` 독립 커밋이라 부기와 운명을 공유하지 않음 — `markRawDataFailed` 1회 수행 유지, 완료 전이·클레임 재취득 없음. ⚠ **현재 구현상 부기 예외는 호출자로 전파**되어 `BatchStage.FAILED` 반환과 `retryQueue.enqueueIfRetryable` 이 생략된다(클레임은 이미 해제돼 수동 재처리는 가능 — 자동 재시도 등록만 유실) | unit | P0 | BatchOrchestrator.java:167-175 · BatchOrchestratorClaimReleaseTest.java:186-207 |
| TC-BATCH-212 | ★인계 진입 오사용 — 클레임 미보유면 원자 클레임으로 폴백 (신규 7차) | `processWithHeldStageClaim` 을 클레임 없이 호출(배치단계 ≠ PROCESSING) | processWithHeldStageClaim(rawSn) | "보유했다"는 검증 불가능한 주장이라 그대로 믿지 않는다 — 배치단계를 읽어 PROCESSING 이 아니면 WARN 후 **일반 경로와 동일한 원자 클레임 수행**(차단 아님 — 차단하면 작업상태 컬럼만 클레임한 복구 형상이 영구 409). 폴백 클레임이 0행이면 SKIPPED. 실 DB: 오사용 호출 실행 중 동시 일반 진입이 **SKIPPED**(구 동작은 2벌 동시 실행) | integration | P0 | BatchTransitionService.java:168-170,193-201 · BatchOrchestratorConcurrentEntryIT.java:292-335 · BatchTransitionServiceTest.java:226-260 |
| TC-BATCH-213 | 인계 진입 오사용 방어의 한계 — 소유자 식별 불가 (신규 7차) | 남이 정상 처리 중(배치단계 PROCESSING)일 때 클레임 없이 인계 진입 호출 | processWithHeldStageClaim(rawSn) | 판정이 "누군가 PROCESSING 을 보유"까지만 가능하고 "그게 이 호출자인가"는 알 수 없어 **통과한다(의도된 잔여 한계)**. 완전 차단은 소유자 토큰 컬럼 도입이 선행돼야 한다 — 결함으로 재분류하지 말 것 | — | P2 | BatchTransitionService.java:141-154(Javadoc "오사용 방어"·"남는 한계") |
| TC-BATCH-214 | 클레임 UPDATE 의 NULL 3값 논리 (신규 7차) | `dataSttsCd` 가 NULL 인 가상 상황 | claimForProcessing | `<>` 비교가 UNKNOWN 이라 0행(클레임 실패)이 되나, 이 컬럼은 DDL `NOT NULL DEFAULT 'PENDING'`(V1/V2/V36) + 엔티티 `nullable=false` + 전 생성 팩토리가 `PENDING` 대입이라 **구조적으로 발생 불가**. `COALESCE` 방어를 두지 않는 이유는 "NULL 도 정상 출발 상태"라는 잘못된 계약을 만들지 않기 위함 | — | P2 | VideoRepository.java:86-92(Javadoc) · LsDataRaw.java:195,219 · V1__phase2_base_schema.sql:112 |

## B-5. 마킹 완료 브릿지 (동시성·가드)

| ID | 케이스명 | 전제 | 입력/조건 | 기대결과 | 계층 | 우선 | 근거(file:line) |
|---|---|---|---|---|---|:--:|---|
| TC-BATCH-050 | 브릿지: 영상 미존재 → skip | findById empty | MarkingCompletedEvent | WARN, 미트리거 + **그 마킹을 `SKIPPED` 로 종결**(`skip()`→`MarkingSkipTxService.terminateSkipped`, PENDING 한정) | unit | P1 | MarkingBatchBridge.java:113-119,187-190 |
| TC-BATCH-051 | 브릿지: PROCESSING/COMPLETED 재트리거 차단 | dataSttsCd ∈ SKIP_BATCH_STAGES | 이벤트 | skip(역전 방지) + 마킹 `SKIPPED` 종결. ⚠ 마킹 API 경로에서는 프리컨디션(stage≠MARKING_READY)이 먼저 412 를 내므로 이 가드는 **방어심도**다 | integration | P0 | MarkingBatchBridge.java:91-92,125-130 |
| TC-BATCH-052 | 브릿지: 비식별 미완료 트리거 차단 | deIdntfYn='N'/'F' | 이벤트 | WARN + `MarkingBatchTriggerReport.skipped(REASON_NOT_DEIDENTIFIED)`, 미트리거 + 마킹 `SKIPPED` 종결. ⚠ 마킹 API 경로에서는 프리컨디션이 먼저 412(방어심도) | integration | P0 | MarkingBatchBridge.java:134-139,187-190 |
...
| TC-BATCH-057 | 브릿지: 이미 claimed면 skip | 두 클레임 모두 false | 이벤트 | `batch already claimed/in-progress or review-owned … skipping` + **방금 커밋된 마킹을 `SKIPPED` 로 종결**해 재마킹 409 영구 잠금을 방지(1차 B-ISSUE-41 해소). 201 응답에 `batchTriggered=false`+사유 | integration | P0 | MarkingBatchBridge.java:162-170,187-190 · MarkingSkipTxService.java:terminateSkipped |
| TC-BATCH-058 | tryCreateBatchQueuedRow: row 존재 → false 멱등 | existsById=true | 호출 | false | unit | P1 | BatchTransitionService.java:299-314 |
| TC-BATCH-059 | tryCreateBatchQueuedRow: 할당형 PK saveAndFlush 즉시 flush | 신규 row | 호출 | INSERT flush, UK 위반 시 예외 전파(호출부가 별도 tx 밖에서 catch) | integration | P0 | BatchTransitionService.java:299-314 |
...
| TC-VLM-032 | 보류는 실패가 아니다 (신규) | 신고 구간 재처리 | BatchReprocessService.retry / Quartz 재큐 | 예외 미발생 → 배치 FAILED 미전이, 재시도 큐 미등록, 시도 상한 미소진. ⚠ **1차 B-ISSUE-61 미해소(3차 실동작 재현)** — VLM 은 규약대로 보류하나 **다음 단계 `FfmpegFrameExtractor` 가 같은 `'F'` 를 예외로 처리**해 배치가 FAILED + 재시도 큐에 등록된다 | integration | P0 | VlmTimeseriesStep.java:303-307 · (파손 지점) FfmpegFrameExtractor.java:185-188 |
...
| TC-VLM-034 | 해소 시 재위탁 배선 (신규) | 신고 resolve → `'F'→'Y'` | DeidentGateReopenedEvent(AFTER_COMMIT) | VlmResumeBridge → VlmWithheldResumeRunner.resumeAsync → run/runWithMarking 재호출 | integration | P0 | VlmResumeBridge.java:34-40 · VlmWithheldResumeRunner.java:62-77 |
...
| TC-BATCH-114 | 구 위치도 계속 허용 — 2-way allowlist (신규) | 비식별본이 `{deid_base}/videos/{rawSn}/` 에 있음 | extractByMarks | 동일하게 채택(전환 전후 행이 섞여도 양쪽 다 읽힘) | integration | P0 | VideoArtifactRootResolver.java:308-320,337-346 |
| TC-BATCH-115 | 심링크 방어: 대상 파일이 원본을 가리키면 거부 (신규) | deid 파일이 원본 영상 심링크 | extractByMarks | `verifyRealPathUnder` 실패 → **예외가 아니라 false** → 기존 RAW only 분기로 흡수. 원본 프레임이 "비식별본"으로 적재되지 않음(CWE-59/CWE-359) | security | P0 | FfmpegFrameExtractor.java:356-367 · VideoArtifactRootResolver.java:469-471,487-497 |
...
| TC-BATCH-118 | 후보 도출 예외 시 구 동작 폴백 (근거 정정) | readableDeidVideoBases RuntimeException | isUnderAllowedDeidBase | 호출 전체가 던지면 `deidentified-path` 단독 판정으로 폴백(fail-secure, 파이프라인 미중단). **내부적으로는 후보별 try/catch 라 한 후보 도출 실패가 나머지를 죽이지 않는다**(B-ISSUE-41 이후) | unit | P1 | FfmpegFrameExtractor.java:317-334 · VideoArtifactRootResolver.java:361-375 |
...
| TC-BATCH-135 | Interpolate: 혼재 타입 트랙 skip | LBL_TYPE distinct>1 | interpolateTrack | WARN skip | unit | P2 | TrackInterpolationStep.java:354-358 |
...
| TC-STREAM-B17 | 스트림 메타 캐시 무효화: 신고('F') 후 즉시 | deident report | evictAfterCommit | 옛 노출본 서빙 안 됨. 대상은 **그 영상 하나**(파생 캐시 미변경 — 확정 정책) | security | P0 | label/service/DeidentReportService.java:275,423,482 |
...
| TC-STREAM-B20 | 비식별 base allowlist **4원소**(정정 — 구 "2-way + 광역 base") | 비식별본이 구 위치 또는 co-locate 위치 · **네 진입점이 모두 실디렉터리** | GET /stream | 허용 base 는 ①`{deid}/videos/{rawSn}` ②co-locate `dirname(원본)/{rawSn}/deid` ③`{deid}/videos/augment` ④`{deid}/videos/resolution` — 프레임 추출기와 **동일 축**(가드 이원화 제거). **구 기재의 "`{deid}` 전체(광역 base)" 원소는 B-ISSUE-41 로 제거됨**. ★**진입점이 심링크면 그 원소는 목록에서 탈락**한다(TC-STREAM-B25) — 4원소는 "정상 형상에서의" 목록이다 | integration | P0 | VideoStreamService.java:590-600 · VideoArtifactRootResolver.java:361-376 |
| TC-STREAM-B23 | ★두 저장소 base 동일 형상에서 **비식별 자리의 원본 심링크 차단** — **파일 레벨**(정정: 구 기재 "B-ISSUE-41 해소" → **부분 해소**) | `STORAGE_RAW_PATH == STORAGE_DEIDENTIFIED_PATH`(=`/nas-storage`, 의도된 온프렘 형상) · `{base}/videos/{rawSn}/deidentified.mp4`(**최종 파일**)가 `{base}/frames/raw/{rawSn}/frame-0.jpg`(원본) 심링크 | GET /stream | **404**. 실경로가 비식별 영상 규약 서브트리 밖이라 허용 base 어디에도 속하지 않는다. *구 동작: 광역 base(`{deid}` 전체) 하위라 realpath 판정을 통과해 **마스킹 전 원본이 200 서빙**됐다(재현 확인)*. ⚠ **이 케이스(파일 레벨)만으로 B-ISSUE-41 이 해소되지 않는다** — 심링크가 **중간 디렉터리**면 좁힌 base 로도 통과했다(TC-STREAM-B25~B28, 9회차 해소) | security | P0 | VideoArtifactRootResolver.java:361-376 · VideoStreamService.java:633-657 |
| TC-STREAM-B24 | 좁히기 회귀 가드 — 산출물 4종 무회귀 (신규) | 동일 base 형상에서 ①구 위치 비식별본 ②co-locate 비식별본 ③증강 파생본(`videos/augment/{부모}/{파생}/`) ④해상도 파생본(`videos/resolution/{부모}/{파생}/`) + 파생 **구 규약**(`…/{부모}/{프리셋}.mp4`) | GET /stream | 전부 **200**. 파생본은 자기 `rawSn` 으로 스트리밍되는데 저장 경로에 부모 `rawSn` 이 끼어 `videos/{rawSn}` 규약에 걸리지 않으므로, 종류 세그먼트(`augment`/`resolution`)까지를 허용 base 로 둔다 | integration | P0 | VideoArtifactRootResolver.java:93,361-376 · StorageSubtreePolicy.java:83-109 |
| TC-STREAM-B25 | ★**중간 디렉터리 자체**가 **같은 base 안**을 가리키는 심링크 — `videos/{rawSn}`(신규, B-ISSUE-41 잔여 CRITICAL) | `STORAGE_RAW_PATH == STORAGE_DEIDENTIFIED_PATH` · `{base}/videos/{rawSn}` **디렉터리 자체**가 `{base}/frames/raw/{other}` 심링크 · DB 경로는 그 아래 파일 | GET /stream | **404**. 실경로의 base 기준 상대경로가 `[frames, raw, {other}]` 라 기대 `[videos, {rawSn}]` 과 불일치 → 허용 base 목록에서 탈락. *구 동작: **탈출이 아니라서** `startsWith` 를 통과 → **마스킹 전 원본이 200 서빙**(RED 실측 — `Expecting code to raise a throwable`)* | security | P0 | VideoArtifactRootResolver.java:465-489 · StorageSubtreePolicy.java:242-257 |
| TC-STREAM-B26 | ★파생 종류 세그먼트 자체가 심링크 — `videos/augment` (신규) | 위와 동일 형상에서 `{base}/videos/augment` 디렉터리가 `{base}/frames/raw/{other}` 심링크 | GET /stream | **404**. 8회차에 신설한 파생 서브트리 2원소도 **동일 취약점 클래스**를 공유했다(coverage-reviewer MEDIUM). *구 동작: 200* | security | P0 | VideoArtifactRootResolver.java:93,368-374,465-489 |
| TC-STREAM-B27 | ★파생 종류 세그먼트 자체가 심링크 — `videos/resolution` (신규) | 위와 동일, `resolution` 세그먼트 | GET /stream | **404**(TC-STREAM-B26 과 대칭). *구 동작: 200* | security | P0 | VideoArtifactRootResolver.java:93,368-374,465-489 |
| TC-STREAM-B28 | ★co-locate 산출 루트 `{rawSn}` 자체가 심링크 (신규) | 동일 base 형상에서 `dirname(원본)/{rawSn}` 디렉터리가 `{base}/frames/raw/{other}` 심링크 · DB 경로는 `…/{rawSn}/deid/*.mp4` | GET /stream | **404**. co-locate 축의 base 도 광역(`dirname(원본)`)이라 같은 자기참조가 성립했다 → 세그먼트 `[{rawSn}]` 불일치로 후보 탈락(fail-secure, 조용히 빠짐) | security | P0 | VideoArtifactRootResolver.java:256-265,316-328,465-489 |
| TC-STREAM-B29 | co-locate `{rawSn}/deid` 진입점 자체가 심링크 (신규 — 회귀 가드) | 동일 base 형상에서 `{rawSn}/deid` 가 `{base}/frames/raw/{other}` 심링크 | GET /stream | **404**. 이 지점은 base 가 이미 좁아(`{rawSn}`) 수정 **전에도** 차단됐다(RED 아님) — 축 전환 후에도 유지됨을 못 박는 가드 | security | P1 | VideoArtifactRootResolver.java:294,465-489 |
...
| ID | 케이스명 | 전제 | 입력/조건 | 기대결과 | 계층 | 우선 | 근거(file:line) |
|---|---|---|---|---|---|:--:|---|
| TC-DEID-030 | 신고: reason blank → INVALID_INPUT | null/blank | report | 400(@Valid 선차단) | unit | P2 | DeidentReportController.java:115,142 · DeidentReportService.java:162-166(⚠라인드리프트 재확인 필요) |
| TC-DEID-031 | 신고: WORKER 본인 배정만(IDOR) | WORKER 타인 srcSn | report | 403 | security | P0 | DeidentReportService.java:122-130(⚠라인드리프트 재확인 필요) |
| TC-DEID-032 | 신고: 부모 RAW PESSIMISTIC_WRITE 락(PII TOCTOU) | 동시 증강 콜백 | doReport | `findByRawSnForUpdate` 로 직렬화 | security | P0 | DeidentReportService.java:179,184 |
| TC-DEID-033 | 신고: 이미 잠금 → CONFLICT | isRawLocked | doReport | 409 | integration | P1 | DeidentReportService.java:193-195 |
| TC-DEID-034 | ~~신고: 전체 라벨 스냅샷+삭제+'F'~~ | — | — | **[폐기 2026-07-30]** ★정책 반전(2026-07-27 사용자 확정) — 신고 시 **라벨을 삭제하지 않고 보존**하며 `LS_LABEL_VERSION` 스냅샷도 남기지 않는다. 신고 구간 PII 노출은 조회 게이트(412)로 차단한다. 대체 케이스 TC-DEID-051 | integration | — | DeidentReportService.java:201-209(⚠라인드리프트 재확인 필요) |
| TC-DEID-035 | 신고: 개인정보 3필드 리셋(stale PII 방지) | 값 존재 | resetPrivacyMetaByRawSn | 익명/가명/PII 3필드 NULL 초기화(벌크 JPQL) | security | P1 | DeidentReportService.java:222-223(⚠라인드리프트 재확인 필요) |
| TC-DEID-036 | 신고: 락 UNIQUE 위반 → CONFLICT(동시) | DataIntegrityViolation | doReport | 409 | security | P1 | DeidentReportService.java:264-268(⚠라인드리프트 재확인 필요) |
| TC-DEID-037 | 신고: APPROVED → TASK_MODIFIED(**META_UPDATED**) | isReviewApproved | doReport | `TaskModifiedEvent(ChangeType.META_UPDATED)`. 라벨이 보존되므로 구 `LABEL_DELETED` 는 폐기 | integration | P1 | DeidentReportService.java:257-260(⚠라인드리프트 재확인 필요) |
| TC-DEID-038 | ~~신고: 라벨 0건이면 스냅샷 스킵~~ | — | — | **[폐기 2026-07-30]** 스냅샷 로직 자체가 제거되어 스킵 분기가 존재하지 않는다(TC-DEID-034 와 동일 사유) | unit | — | DeidentReportService.java:201-209(⚠라인드리프트 재확인 필요) |
| TC-DEID-039 | resolve: actor null → UNAUTHORIZED | null | resolveManually | UNAUTHORIZED | unit | P2 | DeidentReportService.java:382-384(⚠라인드리프트 재확인 필요) |
| TC-DEID-040 | resolve: 신고 없음 → NOT_FOUND | empty | resolveManually | NOT_FOUND | unit | P2 | DeidentReportService.java:385-386(⚠라인드리프트 재확인 필요) |
| TC-DEID-041 | resolve: OPEN 아님 → CONFLICT | RESOLVED/DISMISSED | resolveManually | CONFLICT | unit | P1 | DeidentReportService.java:392-394(⚠라인드리프트 재확인 필요) |
| TC-DEID-042 | resolve: 산출물 미검증 → CONFLICT(fail-closed) | verify 실패 | resolveManually | CONFLICT, OPEN/락/'F' 전부 유지(롤백) | security | P0 | DeidentReportService.java:400,594-(⚠라인드리프트 재확인 필요) |
| TC-DEID-043 | resolve: 성공 → RESOLVED+락해제+'F'→'Y' | 검증 통과 | resolveManually | 'Y' 복원(게이트 자동 해제, 보존된 기존 라벨 그대로 재사용) | integration | P0 | DeidentReportService.java:400-419(⚠라인드리프트 재확인 필요) |
| TC-DEID-044 | verifyArtifact: 경로 없음/파일 부재 → 거부 | null/파일 없음 | verifyDeidentArtifact | CONFLICT | security | P1 | DeidentReportService.java:594-(⚠라인드리프트 재확인 필요) |
| TC-DEID-045 | verifyArtifact: 시간조건(신고 후 재비식별) 미충족 — **엄격 비교로 정정(Phase 2, B-ISSUE-42 해소)** | mtime ≤ 신고시각 | verifyDeidentArtifact | 거부(옛 비식별본 배제). `mtime.isAfter(reportTime)` **엄격 비교**(스큐 관용 완전 제거) — 구 감산 방향 관용(`reportTime.minusSeconds(60)`)이 신고 이전 최대 60초 과거 파일도 통과시키던 결함(B-ISSUE-42, 구 B-ISSUE-102)을 해소했다. 경계값(mtime==신고시각)도 거부로 고정 | security | P0 | DeidentReportService.java:594- (구 CLOCK_SKEW_TOLERANCE_SECONDS 상수 제거됨, ⚠라인드리프트 재확인 필요) |
| TC-DEID-046 | resolve: 배치단계 역행 안 함(CWE-664) | COMPLETED 후 신고 | resolveManually | 'Y' 만 복원, DATA_STTS_CD 유지 | integration | P1 | DeidentReportService.java:412-419(⚠라인드리프트 재확인 필요) |
| TC-DEID-047 | resolveOpenReports(자동): OPEN 일괄 RESOLVED | 배치 비식별 성공 | resolveOpenReports | resolve + releaseRaw + 캐시 evict(멱등) | integration | P1 | DeidentReportService.java:471-490 · DeidentifyStep.java:354 · KpstDeidentTxService.java:358(⚠라인드리프트 재확인 필요) |
| TC-DEID-048 | 신고 목록: status allowlist 밖 → 400 | "X" / SQLi 문자열 | listReports | INVALID_INPUT(@Pattern + 서비스 정규화 이중 방어) | unit | P2 | DeidentReportService.java:445-466(⚠라인드리프트 재확인 필요) |
| TC-DEID-094 | ★신고 목록: 신고자 표시명 `reporterName` (신규 2026-08-04, `main` PR #79 병합) | 신고자 `USER_NO` 가 `MNG_ACCT_USER` 에 존재 | `GET /v1/deident-reports` | 행마다 `reporterName`=`USER_NM` 채워짐. `reporterNo` 원값도 그대로 유지(하위호환 — 필드 추가만). 이름 해석은 페이지의 `USER_NO` **단일 IN 쿼리** 1회(`findByUserNoIn`) — 행마다 조회하는 N+1 금지 | integration | P1 | DeidentReportService.java:478 · DeidentReportListResponse.java:41-52 |
| TC-DEID-095 | 신고 목록: 마스터에 없는 신고자 → 표시명 null (fail-soft, 신규 2026-08-04, `main` PR #79 병합) | 탈퇴·관제 계정 삭제로 `MNG_ACCT_USER` 행 부재 | `GET /v1/deident-reports` | **200 + `reporterName=null`** — 이름 조회 실패가 목록 조회 자체를 깨뜨리지 않는다. `reporterNo` 는 그대로 노출. `reporterNo` 가 null 인 레거시 행도 동일하게 null | security | P1 | DeidentReportService.java:478(⚠라인드리프트 재확인 필요) |
| TC-DEID-049 | 컨트롤러: 라벨링 단계 srcSn 신고 | POST /v1/labels/{srcSn}/deident-report | report | 201, WORKER(본인 배정)/REVIEWER | integration | P1 | DeidentReportController.java:111-119(⚠라인드리프트 재확인 필요) |
| TC-DEID-050 | 컨트롤러: resolve — WORKER 본인/REVIEWER 전체 | POST /v1/deident-reports/{rprtSn}/resolve | resolve | 200/403/409 | integration | P1 | DeidentReportController.java:161-168(⚠라인드리프트 재확인 필요) |
| TC-DEID-051 | ★라벨 보존 — 신고 후 라벨·이력 불변 (신규) | 라벨 N건 보유 영상 | 신고 201 | `LS_DATA_LBL` 행 수·내용 불변, `LS_LABEL_VERSION` 신규 행 0, 라벨셋 버전 bump 도 없음. 신고 구간 노출은 조회 게이트(412)가 담당 | integration | P0 | DeidentReportService.java:201-209(⚠라인드리프트 재확인 필요) |
| TC-DEID-052 | 개인정보 3필드 리셋의 행 단위 감사 (신규) | 3필드 값 보유 프레임 M건 | 신고 201 | 리셋 **직전**에 대상 srcSn 을 확정하고 `LS_DATA_LBL_HSTRY` 에 프레임당 1행(`recordPrivacyMetaResetEvent`). 라벨 델타 0건이라 V139 필터로 `V_COMPLETED_LABEL_CHANGE` 에는 미노출 | security | P0 | DeidentReportService.java:222-253 · V139__filter_zero_change_rows_from_change_view.sql(⚠라인드리프트 재확인 필요) |
| TC-DEID-053 | ★파생영상 신고는 412 로 거부 (신규) | `ORGNL_RAW_SN` non-null | POST 신고(srcSn/rawSn 양쪽) | **412 PRECONDITION_FAILED**. 신고 행 미생성, REVIEWER 알림 없음, **부모 rawSn 미노출**(원본으로 유도하지 않음). 사용자 사유는 `LogSanitizer` 정제 후 WARN 감사 로그로만 보존 | security | P0 | DeidentReportService.java:187,323-334(⚠라인드리프트 재확인 필요) |
| TC-DEID-054 | 비식별 미수행('N'/null) 신고 412 (신규) | `DE_IDNTF_YN='N'`/null | POST 신고 | 412. `'F'`(신고·실패)는 별도 경로(작업락 409)로 처리 | security | P0 | DeidentReportService.java:190,357-365(⚠라인드리프트 재확인 필요) |
| TC-DEID-055 | ★마킹 단계 rawSn 신고 신설 (신규) | 마킹 화면(프레임 컨텍스트 없음) | POST /v1/videos/{rawSn}/deident-report | 201, WORKER/REVIEWER. UNCERTAINTIES #2("미구현") 해소 | integration | P0 | DeidentReportController.java:138-146 · DeidentReportService.java:151(⚠라인드리프트 재확인 필요) |
| TC-DEID-056 | rawSn·srcSn 두 경로가 동일 본체(`doReport`) (신규) | 두 진입점 | report / reportByVideo | 파생 412·비식별 미수행 412·작업락·`'F'` 전이·개인정보 3필드 리셋·스트림 캐시 무효화 **부수효과가 동일** | integration | P0 | DeidentReportService.java:122,151,173(⚠라인드리프트 재확인 필요) |
| TC-DEID-057 | resolve 성공 시 `DeidentGateReopenedEvent` 항상 발행 (신규) | 미승인 영상 포함 | resolveManually | 승인 여부와 무관하게 발행 — 보류된 VLM 위탁 재개 신호가 미승인 영상에도 도달해야 하기 때문 | integration | P0 | DeidentReportService.java:515-523(⚠라인드리프트 재확인 필요) |
| TC-DEID-058 | resolve 성공 + APPROVED 일 때만 `DeidentReportResolvedEvent` (신규) | 검수 승인 영상 | resolveManually | export 재산출·관제 재통지용. 미승인이면 미발행(불필요한 v1 생성 방지) | integration | P0 | DeidentReportService.java:520-522(⚠라인드리프트 재확인 필요) |

> ⚠ **병합 라인드리프트 안내(2026-08-04, main PR #79 병합)**: `main`(신고자 표시명 `reporterName`)과 `qa-0803` Phase 2(시계스큐 정정, `CLOCK_SKEW_TOLERANCE_SECONDS` 제거)가 같은 `DeidentReportService.java` 를 각각 독립적으로 변경했다. TC-DEID-030~058 의 세부 라인 범위는 다음 회차 `file:line` 전수 재확인에서 정밀 정정이 필요하다. TC-DEID-032·033·045 는 이번 병합 작업 중 실측 재확인했다. TC-DEID-094/095(신고자 표시명)는 `main` 원본을 채택했다.
...
| TC-DEID-083 | ★산출물 무결성 판정 단일 원천 (신규) | 18바이트 텍스트 스텁 | isUsableDeidFile | `DeidentArtifactIntegrity.isValidVideoArtifact` = 정규파일 + **≥512B** + 컨테이너 시그니처(ISO-BMFF box / MPEG-TS sync) → false. 구 "존재+>0바이트" 판정 폐기 | security | P0 | KpstDeidentService.java:692-694 · DeidentArtifactIntegrity.java:49,87-110 |
...
| TC-BATCH-174 | 판정은 allowlist `containsAll` (신규) | 프로파일 `local,prd` / `prd1` / `LOCAL` / 미지정 | verify | 전부 엄격(거부) — denylist 가 아니므로 오타·대소문자·혼합·미지정이 자동으로 fail-closed | security | P0 | QuartzClusteringGuard.java:55,96-103 |
...
| ID | 케이스명 | 전제 | 입력/조건 | 기대결과 | 계층 | 우선 | 근거(file:line) |
|---|---|---|---|---|---|:--:|---|
| TC-BATCH-180 | YOLO execute 경계 — 빈 프록시 호출 시 트랜잭션 개시 (신규) | `@Transactional` 없는 IT | 빈(프록시).execute(ctx) | `bumpLabelVersionIn` 이 예외 없이 수행되어 `LBL_VER` +1. 무-트랜잭션이면 즉시 실패 | integration | P0 | YoloAutolabelStep.java:166-170(근거 드리프트 정정, 3차 검증 — 구 155-158) |
...
| TC-BATCH-183 | VLM execute 경계는 **쓰기 가능**(readOnly 아님) (신규) | 마킹 보유 | 빈(프록시).execute(ctx) | 상위 경계가 `runWithMarking` 의 상한을 따라 readOnly 가 아니어야 마킹 전이 저장이 성립 | integration | P0 | VlmTimeseriesStep.java:219-227(근거 드리프트 정정, 3차 검증 — 구 205-227) |

...
> 251 케이스(표 행 실측 — **폐기 행 포함**, 행을 지우지 않으므로) · 계층: unit / integration / security · [← README](README.md) ※ 카운트 = `grep -cE '^\| ~*TC-'`(ID 취소선 폐기 행 포함, 2026-08-05 머지 회차 7 정정 · 회차 5 에서 177→180, 포털 TUS 존재 오라클 차단 `TC-TUS-033~035` 신설 · **회차 6 에서 180→201, F-12 데이터마트 ZIP 다운로드·보존기간 만료 자동 삭제 신설** · **회차 7 에서 201→210, F-12d 화면 배선(다운로드 버튼·만료 예정일 표기) 신설** · **회차 8 에서 210→215, F-12e 대용량 전송 제한시간·전송 중단 안내·진행 표시 신설** · **회차 9 에서 215→228, F-12f 대용량 다운로드 취소(두 경로) 신설**(구 머리말이 이 회차 반영을 빠뜨리고 있었다 — 이번에 정정) · **회차 10 에서 228→237, F-12g~i 방치 업로드 자산의 실패 전이·정리/보존기간 스윕 자기 토글 분리·비동기 스트리밍 응답 서버측 절대 제한시간 신설** · **회차 11 에서 237→241, F-12g 의 하트비트 판정 서술이 신설 직후 거짓으로 확인돼 정정 + 유계 부등식·프레임 대기 상한·바인딩 실패 검증 4건 신설** · **회차 13 에서 241→243, F-13 포털 라벨링 프레임 이동 경로 신설** · **회차 14 에서 243→251, F-12a 데이터마트 다운로드 산출 구조를 검수 승인 export(NIA COCO 확장)와 통일 — TC-PORTAL-132~139 신설, TC-PORTAL-098 폐기**)
...
| 14 | 2026-08-26 | 0건 | 6건(TC-PORTAL-132~137) | 0건 | **★F-14 신설 — 포털 메타 복제본의 `EVNT_ANNO_CN` 이 항상 NULL 이던 결함(CO-20260826-포털메타복제 2건).** 복제 경로는 **네 단**(payload 직렬화·전송 계약 record·복원 빌더·포털 INSERT)이고 그 위에 **복제본 물리 DDL** 축이 하나 더 있는데, control 이 `V128` 로 컬럼을 추가했을 때 **DDL 만 뒤늦게 따라붙고 네 단이 전부 빠져** 컬럼은 있는데 값이 영구히 비었다. ⚠ `db/portal/V4` 주석이 추가 사유를 *"복제하려고"* 가 아니라 *"JPA 파생 조회가 42703 으로 깨지지 않게"* 라고 스스로 밝히고 있다 — **DDL 을 맞췄다는 것이 복제가 된다는 뜻이 아니다.** 세 축(payload·SQL·DDL)이 서로를 못 덮으므로 가드를 셋 다 세웠고, ①~④를 맞추고 DDL 만 미반영하면 앞 두 축이 **전건 green** 인 채 DDL 가드만 붉어짐을 변이로 실증했다. ⚠ **통합시험은 DDL 축을 원리적으로 못 잡는다** — 시험 인프라가 control·포털을 같은 물리 테이블로 묶어 포털 전용 DDL 이 아예 적용되지 않는다. **mutation 실증 5종**(직렬화 입구 되돌리기 · 엔티티만 추가 · ①~④ 맞추고 DDL 만 누락 · 덤프 재생성 누락 · 제외 목록으로 가드 침묵시키기) 전부 원복 확인. **backend FULL 857 XML / 7,813 tests / 0 실패** |
| 13 | 2026-08-26 | 0건 | 2건(TC-PORTAL-130~131) | 0건 | **★F-13 신설 — 포털 라벨링의 프레임 이동이 내부 경로로 나가 접근 거부로 튕기던 결함(CO-019).** 라벨링 화면은 내부(`/label/:id`)와 포털(`/portal/label/:id`) 두 라우트가 **같은 컴포넌트를 재사용**하는데, 프레임 이동 경로가 내부 경로로 하드코딩돼 있어 포털 사용자가 다음 프레임을 고르면 `ChannelGuard(INTERNAL)` 가 `/forbidden` 으로 되돌렸다 — 포털 라벨링이 사실상 **첫 프레임 한 장으로 제한**됐다. 인가가 잘못된 것이 아니라 화면이 잘못된 주소로 이동한 것이며 **가드는 정상 동작했다**(라우트·가드는 손대지 않았다 — 그쪽을 푸는 것은 포털 사용자에게 내부 화면을 여는 인가 결함이 된다). 이동 경로 조립을 화면 안 단일 지점(`frameRoute`)으로 모아 이동 지점이 늘어도 같은 하드코딩이 복제되지 않게 했다. **동작 사양이 바뀐 것이 아니라 회귀 가드가 없어 결함이 살아남은 구간**이라 이번에 케이스를 세운다. ⚠ `replace: true` 는 **불변** — X(닫기)의 뒤로가기가 이전 프레임이 아니라 진입 이전 경로(작업 목록)로 빠지게 하는 계약이다. ⚠ 포털 **업로드** 라벨링(SCREEN-034)은 같은 결함이 없다 — 그 화면은 URL 이 아니라 화면 안 인덱스 상태로 프레임을 넘긴다. **mutation 실증**: 경로 계산을 옛 동작(`/label/` 고정)으로 되돌리면 `포털_채널에서_프레임을_이동하면_포털_경로로_간다` 가 실패함을 확인 |
| 14 | 2026-08-26 | 3건(TC-PORTAL-103 근거 심볼 · TC-PORTAL-104 · TC-PORTAL-105 산출 구조 문구) | 8건(TC-PORTAL-132~139) | 1건(TC-PORTAL-098) | **★F-12a 산출 구조 통일 — 포털 데이터마트 ZIP 의 라벨 산출을 검수 승인 학습데이터 export 와 같은 구조로 맞췄다(CO-021).** 영상 단위 단일 문서 `{rawSn}/labels.json`(좌표를 `points[[x,y]]` 로 싣는 자체 shape)을 **폐지**하고, 프레임마다 `{rawSn}/frames/{FRM_NO 4자리 zero-pad}.json`(**NIA COCO 확장 최상위 9키** `info`·`dataset`·`licences`·`video`·`event`·`image`·`annotations`·`categories`·`type`)을 이미지와 **같은 자리에 이름만 다른 짝**으로 산출한다. 좌표는 `annotations` 항목의 `bbox`·`polygon`·`keypoints` 다. ⚠ **구 정책 → 폐기**: ①**「라벨은 `labels.json` 한 개」 폐기** — 두 형식이 병존하면 같은 라벨의 좌표 표현이 갈리고(「두 번째 진실원」), export JSON 이 다시 읽혀 적재되는 왕복 자산이라는 확정 근거를 포털 산출물만 스키마가 달라 충족하지 못했다(`TC-PORTAL-098` 폐기, 대체 `TC-PORTAL-132`). ②**「폐기 프레임도 담는다」 폐기** — 밖으로 나가는 경로는 폐기분을 뺀다(승인 산출물과 통일, `TC-PORTAL-135`). ③**「이미지가 없으면 그 이미지만 빠지고 어노테이션 문서는 남는다」 폐기** — 프레임 **전체**를 건너뛴다(승인 산출물과 통일, `TC-PORTAL-136`). ★**유지되는 것은 그대로 검증한다**: ZIP 최상위 `{rawSn}/` 접두 · 서버 생성 고정 파일명 · 게이트 3종 **순서**(403→412→410) · 프레임 단위 병합 규칙 · 원본 폴백 금지(AC-034) · 이미지는 비식별벌만. 그 밖에 `TC-PORTAL-103` 의 근거 심볼 드리프트(`resolveFrameImages` → `resolveFrameEntries`)를 정정했다. ⚠ **형제 경로 무변경** — 포털 **업로드 자산** 내보내기의 `portal-upload-{uldSn}-labels.json`(F-7 `TC-PORTALUP-055~058`)은 별개 축이라 손대지 않았다. 위 `docs/v2-wiki/16-portal.md` §16.4a 도 같은 커밋에서 갱신 |
| 15 | 2026-09-02 | 0건 | 0건 | 0건 | **★셸 소유 경계 반전 — 머리 영역과 좌측 주 메뉴를 Host 가 모두 소유한다(케이스 표 동결이라 서술만).** 저작도구는 그 아래 **본문만** 그리고, 목적지 셋(내 작업·내 업로드·증강)은 **본문 상단 이동 탭**으로 오간다. ⚠ **이 축은 두 번 뒤집혔다** — 최초 Host 소유 → 2026-09-01 저작도구 소유 → 2026-09-02 다시 Host 소유. 상대 설계와 같은 쪽으로 수렴했고 그때 남긴 「회신 대기」가 닫혔다. **세 번째로 뒤집지 말 것.** ⚠ **기존 케이스 여덟이 이제 다른 화면 모델을 검증한다** — `TC-PORTAL-110~116` 은 「포털 홈의 **데이터마트 영상 카드**」를 전제하는데, `SCREEN-028` 은 그 사이 **「내 작업」**(본인 저장 작업 표)으로 바뀌었고 **데이터마트에서 영상을 고르는 목록은 Host 소유**로 확정됐다. 즉 그 카드가 이 화면에 더 이상 없다. **케이스가 틀린 것이 아니라 검증 대상 화면이 옮겨간 것**이며, 만료 표기 규칙(날짜까지만 · 값 없으면 자리 비움 · 임박 강조 없음)과 다운로드 규칙(한 번에 하나 · 취소는 정상 종료)은 **새 화면에서도 그대로 유효**하다. ⚠ `TC-PORTAL-117` 은 `PortalLayout` 의 **머리 영역(GNB)** 을 전제한다 — 사양상 포털 채널은 머리 영역을 그리지 않지만, 코드의 숨김이 **포털 채널 빌드에서만** 걸려 그 케이스는 **지금도 통과한다**. 배선이 붙을 때 함께 볼 것. ⚠ **이번 회차에 코드 변경은 없다** — 설계 층만 반영했다(서버 ITEM 9건 · 위키 2파일 · 시안 SD-024 전면 재작성). 코드에 남은 일은 **본문 상단 이동 탭 신설** 하나뿐이며, 좌측 메뉴는 코드에 **생긴 적이 없어 지울 것이 없다**(2026-09-01 결정이 설계에만 있었다) |
| 16 | 2026-09-02 | 0건 | 0건 | 0건 | **★포털 업로드 파이프라인 순서 반전 + 라벨링 화면 통합(케이스 표 동결이라 서술만 · CO-20260902 2건).** ①**순서가 뒤집혔다** — 구 `업로드 → (즉시) 고정 간격 프레임 추출 → 준비 완료 → 라벨링` 이 **폐기**되고 `업로드 → 마킹(자동/수동) → 마킹 위치로 추출 → 준비 완료 → 라벨링` 이 됐다. **고정 간격 추출은 사라진 것이 아니라 자동 마킹의 기본 동작으로 흡수**됐고(간격을 그대로 두면 구 동작과 같은 결과) 단위가 **초에서 프레임 수(기본 300)** 로 바뀌었다. ★**그 귀결로 `UPLOADED` 의 뜻이 「추출 대기」에서 「마킹 대기」로 바뀐다** — 이 상태를 전제하는 기존 케이스(TC-PORTALUP 계열의 상태 전이·방치 판정·보존기간 축)는 **틀린 것이 아니라 검증 대상 구간의 의미가 옮겨간 것**이며, 특히 방치 판정(`stuck-timeout-minutes`)이 이제 **사람의 조작을 기다리는 구간**을 재게 되는데 그 맞물림은 **이번 확정에 없어 근거 없이 케이스를 만들지 않았다.** ②**회복 경로는 재업로드뿐이다** — 재마킹·재추출 기능을 만들지 않으므로 **이미 추출이 끝난 자산의 재마킹 저장은 거부**되고, 「마킹 완료」가 **되돌릴 수 없는 조작**이 되어 **완료 직전 확인 단계가 선택이 아니라 필수 조건**이 된다. ⚠ 자산 삭제는 **추출 중 거부**라 잘못 마킹한 사용자는 추출이 끝날 때까지 기다려야 하는 마찰을 인지·수용했다. ③**상한 초과는 거부가 아니라 절단**이며 **자동은 전 구간 고르게 다시, 수동은 앞에서부터** 남긴다 — 절단 사실과 실제 장수가 완료 확인 창에 함께 드러나야 한다. ④**라벨링 화면 둘이 하나로 합쳐졌다** — `SCREEN-034`(포털 업로드 라벨링) **폐기**, `SCREEN-029` 통합. **저장처는 화면이 가르지 않고 서버가 자산 출처로 판정**하며 **내려받기·내보내기는 라벨링 화면에서 빠져 내 업로드 목록(`SCREEN-033`)으로 옮겼다.** ⚠ **구 사양 폐기** — 「두 화면은 경로도 데이터도 분리돼 있다」. ★**그러나 AI 보조 미제공 범위는 넓어지지 않았다** — 통합은 화면을 합친 것이고 **AI 유무를 가르는 축은 화면이 아니라 자산 출처**다. 업로드 자산 대상 케이스를 「이제 한 화면이니 AI 도 된다」로 고쳐 잡지 말 것. ⑤**포털 이미지 자산은 접수 축에서 이미 폐기**됐고 이번 라운드로 화면·사양에서도 걷혔다 — **다만 자산 종류 값역과 그 직접 투영인 응답 값은 남는다**(이미 적재된 행 판독용). ⚠ **「프레임 이미지」는 영상에서 뽑은 프레임이라 이 축과 무관**하므로 낱말로 훑어 함께 폐기 표기하지 말 것. ⚠ **이번 회차에 코드 변경은 없다** — 설계 확정만 반영했다(창구 `API-238`~`API-241` 넷과 화면 `SCREEN-045` 는 구현 미착수). 마킹 저장은 **기존 마킹 원장 재사용**이고 포털 전용 마킹 표를 만들지 않는다 |
| 17 | 2026-09-08 | 0건 | 0건 | 0건 | **★포털 라벨링의 업로드 자산 갈래를 관제용 라벨링 도구로 흡수(케이스 표 동결이라 서술만 · CO-20260908).** 회차 16 이 「주소를 하나로 합쳤다」로 적은 통합은 **주소만** 하나였고 **본문은 두 벌**로 남아 있었다 — 업로드 갈래는 별도의 미니멀 화면이 그렸고 좌측 도구바·라벨 마스터 사이드바·우측 객체/속성 패널·프레임 필름스트립·캔버스 상단 옵션바·단축키·닫기 확인 모달이 **그쪽에만 없었다**. 이번에 그 화면을 폐기하고 두 출처가 **같은 본문**(관제용 라벨링 도구 + 포털 게이팅)을 쓰게 했다. **사양 변경이 아니라 구현 드리프트 수정**이다 — SCREEN-029 v54·UC-027 v32·AC-1070 v19 가 이미 「자산 출처에 따라 다른 화면으로 갈라지지 않는다」를 못박고 있었고 어긋난 것은 코드뿐이라 설계 ITEM 은 한 건도 고치지 않았다. ★**갈리는 것은 화면이 아니라 창구다** — 조회·이미지·저장이 자산 축(`/portal/uploads/...`)과 프레임 축(`/portal/frames/...`)으로 갈리고, 그 분기는 데이터 계층(출처 어댑터)에만 둔다. ⚠ **BE 창구 통합은 이번 범위 밖**이라 `?source=upload` 표기와 그 어댑터가 남는다(창구가 합쳐지면 둘 다 걷어낸다). ⚠ 폐기 화면의 시험이 지키던 단정(진행 오버레이·취소·저장 중 편집 차단·프레임 0건 상태 안내·미저장 편집 보호·자산 상태 게이트)은 **지우지 않고 통합 화면 주소에서 그대로 승계**했다 — 지우고 끝냈으면 그 회귀 가드가 함께 사라졌다. ⚠ **미저장 편집 보호의 방식이 바뀌었다** — 구 화면은 「dirty 면 덮지 않고 첫 로드에 한해 병합」이었고 지금은 **라벨이 도착하기 전에는 캔버스를 그리지 않아 편집 창 자체를 없앤다**(전체교체 PUT 이라 유실이 곧 작업 통째 유실이다). 그 대가로 프레임 전환에 로딩 화면이 잠깐 보인다(인지·수용). ⚠ **`labelId`·`trackId` 왕복은 업로드 축에서 성립하지 않는다** — 그 축의 요청·응답 계약(API-154/155)에 두 필드가 **아예 없다**(실측). 데이터마트 축은 종전대로 왕복하며 그 가드는 그대로다. 이번 변경이 만든 것이 아니라 **드러난** 계약 공백이라 BE 협의로 넘긴다. |
...
> ★**산출 구조 통일(2026-08-26, CO-021)** — 라벨 산출이 영상 단위 단일 문서(`labels.json`)에서
> **프레임마다 `{rawSn}/frames/{FRM_NO 4자리 zero-pad}.json`(NIA COCO 확장 9키)** 으로 바뀌었다.
> 이미지와 **같은 자리에 이름만 다른 짝**이라 검수 승인 학습데이터 산출물과 같은 구조이고, 그래서
> export JSON 을 다시 읽어 적재하는 경로에 그대로 들어간다. 좌표는 `annotations` 항목의
> `bbox`·`polygon`·`keypoints` 로 표현하며 자체 shape `points[[x,y]]` 는 **두지 않는다**
> (두 형식이 병존하면 같은 라벨의 좌표 표현이 갈린다 — 「두 번째 진실원」).
>
> ⚠ **바뀌지 않은 것은 이 절에서 그대로 검증한다** — ZIP 최상위 `{rawSn}/` 접두 · 서버 생성 고정
> 파일명(TC-PORTAL-106) · 게이트 3종 **순서** 403→412→410(TC-PORTAL-099~101) · 프레임 단위 병합
> 규칙(TC-PORTAL-105) · 원본 폴백 금지(TC-PORTAL-103) · 이미지는 비식별벌만.
>
> ⚠ **형제 경로와 혼동 금지** — 포털 **업로드 자산** 내보내기의 `portal-upload-{uldSn}-labels.json`
> (F-7 절 `TC-PORTALUP-055~058`)은 **별개 축이며 이번 통일 대상이 아니다.** 그 `labels.json` 은
> 그대로 살아 있다.

| ID | 케이스명 | 전제 | 입력/조건 | 기대결과 | 계층 | 우선 | 근거(파일) |
|----|---------|------|----------|---------|------|:--:|----------------|
| ~~TC-PORTAL-098~~ | ~~정상 다운로드 — labels.json + 비식별 프레임 이미지 + 비식별 영상~~ | **폐기 (2026-08-26, CO-021)** — 구 기대결과 `200, ZIP({rawSn}/labels.json + {rawSn}/frames/{FRM_NO}.jpg + {rawSn}/video.{ext})` 가 전제하던 **영상 단위 단일 라벨 문서(`labels.json`, 좌표를 `points[[x,y]]` 로 싣는 자체 shape)가 폐지**됐다. 라벨 산출이 프레임마다 NIA COCO 확장 문서로 바뀌어 이 케이스는 통과해도 폐기된 구조를 고정하게 된다. 대체: TC-PORTAL-132 | - | - | - | - | PortalDatamartDownloadService.java(resolveFrameEntries) |
...
| TC-PORTAL-103 (신규) | 원본 폴백 없음 — 비식별 경로 부재 시 원본 영상은 어떤 경우에도 담기지 않는다 | 비식별 경로 null | 동일 | 원본(비식별 이전) 영상은 ZIP에 포함되지 않는다(AC-034 불변 규칙). 서브트리 검증 실패 프레임도 같은 원칙(`resolveFrameEntries`)으로 사유 코드만 로그에 남기고 조용히 빠지나, 이 조합의 전용 단위 테스트는 아직 없다 | security | High | PortalDatamartDownloadService.java(resolveDeidVideo, resolveFrameEntries), PortalDatamartDownloadServiceTest.java(noDeidVideo_neverFallsBackToOriginal) |
| TC-PORTAL-104 (신규) | 비식별 영상이 없으면 영상 없이 라벨·이미지만 담긴다 | APPROVED, 비식별 영상 이력 없음 | 동일 | 200, ZIP에 video.* 엔트리가 생성되지 않는다(`frames/` 아래 어노테이션 문서·이미지 짝만 담긴다). ⚠ **구 문구 「labels.json + frames만」은 폐기**(2026-08-26, CO-021) — 검증 축(영상 엔트리 부재)은 불변이고 산출 구조 서술만 정정했다 | integration | Med | PortalDatamartDownloadService.java(resolveDeidVideo), PortalDatamartDownloadServiceTest.java(noDeidVideo_zipHasLabelsAndFramesOnly) |
| TC-PORTAL-105 (신규) | 다른 사용자의 저장 라벨은 담기지 않는다(본인만) | 타 사용자 저장분 존재 | 동일 | 각 프레임의 어노테이션 문서 `annotations` 에 본인 저장분만 반영, 타 사용자 저장분은 병합 대상 아님. 병합 판정은 **프레임 단위**라 한 영상 안에서 어떤 프레임은 본인 것, 어떤 프레임은 데이터마트 원본이 될 수 있다. ⚠ **구 문구 「labels.json 에 본인 저장분만 반영」은 폐기**(2026-08-26, CO-021) — 검증 축(타 사용자 저장분 미노출)은 불변 | security | High | PortalDatamartDownloadTxService.java(plan), PortalDatamartDownloadServiceTest.java(otherUsersLabelsNotIncluded) |
...
| TC-PORTAL-132 (신규 — TC-PORTAL-098 대체) | 정상 다운로드 — 프레임마다 어노테이션 문서 + 이미지 **짝** + 비식별 영상 | APPROVED, 본인 저장 라벨 존재, 비식별 프레임 이미지·영상 존재 | GET /v1/portal/datamart/videos/{rawSn}/download | 200, ZIP 에 프레임마다 `{rawSn}/frames/{FRM_NO 4자리 zero-pad}.json`(어노테이션 문서)과 **같은 이름의** `.jpg` 가 짝으로 들어가고 `{rawSn}/video.{ext}` 가 더해진다. `{rawSn}/labels.json` 엔트리는 **존재하지 않는다** | integration | High | PortalDatamartDownloadController.java(download), PortalDatamartDownloadTxService.java(plan), PortalDatamartDownloadService.java(resolveFrameEntries) |
| TC-PORTAL-133 (신규) | 어노테이션 문서는 **NIA COCO 확장 최상위 9키**다 | 위와 동일 | 문서 1건 파싱 | 최상위 키가 `info`·`dataset`·`licences`·`video`·`event`·`image`·`annotations`·`categories`·`type` 9개이고 **그 순서**로 직렬화된다. 검수 승인 산출물과 같은 조립기·같은 문서 타입을 쓰므로 두 산출물의 문서 형태가 갈리지 않는다 | integration | High | NiaAnnotationDoc.java, PortalNiaDocumentFactory.java(build), NiaExportContextAssembler.java(assemble) |
| TC-PORTAL-134 (신규) | 좌표는 `bbox`·`polygon`·`keypoints` 로 표현되고 자체 shape `points[[x,y]]` 는 나오지 않는다 | BBOX·POLYGON 저장분 | 문서의 `annotations` 검사 | BBOX 는 `bbox`, POLYGON 은 `polygon` 필드로 나가며 문서 어디에도 `points` 키가 없다. **도형 분기·좌표 파싱 판정을 복제하지 않고** 승인 경로와 **같은 매핑기 한 벌**을 쓴다 — 복제하면 두 산출물의 좌표 해석이 갈리고 한쪽만 고쳐질 때 조용히 어긋난다(이 통일이 없애려는 바로 그 결함) | integration | High | LabelToAnnotationMapper.java(toAnnotation), AnnotationSource.java, NiaAnnotation.java |
| TC-PORTAL-135 (신규) | 폐기 프레임은 문서도 이미지도 담기지 않는다 | 폐기된 프레임 1건 포함 | 동일 | 그 프레임의 `.json`·`.jpg` 가 **둘 다 없다** — 밖으로 나가는 경로는 폐기분을 뺀다(검수 승인 산출물과 같은 규칙). ⚠ 저작도구 **화면**의 프레임 수는 폐기분을 빼지 않는다(총량이 줄면 작업 진도 변화를 알 수 없다) — **다른 축이니 통일하지 말 것** | integration | High | PortalDatamartDownloadTxService.java(plan), LsDataSrcRepository.java(findNotDiscardedByRawSnOrderByFrameNoAsc) |
| TC-PORTAL-136 (신규) | 비식별 이미지가 없는 프레임은 **프레임 전체가 건너뛰어진다** | 비식별 경로 null 또는 서브트리 검증 실패 | 동일 | 그 프레임의 `.jpg` 뿐 아니라 `.json` 도 담기지 않는다. 원본으로 대체하지 않으며(AC-034) 나머지 프레임 산출은 계속된다(전건 거부 아님). 실패 사유는 코드만 로그로 남긴다(경로 원문 미노출 — CWE-209/117). ⚠ **구 기대결과 「그 이미지만 빠지고 어노테이션 문서는 남는다」는 폐기**(2026-08-26, CO-021 — 검수 승인 산출물과 통일) | integration | High | PortalDatamartDownloadService.java(resolveFrameEntries), DatasetExportWriter.java(원천 이미지 부재·이탈 프레임 건너뜀 — 통일 근거) |
| TC-PORTAL-137 (신규) | 파일명은 승인 산출물과 **같은 단일 지점**을 쓴다(포털 전용 규칙 신설 금지) | FRM_NO 7 · 1234 | 동일 | `0007.json`/`0007.jpg` · `1234.json`/`1234.jpg` 로 4자리 zero-pad 되고 문서와 이미지의 stem 이 같다. 파일명을 만들 수 없는 프레임(FRM_NO 부재·음수)은 이름을 지어내지 않고 그 프레임을 건너뛴다(fail-closed) | unit | Med | ExportFileNaming.java(jsonFileName, imageFileName), PortalDatamartDownloadTxService.java(plan) |
| TC-PORTAL-138 (신규) | 활성 영상 메타가 없으면 **라벨 없는 ZIP 을 조용히 내보내지 않는다** | 활성 영상 메타 스냅샷 부재 | 동일 | 문서를 만들 근거가 없으므로 산출을 중단한다(fail-closed). 빈 문서·기본값으로 때우면 사용자는 성공한 줄 알고 내용 없는 묶음을 받는다. 게이트 ③(데이터마트 노출)을 통과했으므로 정상 흐름에서는 발생하지 않는다 | integration | High | PortalDatamartDownloadTxService.java(plan), NiaExportContextAssembler.java(assemble) |
| TC-PORTAL-139 (신규) | 마스터·트랙 연결이 없는 **기존** 저장분도 오류가 아니라 정상 산출된다(하위호환) | 라벨 마스터·트랙 식별자가 비어 있는 기존 포털 저장 라벨 | 동일 | 그 어노테이션의 `category_id`·`track_id` 만 null 로 나가고 나머지 필드는 정상이며 200 이다. **값을 지어내 채우지 않고 기존 행을 백필하지도 않는다** — 라벨명 소급 매칭은 동명이인·비활성 마스터 오매칭 위험이 있어 두지 않기로 확정돼 있고, 재저장하면 자연 복구된다 | integration | Med | AnnotationSource.java, PortalNiaDocumentFactory.java(build) |
...
## F-13. 포털 라벨링 프레임 이동 경로 (★2026-08-26 신설 — CO-019)

> 라벨링 화면은 내부·포털 두 라우트가 재사용하는 단일 컴포넌트다. 이동 경로가 채널을 따라가지
> 않으면 포털 사용자는 프레임을 넘기는 순간 채널 가드에 걸려 접근 거부 화면으로 튕긴다.
> 라우터·가드는 **정상 동작**하므로 이 절의 검증 대상은 화면이 만드는 **이동 경로**뿐이다.

| ID | 케이스명 | 전제 | 입력/조건 | 기대결과 | 계층 | 우선 | 근거(파일) |
|----|---------|------|----------|---------|------|:--:|----------------|
| TC-PORTAL-130 (신규) | 포털 채널에서 프레임을 이동하면 포털 경로로 간다 | channel=PORTAL, role=PORTAL_USER · 프레임 2장 영상 · 미저장 변경 없음 | `/portal/label/777` 에서 썸네일 「프레임 2」 선택 | `/portal/label/778` 로 `replace: true` 이동 — 내부 경로(`/label/778`)로는 이동하지 않는다(그 라우트는 INTERNAL 채널 가드가 막아 접근 거부 화면이 뜬다) | unit | High | LabelingPage.tsx(frameRoute · performJump), LabelingPageFrameJumpRoute.test.tsx(포털_채널에서_프레임을_이동하면_포털_경로로_간다) |
| TC-PORTAL-131 (신규) | 내부 채널에서 프레임을 이동하면 기존대로 내부 경로로 간다 | channel=INTERNAL, role=WORKER · 프레임 2장 영상 · 미저장 변경 없음 | `/label/777` 에서 썸네일 「프레임 2」 선택 | `/label/778` 로 `replace: true` 이동 — 포털 경로로 새지 않는다(회귀 가드) | unit | High | LabelingPage.tsx(frameRoute · performJump), LabelingPageFrameJumpRoute.test.tsx(내부_채널에서_프레임을_이동하면_기존대로_내부_경로로_간다) |

## F-14. 메타 복제 정합 — 네 단 + DDL 축 (★2026-08-26 신설 — CO-20260826-포털메타복제 2건)

> 포털 복제본을 채우는 경로는 **네 단**(payload 직렬화 → 전송 계약 record → 복원 빌더 → 포털 INSERT)이고,
> 그 위에 **복제본 물리 DDL** 축이 하나 더 있다. 한 단만 빠져도 그 컬럼은 복제본에서 **영구히 빈다**.
> 실사고: control 이 `EVNT_ANNO_CN` 을 추가했을 때 DDL 만 뒤늦게 따라붙고 네 단이 전부 빠져,
> **컬럼은 있는데 값이 항상 NULL** 이었다. 세 축은 서로를 덮지 못하므로 가드가 셋 다 필요하다.
> ⚠ 통합시험은 DDL 축을 **원리적으로 못 잡는다** — 시험 인프라가 control·포털을 같은 물리 테이블로 묶는다.
> 상세: [16-portal.md §16.1b](../v2-wiki/16-portal.md)

| ID | 케이스명 | 전제 | 입력/조건 | 기대결과 | 계층 | 우선 | 근거(파일) |
|----|---------|------|----------|---------|------|:--:|----------------|
| TC-PORTAL-132 (신규) | 승인 시 동결값이 발신함 payload 에도 실린다 — 이음매 | APPROVED event_annotation 보유 영상 | 검수 승인 → `materialize` | `LS_DATASET_VIDEO_META` 동결과 **함께** `LS_META_REPL_OUTBOX.PAYLOAD_CN` 에 `evntAnnoCn` 키가 실리고 동결본과 **JSON 의미 동등**. ★바이트 동일이 아니라 의미 동등이다(직렬화 순서·공백에 묶이면 취약) | integration | High | DatasetVideoMetaSnapshotServiceIT(검수승인시_event_annotation이_스냅샷으로_동결되고_발신함_payload에도_실린다) |
| TC-PORTAL-133 (신규) | 미승인 어노테이션은 payload 에도 실리지 않는다 — 음성 방향 | 미승인(PENDING) event_annotation | 검수 승인 → `materialize` | 동결본이 NULL 이고 발신함 payload 에도 값이 없다. **미승인분이 포털로 새지 않는다** | integration | High | DatasetVideoMetaSnapshotServiceIT(materialize_skipsUnapprovedEventAnnotation) |
| TC-PORTAL-134 (신규) | 포털 프로비저닝 DDL 이 엔티티 전 컬럼을 담는다 | `db/portal/V*.sql` 현재 형상 | 버전 순으로 CREATE + ADD/DROP COLUMN 누적 후 엔티티 `@Column` 집합과 대조 | 엔티티 컬럼이 전부 DDL 에 실재한다. **엔티티에 있는데 DDL 에 없으면 실패**(운영 42703) / DDL 에만 있는 컬럼은 실패 아님 + WARN(레거시·향후 컬럼) — **판정 방향 비대칭이 의도다** | unit | High | PortalMetaReplicaDdlParityGuardTest(포털_복제본_프로비저닝_DDL이_엔티티_전_컬럼을_담는다) |
| TC-PORTAL-135 (신규) | 온프렘 설치가 로드하는 스키마도 엔티티 전 컬럼을 담는다 | `deploy/onprem/db/portal-schema.sql` | 같은 축으로 대조 | 설치가 **실제 로드하는 산출물**이 엔티티와 맞는다. 이 파일이 옮겨지면 조용히 skip 하지 않고 **명시 메시지와 함께 실패**한다(fail-loud) | unit | High | PortalMetaReplicaDdlParityGuardTest(온프렘_설치가_로드하는_포털_스키마가_엔티티_전_컬럼을_담는다) |
| TC-PORTAL-136 (신규) | 덤프 재생성 누락 차단 — 두 정본의 컬럼 집합이 같다 | 프로비저닝 DDL ↔ 설치 로드본 | 두 산출물의 컬럼 집합 대조 | 동일하다. ★**복제본 스키마의 정본은 둘**이라, 원천(`db/portal/V*.sql`)만 늘리고 `gen-schema-sql.sh` 재생성을 잊으면 **실제 설치되는 포털 DB 만 옛 형상**으로 남는다 — 그 형태를 이 케이스가 잡는다 | unit | High | PortalMetaReplicaDdlParityGuardTest(온프렘_포털_스키마가_프로비저닝_DDL과_같은_컬럼집합이다_덤프_재생성_누락_차단) |
| TC-PORTAL-137 (신규) | payload 가드의 제외 목록이 말없이 늘어날 수 없다 — 탈출구 봉쇄 | 계약 정합 가드의 관리 컬럼 제외 목록 | 목록 증감 · 사유 공백 · 제외 이름의 엔티티 실재 | 확정 명단과 다르면 실패하고, 항목마다 **사유를 값으로 강제**한다. ★**제외 목록은 그 자체가 탈출구다** — 계약 시험이 붉어졌을 때 문제 컬럼을 목록에 넣으면 **green 으로 되돌아가** 결손이 통과한다(변이로 실증). docstring 경고만으로는 막지 못한다 | unit | High | PortalMetaReplicaPayloadParityGuardTest(payload_제외목록은_말없이_늘어날_수_없다_가드_탈출구_봉쇄) |

> **세 축이 서로를 못 덮는다는 실증**: ①~④를 전부 맞추고 **DDL 만 미반영**하면 payload 축 가드(9건)·
> SQL 축 가드(3건)가 **전건 통과**하고 DDL 축 가드 2건만 실패한다. 「가드가 있다」와 「그 축이 덮인다」는 다르다.

> **e2e 의 출발점이 곧 커버리지 경계**: `MetaReplicationWorkerIT` 의 복제 e2e 는 payload JSON 을
> **하드코딩해 출발**하므로 「승인 → 동결 → 발신함」 구간을 **덮지 못한다.** 직렬화 입구를 되돌리는 변이를
> 걸어도 그 IT 는 전건 green 이고 TC-PORTAL-132 만 붉어진다. 「복제 검증은 워커 IT 가 전부 덮는다」는
> 인식은 **정정 대상**이다.

> **알려진 한계(인지·수용)**: 이 가드들은 「저장소의 DDL 산출물이 엔티티와 맞는가」까지만 본다 —
> **운영 포털 DB 에 그 DDL 이 실제로 적용됐는지는 저장소가 알 수 없다.** 미적용 환경은 가드 green 인 채
> 런타임 42703 으로 깨진다(§16.1a 의 graceful skip 은 *테이블 부재*만 덮고 *컬럼 부재*는 덮지 않는다).
> 기존 복제분의 빈 `EVNT_ANNO_CN` 은 **백필하지 않는다**(확정) — 해당 영상이 재승인될 때까지 비어 있으며
> 신규 발신함이 생기지 않으므로 자연 복구도 되지 않는다.

...
| TC-DIFF-002 | diff — SKELETON v 변경 감지 | 키포인트 v만 변경 | diff | MODIFIED 감지(삼중값) **+ 응답에 차이가 드러날 것** — `before != after` 이고 `type="SKELETON"`. ⚠ **현행 미해소**: `ShapeDto.fromPoints` 가 2-stride 로 평탄화해 v 를 버리고 `type="POLYGON"` 으로 내려 `before == after`(3차 D-ISSUE-22 = 1차 D-ISSUE-41 이월) | unit | Med | VersionService.java:1180-1197(readPoints, via parseLabelsById:1143-1171 ← computeLabelDiffs:1124-1137) · version/dto/LabelDiffDto.java:67-75(v 유실 지점) |
...
| TC-DIFF-006 | diff — 접근권한(IDOR) | 미배정 WORKER | diff | accessGuard 403(양 버전). ⚠ **현행 미해소**: 해시 조회가 인가보다 앞서 존재 해시=403 / 미존재 해시=404 로 갈려 **버전 존재 여부 오라클**(CWE-209) 성립 — 3차 D-ISSUE-25(1차 D-ISSUE-42 이월) | security | High | VersionService.java:338-349 |
| TC-DIFF-007 | diff — 손상 JSON | 파싱 실패 | diff | 빈 리스트(장애 격리) | unit | Low | VersionService.java:1124-1137(특히 1129-1136) |
| TC-DIFF-008 | **rollback 정상 — 재활성 + 본문 복원** | active 아닌 대상 스냅샷 | rollback(hash,srcSn) | ①`LS_DATA_LBL` full-replace(**LBL_SN·AI메타·TRCK_ID 보존 복원**, 점유 PK 만 신규 발급) ②버전 행은 **대상 스냅샷 재활성**(새 행 적층 없음 — `SAVE_REASON='ROLLBACK'` 폐기) ③`LS_DATA_LBL_HSTRY` 에 롤백 이벤트(actor·시각·대상 해시) ④**`REG_USER_NO`(라벨 작성자) 도 보존**. ⚠ **④는 현행 미해소** — 스냅샷 페이로드에 `regUserNo` 가 없고 복원 INSERT 도 그 컬럼을 안 써서 값이 있던 라벨까지 NULL 로 덮인다(통계가 수동 라벨을 자동으로 오분류) — 3차 D-ISSUE-23(1차 D-ISSUE-43 이월). ※AI 메타 행의 `REG_ID` 가 롤백 수행자로 바뀌는 것은 계약대로(결함 아님), 라벨 속성값(`LS_DATA_LBL_ATTR_VAL`)은 스냅샷에 없어 삭제되는 것이 명시된 한계 | integration | **Critical** | VersionService.java:410-522,535-545 · batch/repository/LsDataLblRepositoryImpl.java:26-29(REG_USER_NO 누락 지점) |
| TC-DIFF-009 | rollback — 빈 스냅샷으로 복원 | 라벨 0 스냅샷 | rollback | 프레임 라벨 삭제(빈 상태) | unit | Med | VersionService.java:663-695(삭제),866-868(restore 빈 스냅샷 조기반환) |
...
| TC-DIFF-018 | rollback — Race 잠금 순서 | 동시 롤백 | rollback | ACTIVE 잠금 → 라벨 교체 직렬화 **+ 종료 후 ACTIVE 행은 정확히 1건**. ⚠ **후자는 현행 미해소** — `findActiveForUpdate` 가 프레임 행 락(`:472`)보다 **먼저** 실행되고 술어가 `activeYn='Y'` 라 경쟁 트랜잭션이 새로 활성화할 행은 잠기지 않는다(write skew) → 동시 롤백 후 **active 2건 잔존**(3차 D-ISSUE-21, 재현 2/2). 라벨 본문 교체 자체는 정상 직렬화됨 | integration | High | VersionService.java:444-449,472 · version/repository/LsLabelVersionRepository.java:29-34 |
| TC-DIFF-019 | rollback — SKELETON 삼중값 무손실 복원 | SKELETON 라벨 | rollback | v 보존(rawPointsJson) | unit | Med | VersionService.java:605-608,789-793,818-831 |
...
| TC-DIFF-024 | 잉여 ACTIVE 자기치유 (신규) | active 행 2건 이상 | rollback(교체 경로 · **멱등 경로 모두**) | 정본 1건 외 전부 비활성 — 조기 반환 경로도 동일 처리. ⚠ **단일 스레드에서만 성립** — 자기치유가 순회하는 목록이 프레임 락 취득 전에 읽힌 stale 목록이라 **동시 롤백에서는 잉여 active 가 남는다**(3차 D-ISSUE-21, TC-DIFF-018 과 동일 근인) | integration | Med | VersionService.java:486-498,535-545,557-563 |

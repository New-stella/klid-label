...
- [ ] **통계 CSV 리포트 실데이터** — **여전히 갭(재확인 2026-08-19)**. `GET /v1/stats/report` 가 `period`(WEEK/MONTH/QUARTER/YEAR) 파라미터를 받고도 그 값과 무관하게 헤더 한 줄(`"month,labeled,reviewed,approvalRate\n"`)만 반환하는 placeholder — 집계 로직 미구현 그대로. 근거: `backend/src/main/java/kr/co/cudo/authoring/stats/controller/StatsController.java`(`report` 메서드). ⚠ 근거 표기 정정: 라인번호(`StatsController.java:110`) 대신 심볼명(`report`)으로 교체(위 「문서 동기화 규칙」 — 라인번호는 리팩토링마다 어긋난다).

...
- [ ] **게시판 (공지/가이드라인 배포)** — v1 운영 편의 기능([13](13-board-practice.md)). v2 범위에 없음.
  - 판단 기준: 운영 공지를 시스템 내에서 배포할 필요가 있는가? (낮은 우선순위, 외부 도구 대체 가능)
- [ ] **연습장 (라벨링 연습 환경)** — v1 신규 작업자 온보딩용([13](13-board-practice.md)). v2 없음.
  - 판단 기준: 작업자 교육/숙련 환경이 필요한가? (낮은 우선순위)

> 위 5개가 "추가 검토" 실질 후보다. 그 외 v1 기능은 아래처럼 **이미 대체됐거나 범위 외**라 추가 대상이 아니다.

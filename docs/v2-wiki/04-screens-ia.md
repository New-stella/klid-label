...
> **SC-005 도구 클릭 → 라벨 선택 모달 → 드로잉 · 좌측 라벨 패널 폐지 · 라벨명 한글 우선 표시**(2026-08-03 사용자 확정, 구속) — 라벨링 캔버스의 조작 흐름을 바꿨다.
...
> - **라벨명 한글 우선 표시** — 표시명 결정을 **공용 함수 하나**(`resolveLabelDisplayName`)로 통일해 라벨 선택 모달·우측 '객체' 목록·객체 속성·라벨 변경 이력·AI 탐지 팝업·포털 업로드 라벨링이 모두 같은 이름을 보여준다. 한글 라벨명은 그대로, 영문은 COCO 매핑(`dtctTypeCd`) 한글 사전 조회, 미등재는 원문. ⚠ 사전은 14건뿐이라 `water` 같은 커스텀 라벨이 영문으로 남는 것은 정상이며, **저장·전송 값에는 치환을 적용하지 않는다**(표시 전용). 라벨 관리 화면(SC-036)은 원문을 편집하는 화면이라 치환 대상이 아니다.

...
> - **SC-012 작업 목록**(`/task`) — 진입 시 **전체(워크플로 상태 필터 없음) · 등록일 최신순**. 배치 상태 축은 종전대로 처리 완료(`status=COMPLETED`) 고정이고, 여기에 **워크플로 상태 축(`workStatus`)이 독립 필터로 신설**됐다(두 축은 AND 결합). **KPI 5카드**(전체 작업 / 미배정 / 작업중 / 검수요청 / 반려)를 클릭하면 그 워크플로 상태로 필터되고, 같은 카드를 다시 누르면 해제(전체)된다. 컬럼 헤더 정렬은 **촬영일시·영상 ID** 두 컬럼(BE 정렬 allowlist `regDt`/`shtDt`/`rawSn` 대응). 검색어(영상명·작업자명)·이벤트유형·작업자 필터도 서버 필터다. ★**"작업중" 카드는 `배정됨 · 검수 미제출`(BE `PENDING`)을 뜻한다** — 시스템에 `IN_PROGRESS` 라는 워크플로 상태는 존재하지 않는다(BE 집계 필드명이 `inProgress` 일 뿐이며, 서버 필터 값으로 `IN_PROGRESS` 를 보내지 않는다).

...
> - ~~**후속 작업(2026-07-31)** — 영상 단위 개인정보 메타 설정 화면 + 별도 저장소~~ → **✅ 완료(2026-08-03)**: `VideoPrivacyMetaPanel` + `LS_DATA_RAW` 3컬럼(V161)로 신설됐고, 이로써 `video` 블록도 사람이 입력한 값을 읽는다. 단 구상과 달리 **"포털=원본 / 관제=비식별" 축 분리는 두지 않았다** — 원본 산출물은 판정 자체를 하지 않기로 확정돼(3필드 null) 원본용 저장소가 불필요해졌기 때문이다. 설정값은 현재 라벨링이 진행 중인 영상에 저장한다.

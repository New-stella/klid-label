...
> - **★수동값이 export를 덮는 범위(중요, 2026-07-31 확정 — 구 "익명여부는 export를 덮지 않는다" 폐기)**: 3필드(익명·가명·개인정보 포함여부) 모두 **원본(`orgnl`) 산출물에만** 수동값 우선 반영된다. **비식별(`deid`) 산출물은 수동값을 무시**하고 시스템 고정값(익명=Y·가명=N·개인정보포함=N)을 쓴다 — 프레임 단위 수동값을 영상 단위 `video` 블록이 태울 수 없어 같은 JSON 안에서 `video`/`image` 가 모순됐기 때문이다. 원본 export가 수동값으로 '익명화됨' 오표기될 수 있다는 점은 사용자가 수용한 사항이며 패널 안내 문구로 명시한다. 상세 → [24 §24.3.3](24-dataset-export.md).
...
> - **후속 작업(신규, 2026-07-31 사용자 결정)** — **영상 단위 개인정보 메타 설정 화면 + 별도 저장소**: 현재 개인정보 3필드의 수동 저장소는 프레임 단위(`LS_DATA_SRC` 3컬럼)뿐이라 **영상 단위 `video` 블록에 태울 원천이 없고, 원본/비식별 구분도 없다.** 이 때문에 비식별 산출물에서는 수동 override 를 억제해 두었다([24 §24.3.3](24-dataset-export.md)). 별도 화면·저장소(포털=원본 / 관제=비식별·증강 축 분리)를 만들면 ①`video`/`image` 가 같은 원천을 읽어 정합이 구조적으로 보장되고 ②비식별 산출물에도 수동값을 제대로 배선할 수 있다. 설정값은 **현재 라벨링이 진행 중인 영상**에 저장한다.

...
> **SC-005 저장 진입점 일원화 — 헤더 `[저장]` 제거 + 도구바 잘림 결함 해소**(2026-08-06 사용자 확정, 구속) — 헤더 우측 `[저장]` 과 좌측 도구바 저장 아이콘이 **같은 `handleSave` 를 부르는 중복 진입점**이었다. 헤더 버튼을 없애고 **좌측 도구바 + `Ctrl+S`** 로 일원화했다.
> - **순서가 중요했다** — 도구바는 짧은 뷰포트(≈690px 이하)에서 **맨 끝 `저장` 버튼이 잘려 클릭 불가**인 알려진 결함(TC-FE-306)이 있었다. 그 상태로 헤더 버튼을 지우면 **알려진 결함 위에 유일한 대안을 없애는** 셈이라, 도구바 스크롤 계약(`TOOLBAR_SCROLL_CLASS`) + 툴팁 portal 분리를 **먼저** 고치고 제거했다.
> - **헤더에 남는 것은 상태**(`저장 중...` / `● 편집 중` / `✓ 저장됨`)이며 진행 표시를 여기로 이관했다. **영상 잠금**(`LOCKED_FOR_REDEIDENT`)은 편집 차단(busy)과 다른 축이라 `LabelingPage` 가 `saveDisabled` 로 도구바에 전달한다. WORKER `[검수제출]` 버튼은 그대로다.
...
> - **회전**은 `0°→90°→180°→270°` 4단계 순환이며 이미지와 라벨을 함께 돌려 보여준다. **회전 중에는 그리기가 잠긴다** — 선택 도구를 뺀 도형·AI 도구가 비활성(툴팁이 사유를 알린다)되고 캔버스도 편집 입력을 봉인한다. 회전 버튼을 누르면 활성 도구가 선택 도구로 되돌아간다. 이유는 포인터 입력 경로가 **회전되지 않은 화면 좌표**를 전제로 설계돼 있어, 회전한 채로 입력을 받으면 사용자가 찍은 위치와 다른 좌표가 저장되기 때문이다.

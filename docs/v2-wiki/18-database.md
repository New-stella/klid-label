...
| `LS_TASK_EVNT_LOG` (V36, **개명 V9**) | 작업 이벤트 로그 — 배정·재배정·**배정 해제**·검수 **시작**/제출/승인/반려·개인정보 선언 변경·프레임 폐기/복원·**영상 제외/복원**의 **행 단위 감사 원장**. ★**검수 점유(「지금 누가 보고 있나」)의 적재처이기도 하다** — 전용 컬럼·별도 표 없이 「검수 시작」 이벤트로 표현하고 조회 시점에 판정한다(→ [12 §12.2.3](12-review-assignment.md)). 이벤트 종류 컬럼에 값 제약이 없어 **새 종류 추가에 마이그레이션이 필요 없다**(상수 추가뿐) — **`VIDEO_EXCLUDE`·`VIDEO_RESTORE`·`UNASSIGN`(2026-09-15 · `ADR-069`)이 그렇게 더해졌고 스키마 변경 0**이다. 영상 제외는 **사유(`RSN`)가 필수**이고 복원은 선택인데, 「왜 감췄나」가 확정 요구라 이 셋은 **자유 문구를 받는다**(개행·제어문자를 제거하고 칸 폭 안으로 제한해 저장한다). ⚠ **프레임 폐기 선례와 취급이 다르다** — 그쪽은 같은 칸에 토큰 하나만 싣고 자유 문구를 금지한다. 둘을 통일하지 말 것. **`ACTOR_ROLE_CD`(행위자 역할, `V38`)** = 그 행위 **시점의** 역할(`ADMIN`/`REVIEWER`/`WORKER`, CHECK 제약) — 볼 때마다 현재 역할을 다시 읽던 것을 바로잡은 것이며 `LS_ISSUE_COMMENT.AUTHOR_ROLE_CD` 선례를 따른다. **NULL 허용은 의도**이고 「역할을 남기지 않던 시기의 기록」을 뜻한다(백필하지 않는다 — 복원할 수 없는 값을 지어내지 않는다). **물리명은 V9(2026-08-15)에서 `LS_TASK_EVENT_LOG` → `LS_TASK_EVNT_LOG`** 로 표준용어 정합됐다(이벤트=사업표준단어 `EVNT`, → [18.3.6](#1836-작업-배정이벤트-로그-테이블-표준용어-정합-v9-2026-08-15)) | [12](12-review-assignment.md) |
...
> 포털 사용자(PORTAL_USER)가 **본인 이미지·영상을 직접 업로드**해 수동 라벨링(BBOX/POLYGON)하는 별도 경로. 내부 파이프라인(비식별→마킹→배치→검수)·데이터마트 View와 **완전 분리**되며 키포인트·선택 객체 AI 추적·VLM(외부 시계열 위탁)·검수·버전관리 미적용 → [16](16-portal.md)·[04](04-screens-ia.md).
> ⚠ **구 서술 폐기(2026-09-15)** — *"오토라벨링·SAM2… 미적용"* 은 **더 이상 사실이 아니다.** `ADR-013` v24 로
> AI 탐지(오토라벨)·AI 분할(SAM2 인터랙티브 분할)·AI 자동 추적은 이 업로드 자산에도 온디맨드로 제공된다
> (배치 오토라벨링 파이프라인은 여전히 미적용 — 온디맨드 AI 보조와는 다른 축) → [16 §16.3b](16-portal.md).
>
...
### 포털 서버간 연동 — 정리 트리거 접수 원장 (V37, 2026-09-09 신설)

| 테이블 | 내용 |
|---|---|
| `LS_DATST_ARNGMT_TRGR` | 포털이 학습데이터셋의 새 버전을 받아 보낸 **정리 삭제 트리거의 접수 기록**. `DATST_ARNGMT_TRGR_SN`(PK) · `DATST_CD` varchar(20) · `VER_NO` varchar(50) · `RCPTN_DT`(수신일시, DEFAULT CURRENT_TIMESTAMP) |

- **축이 다르다** — 위 「포털 자산 업로드」가 포털 *사용자* 의 자산이라면, 이 표는 **서버간 연동**
  (`INT-014`)이 보내온 신호를 받아 두는 자리다. 그래서 포털 사용자 오버레이가 아니라 별도 ERD
  (`ERD-035`)로 두었고, 선례는 같은 축의 메타 복제 발신함(`ERD-026`)이다.
- **`(DATST_CD, VER_NO)` 유일 제약이 재수신 멱등의 근거다.** 삽입을 `ON CONFLICT DO NOTHING` 으로
  걸어 **판정을 조회가 아니라 실행문 자체**에 둔다 — 조회한 뒤 삽입하면 두 트리거가 같은 순간에
  도착할 때 둘 다 통과해 행이 둘 생기고 기산점이 갈린다(파생영상 폐기 삭제에서 세운 원칙과 같은 축).
- **`RCPTN_DT` 는 정리 유예의 기산점이며 재수신으로 갱신하지 않는다.** 갱신하면 중복 수신이 반복될
  때마다 정리가 무한히 미뤄져 영영 일어나지 않는다.
- ⚠ **등록일시 컬럼을 따로 두지 않은 것은 의도다** — 행이 생기는 시각과 트리거를 받은 시각이 같은
  사실이라, 나란히 두면 같은 사실의 **두 번째 진실원**이 되어 나중에 한쪽만 갱신되면 어느 쪽이
  기산점인지 갈린다.
- **외래키가 없다** — 데이터셋 코드는 포털이 소유한 식별자라 이 저장소에 대응하는 부모 표가 없다.
- **보존 규칙의 정본이 아니다** — 기간·기산점 규칙은 `DFEAT-055` 가 소유하고 이 표는 그 규칙이 읽어
  갈 값을 담을 뿐이다. 창구·엔티티에 유예 일수를 복제하지 않는다.

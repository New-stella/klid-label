...
위 `POST /v1/frames/{srcSn}/yolo-track` 백엔드에 붙는 **화면(FE) 배선**이다. 트랙 편집 영역(우측 객체 목록 아래)에 `AutoTrackPanel` 로 배치된다.

⚠ **구 서술 폐기(2026-09-15)** — *"내부 라벨링(`/label/:id`) 전용이다 — 포털 라벨링(`/portal/label/:id`)은
오토라벨·추적 자체를 제공하지 않아(ADR-013) 진입점을 두지 않는다"* 는 **더 이상 사실이 아니다.** 포털
라벨링(`/portal/label/:id`)도 이 패널을 노출한다(2026-09-15 · `ADR-013` v24 · [16 §16.3b](16-portal.md)).
다만 **채널이 다르면 호출하는 API 도 다르다** — 내부는 `POST /v1/frames/{srcSn}/yolo-track`, 포털은
**별도 신설 창구** `POST /v1/portal/frames/{srcSn}/yolo-track`(인가·입력 이미지 해석만 갈아끼우고
추론 본체·응답 계약은 동일)를 쓴다. 경로 조립은 `portalMode` 단일 축에서 파생하는 한 곳(헬퍼)에서만 한다.
이 UI 정리(패널 자체의 표기·동작)는 관제향(`SCREEN-005`)과 **완전히 동일**하고 바뀌지 않는다.
...
#### 포털 전용 AI 보조 창구 (2026-09-15 신설, `ADR-013` v24) — 별도 창구, 같은 본체

포털 라벨링 화면은 **내부 창구를 그대로 여는 것이 아니라 포털 전용 창구**를 부른다. 인가(포털 작업
대상 판정)와 입력 이미지 해석만 채널마다 갈아끼우고, 추론 본체(검출 클래스 재구성·YOLO/SAM 호출·
bulkhead·취소·좌표 정규화·폴리곤 상한/예산·트랙 예산 절단/resume·mock 차단·응답 조립)는 내부 서비스를
그대로 재사용한다. 요청·응답 본문 계약은 대응하는 내부 창구와 동일하다.

| 기능 | 내부(관제향) 창구 | 포털 전용 창구 |
|---|---|---|
| AI 탐지 | `POST /v1/frames/{srcSn}/autolabel` | `POST /v1/portal/frames/{srcSn}/autolabel` |
| AI 분할 | `POST /v1/frames/{srcSn}/sam2-segment` | `POST /v1/portal/frames/{srcSn}/sam2-segment` |
| AI 자동 추적 | `POST /v1/frames/{srcSn}/yolo-track` | `POST /v1/portal/frames/{srcSn}/yolo-track` |
| 슬라이더·대기예산 기본값 | `GET /v1/ai-defaults` | `GET /v1/portal/ai-defaults` |
| 취소 | `POST /v1/ai-requests/{requestId}/cancel`(API-204) | `POST /v1/portal/ai-requests/{requestId}/cancel` |
| 선택 객체 AI 추적 | `POST /v1/frames/{srcSn}/sam2-track` | **없음(404) — 관제향과의 기능 중복이라 포털에 두지 않는다** |

- **비식별 판정(신고 412·재비식별 작업락 409)은 포털 창구에 없다** — 포털에는 비식별 파이프라인 자체가
  없다(*"비식별 이런것도 안들어감"*).
- **요청량 제한**: 사용자별 RateLimiter `portalAiAssist`(60회/분)가 실행 창구 3종(탐지·분할·자동 추적)에
  걸린다. 관제향 내부 창구에는 이 제한이 없다(사용자 단위가 아니라 서킷·bulkhead 로 보호).
- **속도·정밀도 설정은 채널 무관 공유** — `GET /v1/portal/ai-defaults` 응답은 `GET /v1/ai-defaults` 와
  같은 값을 같은 서비스에서 돌려준다(설정 자체는 채널로 나뉘지 않는다).
- 전체 인가·입력 이미지·비식별·요청량 제한 상세는 [16 §16.3b](16-portal.md)가 정본이며, 이 표는
  요약이므로 여기에 중복 규정하지 않는다.
...
- 포털 라벨링(`/portal/label/:id`)은 AI 탐지·AI 분할·AI 자동 추적을 제공하므로 같은 busy 규칙이
  적용된다. **2026-09-02 로 그 화면이 통합돼 본인 업로드 자산도 같은 경로에서 열리고**, ⚠ **구 서술
  폐기(2026-09-15)** — *"업로드 자산에는 AI 진입점이 없어 AI busy 가 발생하지 않고 저장 진행 표시·
  취소만 동작한다"* 는 **더 이상 사실이 아니다.** §16.3b 확정으로 AI 세 기능은 **데이터마트 자산·본인
  업로드 자산 양쪽에 동일하게** 제공되므로, 업로드 자산 라벨링에서도 AI busy 가 발생한다. ⚠ **구 서술
  폐기** — *"**포털 업로드 라벨링**(`/portal/uploads/*`, ADR-013 별도 경로)"* 이라는 **별도 화면은
  폐기**됐다(`SCREEN-034`). ★**AI 유무를 가르는 축은 더 이상 자산 출처가 아니다** — 두 출처 모두
  AI 세 기능을 제공하고, 출처로 갈리는 것은 **저장 결과의 보존 성질**뿐이다(업로드 자산의 AI 자동
  추적 트랙 묶음은 저장 후 사라진다) → [16 §16.3b·§16.5b](16-portal.md).

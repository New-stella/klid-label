...
- 포털 라벨링(`/portal/label/:id`)은 데이터마트 자산에 AI 분할·추적을 제공하므로 같은 규칙이 적용된다. **2026-09-02 로 그 화면이 통합돼 본인 업로드 자산도 같은 경로에서 열리는데**, 업로드 자산에는 AI 진입점이 없어 AI busy 가 발생하지 않고 저장 진행 표시·취소만 동작한다. ⚠ **구 서술 폐기** — *"**포털 업로드 라벨링**(`/portal/uploads/*`, ADR-013 별도 경로)"* 이라는 **별도 화면은 폐기**됐다(`SCREEN-034`). ★**AI 유무를 가르는 축은 화면이 아니라 자산 출처다** → [16 §16.5b](16-portal.md).

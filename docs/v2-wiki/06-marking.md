...
- 비식별 누락 신고: 라벨링 단계(srcSn 기준)·**마킹 단계(rawSn 기준 `POST /v1/videos/{rawSn}/deident-report`) 모두 구현됨** — 부수효과(작업락 + `DE_IDENT_YN='F'` + 검수완료 영상 통지 — 라벨·개인정보 3필드는 보존)는 두 경로가 동일하며 파생영상은 412 로 거부된다 → [08](08-deidentification.md#84-누락-신고-rq-sfr-09-03-uc-016)

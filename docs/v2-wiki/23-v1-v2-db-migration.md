...
- 검수자(REVIEWER) 배정은 **이관 대상 자체가 아니다** — v1 에 영상별 근거가 없기도 하지만, **v2 에는 검수자 배정이 없다**(2026-09-15 폐지, 구속 → [12 §12.1 ★](12-review-assignment.md)). 검수는 배정 없이 전체 대기열에서 집어간다. 라벨 작성자 22명/1,814 영상 기준.
  - ⚠ **구 서술 폐기(2026-09-15)** — *"(운영 재배정)"* 은 **앞으로 운영자가 검수자를 배정할 것**을 전제한 표현이라 사실과 다르다. 이관 후 할 일이 아니다.

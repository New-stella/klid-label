...
| 테이블 | 주요 컬럼 |
|--------|----------|
| `LS_NOTICE` | NOTICE_SN(PK), NOTICE_TITLE(200), NOTICE_CN(TEXT), UPEND_FIX_YN, PBLCN_STTS_CD(DRAFT/PUBLISHED, V80 rename 구 PUB_STTS_CD), PBLCN_DT(V80 rename 구 PUB_DT), REG_ID/REG_DT, MDFR_ID/MDFCN_DT. INDEX(PBLCN_STTS_CD, UPEND_FIX_YN, REG_DT DESC) |
| `LS_NOTICE_ATTACH` | ATCH_FILE_SN(PK, V90 rename 구 ATTACH_SN), NOTICE_SN(FK ON DELETE CASCADE), ORGNL_FILE_NM, STRG_FILE_NM(UUID, V90 rename 구 STORE_FILE_NM), FILE_PATH, FILE_SIZE, REG_DT |

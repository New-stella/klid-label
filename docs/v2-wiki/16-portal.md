...
**등록 규칙**
- **해제본 구성은 우리 검수 승인 산출물 구조로 «가정»한다** — `{영상 키}/v{n}/deid/NNNN.jpg` + 같은 이름 `NNNN.json`(NIA 문서). 버전이 여럿이면 가장 큰 n. **`orgnl/` 은 읽지 않는다**(원본 비노출). 구조가 다르면 추측으로 채우지 않고 **등록 실패(FAILED)**. ⚠ 실제 배포 샘플로 확인되지 않았다 — 샘플을 받으면 `ADR-068` 을 개정한다.

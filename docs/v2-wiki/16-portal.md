...
- PortalLayout (모바일 친화, **좌측 메뉴 보유** — 2026-09-01 확정). 머리 영역은 Host 가 그리고
  그 아래는 좌측 메뉴를 포함해 전부 저작도구가 렌더한다. 메뉴 목적지 셋 — 내 작업 `SC-028` ·
  내 업로드 `SC-033` · 증강 `SC-044`. 사용자 신원은 표시하지 않는다(Host 머리 영역이 보여 준다).
  ⚠ 구 서술 「LNB 없음」은 폐기. 외부 채널측 회신 대기 축이다 — 상세는 [04](04-screens-ia.md) §4.1

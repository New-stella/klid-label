...
>
> ⚠⚠ **`evnt_type` 은 합의됐다 (2026-09-02 사용자 확정, 구속 · `ADR-059`)** — 요청자가 고르지 않고 **서버가 중립값 `ETC` 를 고정 송신**하며
> 세부 유형은 **전송하지 않는다**. 포털 사용자가 본인 자산으로 올린 영상에 그 유형이 **없다**는 사정(관제 인입에서 받아오는 값이 이 경로엔 애초에
> 존재하지 않는다)이 이 확정으로 해소된다 — 값을 지어내지 않고도 요청을 조립할 수 있다 → [14 §14.1](14-augmentation.md).
> ★`ETC` 는 **벤더와 협의가 끝난 값**이다. ⚠ **다만 규격서 판본 대조가 남아 있다** — 우리가 가진 판본(**v1.3**)에는 아직 `ETC` 가 없고
> 허용 값이 **`FLOOD | WILDFIRE` 둘뿐**으로 적혀 있다. **개정판을 받으면 대조**할 것.
>
> ⚠ **구 서술 폐기(2026-09-02)** — *"남은 합의 사항은 `evnt_type` 하나다 … 무엇을 보낼지 정해져야 요청을 조립할 수 있고, 그전까지 값을
> **지어내지 않는다.** 그래서 흐름 시퀀스(`SEQ-019`)도 메시지를 그리지 않는다"* 는 더 이상 사실이 아니다. 미결이던 것은 이제 확정됐다.

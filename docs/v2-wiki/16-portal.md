...
| Method · URL | 설명 | 응답 |
|---|---|---|
| `GET /v1/portal/datasets/{datasetId}/videos?page&size` | 데이터셋에서 등록한 영상 목록. `registrationState`(IN_PROGRESS·DONE·FAILED) + 페이지 + 항목 `rawSn`·`videoName`·`frameCount`·`labelCount`·`entrySrcSn`·`lastSavedAt`. 뒤 둘은 **요청 사용자 기준**(내 작업 목록과 같은 판정) | 200 / 401 / 403(포털 회원 아님) / 409(소재 미준비) |
...
**알려진 공백** — 등록 실패 뒤 재시작 경로 없음 · 같은 데이터셋의 새 버전은 다시 등록되지 않음 · 오래된 등록 행 정리 없음.

...
>
> ⚠⚠ **정책만 확정됐고 코드는 아직 철거 전이다** — 듀얼 데이터소스·복제 워커·발신함·Quartz 잡·메트릭이
> 그대로 돌고 있으므로 §16.1a 는 **철거 전까지의 현행 동작 기록**으로 유효하다. 「이미 걷어냈다」로 읽지 말 것.
>
...
> ⚠ **[폐기] 이 절이 서술하는 복제 축은 2026-08-31 상호 DB 미접근 확정으로 폐기됐다**(위 16.1 ★ 절).
> 아래는 **철거 전까지의 현행 동작 기록**이며 새 설계의 근거가 아니다. 판정 정본은 `INT-009`.

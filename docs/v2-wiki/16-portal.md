...
| 인증 축 | **시스템 주체**(사용자 세션과 무관한 시점에 도착한다) |
...
#### ★ 주체 축 격리는 양방향이다 (`INT-014` · `AC-1103`)

포털 채널 토큰은 **사용자 주체**와 **시스템 주체**로 갈리고, 둘은 같은 서명키·같은 헤더를 쓰므로
**주체 클레임 말고는 가를 수단이 없다.**

| 토큰 주체 | 들어가는 곳 | 다른 축의 창구에 오면 |
|---|---|---|
| 사용자 | `/v1/portal/**` | 거부 |
| 시스템 계정 | `/v1/portal-system/**` | 거부 |

- **뒤쪽은 포털 사용자 채널 *전체* 에 적용된다** — 시스템 주체 토큰은 `ROLE_PORTAL_USER` 를
  **아예 부여받지 못하므로** 저작·업로드·증강 어느 창구에도 도달하지 못한다. 근거는 그 토큰이
  **특정 사용자를 가리키지 않는다**는 것이다 — 토큰 주체를 자원 소유자와 견주어 접근을 가르는
  창구에 그것이 도달하면 그 비교가 **의미를 잃은 채 통과**한다.
- **무엇이 시스템 계정인지는 배포 설정(`PORTAL_SYSTEM_SUBJECTS`)이 정한다.** 비면 아무도 시스템
  주체가 아니어서 이 창구가 닫힌다(fail-closed) — 값이 없을 때 열리는 것이 아니라 닫히는 방향이다.
- ⚠⚠ **포털이 시스템 계정을 쓰는데 그 식별자가 미등록이면, 그 토큰은 사용자 주체로 떨어져 포털
  사용자 권한을 받는다** — 이 설계가 막으려던 바로 그 상태이며 거부가 아니라 **통과**라 조용히
  일어난다. 토큰에 시스템 여부 표식이 없어 목록 없이는 식별할 수단이 없는 **구조적 한계**이고,
  그래서 방어가 코드가 아니라 **운영 절차**에 있다.
...
**`LS_DATST_ARNGMT_TRGR` (V36, 2026-09-09 신설)** — 포털이 보낸 정리 삭제 트리거의 접수 원장.

...
```
관제서버 --(API 연동)--> 포털 적재 (관제서버 책임 · DB 공유 아님)
        ↓
포털 라벨 화면은 저작도구 DB(control)에서 Load
```

> **★관제서버와 포털 사이에는 DB 공유가 없다 (2026-08-27 사용자 확정, 구속)** — 데이터마트 내용은
> **API 연동으로 적재**한다. ⚠ 구 서술 *"관제서버 → 데이터마트 → 포털 **DB** 적재"* 는 **폐기**다.
> 그 문장대로 읽으면 관제가 포털 DB 에 직접 쓰는 구조를 전제하게 되고, 그 전제 위에서 스키마·권한을
> 설계하면 실제로 없는 경로를 만들게 된다.
>
> ⚠ **구 예외 폐기(2026-08-31)** — *"저작도구 → 포털 DB 단방향 복제(§16.1a 메타 복제 배치)는 별개
> 축이라 그대로다"* 는 **더 이상 사실이 아니다.** 바로 아래 ★ 절이 그 예외를 닫았다.

> **★저작도구와 포털은 서로의 DB 에 접근하지 않는다 (2026-08-31 사용자 확정, 구속)** — 데이터 교환은
> **API** 로 설계한다. **양방향 약속**이라 포털도 저작도구 DB 를 참조하지 않는다.
> 따라서 **§16.1a 메타 복제 배치는 폐기 확정**이며 새 설계의 근거로 인용하지 않는다.
>
> ✅ **철거 완료 (2026-08-31)** — 듀얼 데이터소스·복제 워커·발신함·Quartz 잡·메트릭이 **코드에서 제거**됐고,
> 반입 산출물의 포털 DB 생성·스키마 로드 단계와 `PORTAL_DB_*`·`META_REPLICATION_ENABLED` 설정도 함께 사라졌다.
> **되살리지 말 것.** 따라서 §16.1a 는 **철거 이전의 동작 기록**이며 현행 동작이 아니다.
> ⚠ `LS_META_REPL_OUTBOX` **테이블 정의는 남겼다** — 드롭 마이그레이션을 분리했고(되돌릴 수 없는 작업)
> 읽는 코드가 0건이라 무해하다.
>
> **대체 API 필요 여부는 미확정** — 이 복제는 **쓰기 전용**이라 저작도구에 복제본을 읽는 경로가 0건이고
> 포털 설계에도 받는 테이블이 없어 **실 소비처가 양측 어디에서도 확인되지 않는다.** 포털 회신으로 소비처가
> 드러나면 REST 대체 경로를 설계하고, 없으면 대체 없이 철거한다. ⚠ 특히 **이벤트 어노테이션 동결 원문**은
> 이 복제 말고 포털로 나가는 경로가 없다. 판정 정본은 `INT-009`.
>
> ### ★★배포가 채널마다 갈린다 — 저작도구는 두 벌이다 (2026-08-31 확정, 구속 · ADR-012)
>
> 저작도구 **하나가 포털·관제 양쪽에 붙는 구조가 아니다.** 포털에 붙는 저작도구와 관제에 붙는 저작도구가
> **각각 별도 배포**되고 각 배포본은 **자기 별도 PostgreSQL DB 에만 연결**한다. **두 배포본끼리도 직접
> 이어지지 않는다** — 서로의 DB 를 읽지 않고 서로를 직접 호출하지도 않는다.
>
> **자산 전달 경로 — 중계자는 관제다**: ①관제 채널 배포본에서 검수 승인 → 관제로 완료 통지 ②관제가 그
> 자산을 포털로 전달(**압축 파일** 예정) ③이후 포털이 **포털 채널 배포본**에 자산 수신을 API 로
> 알린다 — 그 배포본이 포털이 보유한 자산을 조회해 작업 소재로 삼는다.
...
> ⚠ **위 16.1 의 `PortalLabelService` 실측(control DB 의 승인 자산을 직접 읽는다)은 단일 배포 시절
> 형상이다.** 배포가 갈리면 그대로 성립하지 않으므로 그 코드를 근거로 「포털향도 control DB 를 읽으면
> 된다」고 추론하지 말 것.
...
## 16.1a [폐기·철거 완료] 메타 복제 배치 (control → 포털 outbox)

> ⚠ **이 절이 서술하는 복제 축은 2026-08-31 상호 DB 미접근 확정으로 폐기됐고 같은 날 코드에서 철거됐다**
> (위 16.1 ★ 절). 아래 서술의 클래스·설정·마이그레이션은 **더 이상 존재하지 않는다** — 지금 찾으면 없다.
> 남겨 두는 이유는 ①왜 그 구조였는지 ②왜 없앴는지가 다음 사람에게 필요하기 때문이고, **새 설계의 근거로
> 인용하지 않는다.** 판정 정본은 `INT-009`.
>
> ⚠ 철거 범위에서 **승인 트랜잭션은 발신함 적재만 걷어냈다** — 동결 스냅샷(`LS_DATASET_VIDEO_META`)과 그
> 해시는 그대로다. 해시·동결 적재가 발신함 직렬화보다 앞에서 끝나고 그 직렬화는 어느 쪽의 입력도 아니었다.
> 관제 계약면 `V_COMPLETED_VIDEO` 무변경을 뷰 대조 시험 3종으로 확인했다 → [18](18-database.md)
...
⚠ **위 다섯 단은 전부 철거됐다** — 이 표는 「한 단만 빠져도 조용히 빈 값이 된다」는 **교훈의 근거**로만 남긴다.
...
| **업로드 영상 증강 — 결과 확인·후속 작업·내려받기** | **○** (2026-09-01 확정 — **저작도구가 자체 수행**한다. 요청·결과 확인·후속 작업·내려받기 넷이 모두 저작도구 안에서 끝나며 **외부 채널이 제공하는 증강 요청 창구를 호출하지 않는다**. 자리: 증강 화면 `SC-044`. 후속 라벨링은 `SC-034`. 파생물은 **포털 자산 계보**이며 구체 저장 모델은 미확정) |
| 버전관리·검수 | ✗ |
...
> **적재와 자격은 확정됐다 (2026-08-27 사용자 확정, 구속)** — ①**전용 저장소로 완전 분리**: 마킹도 증강 파생물도 포털 업로드 전용 저장소에만 쌓고
> 내부 마킹·증강 원장을 채널 구분 컬럼으로 **함께 쓰지 않는다.** 함께 쓰면 데이터마트 뷰와 관제 통지에서 포털 자산을 빼는 가드를 경로마다 달아야 하고
> **한 곳만 빠뜨려도 본인만 보게 되어 있는 자산이 외부로 나간다.** ②**요청 자격은 소유자 스코프 + 준비 완료**뿐이다 — 내부 증강의 「검수자 전용 + 검수 완료 영상」
> 조건은 옮기지 않는다(이 경로엔 검수가 없어 그대로 옮기면 아무도 요청할 수 없다).
> ③**외부 위탁 창구는 새로 만들지 않는다** — 확정된 「생성형 AI API 연동명세서 v1.3」의 `request_channel` 이 이미 `CONTROL | PORTAL | AUTHORING` 이고
> 최종 상태 Webhook 도 「생성형 AI → 관제/**포털**/저작」으로 규정돼 있다. `request_user_id`(선택) 로 요청 사용자도 식별된다.
> ⚠ 증강은 **`AUGMENT`+`I2I` 만 지원**이고 `I2V`/`V2V` 는 미지원이라, 내부 경로와 같이 **프레임 이미지만 변환하고 영상 파일은 복사**한다.
>
> ⚠⚠ **남은 합의 사항은 `evnt_type` 하나다** — 그 명세는 이 값을 **필수**로 요구하고 허용 값이 **`FLOOD | WILDFIRE` 둘뿐**인데,
> 포털 사용자가 본인 자산으로 올린 영상에는 그 유형이 **없다**(관제 인입에서 받아오는 값이 이 경로엔 애초에 존재하지 않는다).
> 무엇을 보낼지 정해져야 요청을 조립할 수 있고, 그전까지 값을 **지어내지 않는다.** 그래서 흐름 시퀀스(`SEQ-019`)도 메시지를 그리지 않는다.
...
- PortalLayout (모바일 친화, **좌측 메뉴 미보유** — 2026-09-02 확정). 머리 영역과 좌측 주 메뉴는
  **Host 가 둘 다 그리고** 저작도구는 그 아래 본문만 그린다. 목적지 셋은 **본문 상단 이동 탭**으로
  오간다 — 내 작업 `SC-028` · 내 업로드 `SC-033` · 증강 `SC-044`. 사용자 신원은 표시하지 않는다
  (Host 머리 영역이 보여 준다). 라벨링 화면 둘은 목적지가 아니라 목록에서 행을 눌러 들어가는
  자리이며, 몰입 편집 화면이라 그 탭도 노출하지 않는다.
  ⚠ **이 축은 두 번 뒤집혔다** — 2026-09-01 의 「좌측 메뉴 보유」는 폐기되고 Host 소유로 되돌아왔다.
  **세 번째로 뒤집지 말 것.** 상세는 [04](04-screens-ia.md) §4.1
...
- **업로드 자산**: **파일 먼저, DB 나중** 순서로 삭제한다. 뒤집으면 DB 가 먼저 사라져 어느 파일을
  ...
  더 나쁘므로 DB 행도 남기고 다음 회차에 재후보한다. **딸린 행 정리는 흡수(`ADR-058`)로 달라진다** — 흡수 전에는
  `LS_PORTAL_ULD_FRME`·`LS_PORTAL_ULD_LBL` 둘만 FK `ON DELETE CASCADE` 로 딸려 지워졌다. 공용 원장
  이관 후에는 `LS_DATA_RAW` 를 참조하는 표가 **약 24개**로 넓어지고, 그중
  `LS_DATA_LBL_HSTRY`·`LS_DATA_AUG`·`LS_DATA_AUG_LBL_MAP` 은 **부모 외래키가 없어 연쇄로 정리되지
  않는다** — 오류로 드러나지 않고 조용히 고아 행이 남으므로 지울 대상을 명시적으로 열거해야 한다.

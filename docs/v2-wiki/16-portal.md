...
화면: `SC-028`(포털 홈, 데이터마트 영상 선택 `/portal`), `SC-029`(포털 라벨링 `/portal/label/:id` — **2026-09-02 통합**, 데이터마트 자산과 본인 업로드 자산을 한 화면에서 다룬다), `SC-033`(포털 업로드 `/portal/uploads`), `SC-044`(포털 증강 `/portal/augment`), **`SC-045`(포털 업로드 영상 마킹 `/portal/uploads/:uldSn/marking` — 2026-09-02 신설)**. 코드: `portal/`(7 파일).

> ⚠ **`SC-034`(포털 업로드 라벨링 `/portal/uploads/:uldSn/label`)는 2026-09-02 폐기** — `SC-029` 로 통합됐다(→ 16.5b). 그 화면을 별개 화면으로 전제하는 서술을 되살리지 말 것.
...
>
> ★**포털 채널 배포본도 관제 채널 배포본이 쓰는 기존 `ai-server` 를 그대로 가리킨다** (2026-09-15 사용자
> 확정, 구속 · `ADR-012` v23 · `CO-20260915-포털-라벨링-AI보조-제공`) — 포털 라벨링 화면의 온디맨드
> AI 보조(AI 탐지·AI 분할·AI 자동 추적, 아래 §16.3b)에 쓴다. **별도 AI 서버를 새로 두지 않는다.**
> 포털 배포본 WAS → 기존 ai-server(관제 측 장비) 로 가는 네트워크 경로 개방이 배포 전제다(인프라 확인
> 잔여). 추론 자원은 관제 채널의 내부 작업자와 **공유**하므로 포털 AI 실행 창구에는 사용자별 요청량
> 제한(`portalAiAssist`)이 걸린다.
> ⚠ **구 서술 폐기(2026-09-15)** — *"포털 채널 배포본에는 `ai-server`(YOLO·SAM2 추론)가 붙지 않는다 —
> 오토라벨링·인터랙티브 분할이 없다"*. 이 반전이 여는 것은 **라벨링 화면의 온디맨드 AI 보조 세 기능뿐**이다
> — 추론 이외 AI 파이프라인(외부 시계열 위탁·비식별·**배치 오토라벨링**)은 여전히 **관제 채널 배포본에만**
> 있다. 업로드 영상 마킹 완료는 프레임 추출까지만 이어지고 YOLO/SAM2 배치를 트리거하지 않는다(아래
> §16.3b). 두 축(온디맨드 AI 보조 ↔ 배치 오토라벨링 파이프라인)을 한 낱말 「오토라벨링」으로 묶어
> 되돌리지 말 것.
...
| **AI 탐지(오토라벨) — 온디맨드** | **○** (2026-09-15 확정 — 포털 전용 창구, 아래 §16.3b) |
| **AI 분할(SAM2 인터랙티브 분할) — 온디맨드** | **○** (2026-09-15 확정 — 2026-08-02 제거를 반전. 포털 전용 창구, §16.3b) |
| **AI 자동 추적(여러 객체 트랙 묶음) — 온디맨드** | **○** (2026-09-15 확정 — 2026-08-02 제거를 반전. 포털 전용 창구, §16.3b) |
| **선택 객체 AI 추적(한 객체 추적 — `sam2-track`)** | ✗ (관제향과의 기능 중복이라 포털에 두지 않는다 — §16.3b) |
| **키포인트(SKELETON) 도구** | ✗ (2026-08-02 제거 — ADR-013 정합, 그대로 유지) |
| **배치 오토라벨링 파이프라인**(업로드 영상 마킹 완료 → YOLO/SAM2 자동 실행) | ✗ (온디맨드 AI 보조와는 다른 축 — §16.3b) |
...
| **업로드 영상 이벤트구간 마킹** | **○** (2026-08-26 확정 — **업로드 자산 한정**. 자리: **전용 화면 `SC-045`**(2026-09-02 확정 · 16.5a). 데이터마트 로드분은 대상 아님. ⚠ **구 기재 폐기** — *"자리: 업로드 라벨링 화면 `SC-034` 안"* 은 사실이 아니다. 마킹은 라벨링 화면 안의 모드가 **아니라 별도 화면**이고 그 `SC-034` 자체가 폐기됐다) |
| **업로드 영상 AI 증강 연동 요청** | **○** (2026-08-26 확정 — **업로드 자산 한정**. 자리: 업로드 목록 `SC-033` 의 자산별 액션. 서버가 외부로 보내는 **비동기 위탁**이라 포털 사용자가 추론 엔드포인트를 직접 호출하지 않는다) |
| **업로드 영상 증강 — 결과 확인·후속 작업·내려받기** | **○** (2026-09-01 확정 — **저작도구가 자체 수행**한다. 요청·결과 확인·후속 작업·내려받기 넷이 모두 저작도구 안에서 끝나며 **외부 채널이 제공하는 증강 요청 창구를 호출하지 않는다**. 자리: 증강 화면 `SC-044`. 후속 라벨링은 **`SC-029`**(2026-09-02 통합 — 구 기재 `SC-034` 는 폐기 화면이다). 파생물은 **포털 자산 계보**이며 구체 저장 모델은 미확정) |
| 버전관리·검수 | ✗ |
...
## 16.3b 포털 라벨링 AI 보조 (2026-09-15 신설, 구속 · `ADR-013` v24 · `CO-20260915-포털-라벨링-AI보조-제공`)

포털 라벨링 화면(`SC-029` — 데이터마트 영상·본인 업로드 영상 **두 출처 모두**, 16.5b 통합화면)에
관제향과 같은 세 기능을 **온디맨드**로 제공한다. 근거(사용자 원문): *"포털향 라벨링 화면도 관제향이랑
동일하게 오토라벨링 기능이 들어가야해 … 스켈레톤빼고"* · *"내가 파악한것은 ai 탐지, 분할, ai 자동 추적
3가지 기능"*.

### 창구 5종 — 내부 API 를 여는 것이 아니라 포털 전용 창구를 신설

| Method · URL | 설명 | 응답 |
|---|---|---|
| `POST /v1/portal/frames/{srcSn}/autolabel` | **AI 탐지**(한 프레임 객체 탐지, BBOX/POLYGON). 본문·응답은 내부 `AutolabelRequest`/`AutolabelResponse` 와 동일 계약 | 200 / 400(path·body srcSn 불일치) / 403 / 404 / 412 아님(비식별 미적용, 아래 참조) / 429 |
| `POST /v1/portal/frames/{srcSn}/sam2-segment` | **AI 분할**(클릭/박스 한 객체 분할). 내부 `Sam2SegmentRequest`/`Sam2SegmentResponse` 와 동일 계약 | 200 / 400 / 403 / 404 / 429 |
| `POST /v1/portal/frames/{srcSn}/yolo-track` | **AI 자동 추적**(시작 프레임부터 후속 프레임까지 여러 객체를 한 번에 찾아 트랙으로 묶기). 내부 `YoloTrackRequest`/`YoloTrackResponseDto` 와 동일 계약. **후속 프레임이 시작 프레임과 다른 영상(rawSn)이면 400** | 200 / 400 / 403 / 404 / 429 |
| `GET /v1/portal/ai-defaults` | 정밀도 기본값 + AI 대기 예산. 내부 `GET /v1/ai-defaults` 와 같은 응답(`AiDefaultsResponse`), 같은 서비스 재사용 | 200 |
| `POST /v1/portal/ai-requests/{requestId}/cancel` | 포털 AI 취소(내부 `POST /v1/ai-requests/{requestId}/cancel`(API-204)은 REVIEWER/WORKER + INTERNAL 채널이라 포털 토큰이 닿지 않아 별도 창구가 필요하다). 응답은 내부와 동일하게 항상 200(`cancelled` true/false, 사유 비구분) | 200 |

`POST /v1/portal/frames/{srcSn}/sam2-track`(선택 객체 AI 추적 — 관제향 AI 탐지 팝업의 [트랙] 버튼에
대응하는 창구)은 **두지 않는다** — 부재는 404(또는 405), 스켈레톤 전용 창구도 없다.

⚠ **채널 격리는 양방향**: INTERNAL 토큰으로 포털 창구 호출 403 · PORTAL 토큰으로 내부 창구(`/v1/frames/**`)
호출 403 — 기존 채널 매처(`/v1/portal/**` = CHANNEL_PORTAL + ROLE_PORTAL_USER)를 그대로 쓴다.

### 인가 — 포털 작업 대상 판정, 추론 본체는 내부 서비스 재사용

모든 대상 프레임(자동 추적이면 시작 + 후속 전부)을 `PortalWorkTargetResolver` 로 판정한다 —
**데이터마트 노출(APPROVED) 영상**이거나 **본인 업로드 자산**이면 통과, 대상 행 부재 404 · 남의
자산/미노출 영상 403(기존 포털 축과 동일 문구). 인가를 통과하면 **추론 본체(검출 클래스 서버측
재구성·YOLO/SAM 호출·bulkhead·취소·좌표 정규화·폴리곤 상한/예산·트랙 예산 절단/resume·mock 차단·
응답 조립)는 내부 서비스(`AutolabelOnlineService`·`Sam2SegmentService`·`YoloTrackService`)를
**그대로** 재사용**한다(복제하지 않는다) — 「인가 + 입력 이미지 해석」만 채널마다 갈아끼운다.

### 입력 이미지 — 포털이 이미 사용자에게 보여주는 파일과 같은 파일

출처별로 **포털 이미지 서빙과 같은 파일·같은 경로 판정기**를 쓴다 — 데이터마트 자산은
`PortalLabelService.serveFrameImage`(비식별 경로 컬럼 + 실경로 검증), 본인 업로드 자산은
`PortalUploadService.serveFrameImage`(포털 저장 base + 실경로 봉쇄). 서빙과 AI 가 같은 해석부를
쓰므로 AI 입력이 화면에 안 뜨는 파일을 몰래 읽는 경로가 없다.

### 비식별 판정은 포털 AI 경로에 없다

*"비식별 이런것도 안들어감"* — 비식별 누락 신고 게이트(412)·재비식별 작업락(409)을 포털 AI 창구의
차단 훅에서 **호출하지 않는다**. 포털에는 비식별 파이프라인 자체가 없다(본인 업로드 자산은 원본을
그대로 쓰고, 데이터마트 자산은 이미 검수 승인된 비식별본이다).
⚠ **인지·수용한 비대칭** — 데이터마트 영상이 비식별 누락 신고 구간이면 프레임 **이미지 서빙**은
412 로 막히는데 AI 창구는 막지 않는다. 화면에 이미지가 뜨지 않아 실사용 도달성은 낮지만 구조적으로
남는 비대칭이다. 기존 포털 이미지 **서빙** 경로의 동작 자체는 이 변경으로 바뀌지 않는다.

### 요청량 제한 · 취소

외부 채널이므로 사용자별 RateLimiter(`portalAiAssist`, 60회/분)를 AI 실행 창구 3종(탐지·분할·자동
추적)에 건다 — 초과 429. 자동 추적은 화면이 시간 예산 절단(`truncated`/`resume`)에 따라 이어 보내기
요청을 연속으로 보낼 수 있어 그 연속 호출이 한도에 끊기지 않게 값을 잡는다. 취소는 `X-AI-Request-Id`
규약을 내부와 공유하되, 소유자 판정에 **채널을 포함**한다(포털 사용자 식별자와 내부 사용자 식별자가
같은 문자열이면 서로의 요청을 끊을 수 있는 구멍을 막는다).

### 도구바 구성 (포털만 — 관제향 `SC-005` 무변경)

포털 좌측 도구바는 「그리기」(선택·바운딩 박스·폴리곤) / **「AI 보조」(AI 탐지 · AI 분할 · AI 자동
추적)** / 「보기」(회전·화면 맞춤·영역 확대·그리드) 세 묶음이다. AI 탐지 팝업은 실행 버튼 **하나**
(「탐지 실행」)로 정리하며, 관제향의 [일반]/[트랙] 두 버튼과 「후속 프레임이 없어 추적할 수 없습니다」
안내는 포털에 두지 않는다(구 [트랙]은 실행이 아니라 도구 전환 + 안내 토스트였다 — 중복 진입). AI
자동 추적은 우측 「객체」 탭의 자동 추적 패널이 실행·검토의 유일한 자리다. 속성 패널의 「선택 객체
AI 추적」은 계속 전달하지 않는다(자동 추적과 기능 중복이라 미제공 유지). 이 UI 정리는 **포털만
먼저**다 — 관제향 흐름은 이번에 바꾸지 않고 별도 CO 로 넘긴다.

### 배치 오토라벨링 파이프라인은 여전히 없다

*"마킹 완료하고 오토라벨링 파이프라인은 사용하지 않음"*(사용자 재확인) — 업로드 영상은 마킹 완료 →
마킹 위치 프레임 추출 → 라벨링에서 끝나며, 마킹 완료가 YOLO/SAM2 자동 실행을 트리거하지 않는다(→
16.5a). 이 확정이 여는 것은 **사용자가 라벨링 화면에서 직접 누르는 온디맨드 AI 보조**뿐이다. 두 축을
한 낱말 「오토라벨링」으로 묶지 말 것.

### 보안 판정 반전의 논거

2026-08-03 보안 2차 전수검증이 포털 SAM2 를 HIGH 로 제거한 근거는 *"게이트 없이 원본 픽셀을 추론
서버로 보낸 유출 경로"* 였다. 이번 창구는 인가(포털 작업 대상 판정)와 입력 이미지(포털이 이미
사용자에게 보여주는 파일 — 서빙과 같은 판정기)를 묶어 그 유출 경로를 재생하지 않는다. HIGH 판정
자체를 되살리는 것이 아니라, 그 판정의 전제였던 「게이트 없음」이 이번에는 성립하지 않는다.

### 알려진 한계

본인 업로드 자산의 라벨 저장(전체교체)은 기존 규칙상 라벨 마스터 연결·트랙 번호를 저장하지 않고
라벨명만 남긴다(`PortalUploadLabelService` — 마스터 역추정 금지 규약). 따라서 **업로드 자산에서 AI
자동 추적 결과의 트랙 묶음은 작업본에서만 유지되고 저장 후 재조회하면 사라진다.** 데이터마트 자산
(오버레이 저장)은 trackId·labelId 를 저장하므로 이 한계가 없다.
...
> - 회귀 가드(당시): BE `PortalSam2RemovedTest`(404 + 핸들러 매핑 0건 + 내부 매핑 잔존), FE
...
>
> ⚠ **이 절은 사실 기록으로 남기며 지우지 않는다 — 다만 2026-09-15 에 위 §16.3b 가 이 절의 결론 일부를
> 반전시켰다.** **AI 분할(`sam2-segment`)·AI 자동 추적(대응하는 `yolo-track`)은 포털 전용 창구로
> 재도입됐다** — 이번은 2026-08-02 가 지운 그 `PortalSam2Controller`/`PortalSam2Service` 를 되살리는
> 것이 아니라 **다른 인가·입력 판정을 가진 신규 창구**다(§16.3b 「보안 판정 반전의 논거」). **그대로 남는
> 것**은 **키포인트(SKELETON) 제거**와 **선택 객체 AI 추적(`sam2-track` — 포털 대응 창구 없음, 404 유지)**
> 뿐이다. `PORTAL_HIDDEN_TOOLS` 는 `[TRACK, KEYPOINT]` 로 좁혀졌다(`SAM_SEGMENT` 가 빠졌다).
...
> ⚠ **구 서술 폐기(2026-09-15)** — *"남은 구 근거 'SAM2 자동추적의 trackId 는 FE 세션 opaque 로 미저장'은
> 그대로 유효하다 — 포털에서 SAM2 자동추적 자체가 제거돼 그 경로로 트랙이 생기지 않는다"* 는 **더 이상
> 사실이 아니다.** §16.3b 확정으로 AI 자동 추적(`yolo-track`)이 포털에 재도입돼 그 경로로 트랙이 다시
> 생긴다. **이 절의 결론(트랙 rename/머지 UI·API 미제공)은 바뀌지 않는다** — AI 자동 추적이 만드는
> trackId 는 편집(번호 변경·병합·삭제·분할) 대상이 아니라 자동 추적 결과 그 자체이며, 데이터마트 자산은
> 오버레이에 저장돼 보존되고 업로드 자산은 저장 후 사라진다(§16.3b 「알려진 한계」).
...
| `POST /v1/portal/frames/{srcSn}/autolabel` | **AI 탐지 — 온디맨드**(2026-09-15 신설). 상세는 §16.3b | 200 / 400 / 403 / 404 / 429 |
| `POST /v1/portal/frames/{srcSn}/sam2-segment` | **AI 분할 — 온디맨드**(2026-09-15 재도입 — 2026-08-02 제거된 `PortalSam2Controller` 를 되살리는 것이 아니라 다른 인가·입력 판정을 가진 신규 창구. 상세는 §16.3b) | 200 / 400 / 403 / 404 / 429 |
| `POST /v1/portal/frames/{srcSn}/yolo-track` | **AI 자동 추적 — 온디맨드**(2026-09-15 신설). 상세는 §16.3b | 200 / 400 / 403 / 404 / 429 |
| `GET /v1/portal/ai-defaults` | AI 정밀도 기본값 + 대기 예산(2026-09-15 신설). 상세는 §16.3b | 200 |
| `POST /v1/portal/ai-requests/{requestId}/cancel` | 포털 AI 취소(2026-09-15 신설). 상세는 §16.3b | 200 |
| ~~`POST /v1/portal/frames/{srcSn}/sam2-track`~~ | **선택 객체 AI 추적 — 여전히 없음.** 2026-08-02 제거(ADR-013 정합)를 반전하지 않는다 — 관제향 AI 탐지 팝업 [트랙] 버튼에 대응하는 이 기능은 자동 추적과 중복이라 포털에 두지 않기로 확정(§16.3b) | 404 |
...
## 16.4b 포털 데이터셋 영상 등록 → 목록 → 라벨링 (2026-09-15 신설, `ADR-068` · `API-253`)

> 흐름: **포털 학습데이터 상세 → 저작도구 데이터셋 소재 화면(SCREEN-046) → 영상을 골라 라벨링(SCREEN-029) → 저장은 포털 전용 오버레이에만.**
> 소재 가져오기(API-248·249)가 배포 압축본을 작업영역에 풀면, **같은 백그라운드 작업이 이어서** 해제본의 영상·프레임·기존 라벨을 원장에 등록한다.

| Method · URL | 설명 | 응답 |
|---|---|---|
| `GET /v1/portal/datasets/{datasetId}/videos?page&size` | 데이터셋에서 등록한 영상 목록. `registrationState`(IN_PROGRESS·DONE·FAILED) + **`registrationFailureReason`(FAILED 일 때만 값 — 2026-09-16 · `API-253` v3)** + 페이지 + 항목 `rawSn`·`videoName`·`frameCount`·`labelCount`·`entrySrcSn`·`lastSavedAt`. 뒤 둘은 **요청 사용자 기준**(내 작업 목록과 같은 판정) | 200 / 401 / 403(포털 회원 아님) / 409(소재 미준비) |
| `POST /v1/portal/datasets/{datasetId}/registration` | **등록 재착수(2026-09-16 신설 · `API-262`)** — 표식이 FAILED 일 때만 등록을 다시 착수하고 IN_PROGRESS 로 답한다. DONE 이면 다시 하지 않고 DONE, 진행 중이면 새로 시작하지 않고 IN_PROGRESS(멱등). 본문 없음, **언제나 200 즉답**(조달 착수 202 와 다르다). 응답 `{registrationState, registrationFailureReason}` | 200 / 401 / 403 / 409(소재 미준비·등록 토글 off) / 503(대기열 포화 — 선점 되돌림) |

**등록 규칙**
- **해제본 구성은 개발망 실물 넷(5145·5148~5150)으로 확인했다(2026-09-16 · `ADR-068` v6)** — 프레임 이미지 `NNNN.jpg` 와 같은 이름의 라벨 문서 `NNNN.json` 이 **평평하게 짝**으로 놓이며 영상 폴더·버전 폴더·비식별 폴더가 없다. 해제본을 재귀로 훑어 짝을 찾고, 문서의 영상 파일명이 같은 짝을 한 영상으로 묶는다. **`orgnl/` 은 읽지 않는다**(원본 비노출 — 그런 자리가 들어오는 날 조용히 열리지 않게). 짝이 없거나 문서를 읽을 수 없으면 추측으로 채우지 않고 **등록 실패(FAILED)**.
  ⚠ **구 서술 폐기** — *「`{영상 키}/v{n}/deid/NNNN.jpg` 3단 구조로 가정 · 버전이 여럿이면 가장 큰 n」* 은 우리 승인 산출물 구조를 그대로 가정한 것이었고 실물은 그보다 단순하다. 되살리지 말 것.
- **문서 스키마는 배포본마다 갈린다 — 아는 모양을 모두 읽는다.** 칸마다 기존 자리를 먼저 보고 없을 때만 새 자리를 본다: 프레임 번호 `image.frame_no`/`frame_num` → 최상위 `frame_no` · 크기 `image.width`/`height` → `resolution:[w,h]` · 라벨 배열 `annotations` → `objects` · 분류 이름 `category` → `class`. **라벨 0건 프레임(`"objects": []`)은 정상**이며 실패가 아니다. 실물이 함께 싣는 `confidence`·`camera_id`·`captured_at`·`lat`/`lon` 등은 읽지 않는다(앉을 자리가 없다 — 필요해지면 설계에 먼저 적는다).
- **문서에 영상 파일명이 없으면 배포본 하나를 영상 하나로 본다** — 영상 키는 해제본 옆 요약의 **배포 코드·버전**으로 만들고(`{코드}_{버전}`, 허용 글자 밖은 밑줄 · 클립 식별자 폭에 맞춰 절단), 둘 다 없으면 데이터셋 번호를 쓴다(키를 비우지 않는다). 조달은 등록 경로가 하고 파서는 만들어진 키를 받기만 한다. ⚠ 인지·수용 — 한 배포본에 영상이 여럿이면 전부 한 영상으로 합쳐진다(문서에 가를 값이 없다 · 그때 포털에 문서 구분자를 요구).
- 등록 메타에 **배포 코드·버전**(`portal.dataset_code`·`portal.dataset_version`)을 함께 남긴다 — 포털 정리 삭제 트리거가 그 두 값으로 오기 때문이다. 옛 데이터라 비어 오면 그 키를 쓰지 않는다.
- 영상 원장 출처는 **`SRC_TYPE='PORTAL_DATASET'`**(개인정보 유형 미상 · 비식별 여부 Y · 소유자 없음). 클립 식별자 `PORTAL_DATASET_{datasetId}_{영상 키}` 로 **멱등**.
- **검수 상태(APPROVED)를 꾸며 넣지 않는다** — 넣으면 관제 조회 뷰·통지·산출물 연동이 반응한다. 검수 워크플로 상태 행도 만들지 않는다. ⇒ 포털향에는 검수·승인 흐름이 없어 데이터셋 영상은 **승인이 아니라 출처로** 작업 대상이 된다.
- 프레임 이미지는 **비식별 프레임 영역(`frames/deid/{rawSn}/…`) 으로 복사**한다 — 프레임 이미지 서빙 규약이 그 영역 밖 파일을 허용하지 않기 때문이다. 원본 경로는 비워 둔다.
- 원본 라벨은 사각 박스·폴리곤만 옮기고 키포인트는 건너뛴다. 라벨 마스터는 식별자가 우리 활성 라벨일 때만 잇는다(이름 역매핑 금지).
- 데이터셋 ↔ 영상 연결은 영상 메타 `portal.dataset_id`·`portal.dataset_video_key` — 새 테이블 없음.
- 등록 상태는 해제본 옆 상태 파일(`registration.json`)이다. 목록 조회 때 표식이 없거나 진행 중 표식이 10분 넘게 멈췄으면 **다시 시작**한다(멱등). 토글 `authoring.portal.materials.registration.enabled`(기본 true).
- **★FAILED 표식은 목록 조회가 다시 시작시키지 않는다 — 사람이 「다시 등록」으로 재착수한다 (2026-09-16 · `ADR-068` v7 · `CO-20260916-포털-데이터셋등록-실패-재착수`)**. 실사고: 5149·5150 이 옛 코드에서 `VIDEO_FILENAME_MISSING` 으로 남긴 FAILED 표식이 파서를 고쳐 배포한 뒤에도 그대로 남아, 서버에서 `<PORTAL_STORAGE_PATH>/materials/{id}/current/registration.json` 을 손으로 지워야 풀렸다. 그래서 재착수 창구를 두었고, 목록 응답에 실패 사유를 실어 화면이 사유를 문구로 보인다. **자동 재시작은 두지 않는다** — 구조 불일치는 재착수해도 같은 사유로 다시 실패하고, 자동으로 되풀이하면 확정 실패에서 끝없이 돈다. 같은 답이 돌아올 사유(구조 불일치 계열)에서는 화면이 버튼을 보조 위계로 낮추고 「같은 소재로는 같은 결과입니다」를 말한다. ⚠ 등록 토글이 꺼진 채 표식이 없으면 FAILED 인데 사유가 null 이다(인지·수용 — 화면은 폴백 문구).
- 내부 작업 범위(작업보드·배정·통계·증강 조회)는 `PORTAL_ULD` 와 함께 `PORTAL_DATASET` 도 뺀다.

**포털 작업 가능 판정 = 검수 승인 영상 ∨ 데이터셋 등록 영상** — 프레임 라벨 조회·이미지·저장·ZIP 내려받기가 한 판정을 쓴다. 비식별 누락 신고 게이트는 그대로 함께 걸리고, 거부 문구는 「포털에서 작업할 수 없는 영상입니다.」 하나다. 구 데이터마트 영상 목록은 여전히 승인 영상만 싣는다.
ZIP(§16.4a)은 데이터셋 영상에 동결 메타가 없으므로 문서의 영상·이미지 블록을 **등록 메타와 프레임 원장 값**으로 채우고 없는 값은 비운다. 영상 파일은 없어 묶음에서 빠진다.

**알려진 공백** — ~~등록 실패 뒤 재시작 경로 없음~~(2026-09-16 재착수 창구로 해소) · 같은 데이터셋의 새 버전은 다시 등록되지 않음 · 오래된 등록 행 정리 없음.

⚠ **AI 보조(탐지·분할·자동 추적)는 데이터셋 영상에도 열린다** — 포털 AI 보조 창구의 진입 인가가 위 작업 가능 판정을 그대로 거치기 때문이다(`ADR-068` · `AC-1120`).
...
- PortalLayout (모바일 친화, **좌측 메뉴 미보유** — 2026-09-02 확정). 머리 영역과 좌측 주 메뉴는
  **Host 가 둘 다 그리고** 저작도구는 그 아래 본문만 그린다. 목적지 셋은 **본문 상단 이동 탭**으로
  ...
  (Host 머리 영역이 보여 준다). 라벨링 화면(`SC-029`)과 마킹 화면(`SC-045`)은 목적지가 아니라
  목록에서 행을 눌러 들어가는 자리이며, 몰입 편집 화면이라 그 탭도 노출하지 않는다.
...
- 포털 라벨링 화면은 LabelingPage 재사용 + `portalMode` 분기: 데이터 경로(라벨/이미지/저장)는 포털 전용 API.
  ★**이 화면은 2026-09-02 로 통합돼 데이터마트 자산과 본인 업로드 자산을 함께 연다**(16.5b) — **저장처는 화면이
  가르지 않고 서버가 자산 출처로 판정**하며, **내려받기·내보내기는 이 화면에서 빠져 내 업로드 목록(`SC-033`)이
  가져갔다.**
  라벨 저장은 포털 user-label(`LS_PORTAL_USER_LABEL`) 단방향(내부 `/v1/frames/...` 미호출).
  **AI 탐지·AI 분할·AI 자동 추적은 「AI 보조」 도구바 묶음으로 노출**(2026-09-15 확정 — §16.3b,
  `PORTAL_HIDDEN_TOOLS` 에서 `SAM_SEGMENT` 를 제외해 `[TRACK, KEYPOINT]` 로 좁힘). **선택 객체 AI
  추적(단축키 Shift+T)·키포인트 도구(단축키 K)는 그대로 미노출**(단축키 도움말도 같은 단일 소스로
  게이팅). 검수제출·히스토리·비식별 신고는 계속 미노출(ADR-013). **배치 오토라벨링 파이프라인**(업로드
  영상 마킹 완료 → YOLO/SAM2 자동 실행)은 화면에 진입점 자체가 없다 — 온디맨드 AI 보조와는 다른 축(§16.3b).
  ⚠ **시계열 메타·촬영환경·프레임 설명·개인정보 판정·이벤트 어노테이션 패널이 미노출인 것은 근거가 다르다** — `ADR-013` v10 이 **제공으로 확정**했고 아직 만들지 않았을 뿐이다(포털 메타 API 0건). 그 미노출을 정책 근거로 인용하지 말 것. **Phase 10** — 트랙 rename/머지(연필 버튼)도
...
- **포털 홈 — 만료 예정일 + 작업 데이터 다운로드**(2026-08-18 신설, SCREEN-028): 카드에 본인 저장 라벨의
  ...
  - ★**다운로드 취소**(2026-08-18 신설) — 진행 중인 버튼 옆에 취소 조작이 나타나고(진행 중이 아닐 때는
    ...
    **그보다 앞에서** 사용자 취소만 갈라내 **오류 안내를 띄우지 않는다**(안 가르면 스스로 멈춘
    ...
    다시 받을 수 있는 상태로 돌아온다.
- **[화면 폐기 — 규약은 목록 화면이 승계] 포털 업로드 라벨링(PortalUploadLabelingPage, SCREEN-034) — 원본
  파일 다운로드 제한시간·취소** (2026-08-18 신설 · **2026-09-02 화면 폐기**): ⚠ **화면은 `SC-029` 로 통합되고
  내려받기·내보내기는 내 업로드 목록(`SC-033`)으로 옮겨간다**(16.5b). **아래 규약 자체는 폐기가 아니라 그 목록
  화면이 그대로 승계한다** — 대용량 원본이라는 성질이 화면을 옮긴다고 달라지지 않기 때문이다. 원본 파일(최대 5GB)은 형제 경로(데이터마트 묶음)와 **같은 결함**을 겪고 있었다 —
...
- **포털 업로드 목록(PortalUploadPage, SCREEN-033)**: **2026-09-02 로 내려받기·내보내기를 승계**하고, 「업로드됨」
  (=마킹 대기) 행에 **마킹 화면(`SC-045`) 진입 액션**을 갖는다. ★**라벨링 화면에서 마킹으로 되돌아가는 자리는
  두지 않는다** — 라벨링을 하고 있다는 것은 이미 추출이 끝났다는 뜻이고 그 시점의 재마킹은 만들지 않기로 한
  기능이라(16.5a), 진입 자리를 두면 눌러 봐야 거절되는 자리가 되어 회복 경로를 잘못 안내한다.
- **포털 업로드 목록(PortalUploadPage) — 만료 표기**: 각 자산에 만료 예정일을 **날짜까지만** 병기한다(SCREEN-033).
...
## 16.5c 포털 모습 이관 — 부모 포털의 저작도구 시안을 입힌다 (2026-09-16, 구속)

부모 포털(`KLID_Portal`)이 **우리 화면을 자기 부품으로 다시 그려** 「저작도구 쪽에 넘기는 기준」으로
삼았다(`pages/workspace/authoring`). 그 짜임을 포털 채널 화면에 입혔다. **바뀐 것은 부품과 짜임이고
데이터·동선·창구 판정은 그대로다.**

### 부품을 어떻게 들여왔나 — 별칭이 아니라 복사

- `components/portal/kit/` — 부모 포털 `components/custom` 에서 저작도구 화면이 실제로 쓰는 **폐포 24종**
  (딸린 CSS 16). 바깥 의존은 `krds-react`·`lucide-react`·`react` 뿐이라 끌려오는 것이 없다.
- `components/portal/authoring/` — 편집 면 표시 부품 16종(VideoStage · MarkTimeline · ObjectList · ToolList …).
- `styles/portal/` — 토큰 CSS 6벌 + 화면 짜임 CSS 4벌. `styles/portalLook.ts` 가 **부모 포털 진입점과
  같은 목록·같은 차례**로 싣는다.
- `krds-react@1.1.1` 설치(부모 포털과 같은 판).

★ **별칭(`@portal/*`)으로 부모 저장소를 가리키지 않는다.** 젠킨스는 이 저장소만 체크아웃해
`npm run build:portal` 을 돌리므로 별칭은 **배포에서 깨진다**. 대가(부모가 부품을 고치면 이쪽이 낡는다)는
인지·수용했다. ⚠ **킷·부품 사본을 고치지 않는다** — 고치면 다음 동기화에서 되돌아온다. 우리 쪽에서만
바꿔야 하는 것이 생기면 그 폴더 **밖에** 새 부품을 둔다.

### 채널 격리 — 관제 산출물에 킷 CSS 가 실리지 않는다

킷 CSS 는 `html { font-size: 62.5% }` 를 **전역에 깔아 관제 축 Tailwind 치수를 전부 줄인다.** 그래서
`portalLook` 은 어디에도 정적으로 매달지 않고 **포털 셸(`PortalLayout`)을 라우터가 지연 로드**한다 —
그 호출이 `IS_PORTAL_CHANNEL_BUILD`(산출 시점에 접히는 상수) 가지 안에만 있어 관제 빌드에서 사라진다.

> 실측(산출물 대조): 관제 `dist` 에 `62.5%`·`krds-badge`·`klid-authoring` **각 0건** /
> 포털 `dist` 에는 전용 청크 `PortalLayout-*.css` 로 실린다.

⚠ **예외 하나** — 관제·포털이 **함께 쓰는** 라벨링 화면이 포털 메타 탭을 정적으로 import 해, 그 탭이
끌어오는 킷 CSS 약 57KB 가 관제 라벨링 청크에도 실린다. **관제 화면의 모습은 바뀌지 않는다**(실린 규칙이
전부 클래스로 좁혀져 있다 — `:root` 토큰 정의 0건 · 태그만으로 된 선택자 0건, 산출물 실측).
⚠⚠ **지연 로드로 덜어내려다 되돌렸다 — 다시 시도하지 말 것.** `lazy()` 로 바꾸니 묶음이 갈리는 대신
**공유 청크로 옮겨 가 관제 전 화면이 내려받게 되어 오히려 나빠졌다**(두 형상 모두 실측).

### 시안과 갈리는 자리 — 접근성은 되살린다

시안을 따르되 **시안이 잃는 접근성은 함께 잃지 않는다.**

| 자리 | 시안 | 우리 |
|---|---|---|
| 이동하는 조작 | 킷 `Button` | `Button as={Link}` + **`role="link"`** |
| 잠긴 조작의 사유 | 말풍선만 | 말풍선 **+ 화면 밖 글**(`aria-describedby`) |
| 잠금 | `disabled` | **`aria-disabled`**(초점을 남겨 사유가 닿게 — WCAG 2.1.1) |
| 받는 중 표시 | 취소 버튼만 | 취소 버튼 + **화면 밖 `role="status"`** |

★ **킷 `Button` 은 `role` 기본값이 `"button"` 이라 앵커로 세워도 덮어쓴다.** 주소·가운데 클릭은 살아
있는데 보조기술에는 「버튼」으로 읽혀 **눈으로는 안 보이는 결함**이 된다. 회귀 가드
`src/test/portalKitLinkRole.test.ts` 가 이것을 막는다.

⚠ **킷 말풍선 본문은 `aria-hidden`** 이라(실측) 그것만으로는 보조기술에 닿지 않는다 — 같은 문구가
문서에 두 번 나오는 것은 **의도**다(하나는 눈, 하나는 귀).

### 우리 쪽에만 있는 사실은 지킨다

- 내려받기 문구가 **받는 형식을 밝힌다**(ZIP / JSON). 시안은 그쪽 목 데이터에 형식 차이가 없어
  「내려받기」 한 마디다.
- 「대기」 글리프는 **`Clock`** 이다. 시안은 모래시계지만 이 저장소는 그 축을 `Clock` 으로 통일했고
  시계도 *"실패가 아니라 기다리는 중"* 을 그대로 전한다(회귀 가드 `iconInventoryGuard`).

### 아직 적용하지 못한 것 — 관제 코드 소유

포털 라벨링 화면은 **관제 라벨링 화면에 위임하는 얇은 껍데기**라, 시안의 편집기 틀(`EditorLayout`·도구
레일·캔버스·프레임 띠)과 **전체 화면 안내 넷**(잘못된 프레임 · 업로드 자산 안내 · 불러오는 중 · 들어갈 수
없음), 객체/메타 탭 모양은 **관제 파일이 소유**해 적용하지 못했다. 관제향 불변이 구속이라 손대지 않았고,
적용하려면 그 결정이 먼저 바뀌어야 한다.

## 16.5a 업로드 영상 마킹 화면 (2026-09-02 신설, 구속 · `SCREEN-045`)

포털 사용자가 **본인이 올린 영상에 이벤트 구간을 마킹하고, 그 마킹 위치로 프레임을 뽑는다.**
경로는 라벨링 화면의 형제 자리(`/portal/uploads/:uldSn/marking`)이고 창구는 `API-238`~`API-241`
넷이다 — 영상 스트리밍(구간 요청) · 스트리밍 서명 주소 발급 · 마킹 저장(추출을 함께 시작) · 마킹 조회(재진입).
⚠ **넷 다 기존 포털 축에 하나도 없던 창구다** — 내려받기 창구는 있으나 그건 첨부 다운로드라 재생에 못 쓴다.

**확정 다섯**: ①마킹은 **별도 화면**이다(라벨링 화면 안의 모드가 아니다) ②마킹의 쓰임은 **프레임 추출
위치 지정**이다(외부 시계열 위탁·검수와 무관) ③**자동·수동 둘 다** 제공한다 ④**마킹을 거쳐야 프레임이
추출된다** ⑤저장은 **기존 마킹 원장 재사용**이다(포털 전용 표를 만들지 않는다 — 16.7).

### ★파이프라인 순서가 바뀐다 (가장 중요한 귀결)
...
- ⚠ **구 서술 폐기(2026-09-02)** — *"업로드한 영상은 곧바로 고정 간격으로 프레임을 추출한다
  (`LS_SYSTEM_CONFIG` `portal.upload.frame-interval-sec` 기본 5초)"* 는 **더 이상 사실이 아니다.**
  그 동작은 사라진 것이 아니라 **자동 마킹의 기본 동작으로 흡수**됐다 — 사용자가 간격을 그대로 두면
  **구 동작과 같은 결과**가 나온다. 「업로드하면 곧바로 추출된다」로 되살려 읽지 말 것.
- ★**「업로드됨」 상태의 뜻이 바뀐다** — 「추출을 기다리는 중」이 아니라 **「마킹을 기다리는 중」**이다.
  목록 화면(`SC-033`)은 그 상태의 행에 **마킹 액션**을 두고 안내 문구도 그에 맞춘다.
- 이 순서를 고른 이유는 **낭비 추출도 재추출도 없기 때문**이다 — 마킹이 추출보다 앞서므로 뽑았다
  버리는 프레임이 생기지 않고, 이미 붙은 라벨을 어떻게 할지 정할 일도 없다.

### 내부 마킹 화면에서 가져오는 것과 버리는 것

내부 채널 마킹 화면(→ [06 마킹](06-marking.md))을 원본으로 삼되 **포털에서 성립하지 않는 축을 걷어낸다.**

| 가져온다 | 버린다 | 왜 버리나 |
|---|---|---|
| 영상 재생(구간 탐색·배속) | **비식별 영상 서빙** | 본인 데이터라 비식별을 하지 않는다 — 원본을 재생한다 |
| 마킹 타임라인 | **비식별 누락 신고** | 마스킹을 한 적이 없어 빠졌다고 신고할 대상이 없다 |
| 자동·수동 모드 전환 + 간격 입력 | **검증 이벤트 유형·질문 선택** | 질문은 검증 이벤트 유형에 매이고 그 유형의 조달처는 관제 인입인데 이 경로엔 그 값이 없다. ⚠ 내부 마킹은 2026-09-07 부터 **관제 미수신 영상에서 작업자가 유형을 직접 고르게** 하지만(→ [06 §6.3-2](06-marking.md)) 그 선택의 쓰임은 **외부 시계열 위탁**이라 위탁이 없는 이 경로에는 성립하지 않는다 |
| 마킹 목록·선택 | **위탁 배치 기동** | 포털에는 외부 시계열 위탁이 없다 |
| 단축키 | **작업 배정·검수 연계** | 포털에 검수가 없다 — 인가는 소유자 판정 하나 |

**진입 차단 축도 갈아끼운다** — 내부는 「비식별 완료 + 배치 단계」 2축인데(→ [06 §6.3-1](06-marking.md)),
포털은 **「업로드 완료 + 본인 자산」** 이다.
⚠ `<video>` 가 인증 헤더를 못 싣는 제약은 채널과 무관하므로 **서명 주소 발급 방식은 그대로 필요**하다.

### 회복 경로 — 지우고 다시 올린다 (구속)

**재마킹·재추출 기능을 만들지 않는다.** 마킹을 잘못했으면 **자산을 지우고 다시 올린다.** 본인이 올린
자산이고 보존기간도 짧아 재업로드 비용이 기능을 새로 만드는 비용보다 싸다.

- **이미 추출이 끝난 자산의 재마킹 저장은 거부**하고, 거부 사유에 회복 경로(재업로드)를 밝힌다.
- ★**「마킹 완료」가 되돌릴 수 없는 조작이 된다** — 누르는 순간 추출이 시작되고 회복 경로가 재업로드뿐이다.
  그래서 **완료 직전 확인 단계는 선택 사항이 아니라 이 설계의 필수 조건**이다.
- ⚠ **마찰 하나를 인지·수용한다** — 자산 삭제는 **추출 중에는 거부**되므로, 마킹을 잘못한 사용자는
  **추출이 끝날 때까지 기다렸다가** 지우고 다시 올려야 한다. 그 대기 구간에 무엇을 보여 줄지가 안내에 있어야 한다.

### 자동 마킹 간격의 단위 — 프레임 수, 기본 300 (구속)

**저장·전송의 단위는 프레임 수이고 시간이 아니다.** 근거 셋: ①**수동 마킹의 지점이 이미 프레임 단위**라
자동만 시간 단위로 두면 한 화면 안에서 단위가 갈린다 ②재사용하는 **마킹 원장의 컬럼이 프레임 간격**이다
③시간으로 받으면 초당 프레임 수로 나눠야 하는데 **그 값이 영상마다 달라 같은 입력이 영상마다 다른
저장값**이 된다.

- **기본값은 300 프레임이고 사용자가 바꿀 수 있다.** 내부 채널 자동 마킹과 같은 값이라 두 채널이 같은
  눈금에서 시작하고, ⇒ **기존 시간 단위 운영 설정에서 환산할 필요가 없어졌다**(환산 규칙 미결이 함께 닫혔다).
- 사용자가 바꾼 값은 **그 영상의 마킹에만** 적용되며 **운영 기본값을 바꾸지 않는다.**
- ⚠ **인지·수용한 대가** — 운영 설정이 시간 단위에서 프레임 수로 바뀌고, **초당 프레임 수가 다른 영상들
  사이에서는 같은 설정이 다른 시간 간격**을 뜻한다. 그래서 화면에는 **현재 영상 기준 환산 시간을 병기**한다.

### 지점 수가 추출 장수 상한을 넘으면 — 절단

**거부하지 않고 상한까지만 뽑는다.** 긴 영상에서 촘촘한 간격을 고르면 상한을 넘기 쉬운데, 그때 통째로
거부하면 사용자는 **몇으로 줄여야 하는지 모른 채 시행착오**를 해야 한다.

- ⚠ **절단이 조용히 일어나면 안 된다** — 완료 확인 창이 이미 「뽑힐 프레임 수」를 알리므로, **상한에 걸려
  잘린다는 사실과 실제로 뽑히는 장수**가 그 자리에 함께 드러나야 한다.
- **남기는 쪽은 모드마다 다르다** — **자동은 전 구간에서 고르게 다시 고르고, 수동은 앞에서부터** 채운다.

### 아직 안 정한 것

- 서명 주소의 **클라이언트 바인딩 수단** — 임베딩 환경에서 그 값이 요청에 실리는지 미확인이라, 현재는
  잔여 위험(만료 전 재사용)을 드러내는 데까지만 와 있다.
- 같은 포털 채널 안에서 **삭제는 브라우저 확인창, 마킹 완료는 화면 안 확인 창**이라는 비대칭.

## 16.5b 라벨링 화면 통합 — 포털 라벨링 화면은 하나다 (2026-09-02 확정, 구속)

포털에 라벨링 화면이 **둘**이었다. 작도 도구·미제공 범위·캔버스 코어는 **같았고** 갈린 실질은 「자산
출처」 하나였는데, 그 축이 **메타 다섯 축의 유무**까지 갈라 놓은 것이 어긋난 지점이었다 — 촬영환경·프레임
설명은 본인이 올린 영상에도 붙을 수 있어야 하고, 마킹이 붙으면 더 그렇다.

| | 구 포털 라벨링 `SC-029` | 구 포털 업로드 라벨링 `SC-034` |
|---|---|---|
...
1. **라벨링 화면은 하나다** — 자산 출처와 무관하게 같은 화면(`SC-029`)에서 라벨링한다.
2. **저장처는 화면이 가르지 않는다** — **서버가 자산 출처로 판정**한다(데이터마트 자산은 포털 전용
   오버레이에, 본인 업로드 자산은 자기 자산에). **화면은 그 차이를 알 필요가 없다.**
3. **내려받기·내보내기를 라벨링 화면에서 뺀다** — **내 업로드 목록(`SC-033`)** 이 담당한다.
4. **메타 다섯 축은 통합 화면이 갖는다** — 자산 출처를 가리지 않는다.

- 통합의 바탕은 **기존 포털 라벨링 화면**이다(섹션이 더 완비돼 있고 메타 다섯 축을 이미 갖췄다).
  업로드 라벨링 화면의 고유 요소 중 **프레임 이동은 흡수**하고 **내려받기 두 가지는 목록으로 옮긴다.**
- **`SC-034` 는 폐기**된다 — 내비게이션에서 그 노드가 빠지고, **마킹 화면(`SC-045`)에서 라벨링으로 가는
  자리도 통합 화면을 가리킨다.**
- **경로가 하나로 준다** — 업로드 자산도 이미 공용 원장에 앉으므로 식별자 체계가 같다. **자산 식별자
  하나로 두 출처를 모두 연다.**
- ⚠ **구 서술 폐기(2026-09-02)** — *"「포털 라벨링」과 「포털 업로드 라벨링」은 다른 화면이다. 경로도
  데이터도 분리돼 있다"*(→ [10 §10.6](10-labeling.md))는 **더 이상 사양이 아니다.**
  ⚠ **구 서술 폐기(2026-09-15)** — *"다만 AI 보조의 유무는 여전히 자산 출처로 갈린다 — 업로드 자산에는
  오토라벨·분할·추적 진입점이 없다"*. **더 이상 사실이 아니다.** §16.3b 확정으로 AI 탐지·AI 분할·AI
  자동 추적 세 기능은 **자산 출처와 무관하게 두 출처 모두**에 제공된다(인가만 `PortalWorkTargetResolver`
  가 출처별로 가른다 — 데이터마트 노출 영상과 본인 업로드 자산 둘 다 통과). 다만 **저장 결과의 보존
  성질은 여전히 출처로 갈린다** — 업로드 자산의 AI 자동 추적 트랙 묶음은 저장 후 사라지고(§16.3b 「알려진
  한계」), 데이터마트 자산 오버레이는 보존한다.

**아직 안 정한 것**: 통합 화면이 **자산 출처를 사용자에게 보여줄지**(저장 방식이 달라 — 원본을 못 고치는
쪽과 고칠 수 있는 쪽 — 사용자가 알아야 할 수도 있다) · **데이터마트 자산에도 내려받기를 줄지**(지금은
없다. 목록으로 옮기면서 그 축이 드러난다).
...
- ⚠ **「업로드됨(`UPLOADED`)」의 뜻이 2026-09-02 로 바뀌었다** — 「추출을 기다리는 중」이 아니라
  **「마킹을 기다리는 중」**이다(→ 16.5a). 위 표의 「만료 없음」 규칙 자체는 그대로다. **다만 그 구간이
  이제 사람의 조작을 기다리는 구간이라, 아래 「방치된 업로드 자산 → FAILED 전이」와 어떻게 맞물릴지는
  이번 확정에 들어 있지 않다** — 근거가 없어 여기서 단정하지 않는다.
...
### 정리 삭제 트리거 수신 (2026-09-09 신설, `API-244`)

위 표의 두 축은 **저작도구가 스스로 시각을 재어** 지우는 것이다. 이 절은 축이 다르다 —
**포털이 「새 버전을 받았다」고 알려 오면** 그 신호로 구 해제본 정리를 깨우는 경로다.

`ADR-012` 가 정한 자산 전달 3단계(①관제 채널 승인 → 관제 통지 ②관제가 포털로 전달 ③**포털이
포털 채널 배포본에 알린다**)의 ③이며, 그 창구가 **`POST /v1/portal-system/dataset-cleanups`** 다.

| 항목 | 값 |
|---|---|
| 받는 것 | 데이터셋 코드 · 버전 |
| 응답 | **202**(접수) · 400(검증) · 401(인증) · 403(주체 축) |
| 인증 | **사전 공유 API 키**(`x-api-key`) — 사용자 세션과 무관한 시점에 도착하므로 토큰이 아니다 |
| 기산점 | 접수 원장 `LS_DATST_ARNGMT_TRGR.RCPTN_DT` — **처음 받은 시각** |

- **★재수신은 성공 무처리이고 기산점을 갱신하지 않는다.** 갱신하면 중복 수신이 반복될 때마다
  정리가 무한히 미뤄져 **영영 일어나지 않는다.** 판정은 조회가 아니라 **삽입문 자체**에 걸린다
  (`ON CONFLICT DO NOTHING` + `(데이터셋코드, 버전)` 유일 제약) — 조회한 뒤 삽입하면 두 트리거가
  같은 순간에 도착할 때 둘 다 통과해 행이 둘 생기고 기산점이 갈린다.
- **알지 못하는 데이터셋도 같은 202** 다. 오류로 답하면 포털이 지울 것도 없는 신호를 계속
  재전송하고, 대상 유무를 응답으로 드러내면 그것이 존재 여부를 탐색하는 수단이 된다.
  ⇒ **202 는 「접수했다」이지 「지울 대상이 있었다」가 아니다.**
- **받은 즉시 지우지 않는다** — 사용자가 그 해제본을 물고 저작하는 중일 수 있어 받자마자 지우면
  그 작업이 그 자리에서 깨진다. 유예 기간의 값은 이 창구가 아니라 **보존 규칙(`DFEAT-055`)** 이
  소유하며, 창구 코드에 일수 상수를 두지 않는다.

#### ★ 인증은 사전 공유 키다 — 2026-09-09 개정 (`INT-014` · `AC-1103`)

포털 회신으로 이 창구의 인증이 **사전 공유 고정 문자열**(`x-api-key`)로 확정됐다. 부르는 쪽이
사람이 아니라 포털 서버라 서명·만료·주체 클레임이 없다.

- **키가 아니면 401 이다 — 토큰을 함께 실어도 마찬가지다.** 이 경로에서는 앞선 인증을 지우고 키만
  보므로 「인증은 됐고 권한이 없다」(403)로 새지 않는다. 키가 틀렸다는 사실이 응답에 남는다.
- **키가 설정되지 않으면 창구가 닫힌다**(전건 401). 값이 없을 때 열리는 것이 아니라 **닫히는**
  방향이라 안전하다. 설정 키는 `PORTAL_CLEANUP_API_KEY`.
- ⚠ **관제지원 수신 채널이 쓰는 키와 섞지 않는다** — 연동 축마다 별도 키이며, 한 축이 유출돼도
  다른 축으로 번지지 않게 한다.
- 비교는 **상수시간**이다 — 일치하는 접두 길이에 따라 걸리는 시간이 달라지면 그 차이로 키를
  한 글자씩 좁혀 갈 수 있다.

⚠⚠ **[폐기] 「주체 축 격리는 양방향이다」는 이 창구에 더 이상 적용되지 않는다.** 그 규칙은 토큰에
주체 클레임이 있다는 전제 위에 있었는데 이 창구에는 토큰이 없다. **격리를 푼 것이 아니라 격리할
대상이 없어진 것**이다 — 그 규칙이 막던 위험은 *"시스템 계정 토큰이 포털 사용자 권한을 얻는다"*
였고, 그 토큰 자체가 이 축에서 사라졌다.
⚠ **사용자 컨텍스트 축(저작 현황 요약)에는 그 격리가 여전히 유효하다** — 거기는 토큰 축이다.
인증 수단이 **창구마다 갈린다**는 것이 이 연동의 현재 모습이며, 그 정본은 `INT-014` 다.

⚠ **같은 축이 두 번 뒤집혔다** — `INT-014` 는 한때 *"별도 API 키 헤더는 쓰지 않는다"* 를 규칙으로
갖고 있었고 그 근거가 포털 회신이었는데, 포털이 그 방식으로 되돌아왔다. **세 번째 반전이 오면
그 경위를 먼저 확인할 것.**

⚠ **일일 저작 집계 창구는 인증 수단이 미확정**이다(`API-243`) — 같은 접두를 쓰지만 이번 회신이
다루지 않았다. 그 창구가 생길 때 인가 배선을 갈라야 한다.

#### 아직 하지 않은 것 (범위 밖 — 결손이 아니다)

| 항목 | 상태 |
|---|---|
| **실제 해제본 삭제 실행** | 미구현 — 지울 대상인 해제본 저장소가 아직 서지 않았다. 현재는 트리거를 접수해 기산점을 남기는 데까지다 |
| **신규 데이터마트 조달**(포털 소재 조회 창구 호출 · 압축 해제 · 적재) | 미구현 — 그 창구는 **포털 소유이고 응답 스키마가 회신되지 않았다** |

⚠ **실동작 선행 조건 둘이 남아 있다** — 포털의 시스템 계정 식별자 통보, 공유 서명키 값 교환.
둘 다 배포 설정이라 구현을 막지는 않으나, 채워지기 전까지 이 창구는 전 환경에서 닫혀 있다.
대외 규격은 `docs/contracts/portal-dataset-cleanup-trigger-contract.md`.

## 16.7 관련 데이터 (DB)

`LS_PORTAL_USER_LABEL` (V47) — `PORTAL_USER_NO`, 원본 `LS_DATA_LBL` 미수정. → [18](18-database.md).

**마킹 원장 재사용 (2026-09-02 확정)** — 포털 업로드 영상의 마킹은 **기존 마킹 원장(`LS_MARKING`)에
그대로 앉는다.** 포털 전용 마킹 표를 새로 만들지 않으며, 자동 간격은 그 표의 **프레임 간격 컬럼**을 쓴다.
여기에 **사용자 식별자 폭 확대**가 따라붙는다(라벨 원장에 이미 같은 조치를 한 선례가 있다 — `ADR-058`).
→ [18](18-database.md)

> ⚠ **§16.3 의 「전용 저장소로 완전 분리」(2026-08-27)와 충돌하는 것으로 읽지 말 것** — `ADR-058`(2026-09-02)이
> **판정 기준을 「전용 표인가」에서 「원본을 덮어쓰지 않는 단방향 오버레이인가」로 옮겼다.** 마킹은 오버레이가
> 아니라 자기 자산의 속성이라 공용 원장에 앉고, 오버레이인 `LS_PORTAL_USER_LABEL`(과 신설 예정인 포털 전용
> 메타·이벤트 어노테이션 오버레이)만 전용으로 남는다. ⚠ **증강 파생물의 적재 위치는 이번 확정에 없다** —
> 마킹 결론을 그리로 옮겨 적지 말 것.

**`LS_DATST_ARNGMT_TRGR` (V37, 2026-09-09 신설)** — 포털이 보낸 정리 삭제 트리거의 접수 원장.
`DATST_CD`·`VER_NO`·`RCPTN_DT` 3칸(+PK)이며 `(DATST_CD, VER_NO)` 유일 제약이 **재수신 멱등의 근거**다.
⚠ **등록일시 컬럼을 두지 않은 것은 의도다** — 행이 생기는 시각과 트리거를 받은 시각이 같은 사실이라
나란히 두면 두 번째 진실원이 된다. → [18](18-database.md)

...
| **레거시 구간행** | `0-8` · `8-16` … (구 describe 산출물) | ⚠ **구 서술 폐기(2026-09-14 사용자 확정)** — *"읽기 전용 병기 — 삭제·숨김하지 않는다"* 는 더 이상 우리 규칙이 아니다. 그 값은 사람의 작업 결과가 아니라 **목서버가 만든 가짜 데이터**였다. **앞으로 만드는 화면은 이 부류에 대응하지 않는다.** 246 실측 결과 이 꼴의 행은 0건이었고(구간 문장은 `vlm.description` 본문 안에 있었다), 목서버가 만든 시계열 결과는 삭제했다 | 미포함 |
...
- **레거시 구간은 BE 계약상 편집 가능**(`items`)하지만 화면이 편집 동선을 주지 않는다. ⚠ *"값 보존은 확정 정책이라 **표시는 반드시 유지**한다"* 는 **2026-09-14 폐기** — 새 화면(시계열·이벤트 어노테이션 창)은 이 부류를 표시하지 않는다. 아래 정렬 규칙은 남아 있는 코드 경로를 읽을 때의 설명이다. 정렬은 `start_sec` **숫자** 오름차순 — 문자열 정렬이면 구간이 10개를 넘는 순간 `10-18` 이 `8-16` 앞으로 와 시간축이 깨진다.
...
>
> ⚠ **③ 레거시 구간 이어붙임은 근거가 사라졌다(2026-09-14)** — 그 값이 목서버가 만든 가짜 데이터로 판명돼 화면 대응을 폐기했고 246 에서도 지웠다. **다만 코드의 ③ 경로는 아직 그대로**이며, 제거하면 조달 순서가 ①→②→`null` 로 바뀐다 — 별건으로 판단한다(설계 먼저).

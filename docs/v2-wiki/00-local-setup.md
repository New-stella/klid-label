...
> 2. backend(2단계) 기동 전에 `export DEIDENTIFY_MOCK_MODE=false KPST_DEID_BASE_URL=http://localhost:9400 VLM_SERVICE_URL=http://localhost:9400 CONTROL_NOTIFY_URL=http://localhost:9400 AUGMENT_API_BASE_URL=http://localhost:9400 WEBHOOK_CALLBACK_BASE_URL=http://localhost:8080/api` 를 설정한다(mock-server 가 host 루프백에서 콜백을 backend 로 되돌려 보내야 하므로 `WEBHOOK_CALLBACK_BASE_URL` 도 `localhost` 로 맞춘다).

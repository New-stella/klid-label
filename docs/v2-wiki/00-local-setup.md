...
| VLM 시계열 | **목 서버로 실 HTTP 연동** | `VLM_SERVICE_URL=http://klid-mock-server:9400` + `VLM_ALLOW_INSECURE_URL=true`(평문·내부 호스트 완화, local/dev 전용). `verify` 위탁 → 목이 `/v1/vlm/callback` 으로 결과 콜백(`results` 는 **단일 객체** `{accuracy, description}`). ⚠ **구 서술 폐기(2026-08-19 — `ADR-049`)** — *"`VLM_CLIENT_ENABLED=false` 면 단계가 통째로 SKIPPED 되어 결과가 빈다"* 는 사실과 다르다. **토글 자체가 폐지**됐고 미연동 판정은 **주소 주입 여부**다 — 주소가 비면 검증을 생략해 **기동은 성공**하되 **위탁 시점에 실패**한다. 건너뛰려면 사람이 시계열 묶음을 스킵해야 하고 그 사실이 이력에 남는다. 완화 플래그 없이 평문·내부 주소를 채우면 `VlmUrlPolicy` 가 빈 생성을 막아 **기동이 실패**한다 |
...
| `VLM_SERVICE_URL` / `VLM_ALLOW_INSECURE_URL` | `http://klid-mock-server:9400` / true | VLM `verify` 위탁 + 콜백 수신. 완화 플래그는 local/dev 프로파일에서만 인정 — 그 밖(또는 `ENV=stg\|prd` 표식)에서 true 면 기동 실패. ⚠ `VLM_CLIENT_ENABLED` 는 **폐지**됐다(`ADR-049`) — 주소가 비어 있는 것이 곧 미연동이다 |

...
- **VLM 실추론 미지원** — 위탁·콜백 왕복은 목 서버(`klid-mock-server:9400`)로 실제 수행하되 응답 내용은 규격에 맞는 더미다(연동 경로 검증용이며 실 벤더 추론 결과가 아니다). 시계열 메타 패널에는 `vlm.description`(서술) + `vlm.accuracy`(일치도) 2행이 뜬다. ⚠ 구 서술("`describe` 구간이 영상 길이 전체를 덮어 구간 수·시각이 맞아떨어진다")은 **폐기** — BE 는 `verify` 만 호출하므로 구간 배열이 적재되지 않는다.

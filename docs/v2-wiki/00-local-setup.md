...
| **VLM `verify` 시계열** | BE 가 쓰는 경로는 `verify` 하나이며, 목은 고정 mock 값(`accuracy=0.8` + 이벤트 유형별 한국어 서술 1건)을 콜백한다. ⚠ 목의 `describe`(구간 배열, window 8초·최대 450구간·길이 3단 폴백) 시뮬레이터는 **남아 있으나 BE 가 호출하지 않는다** — 구 서술이 이 절을 `describe` 기준으로 적고 있던 것을 정정 |
...
- **시계열 실추론 미지원** — 위탁·콜백 왕복은 목 서버(`klid-mock-server:9400`)로 실제 수행하되 응답 내용은 규격에 맞는 더미다(연동 경로 검증용이며 실 벤더 추론 결과가 아니다). 위탁은 **묘사·추가 질문 두 창구로 각각** 나가며, 시계열 메타 패널에는 `vlm.description`(서술) 1행이 뜨고 추가 질문 결과는 이벤트 어노테이션의 답변 초안으로 들어간다. ⚠ 구 서술("`vlm.accuracy`(일치도) 행이 함께 뜬다" · "구간 배열이 적재된다")은 **둘 다 폐기** — 판정 창구를 연동하지 않으므로 일치도는 새로 생기지 않는다.

...
- **GET 응답은 수동값 우선 + 기본상수 프리필 + `*Source`(MANUAL/DERIVED) 병기**다. 기본상수를 화면이 하드코딩하지 않게 하려는 것이며, 상수의 단일 원천은 `ExportPrivacyPolicy.DEID_DEFAULT_*` 다.
  ...
  - "누가 실제로 손댔나"는 컬럼이 아니라 **`LS_TASK_EVNT_LOG` 감사**가 독립적으로 담당한다(INSERT 자동 채움은 감사를 남기지 않고, 사람의 PUT 만 남긴다).
...
- **★영상 축 변경은 행 단위로 감사된다** (2026-08-03 2차) — `LS_TASK_EVNT_LOG` 에 `PRIVACY_META_UPDATE`(저장) 1행(actor·시각·사유 고정 문구). **판단값(Y/N)은 남기지 않는다**(CWE-359 — 개인정보 유무 자체가 민감 신호이고 작업 이력 화면에 노출된다). 라벨 이력(`LS_DATA_LBL_HSTRY`)은 `SRC_SN NOT NULL` 인 프레임 스코프라 영상 축 행을 담지 못할 뿐이며, rawSn 스코프 감사 축은 원래부터 있었다.
...
| 영상 축 `LS_DATA_RAW.*_INCL_YN` | `changePrivacyMeta(null,null,null)` + `LS_TASK_EVNT_LOG PRIVACY_META_RESET` 감사 | **보존**(리셋 없음) |
...
- **`vlm.accuracy`(일치도)는 export 에 넣지 않는다** — 화면 전용이다(R12, [09 §9.4-1](09-vlm-timeseries.md)). ⚠ 그 값은 이제 **과거 적재분에 한한다** — 판정 창구를 연동하지 않으므로 새로 생기지 않는다. 기존 행을 지우지 않으므로 이 제외 규칙 자체는 그대로 유효하다.

...
- **결과 조회(`GET /v1/augments/{jobId}/result`, SC-023)는 해상도 파생의 프레임 비교쌍을 반환**한다 — 좌=원본 비식별 프레임 / 우=파생 프레임(둘 다 `/v1/frames/{srcSn}/deid-image` 경로만 노출, 스토리지 경로 미노출), `(RAW_SN, FRM_NO)` 동등 조인 + 기본 12장 페이징. **외부 위탁 증강(WINTER/NIGHT/RAIN)은 여전히 빈 배열**(외부 SFR-07 연동 이후 제공). 파생↔프리셋 판별은 **파생 영상이 스스로 보유한 `LS_DATA_RAW.AUG_TYPE_CD` 컬럼 단일 원천**이다(V148 신설 + V149 백필, 신규 파생은 생성 팩토리가 채움) — `VMS_CLIP_ID` 포맷(이중 접두 `_RESL_RESL_480P_`·구형 `_RES_RES_480P_` 등 실데이터 드리프트)은 판별에 관여하지 않는다. 구 중앙 파서 `video/util/AugTypeParser` 는 제거됐다. 코드: `augment/service/AugmentResultViewService`

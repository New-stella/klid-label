...
- 위탁 유형은 **`AUGMENT` 단일값**이다 — 무엇으로 바꿀지는 아래 **생성 조건 5항목**(시간대·계절·날씨·지형·심각도)이 정한다. 해상도 변경(RESOLUTION)은 §14.3 내부 수행 — 단, 2026-07-21부터 처리 결과 자체는 증강과 동일하게 새 파생영상(RAW_SN)을 생성한다(외부 위탁 여부만 다름)
  > ⚠ **구 서술 폐기(2026-09-02 · `ADR-059`)** — *"위탁 유형 3종: **WINTER / NIGHT / RAIN** (날씨·계절·시간)"* 는 사실과 다르다. 세 값은 단일값 `AUGMENT` 로 합쳐졌고, 겨울·야간·우천은 처리 종류가 아니라 **생성 조건 프리셋**으로 제시된다(§14.6). 처리 종류는 **「증강 AI」와 「해상도 변경」** 둘이다.
  > ⚠ **이미 만들어진 파생본의 구 코드값(`WINTER`·`NIGHT`·`RAIN`)은 백필하지 않고 보존한다** — 따라서 조회·표시·정렬 경로는 **옛 값과 새 값을 모두 견뎌야 한다**. 이 사실을 지우지 말 것.
- **★이벤트 유형은 요청자가 고르지 않는다 — 서버가 중립값 `ETC` 를 고정 송신한다 (2026-09-02 사용자 확정, 구속 · `ADR-059`)** — 증강 요청 화면에서 **이벤트 유형(침수/산불) 선택과 침수 세부 유형이 제거**됐고, 세부 유형은 **전송하지 않는다**. ★`ETC` 는 **벤더와 협의가 끝난 값**이다. ⚠ 다만 우리가 가진 규격서 판본(**v1.3**)에는 아직 그 값이 없어 `evnt_type` 허용값이 **`FLOOD | WILDFIRE`** 로 적혀 있다 — **개정판을 받으면 대조**해야 한다. 포털 경로의 같은 미결 항목도 이 확정으로 닫힌다 → [16](16-portal.md).
- **표시 정렬 순서(`AUG_ORDER`)** — `AUGMENT` → `WINTER` → `NIGHT` → `RAIN` → `RESOLUTION` → `RESL_1080P` → `RESL_720P` → `RESL_480P`. 앞쪽이 증강 축, 뒤쪽이 해상도 축이다. ⚠ 구 코드값을 백필하지 않으므로 **정렬표에서 빼지 말 것** — 빼면 기존 파생본이 순서 꼬리로 밀려난다. 근거: `AugmentReviewService(AUG_ORDER)`.
  > ✅ **코드 반영 완료 (2026-09-02 실측)** — `AUG_ORDER` 가 `AUGMENT` 를 선두로 여덟 값을 정렬하고(`AugmentReviewService(AUG_ORDER)`), 위탁 바디는 서버 고정 `ETC` 를 싣는다(`augment/integration/dto/GenAiJobSubmitRequest(EVENT_TYPE_ETC)`). 요청 DTO 에는 이벤트 유형·세부 유형 필드가 **없고**(`augment/dto/AugmentRequestRequest`), FE 처리 종류도 둘이다(`features/augment/types.ts(PROCESS_KINDS)`). 목서버도 `ETC` 를 받는다.
  > ⚠ **구 서술 폐기(2026-09-02)** — *"코드는 아직 이 확정을 따르지 않는다"* 는 **반영 도중의 워킹트리를 읽은 서술**이었다. 축마다 착지 시점이 달라 그 사이에 관측하면 절반만 반영된 것처럼 보인다 — **반영 중인 변경의 구현 상태를 위키에 적을 때는 착지 후에 적을 것.**
- **요청 시 생성 조건(prompt) 5필드 필수 (2026-07-31)** — REVIEWER 가 `time`/`season`/`weather`/`terrain`/`severity` 를 입력하면 **전송 형태는 최상위 `mtdt` + `prompt` 분리(연동명세서 v1.3)** 로 나간다 — 생성 조건 다섯 항목은 최상위 **`mtdt`** 객체로, 자유 지시문은 **별개 최상위 `prompt`(문자열 ≤1000, 선택)** 로 싣는다. 보관(`LS_DATA_AUG.PROMPT_CN`)도 **분리한 그 형태 그대로** 한다 — 나간 값과 보관값이 갈라지지 않게 하기 위함이다. 구 구현의 "증강 유형별 서버 고정 문구"는 폐기. ⚠ **증강 유형(`AUG_TYPE_CD`)은 prompt 에서 파생하지 않는다** — 자유 문자열이 유형으로 흘러가면 파생 산출물 경로(`.../{augTypeCd}.mp4`) 순회(CWE-22)와 `RESL_` 네임스페이스 침범(검수 우회)이 열린다. 유형의 단일 원천은 `types[]` enum(단일값 `AUGMENT`)이다.
  - **다섯 필드는 닫힌 허용 코드다** — `time` `DAWN`\|`DAY`\|`DUSK`\|`NIGHT` · `season` `SPRING`\|`SUMMER`\|`AUTUMN`\|`WINTER` · `weather` `CLEAR`\|`CLOUDY`\|`RAIN`\|`SNOW`\|`FOG`\|`WINDY` · `terrain` `ROAD`\|`UNDERPASS`\|`RIVER`\|`URBAN`\|`RESIDENTIAL`\|`RURAL`\|`MOUNTAIN`\|`FOREST` · `severity` `LOW`\|`MEDIUM`\|`HIGH`. ⚠ 이 코드들은 **생성 조건 축**이라 증강 종류(`AUG_TYPE_CD`)와 무관하다 — `season=WINTER` · `time=NIGHT` · `weather=RAIN` 을 구 증강 종류 `WINTER`/`NIGHT`/`RAIN` 과 **같은 것으로 읽지 말 것**(축이 다르다).
  - **`prompt` 에 객체를 실으면 `400 INVALID_PARAMETER`** — 명세가 *"prompt 값으로 객체를 전달하지 않습니다"* 를 명시한다. 구 객체 형식(`prompt.condition`/`prompt.text`)과 최상위 `condition` 은 **계약에서 제외**됐다. `prompt` 의 타입을 dict/Map 으로 되돌리지 말 것. 근거: `augment/integration/dto/GenAiJobSubmitRequest(mtdt, prompt)` · 회귀 가드 `AugmentRequestContractTest`.
  > ⚠ **구 서술 폐기(2026-09-02)** — *"명세서 v1.1 §4.1 `prompt`(자유 구조 dict)로 **가공 없이 그대로** 전송되고, 같은 값이 `LS_DATA_AUG.PROMPT_CN`(JSON 원문)에 보관된다"* · *"값은 **자유 문자열**이다(계약이 허용값 enum 을 정의하지 않으므로 우리가 좁히지 않는다) — 형식 검증은 필수·공백금지·50자 상한 + **보이지 않는 문자 제거**(`VisibleTextNormalizer` — 제어문자에 더해 NBSP(U+00A0)·ZWSP(U+200B)·BOM(U+FEFF)·WJ(U+2060)·U+2028/2029·RLO(U+202E) 등 Cf/Zl/Zp/Zs 카테고리)뿐"* 은 사실과 다르다. **정본은 v1.3(갱신일 2026-08-12)** 이고 다섯 필드 전부 허용 코드가 닫혀 있다.
  > ⚠ **다만 그 시절이 남긴 제약은 그대로 유효하다** — 공용 `ControlCharNormalizer` 를 넓히지 않고 **전용 정규화기를 분리**했던 이유(그 공용 유틸이 **SQL 표현식(`translate`)과 등가**여야 하는 제약(목록 필터 옵션 왕복)을 지고 있어, 한쪽만 제거 집합을 넓히면 이벤트유형 필터가 조용히 0건이 된다)는 값이 닫힌 코드가 된 지금도 성립한다. 그 제약을 근거 없이 풀지 말 것.
- **★같은 (영상 × 종류) 재요청은 몇 번이든 허용 (2026-07-31 사용자 확정, 구속)** — 구 "요청 1회 = 파생영상 1건" 정책(사전 조회 409 + 부분 유니크 `UK_LS_DATA_AUG_ACTVTN` V143)은 **폐기**했다(V153 DROP). 근거: 증강 결과 이미지는 요청마다 다르게 생성되므로 원하는 결과가 안 나오면 같은 영상·종류로 다시 요청하는 것이 **정상 운영 동선**이다. 연타(오조작) 방어는 **FE 단독 책임**이며 **속도 제한(RateLimiter)도 두지 않는다**(자원 소모 CWE-770 은 사용자가 인지·수용한 잔여 위험 — "보안 강화" 명목으로 되살리지 말 것). ⚠ 해상도 파생 전용 `UK_LS_DATA_AUG_RESL`(V125)은 성격이 다르므로 **그대로 유지**된다.
- **★반복 요청의 귀결 ① 결과 구분축은 prompt** — 같은 (영상 × 종류) 파생이 여러 건 공존하므로 `GET /v1/augments/{jobId}/result` 가 **외부 위탁 항목을 항목(=`DATA_AUG_SN`) 단위로** 내려주고 각 항목에 `prompt`(전송 원문 JSON 문자열 — `mtdt`+자유 지시문을 **분리한 형태 그대로**, 재가공 없음)를 싣는다. 항목을 구분하는 축은 `type` 이 아니라 `id`+`prompt` 다. 구 구현은 이 항목을 제외해, 조건을 볼 수 있는 경로가 accept/reject **응답**뿐이었다 — 즉 **결정을 내린 뒤에야**, 게다가 재전이가 CONFLICT 로 막혀 **다시 조회할 수 없었다**(R9 미충족). 이 엔드포인트는 REVIEWER 전용이라 노출 범위는 accept/reject 와 동일하다(목록 `GET /v1/augments` 는 WORKER 도 허용되므로 prompt 를 싣지 않는다). 외부 위탁 항목도 **`framePairs` 를 채우고 `derivativeRawSn` 을 내려준다** — 짝짓기 근거는 `LS_DATA_AUG.NEW_RAW_SN`(V155)이며 유형·시각 추정을 쓰지 않는다(구 서술 "framePairs 는 비어 있고 derivativeRawSn 은 항상 null" 은 V155 로 해소).
- **★반복 요청의 귀결 ② 파생 영상 식별자는 시각이 아니라 증강행 PK** — `LS_DATA_RAW.VMS_CLIP_ID` 를 `{부모}_AUG_{종류}_{DATA_AUG_SN}` 로 만든다(구: `System.currentTimeMillis()`). 동시 콜백 2건이 같은 밀리초에 도달하면 `UK_LS_DATA_RAW_VMS_CLIP` 위반 → 콜백 tx 롤백 → 파생 미생성 + PENDING 잔류 → 만료 스윕 FAILED 로 **이미 생성된 외부 결과물이 유실**된다(2노드 Active-Active). 포맷은 그대로라 데이터마트 뷰·동결 메타는 영향 없다(증강 종류 판별은 `VMS_CLIP_ID` 가 아니라 `AUG_TYPE_CD` 컬럼이 단일 원천이라 애초에 접미와 무관하다 — 구 마커 역파서 `AugTypeParser` 는 제거됨). 상한(VARCHAR(128)) 초과 시 앞쪽(부모 부분)만 잘라 유일 접미를 보존한다.
...
- **업스케일(확대)도 허용** — 구 `targetH>=srcH` 400 거부 가드 제거. **산출 크기가 원본과 같아지는** 프리셋만 스킵하고 나머지는 3종 전부 생성
- **★프리셋은 고정 캔버스가 아니라 크기 상한이다 — 종횡비 보존 가변 캔버스 (2026-08-26 사용자 확정, 구속 · `ADR-018` · CO-018)**: 원본의 **짧은 변**을 프리셋의 짧은 값(1080/720/480)에 맞추는 배율을 구하고, 그 배율로 계산한 **긴 변**이 프리셋의 긴 값(1920/1280/854)을 넘을 때만 긴 변이 상한에 맞도록 배율을 낮춘다. 산출 프레임 크기는 그 배율의 실제 결과이며 **남는 영역을 채우는 패딩이 없다**. 720p 기준 산출 예 — `1920x1080 → 1280x720` / `1440x1080 → 960x720`(짧은 변 기준) / `640x480 → 960x720`(업스케일) / `1080x1920 → 720x1280`(세로 영상) / `2560x1080 → 1280x540`(긴 변 상한).
  - ⚠ **구 서술 폐기(2026-08-26)** — *"라벨/이미지 좌표를 해상도 배율(scaleX=targetW/srcW, scaleY=targetH/srcH)로 재계산"*. 그 축별 독립 배율은 비-16:9 원본을 **왜곡**시켜 `E-ISSUE-26` 으로 이미 폐기됐고, 그 뒤 「종횡비 보존 + 검정 레터박스(16:9 고정 캔버스)」로 바뀌었다가 **지금은 그 검정 패딩도 폐기**됐다(학습데이터 산출물에 검정 띠가 실려 나가 학습 노이즈가 되고, 파생본의 유일한 소비자인 관제·데이터마트로 그대로 간다). 경위 3단계 — 축별 독립 배율(왜곡) → 종횡비 보존 + 검정 패딩 → 종횡비 보존 + 가변 캔버스(현행). **중간 단계(검정 패딩)를 현재 사양으로 적지 말 것.**
  - ⚠ **박스 피팅(`min(targetW/srcW, targetH/srcH)`)으로 되돌리지 말 것** — 두 값을 모두 상한으로만 쓰면 세로 영상이 `405x720` 처럼 과소 산출되어, 짧은 변이 405 인데 720p 라 불리는 불일치가 생긴다(사용자가 명시적으로 기각한 안). 새 규칙은 짧은변/긴변으로 **정렬한 뒤** 비교한다.
  - **스킵 판정도 함께 바뀐다** — 「프리셋 값이 원본 해상도와 같은가」가 아니라 **「산출 크기가 원본과 같은가」**다. 구 판정을 두면 예컨대 원본 `960x720` 에 720p 프리셋(1280x720)이 스킵되지 않아 **원본과 동일한 파생본**이 만들어진다. 반올림 때문에 배율이 정확히 1.0 이 아니어도 산출 크기가 같아질 수 있으므로 **배율이 아니라 산출 크기로** 판정한다.
- **라벨/이미지 좌표를 가로·세로 같은 균일 배율로 재계산해 파생영상에 적재**(BBOX/POLYGON/세그멘테이션/키포인트 전 종류) — 구 '좌표 미제공' 폐기. **오프셋 가산이 없어**(`x' = x*scale`) `SCALE_X`/`SCALE_Y` 두 컬럼만으로 원본 좌표가 **완전히 역산**된다. 두 컬럼은 유지되지만 이제 **항상 같은 값**이다 — "X축 스케일"/"Y축 스케일"처럼 서로 다를 수 있다는 정의는 확정 사양과 모순이다.
- ⚠ **파생 개수가 줄어드는 원본이 있다 — 결함이 아니다**: 4:3 원본(예 `1440x1080`)은 짧은 변이 이미 1080 이라 `RESL_1080P` 의 배율이 1.0 이 되어 **스킵**된다(파생 3건 → 2건). 구 고정 캔버스 규칙에서는 `1920x1080` 필러박스 파생이 만들어졌으므로 운영자 눈에는 개수가 준 것으로 보인다.
- ⚠ **리사이저에는 「산출 크기」가 아니라 「프리셋 상한」을 넘긴다** — `Java2DImageResizer.resize(src, dst, w, h)` 의 두 수치는 **상한**이고 리사이저가 배율·산출 크기를 스스로 도출한다. 이미 도출이 끝난 산출 크기를 넘기면 같은 원본에 변환이 **두 번** 걸려, 긴 변 상한이 정확히 걸리고 반올림이 올림으로 떨어지는 구간(`2560x1080`+480p · `1366x768`+1080p)에서 산출 파일이 **1px 작아진다** — 라벨 최우측 좌표가 이미지 밖으로 나가고 응답·DB 가 보고하는 크기와 실제 파일이 갈린다. 픽셀과 라벨이 **같은 계산기·같은 입력**을 쓰는 것이 이 축의 불변식이다.
- **응답 `targetW`/`targetH` 는 요청 프리셋 수치가 아니라 실제 산출 크기다**(생성 `POST /v1/videos/{rawSn}/resolution`·조회 `GET /v1/videos/{rawSn}/resolution` 공통). 예: 원본 `1440x1080` 을 `RESL_720P` 로 요청하면 `960x720`.
- **기존 파생본은 재생성하지 않는다**(사용자 확정) — 검정 패딩이 든 채 존치하고 **신규 생성부터** 새 규칙이 적용된다. 백필 마이그레이션을 두지 않는다.
...
- **결과 조회(`GET /v1/augments/{jobId}/result`, SC-023)는 프레임 비교쌍을 반환**한다 — 좌=원본 비식별 프레임 / 우=파생 프레임(둘 다 `/v1/frames/{srcSn}/deid-image` 경로만 노출, 스토리지 경로 미노출), `(RAW_SN, FRM_NO)` 동등 조인 + 기본 12장 페이징.
  - **★외부 위탁 증강(`AUGMENT` — 구 코드값 `WINTER`·`NIGHT`·`RAIN` 파생본 포함)도 쌍을 채운다 (2026-07-31 확정)** — 구 구현은 해상도 파생에만 적용하고 외부 위탁은 `framePairs=[]`·`totalFramePairs=0` 을 **상수**로 반환해, 정작 **검수(채택/반려) 대상인 유일한 유형**의 비교 이미지가 영구히 0장이었다. 그 상태에서 FE 는 "외부 연동 이후 표시됩니다" 를 띄우면서 채택/거부 버튼을 활성으로 그려, "이미지를 비교해 보고 사용 유무를 선택" 요구의 등재 게이트가 의례적 절차가 됐다. 짝짓기 근거는 유형별로 다르다 — **외부 위탁=`NEW_RAW_SN`(V155)** / **해상도=파생이 스스로 보유한 `LS_DATA_RAW.AUG_TYPE_CD` 컬럼**(V148 신설 + V149 백필). 해상도를 `NEW_RAW_SN` 으로 **통일 금지** — 백필이 없어 기존 해상도 파생은 그 값이 NULL 이다.
  - **★항목 상태축 `resultState` 신설** — `GENERATING`(생성 중) / `PREPARING_FRAMES`(생성 완료·비교 이미지 반입 중 = 0장이 **정상**) / `READY` / `WITHHELD`(파생이 비식별 신고 구간) / `GENERATION_FAILED`(dead-letter — 기다려도 안 생김) / `CANCELED` / **`PURGED`**(폐기 유예 경과로 실삭제 — 영구히 이미지 없음) / **`DERIVATIVE_UNLINKED`**(V155 이전 외부 위탁 증강 — 파생 영상 매핑(`NEW_RAW_SN`)이 없어 비교 이미지를 **영구히** 제공할 수 없음). `decision` 만으로는 ①0장인데 정상 vs 영구 실패 ②**생성 실패 vs 사람의 반려**(둘 다 `REJECTED`) 가 구분되지 않아 화면이 사실과 다른 문구를 띄웠다. 응답 공통 `message` 는 항목별 사정을 표현할 수 없으므로 항목 단위 필드로 둔다. `PURGED` 는 **최우선 판정**이다 — 실삭제된 항목은 쌍이 0장이라 그냥 두면 `PREPARING_FRAMES`("곧 옴")로 계산돼 같은 응답의 `discard.purged=true`("영영 없음")와 정면으로 모순된다.
  ...
  - **★항목 축 복구 가능 여부 `restoreEligible` 신설 (2026-08-01)** — **화면이 복구 버튼을 그리는 유일한 근거**이며, BE 가 복구 API(`AugmentDiscardService`)의 **사전조건 그대로** 계산한다: **실삭제되지 않았고**(`discard.purged != true`) **최신 검수 행이 `REJECTED` 다**. 해상도 파생은 항상 `false`.
    - **표식 유무는 조건이 아니다 (2026-08-01 보정)** — 구 계산은 "**열린 폐기 표식이 있다** OR 최신 검수 행이 `REJECTED`" 였는데, `restore()` 는 표식을 닫은 **뒤** 검수 행을 찾아 `reopen()` 하고 그 행이 없거나 `REJECTED` 가 아니면 **409** 다. 즉 표식 축만으로는 **또 다른 죽은 버튼**(스윕이 검수 행만 먼저 지운 창)이 남았다. 두 진입 경로(`restore` / 표식 없는 `restoreWithoutMark`)는 **수용 조건이 같다** — 둘 다 마지막에 그 검수 행을 `REJECTED → PENDING` 으로 되돌리기 때문이다. 운영 정상 형상에서는 버튼이 줄지 않는다(표식은 `reject()` 트랜잭션에서만 생겨 항상 `REJECTED` 검수 행이 동반한다).
    - **★"최신 검수 행" 의 정의는 코드에 한 곳뿐이다 — `LsDataAugRvw.RECENCY_ORDER`(`REG_DT` 최대, 동률이면 PK)** — 정의가 **둘** 있었다: 복구 경로는 리포지토리의 `REG_DT DESC` 첫 행, 조회 경로는 메모리에서 `RVW_DT` 비교. `LS_DATA_AUG_RVW` 에는 `DATA_AUG_SN` 유니크가 **없고** 중복 행 정리 마이그레이션도 **금지**라 한 증강에 검수 행이 2건 이상 공존할 수 있는데, 그때 두 선택자가 **서로 다른 행**을 골라 화면은 "반려됨 → 복구 가능", API 는 404 를 냈다(무한 재시도). 정본을 **복구 경로가 쓰던 `REG_DT`** 로 맞췄다 — 화면은 *BE 가 실제로 행동할 대상*을 보여야 하며, 다른 행에서 파생한 `decision` 을 보여주는 것 자체가 오도다. 중복 행을 **유니크 제약이나 정리 마이그레이션으로 없애려 하지 말 것**(백필 금지 · accept/reject 는 행 잠금으로 직렬화한다). 회귀 가드: `AugmentRestoreEligibilityCouplingTest.중복_검수행이_있어도_조회와_복구가_같은_행_REG_DT최대_을_본다`.
    - **FE 가 조건을 재유도하면 반드시 어긋난다** — `decision=REJECTED` 는 **세 입력**(사람의 반려 · 폐기 표식 · **생성 영구 실패**)에서 나오는데 복구 API 는 **앞 둘만** 받는다. `decision === 'REJECTED' && !discard.purged` 로 재유도하던 구 화면은 dead-letter 항목(검수 행·표식 둘 다 없음 → **100% 404**)에 버튼을 그렸고, 재조회해도 세 필드가 그대로라 **무한 재시도**가 됐다(서버측 중복 차단·속도 제한을 두지 않는 확정 정책이라 막는 층이 없다). 같은 화면이 "생성에 실패해 비교 이미지가 만들어지지 않았습니다" 옆에 복구 버튼을 띄우는 자기모순이기도 했다.
    - **⚠ `discard.restorable` 과 다른 축이다(합치지 말 것)** — 저쪽은 **폐기 표식 자체**의 힌트라 **표식이 없으면 필드째 없어**(`discard=null`) 표식 없는 반려(그랜드퍼더링)를 표현할 수 없고, 실삭제 클레임(`DEL_PRCS_DT`) 구간에서 보수적으로 `false` 를 낸다. `restoreEligible` 은 그 구간에서도 복구가 실제로 성립하므로 `true` 다 — **한 응답에서 두 값이 갈리는 것이 정상**이다. **FE 는 `restoreEligible` 만 참조한다**(프로덕션 컴포넌트에서 `discard.restorable` 참조 0). 그럼에도 `restorable` 을 제거하지 않는 이유는 하위호환 + "지금 되돌릴 수 있는 표식인가" 라는 유예 안내 문구용 의미가 유효해서다.
    - **★두 판정은 테스트로 결합돼 있다** — `restoreEligible=true` 면 `POST /restore` 가 200, `false` 면 4xx 임을 **같은 테스트 안에서** 단언한다(`AugmentRestoreEligibilityCouplingTest` 7건: 열린 표식+REJECTED / 표식 없음+REJECTED / 표식만 있고 검수 행 없음 → 409 / dead-letter → 404 / 미결정 → 404 / 실삭제 → 409 / 중복 검수 행). GET 만 단언하던 구 테스트는 사전조건이 바뀌어도 **전부 초록**이라 이 결함 클래스를 놓쳤다 — 새 사전조건이 생기면 이 결합 테스트부터 고친다.
    - **추가 쿼리 0** — 두 입력(최신 검수 행 · 폐기 축)은 조회 서비스가 이미 **배치 1회씩** 읽어 들고 있는 맵이며, 판정 메서드는 리포지토리를 참조할 수 없는 `private static` 이다.
    - **최종 판정이 아니다** — 복구 API 가 행을 잠그고 재판정하므로 버튼을 그린 뒤에도 404/409 가 날 수 있다(레이스). 인가 판단에 쓰지 않는다.
    - **경로·사유는 싣지 않는다(CWE-209/359)** — `VDO_FILE_PATH`(NAS 실경로)는 응답 DTO 에 **필드로도 만들지 않고**, `DSCD_RSN` 은 이미 `rejectReason` 으로 나가므로 중복 축을 두지 않는다. 해상도 파생은 검수 대상이 아니라 표식이 생길 수 없어 항상 `null`. 조회는 검수 행과 동일하게 **배치 1회**(항목 수 무관, 인덱스 `IX_LS_DATA_AUG_DSCD_LOOKUP` V158 — 기존 4개는 전부 부분 인덱스이거나 다른 컬럼이라 이 조회를 커버하지 못해 seq scan 이었고, 비석 테이블은 retention 정리가 없어 시간이 갈수록 나빠진다).
  ...
  - **★"센 뒤 드롭" 해소 (2026-08-01)** — 구 구현은 `totalElements` 를 확정한 **뒤** 페이지 슬라이스 루프에서 해상도 파생만 두 지점(**파생 미해석** · **파생본이 신고 구간**)에서 드롭해, 총량이 실제 `results` 합계보다 컸다(FE 페이저가 존재하지 않는 페이지를 그리고 마지막 페이지가 비었다). 드롭 술어를 **항목 구성 시점**(`resolveItems`)으로 옮겨 두 값이 **같은 집합**에서 나오게 했다 — 새 드롭 조건이 생기면 반드시 이 단계에 넣는다. 비용 증가는 없다(파생 해석은 종전에도 jobId 단위 1회, 신고 게이트 대상은 해상도 파생뿐이라 영상당 최대 3건).
  ...
  - 하위호환: 신규 파라미터는 전부 optional 이고 **기존 `page`/`size` 의 기본값·의미는 불변**이라 구 호출이 그대로 동작한다. FE 항목 페이저 배선은 후속(Phase 5). 코드: `augment/service/AugmentResultViewService`
  - **★파생↔프리셋 판별은 `LS_DATA_RAW.AUG_TYPE_CD` 컬럼 단일 원천이다** — 파생 영상이 스스로 보유한다(V148 신설 + V149 백필, 신규 파생은 생성 팩토리 `createFromAugment`/`createFromResolution` 이 생성 시점에 채움). `VMS_CLIP_ID` 포맷(이중 접두 `_RESL_RESL_480P_`·구형 `_RES_RES_480P_` 등 실데이터 드리프트)은 **판별에 관여하지 않으며** 구 중앙 파서 `video/util/AugTypeParser` 는 제거됐다. 그래도 **"해상도 파생을 찾지 못해 항목을 드롭"하는 경로는 남는다** — ①V149 백필이 종류를 판별하지 못해 `AUG_TYPE_CD` 가 NULL 로 남은 과거 파생 ②계약 밖 코드(레거시 `RESOLUTION` 등) ③파생이 아직 비식별 확정(`DE_IDNTF_YN='Y'`)되지 않음. 그래서 이 드롭도 위 "센 뒤 드롭" 규칙대로 `resolveItems` 단계에서 소진한다.
...
- `LS_DATA_AUG.AUG_PROC_STTS_CD`: PENDING / ACCEPTED / REJECTED / **CANCELED**(V154 신설, §14.4.1)
...
- **예외 2가지**: ①**해상도 파생**(`AUG_TYPE_CD` `RESL_` 접두) — 검수 대상이 아니라 accept/reject 진입 자체가 400 이라 검수 행이 영영 생기지 않는다. 예외가 없으면 해상도 파생 전량이 작업목록에서 사라진다. ②**그랜드퍼더링**(`NEW_RAW_SN` 매핑이 없는 V155 이전 파생) — 어느 증강 행이 만든 파생인지 알 수 없어 무조건 미등재로 두면 고아 배정(배정 행은 있는데 목록에는 없어 접근 불가)이 된다.
...
- **★그랜드퍼더링 항목은 `resultState=DERIVATIVE_UNLINKED` 로 사실대로 표시한다 (2026-08-01 — 구 "알려진 한계" 해소)**: `NEW_RAW_SN` 매핑이 없는 기존 증강 항목은 프레임 비교쌍을 만들 수단이 없어 쌍이 **영원히 0장**인데, 구 구현은 총량만 보고 `PREPARING_FRAMES`("생성이 완료되어 비교 이미지를 반입하고 있습니다")를 내려 화면이 **영원히 오지 않을 것을 곧 온다고** 말했다. 판정 순서는 `WITHHELD` **뒤**(신고는 해소 가능한 사실이라 더 시급), 총량 판정 **앞**(총량에 맡기면 그 결함 그대로)이며 **외부 위탁 전용**이다(해상도 파생은 매핑을 못 찾으면 결과 조회가 항목 자체를 드롭한다).
  - **백필하지 않는다** — `NEW_RAW_SN` 역추정은 시각 기반이라 **다른 요청의 파생본**을 가리킨다(V155 가 이미 폐기한 방법). 비교 이미지를 실제로 **보여주는 것**은 백필 없이 불가능하지만, **거짓 문구를 멈추는 데는 백필이 불필요**하다.
  - **`GENERATION_FAILED` 로 재분류하지 않는다** — 이 항목의 **생성은 성공**했고 파생 영상도 실재한다. 없는 것은 증강 행 ↔ 파생 영상을 잇는 **매핑**뿐이다.
  - **`reviewable` 은 유지된다(의도된 판단)** — 비교 이미지 0장인 항목의 승인은 형식적이지만, `false` 로 막으면 이 항목들이 **영구히 등재되지 못해**(등재 게이트가 리뷰 축) 이미 배정된 WORKER 의 영상이 작업목록에서 사라지는 **고아 배정**이 된다. 즉 REVIEWER 가 비교 이미지를 보지 못한 채 accept 하는 것은 남은 **의도된 잔여 상태**이며, 화면은 그 사실을 문구로 알린다("이전에 생성된 결과물이라 비교 이미지를 제공할 수 없습니다.").
...
- **복구 실패 응답 — 코드가 두 종류다(구 서술 "409 두 종류" 는 사실과 다르다)**: **404** `"복구할 폐기 이력이 없습니다."`(표식도 `REJECTED` 검수 행도 없어 되돌릴 결정 자체가 없음 — `ErrorCode.NOT_FOUND`) · **404** `"증강 결과를 찾을 수 없습니다."` · **409** `"복구할 검수 이력이 없습니다."`(표식은 열려 있는데 스윕이 검수 행을 먼저 지운 레이스) · **409** `"유예 기간이 지나 이미 삭제된 파생영상입니다…"`(실삭제 커밋과 경합) · **400** `"복구 사유는 필수입니다."`. 화면은 **어느 축으로도 분기하지 않고** 서버 안내를 그대로 노출한 뒤 결과 쿼리를 무효화해 재동기화한다(메시지 문자열로 가르면 문구 변경 시 화면이 조용히 깨진다). 정상 형상에서 이 실패들은 **레이스에서만** 난다 — 버튼 가시성은 §14.3 `restoreEligible` 이 정하므로 "누르면 반드시 실패하는 버튼" 은 없다.
...
`LS_DATA_AUG`(증강·상태 — 해상도 파생도 `AUG_TYPE_CD='RESL_*'` 로 통합 적재, 부분 유니크 인덱스 `UK_LS_DATA_AUG_RESL` V124), `LS_DATA_AUG_RVW`(검수·`LBL_INTGRT_PCT`·`REJECT_RSN`), `LS_DATA_AUG_LBL_MAP`(원본-증강/해상도 파생 공통 라벨 매핑·`COORD_RECALC_YN`/`SCALE_X`/`SCALE_Y`), **`LS_DATA_AUG_DSCD`**(폐기 원장 — 표식·유예·복구·실삭제 비석, V156). 구 전용 테이블 `LS_RESOLUTION_EXPORT`·`LS_RESOLUTION_LBL_MAP`은 폐기(V125). → [18](18-database.md).

## 14.6 통합 단일 선택 UX (SCR-AUG-001)

증강 요청 화면(`/augment`, REVIEWER)은 처리 종류와 대상 영상을 **각각 1건만** 고르는 단일 선택 흐름이다.

1. **처리 종류 선택** — 카드 **둘**(증강 AI · 해상도 변경)을 `radiogroup`(로빙 tabindex·화살표 탐색, WCAG 4.1.2)으로 하나만 선택. '해상도 변경' 선택 시에만 타겟 해상도(1080P/720P/480P) 선택 UI 노출. 종류를 바꾸면 타겟 해상도·해상도 결과가 초기화된다.
   > ⚠ **구 서술 폐기(2026-09-02 · `ADR-059`)** — *"카드 4개(겨울/야간/우천/해상도 변경)"* 는 사실과 다르다. 겨울·야간·우천 카드는 없어지고 **생성 조건 프리셋**(아래 3번)으로 옮겨갔다.
2. **대상 영상 선택** — 검수 완료(`DATA_STTS_CD=COMPLETED` + `RVW_STTS_CD=APPROVED`) 영상만 라디오로 1건 선택(검색·이벤트 필터·페이징, 페이지 이동 후에도 선택 보존).
3. **생성 조건(프롬프트) 입력 (2026-07-31, 증강 AI 카드 전용)** — 증강 카드 선택 시 `AugmentPromptFieldset` 이 시간대/계절/날씨/지형/심각도 5필드 입력폼을 노출한다. 전부 필수이며 값은 **닫힌 허용 코드**에서 고른다(§14.1 — 자유 문자열이 아니다). BE 와 동일 검증(`features/augment/promptValidation.ts(validateAugmentConditions)`)을 **미리** 수행해 제출 전 400 을 막는다.
   > ⚠ **구 서술 폐기(2026-09-02)** — *"전부 필수·자유 문자열이며 BE 와 동일 검증(`validateAugmentPrompt`)을 미리 수행해"* 는 사실과 다르다. 다섯 항목은 v1.3 에서 **허용 코드로 닫혔고**, 자유 서술을 실을 자리는 별개 최상위 `prompt`(문자열 ≤1000, 선택)다. ⚠ **필수 여부는 이 정정의 대상이 아니다**(다섯 항목 모두 필수 ↔ 벤더 계약의 「최소 한 개」 사이 층간 모순은 별건).
   - 입력값이 **외부 생성형 AI 로 그대로 전송**된다는 개인정보 경고를 상시 노출한다. 해상도 변경 카드에는 이 입력폼이 없다(외부 위탁이 아니므로 prompt 개념 자체가 없음).
   - **겨울·야간·우천은 생성 조건 프리셋으로 제시된다 (2026-09-02 사용자 확정, 구속 · `ADR-059`)** — 프리셋을 고르면 다섯 항목이 채워지고, **이미 입력한 항목은 프리셋을 바꿔도 덮어쓰지 않는다**(사용자가 손댄 값을 시스템이 되돌리지 않는다). 구 처리 종류 카드가 이 자리로 내려온 것이며 **유형(`AUG_TYPE_CD`)에는 관여하지 않는다** — 유형은 프리셋과 무관하게 `AUGMENT` 단일값이다.
   - ⚠ **이벤트 유형(침수/산불)·침수 세부 유형 입력은 이 화면에서 제거됐다** — 서버가 중립값 `ETC` 를 고정 송신하고 세부 유형은 보내지 않는다(§14.1).
4. **실행(submit) 시나리오 분기**:
   - 증강 AI(`isAugmentKind`) → `POST /v1/augments/request`(videoIds·types 길이 1 배열) → 성공 시 토스트 + 결과화면(`/augment/result/{jobId}`) 네비게이션. **단건 계약이 정본**이라 2건 이상은 400(`@Size(max=1)`, 서비스도 동일 규칙 fail-closed 재확인)이고, 응답은 요청 개수 echo 가 아니라 **실제 생성 수(`createdCount`)** 를 담는다. **생성 0건은 성공이 아니다** — 프레임 미추출 영상은 412(`PRECONDITION_FAILED` + `data.skippedVideoIds`)로 거부한다(구 동작: 조용히 스킵 후 200 = silent no-op).
...
5. **실행 버튼 비활성 조건**: 종류 미선택 · 영상 미선택 · (해상도 종류인데 타겟 해상도 미선택) · (증강 AI 인데 프롬프트 5필드 미충족) · 처리 중(`isPending`).

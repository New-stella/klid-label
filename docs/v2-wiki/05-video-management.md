...
- **조치 버튼**: `배치 재실행`(기존 `POST /v1/videos/{rawSn}/batch/retry`) + VLM · AI 탐지 · AI 분할 단계의 `건너뛰기`/`되돌리기`. 전부 **REVIEWER 전용**이며 화면 게이팅은 UX 편의일 뿐 실제 강제는 BE(403)다.
...
- ⚠ **시계열 묶음의 이름은 여전히 `VLM` 이다** — 사양 본문은 이 묶음을 「시계열」이라 부르지만 2026-08-13 확정은 오토라벨 한 건이라 바꾸지 않았다(설계·구현 불일치로 별건 처리).
...
- **7단계를 7칸으로 그리지 않는다** — 오토라벨에 해당하는 마지막 세 단계(AI 탐지 · AI 분할 · 보간)를 **「오토라벨링」한 칸**으로 접어 **5칸**(비식별 · 마킹 · VLM · 프레임추출 · 오토라벨링)으로 보여준다.

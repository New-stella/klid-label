...
★**조회 범위부터 역할이 가른다 (2026-08-25 · CO-009)** — 검수자는 전체 영상을, 라벨링 작업자는 **본인에게 `LABELER` 로 배정된 영상만** 본다. **범위 제한은 거부가 아니라 결과 축소**이며 배정이 하나도 없으면 403 이 아니라 **빈 목록**이다(목록을 부르는 행위 자체는 정상이다). 사용자 축은 **인증 주체에서만** 나오고 요청 파라미터로 넓히거나 다른 사용자를 지정할 수 없다 — `VideoListFilter` 에 사용자 축 필드를 두지 않는 것이 그 강제다(CWE-639).
...
- ★**`clearedStages` 추가**(2026-08-21 · ADR-050) — **건너뛰기가 해제된** 작업 묶음 목록이며, 사람이 직접 해제한 것과 재수행에 따라 자동 해제된 것을 구분하지 않고 함께 담는다. **화면 세션과 무관한 영구 상태**다(없으면 빈 배열 · 값 공간은 `skippedStages` 와 같다).
...
- ★**노출 조건은 「배치가 실패했거나, 실패·건너뜀·해제 표시가 있는 묶음이 하나라도 있으면」이다**(2026-08-12 · **2026-08-21 확대**) — 실패했을 때만 노출하면 **건너뛴 뒤 재기동이 성공한 영상에서 이 영역이 통째로 사라진다.** 스킵 표식은 영구라 그 단계는 이후 모든 재기동에서 조용히 건너뛰어지는데 화면에는 완료로 보이고, 해제할 창구가 어디에도 남지 않는다(막는 조작에는 되돌리는 길을 함께 둔다).
...
- **조치 버튼**: `배치 재실행`(기존 `POST /v1/videos/{rawSn}/batch/retry`) + 시계열 · 오토라벨링 **묶음**의 `건너뛰기`/`재수행`. 전부 **REVIEWER 전용**이며 화면 게이팅은 UX 편의일 뿐 실제 강제는 BE(403)다.
  ...
  - **재수행 노출 대상은 「건너뛴 적이 있는 묶음」**(건너뜀·해제 모두)이다 — 그 외에는 서버가 400 이므로 두지 않는다.

...
- **배정 시나리오는 작업 목록(SC-013 영역)과 완전 정합**: ① 목록에 **배정자 컬럼** 표시(미배정/배정자명) ② 배정된 영상은 **"재배정" 버튼**으로 전환되고 클릭 시 **현재 배정자가 사전선택**된 재배정 모달(동일자 저장 비활성) ③ 검수 완료(`assignStatus='COMPLETED'`) 영상은 재배정 버튼 미노출(BE 완료 재배정 차단과 정합) ④ 배정/재배정 성공 시 `VIDEO_KEYS`·`['assignments']` 무효화로 행 즉시 갱신

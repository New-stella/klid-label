...
| 조작 | 동작 | 편집과의 관계 |
|------|------|--------------|
| **왼쪽/오른쪽 90° 회전** | `0°→90°→180°→270°` 4단계 순환. 이미지와 라벨을 함께 돌려 **보여주기만** 한다(회전이 이미지 좌표계가 아니라 뷰 좌표계에 걸리므로 이미지↔캔버스 환산식은 그대로다) | **회전 중에는 그리기가 잠긴다** — 선택 도구를 뺀 **도구 버튼**(바운딩 박스 · 폴리곤 · AI 분할 · AI 추적 · 스켈레톤)이 비활성되고(툴팁이 "회전 중에는 사용할 수 없습니다" 로 사유를 말한다) 캔버스도 라벨·오버레이의 포인터 입력을 끊는다. 회전 버튼을 누르면 활성 도구가 **선택 도구로 되돌아간다** |

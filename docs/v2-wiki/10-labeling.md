...
| 조작 | 동작 | 편집과의 관계 |
|------|------|--------------|
| **왼쪽/오른쪽 90° 회전** | `0°→90°→180°→270°` 4단계 순환. 이미지와 라벨을 함께 돌려 **보여주기만** 한다(회전이 이미지 좌표계가 아니라 뷰 좌표계에 걸리므로 이미지↔캔버스 환산식은 그대로다) | **회전 중에는 그리기가 잠긴다** — 선택 도구를 뺀 **도구 버튼**(바운딩박스 · 폴리곤 · AI 분할 · 스켈레톤)이 비활성되고(툴팁이 "회전 중에는 사용할 수 없습니다" 로 사유를 말한다) 캔버스도 라벨·오버레이의 포인터 입력을 끊는다. 회전 버튼을 누르면 활성 도구가 **선택 도구로 되돌아간다** |
...
**AI 추적은 좌측 도구바에 두지 않는다.** 사양 SCREEN-005 §좌측 도구바의 버튼은 **선택/이동 · 바운딩박스 · 폴리곤 · AI 분할 · 키포인트 · AI 탐지 여섯**이며 AI 추적은 그 목록에 없고, §라벨링 캔버스가 *"AI 추적은 우측 패널 '객체' 탭에서 대상 객체를 펼쳤을 때 노출되는 버튼으로 실행한다"* 고 못박는다.

...
- ★**프리셋은 「이벤트 + 라벨」 둘뿐이다 (2026-08-25 사용자 확정, 구속 · `V17`)** — 프리셋 자체는 **이름과 설명을 갖지 않는다**(`PRESET_NM`·`EXPLN` 컬럼 제거). 식별 축은 **`EVNT_TYPE_CD` 하나**이며 필수다(`NOT NULL`, 미지정 400). 사람이 읽는 이름은 이벤트유형 마스터의 표시명을 **서버가 응답에 실어 준다**(`eventTypeNm`).

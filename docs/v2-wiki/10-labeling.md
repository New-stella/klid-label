...
- **판정은 "필드가 왔는가"가 아니라 "값이 회차와 다른가"다.** 화면은 폐기를 건드리지 않아도 회차 값을 그대로 실어 보내므로(§13.8.1의 왕복 계약), 필드 존재로 판정하면 라벨만 고친 정상 저장까지 막힌다. 반대로 경로(프레임 단위/영상 단위) 단위로 항상 열어 두면 회차를 한 번 불러오는 것만으로 차단이 우회된다. 회차를 알 수 없는 프레임(`resolved=false`)은 비교 기준이 없으므로 보수적으로 **새 조작**으로 본다.

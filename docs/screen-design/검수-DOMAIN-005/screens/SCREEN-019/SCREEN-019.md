...
type: screen_spec
version: 37
last_updated_at: 2026-08-21T00:28:17.221Z
domain: DOMAIN-005
project_id: 4ece2c3f-8e99-46f5-9580-71108a76e578
synced_at: 2026-08-21T00:40:31.700Z
sync_session: 9
stale: true
status: CHANGED
prev_version: 36
...
> ⚠️ **버전 변경 감지 — logicraft v36 → v37**
> change_summary: 비식별 미완료 영상의 승인 거부 축을 본문과 대조한 결과 이미 반영되어 있어 본문을 고치지 않았다. 대조한 자리는 purpose 의 마지막 문장(비식별 미완료 영상은 승인이 거부되므로 승인 버튼을 비활성으로 두고 사유를 미리 안내하며, 승인만 막고 반려·라벨 확인·프레임 이미지 열람·검수 착수는 막지 않는다), 검수 액션 섹션의 승인 버튼 note(비식별 완료 전 비활성 + 옆 안내가 사유를 미리 알린다), 같은 섹션의 비식별 완료 전 승인 불가 안내(Alert, review.deidentCompleted 바인딩), 그리고 그 섹션 description 의 해당 문단(사유를 다 적고 승인을 누른 뒤에야 거부되는 동선을 만들지 않는다 + 막는 범위는 승인 하나뿐이다)이다. 이 안내를 프레임 이미지 로드 실패 힌트의 비식별 재처리 대기(비식별 누락 신고)와 합치지 않는다는 축 구분도 이미 적혀 있어 그대로 두었다. 검수 목록 응답에 비식별화 완료 여부를 표시 전용으로 싣는 축은 이 화면의 계약이 아니라 검수 목록 화면(SCREEN-018)이 소비하는 목록 조회의 축이라 여기에 옮겨 적지 않았다 — 이 화면은 그 목록 조회를 소비하지 않는다. 다만 이 화면의 안내가 바인딩하는 값을 검수 상세 조회(API-009) 응답이 아직 담고 있지 않아, 화면이 승인 가능 여부를 미리 판단할 근거를 상세 조회에서 받지 못한다. 이는 화면 사양의 공백이 아니라 그 조회 계약의 공백이라 이번에 여기서 채우지 않았다.
...
REVIEWER가 영상 검수 상세를 확인하고 승인 또는 반려를 결정하는 화면. 상단에 영상 메타 정보와 상태를 보여주는 헤더, 그 아래 프레임 이동 컨트롤이 있는 상단바, 캔버스 위의 프레임 썸네일 스트립, 중앙의 읽기 전용 라벨 캔버스, 우측에 객체/메타/이슈 3개 탭으로 구성된 패널이 배치된다. 승인·반려 액션은 검수 헤더가 단독으로 담당한다. 우측 패널의 '객체' 탭은 프레임 라벨 목록·선택 객체 속성·검수 메모를 다루고, '메타' 탭은 이벤트 어노테이션과 외부 시계열 메타를 검토하며, '이슈' 탭은 서버 연동 문의 스레드를 다룬다. 영상에 라벨이 하나도 없는 상태에서 승인을 시도하면 확인 절차를 한 번 더 거친다. 승인은 작업 완료를 의미한다. 접근: REVIEWER. 이미 완료(승인)된 영상 중 검수 승인 이후 라벨·메타가 수정되어 다시 승인이 필요한 영상은 재검토 필요로 표시되며, 이 상태에서는 완료 상태여도 승인 버튼이 다시 활성화되어 재승인할 수 있다. 영상의 비식별이 아직 완료되지 않은 영상은 승인이 거부되므로 승인 버튼을 비활성으로 두고 사유를 미리 안내한다 — 이 조건은 승인만 막고 반려·라벨 확인·프레임 이미지 열람·검수 착수는 막지 않는다.
...
- **source_hash**: 5866d2e2e56c15b37f3d0e7e8c0c8fc21508a9ee6fad2e032fd24e3f8a945874
- **generated_at**: 2026-08-19T18:43:30.704Z

...
> change_summary: ADR-013 v10 정합. 「이 역할에 두지 않는 것」에서 시계열 메타를 빼고 외부 시계열 분석 서버로 나가는 위탁 연동(호출·콜백)으로 대체했다. 미제공의 축은 위탁 연동이지 그 결과물을 사람이 보고 고치는 일이 아니기 때문이다. 데이터마트에서 불러온 영상의 메타(촬영환경·프레임 설명·개인정보 판정·시계열 메타)와 이벤트 어노테이션을 확인·수정·추가하는 권한을 문단으로 신설하고, 그 결과가 포털 전용 저장소로만 가고 데이터마트와 원본 동결본을 수정하지 않는 단방향임을 함께 적었다. 본인이 올린 영상에 한해 이벤트구간 마킹과 AI 증강 연동 요청을 수행한다는 서술도 업로드 경로에 더했다. 분할 보조(인터랙티브 분할·자동추적·키포인트) 미보유는 그대로 유지하되 그 근거가 외부 채널이 추론 엔드포인트를 직접 호출하지 않게 하려는 보안 판단임을 명시해 약화하지 않았고, 오토라벨링·검수·버전관리·기여도 점수 미보유도 유지했다. 포털 라벨링 화면 권한의 조건문을 같은 축으로 정밀화하고 변경 근거 결정을 brownfield 에 연결했다.

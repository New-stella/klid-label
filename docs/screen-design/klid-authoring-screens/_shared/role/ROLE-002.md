...
> change_summary: 확정된 화면 접근 축에 권한 매트릭스를 맞춘다. 영상 처리 현황과 영상 상세 두 화면이 검수자 전용으로 확정되어 이 역할의 접근 권한을 두지 않는다 — 두 화면은 배치 처리 상태 확인에 더해 재시도·건너뛰기·재수행 같은 운영 조치를 제공하는 자리이며, 그것은 라벨 수정·검수 제출을 맡는 이 역할의 축이 아니다. 배열 원소는 지우지 않고 폐기 표기로 남긴다 — 권한 매트릭스는 누가 무엇을 할 수 있었는지의 근거라, 원소를 지우면 결정을 되돌릴 때 판단 근거가 사라진다. 폐기 전 조건은 각각 "영상 처리 현황 — 본인에게 배정된 영상만 목록에 보이며, 조회만 가능하고 마킹 진입·배정 동선은 없다" 과 "영상 상세 — 본인 배정 영상" 였다. 비식별 처리 상태·이력 확인은 제거하지 않고 범위를 좁힌다 — 이력이 드러나는 자리가 검수자 전용 화면뿐이라 이력 축은 닫히고, 이 역할에 남는 신호는 마킹 화면의 진입 차단 안내뿐이다. 정정 전 조건은 "본인 배정 영상의 비식별 처리 상태·이력 확인" 였다. 라벨링·마킹·검수 제출·비식별 누락 신고를 비롯한 나머지 권한은 손대지 않는다.

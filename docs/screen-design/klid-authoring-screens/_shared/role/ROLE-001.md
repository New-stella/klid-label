...
> ⚠️ **버전 변경 감지 — logicraft v10 → v12**
> change_summary: 영상 처리 현황 화면 권한의 조회 범위 한정구를 걷어낸다. 이 화면은 검수자만 진입하므로 다른 조회 범위와 대비할 항이 없고, 한정구를 남기면 존재하지 않는 좁은 범위가 따로 있는 것처럼 읽힌다. 권한 자체는 변하지 않는다 — 대상 화면·행위 목록 모두 그대로다.

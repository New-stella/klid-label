...
영상 처리 현황(영상 상세, 작업 목록 확장 행 등)에서 배치 파이프라인 진행 상황을 시각화할 때 사용한다. stages 가 없는 기존 영상에서는 아무것도 렌더되지 않으므로 상위 화면이 StatusBadge 등으로 폴백 표시를 준비해야 한다. 진행 중이거나 대기 중인 단계가 남아 있고 실패한 단계가 없으며 영상이 최종 완료 상태가 아니면 상위 화면이 주기적으로 상세를 다시 조회해 단계 표시를 갱신한다 — 실패로 멈췄거나 모든 단계가 끝났으면 다시 조회하지 않는다. 마킹 화면에는 두지 않는다 — 잔여 배치는 마킹 완료가 트리거라 마킹 중에는 확인할 진행이 없기 때문이다.
...
마킹 도구 모음. 자동/수동 모드 전환(탭 UI, 단축키 1=수동/2=자동 — 전역에서 항상 발화하며 입력 필드 포커스 시에는 제외), 자동 모드 전용 프레임 간격 입력, 초기화(로컬 마킹 전체 삭제), 마킹 완료 제출, 현재 마킹 건수 표시로 구성된다. 이벤트명은 이 도구에서 입력받지 않는다 — 관제 인입값(검증이벤트유형)에서 서버가 영상 단위로 자동 소싱해 외부 시계열 위탁에 반영한다. 관제가 그 값을 보내지 않은 영상에서는 화면이 검증 이벤트 유형 선택을 노출하며, 그 영상에서는 유형을 고르지 않으면 마킹을 완료할 수 없다 — 외부 시계열 위탁에 싣는 질문 문구는 어느 마킹 방식에서도 반드시 값이 있어야 하는데(수동은 작업자가 직접 고르고 자동은 그 유형의 첫 번째 질문이 자동 선택된다) 질문을 고르려면 유형이 먼저 정해져야 하기 때문이다. 다만 완료 버튼을 비활성화하지는 않는다 — 유형이 빈 채로 누르면 토스트로 사유를 알리고 제출만 막으며, 이는 마크 0건 검사와 같은 방식이고 두 모드에 모두 적용된다. 유형과 질문을 고르는 자리는 이 도구가 아니라 검증 이벤트 유형·질문 선택 영역이며, 이 도구는 그 선택 결과를 완료 제출 게이팅에만 쓴다. 수동 모드에서는 단축키 안내(Space=마킹/Del·Backspace=삭제/Enter=완료)를 대신 노출한다.
...
- **type**: () => void
- **required**: true
- **description**: 마킹 완료 제출. 버튼은 항상 클릭 가능하다 — 수동 모드에서 마크 0건이면 토스트 경고로 제출만 막고, 자동 모드는 마크 건수 검사 대상이 아니라 간격 값 검증만 통과하면 제출된다. 검증 이벤트 유형 선택을 노출한 영상에서 유형을 고르지 않은 채 누르면 두 모드 모두 같은 방식으로 토스트 경고를 띄우고 제출만 막는다
...
자동/수동 모드 전환은 tablist/tab 시맨틱을 제공하는 탭 컴포넌트로 구현한다. 마킹 완료 버튼은 제출을 막아야 하는 상황(마크 0건, 검증 이벤트 유형 미선택)에서도 비활성화하지 않고 토스트로 사유를 알린다 — 버튼을 비활성화하면 초점을 받지 못해 보조기술 사용자에게 사유가 전달되지 않는다. 제출 진행 중(submitting)일 때만 예외로 비활성화한다.
...
작업 배정 모달. assign(단건 신규)/reassign(기존 배정 재배정)/bulk(선택 다건 일괄) 3모드 공용. 상단에 영상 정보 박스(단건은 영상명, bulk는 대상 영상 중 최대 3건을 칩으로 미리 보여주고 나머지는 '외 N건'으로 요약 + 모든 선택 영상에 동일 작업자가 배정됨을 안내) + 작업자 select(필수) 로 구성된다. [폐기] 검수자 select(REVIEWER 역할이면 기본값=로그인 사용자로 변경 가능, WORKER 역할이면 읽기 전용 표시)와 그 후보 목록 조회는 두지 않는다 — 배정의 대상은 작업자뿐이며 검수자를 영상에 배정하는 절차를 두지 않는다. 작업자 후보 목록은 REVIEWER 역할이면서 모달이 열려 있을 때만 조회한다(WORKER 화면에 마운트돼 있어도 무조건 호출되면 403이 나기 때문). reassign 모드에서 현재 배정된 작업자와 동일한 작업자를 다시 고르면 저장 버튼이 비활성화되고 경고 문구가 노출된다. 저장 시 assign/bulk는 생성 요청, reassign은 수정 요청을 호출하며 성공 시 토스트 안내 후 모달을 닫는다. 작업 배정/목록 화면, 영상 목록/처리 현황 화면(마킹 진입 팝업에서 '수동' 선택 시 단건 신규 배정 모드로도 진입).
...
배정 이력 드로어. 우측 슬라이드 패널(createPortal, 포커스 트랩 + 복귀)로 표시되며 ESC·배경클릭·X 버튼으로 닫힌다. 헤더(시계 아이콘 + '배정 이력' 제목 + 닫기) + 대상 작업(영상명) 박스 + 타임라인으로 구성된다. 타임라인은 이 창구가 돌려주는 이벤트를 그대로 그린다 — 종류로 거르는 질의 항목이 없어 값역에 있는 종류가 모두 실린다. 지금 그리는 종류는 배정(ASSIGN)·재배정(REASSIGN)·검수 시작(START_REVIEW)·검수 제출(SUBMIT)·제출 취소(CANCEL_SUBMIT)·승인(APPROVE)·반려(REJECT)·개인정보 선언 변경(PRIVACY_META_UPDATE)·개인정보 선언 초기화(PRIVACY_META_RESET)·프레임 폐기(FRAME_DISCARD)·프레임 복원(FRAME_RESTORE)·시작 버전 적용(START_VERSION_APPLY)이다. ★종류를 개수로 적지 않는다 — 열거가 곧 목록이며, 숫자로 적으면 값이 늘 때마다 그 숫자가 틀린다. 값역이 넓어지면 아직 문구를 정하지 않은 종류가 실려 올 수 있으므로, 그때도 빈칸이나 코드값을 그대로 내보이지 않는다. 각 항목은 이벤트 종류별 좌측 점 색상(의미 상태색 토큰 — APPROVE=성공, REJECT=실패, REASSIGN/CANCEL_SUBMIT=주의, ASSIGN/SUBMIT/START_REVIEW=정보, 개인정보 선언 변경·초기화와 FRAME_DISCARD/FRAME_RESTORE/START_VERSION_APPLY 는 중립 톤 — 배정·검수가 앞으로 나아간 일은 정보 톤, 무엇이 언제 바뀌었는지를 남긴 기록은 중립 톤이라는 기존 가름을 그대로 잇는다) + 일시 + '{행위자} — {행위 설명}' 패턴 문구(예: '{A} — {B} 작업자에게 배정', '{A} — 개인정보 선언 저장') + 행위 시점 역할 + 반려 사유(있을 때만)를 보여준다. 개인정보 감사 이벤트 2종은 배정·검수 진행 자체가 아니라 개인정보 선언값이 언제 바뀌었는지를 기록하는 용도이며, 판단값(Y/N) 자체는 노출하지 않고 고정 사유 문구만 보여준다. BE 응답은 합성 ASSIGN 이벤트를 항상 첫 행으로 내려 정상 응답은 최소 1건이므로 빈 상태는 비정상/방어 케이스에서만 노출된다.

★검수 시작의 문구는 '{행위자} — 검수 시작'이다. 이 부품은 '{행위자} — {행위 설명}' 꼴이라 문구를 정해 두지 않으면 그 자리에 빈칸이나 코드값이 그대로 노출된다. 같은 이유로 프레임 폐기는 '{행위자} — 프레임 폐기', 프레임 복원은 '{행위자} — 프레임 복원', 시작 버전 적용은 '{행위자} — 시작 버전 적용'으로 적는다.

★같은 사람이 잇달아 남긴 검수 시작은 한 줄로 합쳐 보인다. 보던 사람이 다시 들어오면 잡은 시각을 뒤로 미루려고 검수 시작이 새로 쌓이는데, 그것을 그대로 펼치면 같은 문구가 여러 줄 이어져 타임라인이 그 사람의 재진입으로 덮인다. 합친 줄에는 최초로 시작한 시각과 마지막으로 다시 들어온 시각을 함께 보인다 — 최초 시각을 마지막 값으로 덮어쓰면 언제 처음 열었는지가 이력에서 사라진다. 합치는 범위는 행위자와 종류가 모두 같은 연속 구간뿐이며, 사이에 다른 사람이나 다른 종류의 이벤트가 끼면 합치지 않는다.

★행위 시점 역할은 각 항목에 함께 보인다. 그 행위를 한 시점에 기록된 값이라 그 사람의 역할이 나중에 바뀌어도 따라 바뀌지 않는다 — 관리자가 승인한 항목은 관리자로 남는다. 역할 칸이 생기기 전에 쌓인 이력은 역할을 복원할 수 없어 비어 있을 수 있으며, 그때는 역할을 비워 보이고 지어낸 값이나 지금 역할로 메우지 않는다.
...
검수자↔작업자 이슈 소통 채널 패널. 반려(REJECTION) 이력과 문의(INQUIRY)를 하나의 스레드 목록으로 통합 표시하며, 헤더에 미해결 문의 건수 배지를 함께 보여준다. mode='worker'(라벨링 화면)일 때만 헤더의 '문의' 토글로 새 문의 작성 폼을 열고 닫을 수 있다(본문 1~1000자, 등록 성공 시 폼이 자동 닫힌다). 각 스레드 카드는 유형·상태 배지(반려/문의, 미해결/답변완료/해결) + 본문 + 작성자 표시 + 댓글 목록을 보여주며, mode='reviewer'일 때만 미해결 문의 스레드에 '해결 처리' 버튼이 노출된다. 둘 다 댓글 작성은 가능하다(본문 1~1000자). 미해결 문의가 해결되면 그 스레드의 댓글 입력이 잠기며(반려 이력은 해결된 뒤에도 댓글을 계속 남길 수 있어 이 잠금 규칙이 적용되지 않는다). 다른 사용자가 같은 문의를 먼저 처리한 경우(409 충돌) 스레드 카드에 동시 처리 안내를 인라인으로 보여주고 최신 상태를 다시 조회한다. 대상 영상 식별자(videoId)가 없으면 탭 자체는 계속 노출하되 패널 자리에 '영상 정보가 없어 이슈 스레드를 사용할 수 없습니다'라는 안내만 보여주고 패널은 렌더하지 않는다. 목록 조회, 문의 등록, 댓글 추가, 해결 처리 4개 동작을 각각 별도 API 로 수행한다. 본문은 텍스트 노드로만 렌더되고(dangerouslySetInnerHTML 미사용 — XSS 방어) 개행은 보존된다.
...
- **required**: true
- **description**: worker=라벨링 화면(문의 등록+댓글), reviewer=검수 화면(댓글+해결 처리).
...
영상 축(video)/프레임 축(frame) 개인정보 3필드(익명·가명·PII 포함여부)를 편집하는 패널 — 각 필드는 라디오 칩 3지(Y/N/미입력)로 표현하고, 사용자가 직접 고르지 않은 값은 *Source(MANUAL/DERIVED) 태그로 병기한다. 구역 제목 아래에는 반영 범위 안내(저장한 값은 비식별 학습데이터에 반영되며 원천은 판정하지 않는다는 문장)를 두는데, 이 안내는 메타 탭 패널이 소유한 도움말 토글이 켜졌을 때만 보이며 기본은 감춤이다 — 토글은 이 부품이 아니라 패널 머리가 소유하고 탭의 설명 문장을 한꺼번에 여닫는다. 문장 자체는 그대로 두고 노출 조건만 다르다.
...
- **type**: boolean
- **required**: false
- **description**: 메타 탭 패널이 소유한 도움말 토글의 상태. 참일 때만 구역 제목 아래 반영 범위 안내를 보인다. 기본은 거짓(감춤)이다.

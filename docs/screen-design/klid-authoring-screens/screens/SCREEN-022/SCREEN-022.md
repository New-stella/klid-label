...
type: screen_spec
version: 45
last_updated_at: 2026-08-27T14:03:25.360Z
domain: DOMAIN-000
project_id: 4ece2c3f-8e99-46f5-9580-71108a76e578
synced_at: 2026-08-27T20:53:19.477Z
sync_session: 31
stale: false
status: CHANGED
prev_version: 43
...
> ⚠️ **버전 변경 감지 — logicraft v43 → v45**
> change_summary: 생성 조건의 필수 여부를 요청 창구의 실제 규칙에 맞춘다. 이 화면 사양은 다섯 항목을 모두 선택으로 두고 최소 한 항목만 채우면 요청이 성립한다고 적었는데, 그 기준은 외부 연동 계약의 것이고 우리 요청 창구는 다섯을 모두 필수로 받는다. 그대로 만들면 네 항목이 빈 요청이 서버에서 전량 거부되므로 화면이 만들 수 없는 상태를 사양이 허용하고 있던 셈이다. 조건 입력 블록과 하단 고정 바의 제출 가능 조건, 요청 불가 사유 문구의 열거를 함께 고쳤다. 더 엄격한 쪽을 택한 이유와 그것이 외부 계약과 다르다는 사실을 남겨 다음에 읽는 사람이 어느 한쪽으로 오독하지 않게 했다. 같은 모순이 요청 창구 문서에서 먼저 발견돼 정정됐고 이 화면에 남아 있던 것을 이어서 닫는다. 아울러 자유 입력을 드롭다운으로 좁히면 미선택 상태에서 필드 오류가 뜰 계기가 사라진다는 점을 사유 문구 항목에 명시해, 그 문구가 왜 유일한 안내 자리인지 근거를 남겼다.
...
REVIEWER 가 검수 완료(승인) 영상 1건에 처리 종류 하나(겨울/야간/우천 증강 또는 해상도 변경)를 단일 선택해 요청하는 화면(/augment). 증강 3종(WINTER/NIGHT/RAIN)은 외부 위탁 잡 요청(POST /v1/augments/request), 해상도 변경은 저작도구 직접 수행(POST /v1/videos/{rawSn}/resolution)이다.

[증강 3종 — 이벤트 유형과 생성 조건] 증강을 고르면 이벤트 유형·생성 조건·자유 지시문 블록이 나타난다. ①이벤트 유형은 침수·산불 중 하나로 필수이며, 침수를 고른 경우에만 세부 유형(도로 침수·하천 범람·지하차도 침수·도심 침수·기타)을 선택으로 함께 고른다(산불에는 세부 유형이 없어 노출하지 않는다). ②생성 조건은 시간대·계절·날씨·지형·심각도 다섯 항목이며 각각 정해진 보기 중에서 고른다(자유 입력이 아니다). 다섯 항목 모두 선택이되 최소 한 개는 골라야 한다 — 조건이 하나도 없으면 생성 결과가 비결정적이 된다. ③자유 지시문은 1,000자 이내 선택 입력이며, 생성 조건과 충돌하는 표현을 적으면 생성 조건이 우선하고 무시된 표현이 경고로 회신된다. 선택·입력한 값은 개인식별정보 입력 금지 안내와 함께 그대로 외부 생성형 AI 로 전송되고 요청 원문이 보관된다. 증강 종류는 이 값에서 파생하지 않고 사용자가 카드로 고른 값이 단일 원천이며, 종류를 고르면 그에 맞는 생성 조건이 기본값으로 채워지고 검수자가 바꿀 수 있다. 해상도 변경은 외부 위탁이 아니라 이 블록을 받지 않는다.
...
- **note**: 증강 3종 선택 시에만 노출하며 필수다. 외부 연동 계약이 침수·산불 두 유형만 지원하므로 그 밖의 값은 보내지 않는다. 영상의 관제 이벤트 코드에서 자동 변환하지 않고 검수자가 직접 고른다 — 관제 코드와 외부 유형은 분류 축이 달라 자동 변환은 추정이 된다.
- **type**: Select
- **label**: 이벤트 유형 (침수 / 산불)
...
- **note**: 이벤트 유형이 침수일 때만 노출하며 선택 입력이다. 산불에는 계약상 정의된 세부 유형이 없어 노출하지 않고 값을 보내지도 않는다 — 산불의 세부 조건은 생성 조건과 자유 지시문으로 전달한다.
- **type**: Select
- **label**: 침수 세부 유형 (도로 침수 / 하천 범람 / 지하차도 침수 / 도심 침수 / 기타)
...
- **note**: 증강 3종 선택 시에만 노출하는 선택 입력. 1,000자 이내이며 글자수 카운터를 병기한다. 초과분을 잘라 보내지 않고 요청을 거부한다. 생성 조건과 충돌하는 표현(예: 밤을 고르고 지시문에 맑은 낮이라고 적는 경우)은 생성 조건이 우선하며, 무시된 표현은 경고로 회신되어 결과 화면에 표시된다.
- **type**: Textarea
...
- **note**: 해상도 변경 종류를 고른 동안에만 대상 영상의 파생 확정 상태를 주기 조회해 표시한다(증강 3종 경로는 이 축과 무관하다). 항목마다 선택한 프리셋·실제 산출 프레임 크기·파생 영상 번호·확정 상태(진행 중/검수 대기/실패)를 보이고, 진행 중 건수와 실패 건수를 요약으로 함께 알린다. 바로 위의 생성 응답 블록은 '예약됨'까지만 말하므로, 확정 단계에서 난 실패는 이 블록에서만 드러난다. 실패가 있으면 해당 해상도만 다시 요청할 수 있음을 안내한다.
- **type**: Custom
- **label**: 해상도 파생 확정 현황
...
- **description**: 증강 잡 이력 6건 카드 그리드(JobCard). GET /v1/augments page=0 size=6(API-059). 응답은 영상 단위 그룹핑이며 작업 식별자/영상 번호/CCTV명/요청된 처리 종류 목록(위탁 3종)/해상도 파생 목록/상태/요청 일시/완료 일시/요청 영상 수 9필드로 제공한다. 카드는 CCTV명·상태 배지·처리 종류 배지(해상도 파생은 별도 톤으로 구분, 증강 없이 해상도 파생만 있으면 '파생' 배지 병기)·영상 건수·요청 일시를 표시한다. 카드 클릭 시 /augment/result/{jobId} 이동. 전체 건수 배지. 로딩 Skeleton, 에러 ErrorState, 빈 상태 EmptyState.
...
- **note**: 증강 3종을 선택한 상태에서 이벤트 유형 미선택·생성 조건 항목 중 하나라도 미선택·자유 지시문 길이 초과 중 하나라도 해당해 처리 요청 버튼이 비활성일 때만 노출 — 무엇이 모자란지 알려 버튼이 왜 안 눌리는지 찾지 못하는 상황을 막는다(RESOLUTION 종류에는 해당 없음). 드롭다운은 사용자가 건드리지 않으면 필드 오류가 뜰 계기가 없고 제출 버튼도 비활성이라 클릭 핸들러가 돌지 않으므로, 이 문구가 사유를 알려 주는 유일한 자리다.
- **type**: Text
...
- **description**: 화면 하단 고정 바. 선택 요약(종류 × 영상 #N — 곱연산 아님). 취소(navigate(-1)) / 처리 요청 버튼. canSubmit = 종류 1개 선택 && 영상 1건 선택 && (RESOLUTION 종류면 preset 필수, 증강 3종이면 이벤트 유형이 선택돼 있고 생성 조건 다섯 항목이 모두 선택돼 있으며 자유 지시문이 길이 제한을 지킬 것) && !isPendingAny. 증강 3종을 고른 상태에서 이벤트 유형·생성 조건·자유 지시문 중 하나라도 미충족이면 선택 요약 아래에 버튼 비활성 사유를 안내하는 문구를 함께 노출한다(아래 경고 문구 컴포넌트). 제출 분기: 증강 종류(isAugmentKind: WINTER/NIGHT/RAIN)는 POST /v1/augments/request 위탁 요청 → 성공 시 결과화면 이동(이동 대상은 요청 본문의 videoIds[0]); RESOLUTION 은 POST /v1/videos/{rawSn}/resolution 저작도구 직접 수행 → 성공 시 변환 결과 inline 표시. 성공/실패 토스트.
...
- **description**: 
- **source_hash**: a8985b6cff25448fc73dd49a313b385c971852e4ad46929d8e559063fb88ea13
- **generated_at**: 2026-08-27T01:18:39.669Z

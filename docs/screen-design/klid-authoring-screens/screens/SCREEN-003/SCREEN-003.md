...
synced_at: 2026-09-14T05:34:04.708Z
sync_session: 39
stale: false
status: CHANGED
prev_version: 14
...
> change_summary: 세션 교체로 역할이 아직 확보되지 않은 상태(미확보)도 이 화면의 도착 사유가 아니라는 조건을 더한다. 직전 리비전이 담은 「역할을 확인하지 못한 상태는 여기로 오지 않는다」를 뒤집지 않고 한 갈래를 넓힌 것이다 — 확보 결과의 재사용은 세션에 묶여, 세션이 끊긴 뒤 같은 인계 토큰으로 다시 서면 역할이 아직 상태에 앉지 않은 미확보가 되는데 그 상태를 판정에 태우면 사용자는 자기에게 권한이 없다고 잘못 안내받는다. 원인만 다를 뿐 확인하지 못한 것을 확인한 것처럼 다루는 같은 오류다. 이로써 도착 조건은 「확보가 끝났고 역할이 확인됐으며 그 역할로는 접근할 수 없다」로 한정되고 미확보·확보 중·확보 실패 셋 다 도착 사유에서 빠진다. 확인 실패 안내와 재시도 수단은 이 화면이 아니라 세션 인계 진입 화면(SCREEN-001)의 '인증 실패 안내'가 소유하므로 여기에 두지 않았다. purpose 에 두 문장, '접근 거부 안내 hero' 섹션 설명에 세 문장을 삽입했다. 화면 구성(섹션·부품)·링크·상태는 그대로이며 기존 문장을 바꾸거나 지우지 않았다(순수 삽입 — 문자 단위 diff 에서 replace opcode 0).

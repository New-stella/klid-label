...
## § 2026-09-17 갱신 — 일괄 처리 결과 창 · 비식별 실패 재시작 · 제외/복원 (SCREEN-008 v55 정합)

### 무엇을 바꿨나
- **일괄 작업 바 안내 교체 (유일한 문구 교체)** — 구 문구 「선택한 영상 중 실패 2건만 재시작 대상입니다. 시계열 일괄 조작은 선택한 3건 전부에 적용되며, 되는 것만 처리하고 거부된 건은 사유와 함께 돌려줍니다.」는 사양과 어긋난다(재시작 대상을 배치 상태 '실패'로 좁히고 건수를 화면이 셌다). 구현 문구로 바꿨다 — 「재시작은 배치가 실패한 영상과 비식별이 실패한 영상에, 시계열 건너뛰기는 … 거부된 건은 사유와 함께 돌려줍니다. 시계열 작업이 실패한 영상만 모으려면 위 「작업 묶음 실패」 필터를 쓰세요.」 건수는 결과 창이 말한다.
- 재시작 버튼 건수 `2건` → `3건`(선택 전건을 보낸다). 재시작·재수행 버튼을 결과 창을 여는 `:target` 링크로 바꿨다.
- **일괄 처리 결과 창 2종 추가** — `#modal-bulk-result`(일괄 재시작 · 접수 2 / 거부 1) · `#modal-bulk-result-none`(시계열 일괄 재수행 · 접수 0 / 거부 3, 「한 건도 접수되지 않았습니다. 아래 영상별 사유를 확인하세요.」). 오버레이 참고 패널에 **선두 비식별 실패 영상이 거부된 결과** 미리보기 카드를 더했다.
- **「제외됨 N건」** — 표 머리 총 건수 옆, 0건이어도 상시. 눌린 상태는 면·경계·안내 문구로 구분한다.
- **행 제외 동작** — 기본 목록에만. 배정된 행(#0042·#0041)은 미리 비활성하고 버튼 아래 줄에 「먼저 배정을 해제하세요」.
- **제외분만 보는 목록** — 참고 패널(`#excluded-view`). 선택 체크박스·일괄 작업 바 없음, 행 동작은 복원·상세만.
- **영상 제외 사유 팝업(`#modal-exclude`) · 영상 복원 확인 팝업(`#modal-restore`)** 추가.

### 왜 이렇게 했나
- 결과 창의 사유는 서버가 돌려주는 문구를 그대로 썼다(화면이 상태코드로 재해석하지 않는다는 구현 원칙).
- 거부 영상 항목은 좌측 error 막대 + 영상명 + 사유 문장으로 — 색만으로 구분하지 않는다. 전건 거부는 숫자 0 옆에 경고 문장(아이콘 동반)을 따로 둔다.
- 새 CSS 는 파일 끝 §18 에 **덧붙이기만** 했다. 기존 규칙을 바꾸지 않고 뒤에서 덮은 것: `.table-head { align-items: center }`(머리에 버튼이 들어와 baseline 정렬이 어긋난다) · `.cell-action { flex-wrap: wrap }` · 일괄 바의 안내 문장을 버튼 줄 아래 한 줄로(`order`).

### 컴포넌트 · 토큰 매핑
| 요소 | 매핑 |
|---|---|
| 결과 창 | UI-004 Modal — 본문 `.result-summary`·`.result-list`(n-0 면 · e-5 좌측 막대) |
| 전건 거부 문장 | 인라인 안내 `.alert-error`(e-0 / e-2 / e-7) — 기존 `.alert-info`·`.alert-warn` 와 같은 골격 |
| 제외 확정 | UI-001 Button danger(`.btn-danger` e-5 / 흰 글자 — SCREEN-005 시안과 같은 값) |
| 제외 사유 | UI-027 Textarea · maxlength 500 (구현 상한과 같다) |
| 제외됨 N건 | ⚠️ 카탈로그 미등재(구현 `ExcludedCountToggle`) — 디자인은 ghost Button + `aria-pressed` 로 그렸다. 미등록 |
| 결과 창 본문 | ⚠️ 카탈로그 미등재(구현 `BulkRetryResultModal`) — 미등록 |
| 제외/복원 팝업 | ⚠️ 카탈로그 미등재(구현 `VideoExcludeReasonModal`·`VideoRestoreConfirmModal`) — Modal 조립으로 그렸다. 미등록 |

### 검증
- **문자 단위 diff (원본 대비)** — html: `replace` 13(전부 안내 문구 교체 · 버튼 건수 2→3 · 재시작/재수행 `button`→`a`) + `insert` 18 · `delete` 0 / css: `insert` 1 뿐.
- **D4** — 추가분 색 hex 0건(새로 잡힌 `#0029`·`#0031` 은 영상 식별자 텍스트).
- **D5 (AA 4.5:1)** — e-7×e-0 8.01 · 흰×e-5 4.56 · p-7×p-0 9.42 · n-7×n-0 7.95 · n-8×n-0 11.08 · n-6×흰 6.30 · n-7×흰 8.68 · n-10×흰 18.43 · n-6×n-0 5.77 — 전부 통과.
- **문구 대조** — 창 제목·안내·도움말·서버 사유 15종이 구현 코드(프론트·서버)에 그대로 있다.

### 구현과 다른 점 (시안은 사양을 따랐다)
- 구현은 시계열 전체 건너뛰기가 켜져 있으면 일괄 바에 「재수행한 영상은 스위치를 그대로 둬도 이후 시계열 위탁 대상이 됩니다」를 덧붙인다 — 사양(v55)에 없어 시안에 넣지 않았다.
- 구현의 결과 창 제목은 조작별(「일괄 재시작 접수 결과」·「시계열 일괄 재수행 결과」)이고 사양은 「일괄 처리 결과」 하나로 부른다 — 시안은 구현 제목을 따랐다.
- 구현은 표 머리 왼쪽에 「전체 N건 (x/y 페이지)」를 두고, 시안은 기존대로 제목 + 「총 N건」이다.

### 역등록 (Phase 5)
- SD-013 **v7 → v8** · render `main` / surface `page` · `action: replace` · sanitizer 제거 0건.
- 게시본 재조회 대조 — CSS 원문 일치, HTML 차이는 서버의 머리 정규화(`<link href="main.css">` 주입·`<meta … />`)뿐, 한글 2,491자 일치.
- 게시본 렌더 PNG 와 로컬 렌더 PNG 의 픽셀 차이는 로딩 스켈레톤 카드 안뿐(쓸어내리기 애니메이션 시점 차이).

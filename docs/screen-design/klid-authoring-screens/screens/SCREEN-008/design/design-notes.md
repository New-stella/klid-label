...
### 무엇을 얹었나
- **일괄 액션바에 세 버튼 추가** — 시계열 건너뛰기 · 되돌리기 · 재수행. 기존 「일괄 재시작」과 「일괄 배정」 사이에 구분자(`bulk-sep`)로 묶어 **배치 조작 그룹 / 배정 / 선택 해제** 세 덩어리로 읽히게 했다. 버튼 종류·크기는 기존과 같은 `btn-secondary btn-sm` 이라 위계가 흔들리지 않는다(배정만 primary 유지).

...
synced_at: 2026-09-08T00:20:31.854Z
sync_session: 36
stale: true
status: CHANGED
prev_version: 41
...
> ⚠️ **버전 변경 감지 — logicraft v41 → v54**
> change_summary: 이어쓰기가 여는 자리를 세 갈래에서 네 갈래로 넓혔다. 넷째는 검수 승인 상태를 잃어 진입이 허용되지 않는 데이터마트 영상이며, 그 행에는 프레임이 멀쩡히 있어 종전 규칙대로면 들어갈 수 있는 것처럼 보이고 눌러야 거부됐다 — 창구가 그 판정을 해서 자리를 비워 주는 쪽으로 확정됐으므로 그 자리의 뜻이 「이어서 작업하러 들어갈 수 있는 대상 프레임」으로 좁혀졌다. 뒤 두 갈래(프레임 없음·진입 불가)는 목록이 이어서 작업을 미리 막는다는 뜻임을 함께 적었다. 갈래 수를 적은 자리를 먼저 전수로 셌다 — 본문의 「세 갈래」 3건 가운데 여는 자리 축은 2건이고 나머지 1건은 「진입은 세 갈래다」로 진입 경로 축이라 그대로 두었다. 한 곳만 고치면 같은 본문 안에서 셋과 넷이 공존하고, 축이 다른 자리까지 고치면 진입 경로가 넷으로 잘못 늘어난다. 함께 적은 것 둘은 목적 서술 글자 상한(2000) 여유가 모자라 로딩·에러 섹션에 담았다 — 그 섹션이 이미 안내 문구를 가르지 않는다는 축을 소유하고 있어 자리가 맞다. ①그 자리가 비는 사유는 둘인데 값은 하나로만 말하므로 화면이 두 사유를 가르지 못한다(인지·수용한 대가이며 공통 문구를 쓴다) ②목록이 미리 막는 것은 이 화면의 진입 거부를 대신하지 않는다 — 서버 가드를 걷어내면 주소를 직접 쳐서 들어올 수 있으므로 직접 들어와도 캔버스를 렌더하지 않고 안내만 표시한다. 직전 회차에 넣은 것은 하나도 되돌리지 않았다 — 「그 자리는 응답이 알려 주므로 화면이 추측하지 않는다」와 「그 자리 값을 저장 이력 판정에 쓰지 않는다」와 그 근거가 그대로 있고 갈래 수만 넷으로 넓혔다. 값의 출처를 화면에 표시하지 않고 전송 판정에만 쓴다는 확정, 메타 다섯 축과 이벤트 어노테이션의 표시·수정·추가, 미제공 축, 소비 창구 목록, status·구현 상태, 정적 렌더는 무변경이며 배열 원소 삭제 0건이다.

...
type: screen_spec
version: 34
last_updated_at: 2026-09-08T12:03:42.171Z
domain: DOMAIN-000
project_id: 4ece2c3f-8e99-46f5-9580-71108a76e578
synced_at: 2026-09-14T05:34:04.706Z
sync_session: 39
stale: true
status: CHANGED
prev_version: 28
...
> ⚠️ **버전 변경 감지 — logicraft v28 → v34**
> change_summary: 아직 확보하지 않은 상태(미확보)도 이 화면의 도착 사유가 아니라는 조건을 더한다. 직전 리비전이 담은 '역할 확인 조회가 실패한 상태는 도착 사유가 아니다'를 뒤집지 않고 한 갈래를 넓힌 것이다. 확보 결과는 세션에 매여, 세션이 끊겼다 같은 인계 토큰으로 다시 서면 앞서 확보한 결과를 다시 쓰지 않고 새로 확보하므로 그 순간이 미확보 상태가 된다. 그 상태를 역할 없음으로 읽으면 관리자가 이미 있는 시스템에서 최초 관리자 등록 안내가 뜨는데, 이는 조회 실패가 '아직 관리자가 없습니다'로 둔갑하는 것과 같은 거짓말이며 원인만 조회 실패에서 조회 미시작으로 다를 뿐이다. 따라서 이 화면에 오려면 확보가 끝났고 그 결과가 역할 없음이어야 하며, 아직 확보하지 않았거나 확보하는 중이거나 확보에 실패한 상태는 어느 것도 도착 사유가 아니다. 확보 응답은 그것을 요청한 세션에만 반영해 다른 세션의 판정에 끼어들지 않는다는 조건도 함께 담았다. 확인 실패를 알리고 사람이 다시 시도하게 하는 자리는 이 화면이 아니라 세션 인계 진입 화면(SCREEN-001)의 인증 실패 안내가 소유하므로 여기에 재시도 수단을 두지 않는다. purpose 에 한 문단을 삽입하고 '안내 헤더' 섹션 설명에 렌더 조건 한 문장을 덧붙였다. 기존 문장·섹션·부품·링크는 하나도 바꾸거나 지우지 않았다(순수 삽입 — 문자 단위 diff 에서 치환·삭제 0).
...
- **description**: 창구가 열린 상태에서만 보이는 머리말 — 진입 시점에 조회한 창구 개폐(API-245)가 열림일 때만 렌더하고, 닫힘이면 이 섹션 대신 권한 요청 안내를 보인다. 관리자가 한 명도 없는 동안에만 열리는 창구임을 알리고, 진입한 사용자에게 최초 관리자 등록이 필요함을 안내. 제목 '관리자 등록' + 보조 설명(이 시스템에는 아직 관리자가 없습니다. 배포 시 설정한 공유 패스워드로 본인을 최초 관리자로 등록하세요.) 이 보조 설명은 열림 상태에서만 나타나므로 관리자가 이미 있는 시스템에서는 표시되지 않는다. 역할 확인 조회나 창구 개폐 조회가 실패한 상태에서는 이 섹션을 렌더하지 않는다 — 실패를 관리자가 없다는 안내로 바꾸면, 실제로는 관리자가 있는데도 없다고 말하는 것이 된다. 역할을 아직 확보하지 않았거나 확보하는 중인 상태에서도 같은 이유로 이 섹션을 렌더하지 않는다 — 확보가 끝나 역할 없음으로 확정된 뒤에만 이 안내가 사실이 된다.

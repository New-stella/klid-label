...
type: screen_spec
version: 32
last_updated_at: 2026-09-07T03:54:56.401Z
domain: DOMAIN-000
project_id: 4ece2c3f-8e99-46f5-9580-71108a76e578
synced_at: 2026-09-08T00:20:31.831Z
sync_session: 36
stale: true
status: CHANGED
prev_version: 26
...
> ⚠️ **버전 변경 감지 — logicraft v26 → v32**
> change_summary: 진입 시점에 창구 개폐를 조회해(API-245) 열림·닫힘 중 한 모습을 처음부터 보이도록 개폐 판정 기전을 바꾼다. 구 사양은 개폐를 관리자 등록 제출 응답으로 사후에 갈랐고, 그래서 관리자가 이미 있는 시스템에서도 '이 시스템에는 아직 관리자가 없습니다'가 무조건 렌더되어 사용자가 패스워드를 넣어 제출한 뒤에야 거절을 받는 막다른 화면이 됐다. 화면이 거짓을 말하지 않게 하는 것이 이 변경의 목적이다. 함께 담은 것: (1) 이 화면으로 보내는 판정 축이 「역할 없음」 하나가 아니라 「역할 없음 + 창구 개폐」 둘이라는 점, (2) 진입 시 조회 실패는 이 화면으로 낙하하지 않는다는 점 — 인증 실패는 상위 시스템 로그인으로, 그 밖의 장애는 오류 상태로 간다, (3) 조회와 제출 사이의 경합으로 제출이 창 닫힘으로 거절되는 갈래는 사전 조회가 없애지 못하므로 그대로 유지한다는 점. consumes_apis 에 API-245 를 더하고 개폐로 갈리는 두 섹션(안내 헤더·권한 요청 안내)의 references_apis 에 반영했다. 기존 API-007 과 401/403/409/429/400 분기, 역할 선택지를 두지 않는다는 규칙은 그대로 둔다.

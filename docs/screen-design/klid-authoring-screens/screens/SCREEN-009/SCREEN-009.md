...
type: screen_spec
version: 56
last_updated_at: 2026-08-26T04:03:48.232Z
domain: DOMAIN-000
project_id: 4ece2c3f-8e99-46f5-9580-71108a76e578
synced_at: 2026-08-26T04:28:14.089Z
sync_session: 27
stale: true
status: CHANGED
prev_version: 54
...
> ⚠️ **버전 변경 감지 — logicraft v54 → v56**
> change_summary: 정적 HTML 와이어프레임 자동 생성 — 1440×auto (14.4KB)
...
- **note**: 판정 축은 재수행을 눌렀는가가 아니라 그 묶음의 산출물이 있는가다 — 최초 처리로 만든 산출물과 재수행으로 만든 산출물은 구분하지 않는다. 답할 질문이 이 묶음에 지금 조치가 필요한가이지 과거에 재수행했는가가 아니기 때문이며, 과거 이력은 감사 기록에 그대로 남는다. 건너뛰기는 사유를 받고 서버가 내려주는 실패한 묶음에만 노출한다. 건너뛴 적이 있는 묶음에 재수행 버튼을 하나 두고 해제 버튼은 두지 않는다 — 범위는 고르지 않는다(묶음이 곧 범위다). 오토라벨 재수행은 사람이 손댄 보간 라벨을 새로 계산된 값으로 바꾸므로 고르는 시점에 알린다. 배지는 상태를, 버튼은 조작을 맡는 서로 다른 요소라 같은 형태로 그리지 않는다. ★검수가 완료된 적 있는 영상은 두 묶음이 갈린다 — 시계열 재수행은 그대로 누르고 오토라벨 재수행은 비활성 + 사유 툴팁이다(라벨을 다시 만들어 승인 시점 스냅샷과 어긋난다) — 되돌릴 수 없어 미리 알린다. 파생영상에는 이 영역의 조작을 노출하지 않는다.
- **type**: Custom
...
- **description**: 
- **source_hash**: b6335750951a48cddd2facfe4ea8e598dae0e091c301a229af8633ca1f53c404
- **generated_at**: 2026-08-26T04:03:48.232Z

...
type: screen_spec
version: 52
last_updated_at: 2026-08-21T00:19:24.728Z
domain: DOMAIN-000
project_id: 4ece2c3f-8e99-46f5-9580-71108a76e578
synced_at: 2026-08-21T00:40:34.318Z
sync_session: 20
stale: true
status: CHANGED
prev_version: 51
...
> ⚠️ **버전 변경 감지 — logicraft v51 → v52**
> change_summary: 건너뛰기 해제 버튼을 이 화면에서 걷어내고 재수행이 건너뛴 상태를 직접 수락하도록 맞춘다 — 되살리려는 사람에게 해제와 재수행은 한 가지 일이고, 나누어 두면 해제만 하고 재수행을 잊었을 때 그 영상이 건너뛰지도 수행하지도 않은 상태로 남는다. 재수행 버튼의 노출 근거도 건너뛴 묶음과 해제한 묶음의 합집합으로 바꾼다 — 재수행이 해제 표식을 남기고 나면 건너뛴 목록에서 빠지므로, 해제한 묶음을 받지 않으면 한 번 재수행한 영상을 화면에서 다시 재수행할 수 없다. 건너뛰기는 그 묶음이 실패한 영상에만 노출한다. 서버의 해제 요청 자체는 남아 있으나 이 화면에서 부르지 않는다.

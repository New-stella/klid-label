...
type: screen_spec
version: 49
last_updated_at: 2026-09-01T07:34:51.076Z
domain: DOMAIN-000
project_id: 4ece2c3f-8e99-46f5-9580-71108a76e578
synced_at: 2026-09-01T08:09:59.465Z
sync_session: 36
stale: false
status: UNCHANGED
prev_version: null
...
- **note**: 두 경로 공통 필수 4종 + 경로에 따라 갈리는 1종. 영상 클립 ID*(저장 파일명이 됨, 영문/숫자/_/- 64자) · CCTV ID*(영문/숫자/_/- 64자) · 출처유형*(select) · 지자체코드*(숫자 1~10자리) · 촬영일시(datetime-local — 관제 인입 재현에서는 선택이고 파이프라인 즉시 실행에서는 필수다. 현재 시각이 미리 채워져 있어 손대지 않아도 채워진 상태다). 출처유형은 화면이 요구하지만 파이프라인 즉시 실행에서는 서버로 전달되지 않고, 관제 인입 재현에서도 비우면 기본값이 적용되므로 서버가 요구하는 항목은 아니다.
- **type**: Custom
- **label**: 식별 정보

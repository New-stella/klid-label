...
type: screen_spec
version: 52
last_updated_at: 2026-09-02T11:24:47.924Z
domain: DOMAIN-000
project_id: 4ece2c3f-8e99-46f5-9580-71108a76e578
synced_at: 2026-09-08T00:20:31.852Z
sync_session: 36
stale: true
status: CHANGED
prev_version: 42
...
- **note**: 선택값은 표시용 메타데이터일 뿐이며 비식별 처리 여부에는 영향을 주지 않는다 — 비식별은 업로드된 모든 영상에 대해 예외 없이 자동 실행된다. 파이프라인 즉시 실행에서는 서버가 이 값을 반드시 받으므로 비운 채로 보낼 수 없으나, 라디오라 늘 하나가 골라져 있어 사용자가 따로 고르지 않아도 채워진다. 관제 인입 재현에서는 서버로 전달되지 않는다.
- **type**: RadioGroup
- **label**: 개인정보 유형

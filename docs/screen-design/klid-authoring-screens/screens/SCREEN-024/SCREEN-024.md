...
type: screen_spec
version: 27
last_updated_at: 2026-08-27T10:16:36.294Z
domain: DOMAIN-000
project_id: 4ece2c3f-8e99-46f5-9580-71108a76e578
synced_at: 2026-08-27T20:53:19.479Z
sync_session: 31
stale: true
status: CHANGED
prev_version: 24
...
> change_summary: 정적 HTML 와이어프레임 자동 생성 — 1440×auto (10.8KB)
...
REVIEWER가 사용자 목록을 조회·검색·필터링하고 역할을 수정하는 관리 화면. 관리자 페이지에 속해 관리자 패스워드 확인을 거쳐야 도달한다. 조회와 쓰기의 요건이 다르다 — 목록·단건 조회는 검수자 권한만으로 되고, 역할 변경에만 유효한 관리자 단기 유효창이 함께 필요하다. 유효창은 검수자 권한을 대체하지 않고 가산되며 역할을 승격시키지 않는다. 계정 활성 여부는 외부 시스템(관제서버) 소유값이라 읽기 전용 배지로만 표시한다. 역할이 아직 배정되지 않은 자동등록 사용자는 '미배정'으로 표시되며, 역할을 선택해야 저장할 수 있다. 접근: REVIEWER.
...
- 조회와 쓰기를 갈랐다. 목록·단건 조회는 검수자 권한만으로 되고 유효창을 요구하지 않는다 — 조회까지 막으면 작업 배정 흐름이 끊긴다.
- 유효창은 인가를 대체하지 않고 가산된다. 검수자 권한은 그대로 필요하며 유효창이 역할을 승격시키지 않는다.
...
- **description**: 
- **source_hash**: 7db4850352057656b13f979b9ce733c64eeb3ea6f87bb9b93450deda502e7446
- **generated_at**: 2026-08-27T10:16:36.293Z

...
- **description**: "실패" 등 위험·실패 계열 표시 — error 톤(error 스케일의 가장 옅은 단계를 배경, 진한 단계를 텍스트로 쓰고 같은 스케일 밖으로 나가지 않는다). 대비 실측값은 아직 없다 — 구현 전 실제로 측정해 본문 기준을 넘는지 확인하고 그 수치를 이 설명에 기록한다.

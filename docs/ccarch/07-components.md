...
{
  "type": "COMPONENT",
  "title": "GiteaClient (Gitea Contents API REST 어댑터)",
  "content": "**책임**\n- Gitea Contents API (PUT/GET/DELETE /repos/{owner}/{repo}/contents/{path}) 어댑터\n- 라벨 저장 시 한글 커밋 메시지 + frame·변화 카운트 enrichment\n- Resilience4j: timeout 70s, CircuitBreaker(failure-rate 50%, window 10), Retry max=3\n- stg/prd 에서 Gitea 호출 실패 시 fallback 큐 활성화\n\n**구현 위치**: `backend/.../common/client/GiteaClient.java`\n**의존**: if-gitea-contents\n**호출자**: comp-version-service, comp-label-service (라벨 일괄 저장 시 SaveCommitFlow)\n\n**역할 분리 근거**: VersionService(APPLICATION 레이어 — 라벨 버전 도메인 로직)와 GiteaClient(INFRA 레이어 — 외부 시스템 REST 어댑터)는 의존성과 변경 사이클이 다르므로 별도 컴포넌트로 식별. 10-traceability.md의 매트릭스가 별도 노드로 추적하는 데 정합.",
  "attrs": {"layer": "INFRA"},

...
| 2026-06-25 | 1.1 | 박찬기 | 김현욱 | 전체 개발범위(R1 외 구현분: 포털·게시판·이슈·통지회복성·라벨속성·프리셋·View 5종 등) ERD 통합 — design-full |
| 2026-07-20 | 1.2 | - | - | 데이터라벨이력(LS_DATA_LBL_HSTRY) 신규 반영(R1~R12) — 기존 누락 엔티티 KLID-AT-EN-046으로 신설. 용도 확장(삭제 전용 감사 → 변경 이력 ADDED/UPDATED/DELETED), CHG_KIND_CD·REG_ID 속성 추가(V112). ERD-002(라벨링) 관계·속성 반영 |
| 2026-07-20 | 1.3 | - | - | 데이터마트 적재 View(ERD-016) 재편(V114) — 라벨 내용 뷰 2종(EN-043 V_COMPLETED_LABEL·EN-044 V_COMPLETED_LABEL_ATTR) 제거(라벨 본문=검수 승인 export 폴더 JSON), EN-041(V_COMPLETED_VIDEO)에 EXPORT_PATH_NM·FRAME_CNT 추가, EN-043을 검수완료라벨변경점뷰(V_COMPLETED_LABEL_CHANGE)로 재정의·EN-044 결번. View 5종→4종. 관제 연동 계약 변경(협의 대상) |
| 2026-07-21 | 1.4 | - | - | 데이터라벨이력(LS_DATA_LBL_HSTRY, EN-046) 저장이벤트 재구조화(V114) — '라벨 1건=1행'에서 '저장 이벤트=1행 + diff 페이로드'로 전환. LBL_SN·CHG_KIND_CD 제거, ADD_CNT/MDFCN_CNT/DEL_CNT·CHG_DTL_CN(diff JSON) 추가. ERD-002 속성·관계 반영 |
...
| KLID-AT-ERD-015 | 영상 업로드 세션 |
| KLID-AT-ERD-016 | 데이터마트 적재 View |
...
erDiagram
    ...
    LS_RESOLUTION_EXPORT }o--|| LS_DATA_RAW : "파생영상 참조(신규 RAW)"
    LS_RESOLUTION_EXPORT ||--o{ LS_RESOLUTION_LBL_MAP : "좌표 재계산 라벨 매핑"
    ...
    LS_RESOLUTION_EXPORT {
        bigint RESL_EXPORT_SN PK
        bigint DATA_RAW_SN FK
        string GOAL_RESL_CD
        bigint NEW_RAW_SN FK
    }
    LS_RESOLUTION_LBL_MAP {
        bigint RESL_LBL_MAP_SN PK
        bigint RESL_EXPORT_SN FK
        bigint ORGNL_DATA_LBL_SN FK
...
erDiagram
    ...
    LS_DATA_SRC ||--o{ LS_DATA_LBL_HSTRY : "라벨 저장 이벤트 이력"
    LS_DATA_LBL {
        bigint LBL_SN PK
        bigint SRC_SN FK
        string LBL_TYPE_CD
...
#### 1.15 KLID-AT-ERD-015 — 영상 업로드 세션

```mermaid
erDiagram
    LS_TUS_UPLOAD ||..o| LS_DATA_RAW : "완료 시 원시영상 합류"
    LS_TUS_UPLOAD {
        string ULD_ID PK
        string USER_NO
        number ULD_LEN
...
> ※ LS_TUS_UPLOAD 는 관리 화면 대용량 영상의 TUS 1.0 재개 가능 업로드 세션으로, 업로드 완료 시점에만 LS_DATA_RAW 1행을 생성해 합류한다(논리 점선, 물리 FK 없음). 1차 적재 경로는 관제 학습용 설정 기반 주기 배치 픽업이며 본 업로드 세션은 1차 적재 경로가 아니다(폐지 예정).

#### 1.16 KLID-AT-ERD-016 — 데이터마트 적재 View

```mermaid
erDiagram
    LS_DATA_RAW ||--o{ V_COMPLETED_VIDEO : "검수완료 영상 메타"
    LS_RAW_DATA_STATUS ||--o{ V_COMPLETED_VIDEO : "APPROVED 필터"
    LS_DATA_SRC ||--o{ V_COMPLETED_FRAME : "검수완료 프레임 페어"
    LS_DATASET_EXPORT ||--o{ V_COMPLETED_VIDEO : "export 폴더 경로(최신 SUCCEEDED)"
    LS_DATA_LBL_HSTRY ||--o{ V_COMPLETED_LABEL_CHANGE : "검수완료 라벨 변경점"
    LS_DATA_SRC ||--o{ V_COMPLETED_LABEL_CHANGE : "RAW_SN 조인"
    LS_DATA_META ||--o{ V_COMPLETED_META : "승인 시계열 메타"
    LS_DATA_META_REVIEW ||--o{ V_COMPLETED_META : "RVW_STTS=APPROVED 필터"
    V_COMPLETED_VIDEO {
        bigint RAW_SN
        string VMS_CLIP_ID
        string DE_IDNTF_YN
        ...
        number DURATION_SEC
        bigint ORGNL_RAW_SN
        string REVIEW_STTS_CD
        date REVIEW_COMPLETED_AT
        string EXPORT_PATH_NM
        number FRAME_CNT
    }
    V_COMPLETED_FRAME {
        bigint SRC_SN
        ...
        string DEIDENTIFIED_PATH
    }
    V_COMPLETED_LABEL_CHANGE {
        bigint LBL_HSTRY_SN
        bigint RAW_SN
        bigint SRC_SN
        bigint LBL_SN
...
> ※ 본 ERD의 4개 항목(V_COMPLETED_*)은 물리 테이블이 아닌 읽기 전용 View 이며, 검수 완료(LS_RAW_DATA_STATUS.DATA_STTS_CD='APPROVED', 메타는 LS_DATA_META_REVIEW.RVW_STTS_CD='APPROVED') 영상만 노출하는 평면 인터페이스다. 관제서버가 완료/수정 통지 수신 후 RAW_SN 으로 단순 SELECT 해 데이터마트에 영상 1건=1행으로 UPSERT 한다. 모두 `CREATE OR REPLACE VIEW` 라 멱등이며, 데이터마트 소비 컨트랙트(출력 컬럼명)는 물리 컬럼 rename(V60/V61) 이후에도 alias 로 보존된다. 비식별 영상 파일 경로는 운영 컨벤션(STORAGE_RAW_PATH→STORAGE_DEIDENTIFIED_PATH) 치환으로 도출하므로 View 에는 원본 경로만 노출한다(애플리케이션 레이어 책임). **V114 재편**: 라벨 내용 뷰(V_COMPLETED_LABEL·V_COMPLETED_LABEL_ATTR)를 제거하고(라벨 본문=검수 승인 export 폴더 JSON) V_COMPLETED_VIDEO 에 export 폴더 경로(EXPORT_PATH_NM)·프레임수(FRAME_CNT)를, V_COMPLETED_LABEL_CHANGE 에 라벨 변경점(LS_DATA_LBL_HSTRY)을 배치했다 — 관제 연동 계약 변경(협의 대상).
...
| ORGNL_RAW_SN | 원본원시영상일련번호 | 숫자 | - | N | N | Y | Y | - | 논리 FK(증강 원본 자기참조) |
...
> **2026-07-21 정책 재반전**: 산출 추적 1행만 남기던 구 역할에서, **파생영상(새 원시영상) 생성 추적** 행으로 의미가 확장됐다. `NEW_RAW_SN`(파생 원시영상 역참조)이 추가되었고, 실제 라벨 좌표 재계산 매핑은 신규 엔티티 `해상도변경라벨매핑`(KLID-AT-EN, LsResolutionLblMap)이 담당한다(V122, `LS_DATA_AUG_LBL_MAP` 미재사용 — DATA_AUG_SN NOT NULL 제약으로 증강 이력 오염 방지).
...
| 속성명 | 동의어 | 타입 | 길이 | NOT NULL (Y/N) | PK (Y/N) | FK (Y/N) | INX (Y/N) | 기본값 | 제약조건 |
|-------|--------|------|------|--------|------|------|------|-------|--------|
| RESL_EXPORT_SN | 해상도변경산출일련번호 | 숫자 | - | Y | Y | N | N | 자동증가 | - |
| DATA_RAW_SN | 원시영상일련번호 | 숫자 | - | Y | N | Y | Y | - | 논리 FK(검수완료·비증강본만) |
| GOAL_RESL_CD | 목표해상도코드 | 문자 | 16 | Y | N | N | N | - | (DATA_RAW_SN,GOAL_RESL_CD) 유니크 |
| ORGNL_W | 원본가로 | 숫자 | - | Y | N | N | N | - | 첫 프레임 실측 |
| ORGNL_H | 원본세로 | 숫자 | - | Y | N | N | N | - | 첫 프레임 실측 |
| TARGET_W | 목표가로 | 숫자 | - | Y | N | N | N | - | 종횡비 보존 산정(업스케일 허용) |
| TARGET_H | 목표세로 | 숫자 | - | Y | N | N | N | - | 프리셋 기준 |
| FRAME_CNT | 프레임개수 | 숫자 | - | Y | N | N | N | - | 리스케일 프레임 수(예약 시 0→확정 시 실측) |
| OUTPUT_DIR_PATH | 출력디렉토리경로 | 문자 | 500 | Y | N | N | N | - | 내부 추적용(응답 미노출) |
| **NEW_RAW_SN** | **파생원시영상일련번호** | **숫자** | **-** | **N** | **N** | **Y** | **Y** | **-** | **(신규 V122) 파생영상 역참조, 예약 커밋 이후 연결** |
| REG_ID | 등록자아이디 | 문자 | 64 | N | N | N | N | - | 감사 추적 |
| REG_DT | 등록일시 | 날짜 | - | Y | N | N | N | 현재일시 | - |

#### 2.5b KLID-AT-EN — 해상도변경라벨매핑 (신규, V122)
...
| 속성명 | 동의어 | 타입 | 길이 | NOT NULL (Y/N) | PK (Y/N) | FK (Y/N) | INX (Y/N) | 기본값 | 제약조건 |
|-------|--------|------|------|--------|------|------|------|-------|--------|
| RESL_LBL_MAP_SN | 해상도변경라벨매핑일련번호 | 숫자 | - | Y | Y | N | N | 자동증가 | - |
| RESL_EXPORT_SN | 해상도변경산출일련번호 | 숫자 | - | Y | N | Y | Y | - | 논리 FK(→해상도변경산출) |
| ORGNL_DATA_LBL_SN | 원본라벨일련번호 | 숫자 | - | N | N | Y | Y | - | 논리 FK(→데이터라벨, 원본) |
| DATA_LBL_SN | 라벨일련번호 | 숫자 | - | Y | N | Y | Y | - | 논리 FK(→데이터라벨, 파생 좌표 스케일 복사본) |
| COORD_RECALC_YN | 좌표재계산여부 | 문자 | 1 | Y | N | N | N | 'Y' | 해상도 변경은 항상 'Y' |
| SCALE_X | X축스케일비율 | 숫자 | - | N | N | N | N | - | =목표가로/원본가로 |
| SCALE_Y | Y축스케일비율 | 숫자 | - | N | N | N | N | - | =목표세로/원본세로 |
| REG_ID | 등록자아이디 | 문자 | 30 | N | N | N | N | - | 감사 추적 |
| REG_DT | 등록일시 | 날짜 | - | Y | N | N | N | 현재일시 | - |
...
| LBL_NM | 라벨명 | 문자 | 64 | Y | N | N | Y | - | 활성(USE_YN='Y') 한정 LOWER(TRIM) 부분 유니크(UK_LS_LABEL_NM_CI, V120)가 단독 유일성 강제. all-rows exact UNIQUE(UK_LS_LABEL_NAME)는 V121 제거(soft-delete 이름 재사용 허용) |
...
| RESP_PAYLOAD_CN | 응답페이로드내용 | 문자 | - | N | N | N | N | - | 외부 호출 응답 |
| REG_ID | 등록자ID | 문자 | 30 | N | N | N | N | - | - |
| REG_DT | 등록일시 | 날짜 | - | Y | N | N | N | 현재일시 | - |
| MDFCN_ID | 수정자ID | 문자 | 30 | N | N | N | N | - | - |
| MDFCN_DT | 수정일시 | 날짜 | - | N | N | N | N | - | - |
...
#### 2.40 KLID-AT-EN-040 — 영상업로드세션

| 엔티티 ID | KLID-AT-EN-040 | 엔티티명 | 영상업로드세션 |
|---------|---|-------|---|
| 관련 클래스 ID | - ※ | 관련 클래스명 | - ※ |
| 관련 유스케이스 ID | - ※ |
| 엔티티 설명 | 관리 화면 대용량 영상의 TUS 1.0 재개 가능 업로드 세션. 청크 오프셋·전체 길이·상태를 보유하고 완료 시점에만 원시영상 1행을 생성해 합류한다. 조건부 갱신(예상 오프셋 일치)과 낙관적 잠금으로 동시 청크 전송을 직렬화하고, 만료(생성 +24시간) 세션은 정리 잡이 회수한다. ※ TUS 업로드는 1차 적재 경로가 아닌(폐지 예정) 인프라 세션 엔티티로 D1에서 업로드 서비스가 행위를 담당하고 별도 엔티티 설계 클래스를 두지 않는다. |

| 속성명 | 동의어 | 타입 | 길이 | NOT NULL (Y/N) | PK (Y/N) | FK (Y/N) | INX (Y/N) | 기본값 | 제약조건 |
|-------|--------|------|------|--------|------|------|------|-------|--------|
| ULD_ID | 업로드세션식별자 | 문자 | 36 | Y | Y | N | N | - | UUID(위치 헤더 노출) |
| USER_NO | 세션소유자번호 | 문자 | 64 | Y | N | N | Y | - | 본인 검증(HEAD/PATCH/DELETE) |
| ULD_LEN | 업로드전체길이 | 숫자 | - | Y | N | N | N | - | 전체 파일 크기(bytes) |
| ULD_OFFSET | 업로드오프셋 | 숫자 | - | Y | N | N | N | 0 | 조건부 갱신으로 직렬화 |
| STTS_CD | 상태코드 | 문자 | 16 | Y | N | N | Y | IN_PROGRESS | IN_PROGRESS/COMPLETED/EXPIRED |
| FILE_PATH | 파일경로 | 문자 | 500 | Y | N | N | N | - | 임시 파일 절대경로(UUID 강제) |
| FILE_NM | 파일명 | 문자 | 255 | N | N | N | N | - | 표시용 원본 파일명 |
| VMS_CLIP_ID | VMS클립아이디 | 문자 | 128 | N | N | N | N | - | 완료 시 영상 합류 메타 |
| CCTV_ID | CCTV아이디 | 문자 | 64 | N | N | N | N | - | 완료 시 영상 합류 메타 |
| EVNT_TYPE_CD | 이벤트유형코드 | 문자 | 32 | N | N | N | N | - | - |
| LCLGV_CD | 지자체코드 | 문자 | 10 | N | N | N | N | - | - |
| PRVC_TYPE_CD | 개인정보유형코드 | 문자 | 8 | N | N | N | N | - | ANONY/PRVC/PSDO |
| SHT_DT | 촬영일시 | 날짜 | - | N | N | N | N | - | - |
| RAW_SN | 원시영상일련번호 | 숫자 | - | N | N | Y | N | - | 완료 시 생성 영상(멱등 응답) |
| EXPD_DT | 만료일시 | 날짜 | - | Y | N | N | Y | - | 생성 +24시간(TTL 회수) |
| VER | 버전 | 숫자 | - | Y | N | N | N | 0 | 낙관적 잠금 |
| REG_DT | 등록일시 | 날짜 | - | Y | N | N | N | 현재일시 | - |
| MDFCN_DT | 수정일시 | 날짜 | - | Y | N | N | N | 현재일시 | - |

#### 2.41 KLID-AT-EN-041 — 검수완료영상뷰

| 엔티티 ID | KLID-AT-EN-041 | 엔티티명 | 검수완료영상뷰 |
|---------|---|-------|---|
| 관련 클래스 ID | - ※ | 관련 클래스명 | - ※ |
| 관련 유스케이스 ID | KLID-AT-UC-009 |
| 엔티티 설명 | 데이터마트 적재용 검수 완료 영상 메타 View(`V_COMPLETED_VIDEO`). **구 정의(V52)는 원시영상 단독**이었으나 V101/V102 에서 검수 승인 시 동결된 스냅샷(데이터셋영상메타 LS_DATASET_VIDEO_META, ACTIVE_YN='Y') 기반으로 재정의됐다 — 동결 메타(cctv명·좌표·코덱·fps·해상도 등 18컬럼)는 스냅샷에서, 상태·버전은 라이브(원시영상·영상진행상태)에서 조달하고, 라이브 APPROVED 게이트로 "스냅샷 존재 AND 현재 라이브 APPROVED" 일 때만 노출한다(재검수·반려본 오노출 차단). 증강 원본 추적은 ORGNL_RAW_SN(V82/V95, 구 PARENT_RAW_SN 폐지), 검수완료일시는 스냅샷 동결값(RVW_CMPL_DT)이다. 물리 테이블이 아닌 읽기 전용 View 이며 관제서버가 RAW_SN 으로 단순 SELECT 한다. 물리 컬럼 rename 이후에도 데이터마트 소비 컨트랙트(출력 컬럼명)는 alias 로 보존한다. **V114**: 최신 SUCCEEDED export(LS_DATASET_EXPORT)를 LEFT JOIN LATERAL(LIMIT 1)로 붙여 export 폴더 경로(EXPORT_PATH_NM)·프레임수(FRAME_CNT)를 끝에 노출한다(미export 영상은 두 값 NULL). ※ View(읽기 전용 산출)로 D1 설계 클래스를 두지 않는다. |

| 속성명 | 동의어 | 타입 | 길이 | NOT NULL (Y/N) | PK (Y/N) | FK (Y/N) | INX (Y/N) | 기본값 | 제약조건 |
|-------|--------|------|------|--------|------|------|------|-------|--------|
| RAW_SN | 원시영상일련번호 | 숫자 | - | Y | N | N | N | - | 영상(작업) 식별자 |
| VMS_CLIP_ID | VMS클립아이디 | 문자 | 128 | Y | N | N | N | - | - |
| VMS_CCTV_ID | VMS_CCTV아이디 | 문자 | 64 | Y | N | N | N | - | - |
| EVNT_TYPE_CD | 이벤트유형코드 | 문자 | 32 | N | N | N | N | - | - |
| LCLGV_CD | 기관코드 | 문자 | 32 | N | N | N | N | - | - |
| PRVC_TYPE_CD | 개인정보유형코드 | 문자 | 16 | Y | N | N | N | - | - |
| PRVC_YN | 개인정보포함여부 | 문자 | 1 | Y | N | N | N | - | - |
| DE_IDNTF_YN | 비식별여부 | 문자 | 1 | Y | N | N | N | - | DE_IDENT_YN alias |
| ORIGINAL_VIDEO_PATH | 원본영상경로 | 문자 | 500 | Y | N | N | N | - | RAW_FILE_PATH_NM alias(원본만 노출) |
| CAPTURED_AT | 촬영일시 | 날짜 | - | N | N | N | N | - | SHT_DT alias |
| DURATION_SEC | 영상길이초 | 숫자 | - | N | N | N | N | - | VDO_LEN_SEC alias |
| ORGNL_RAW_SN | 원본원시영상일련번호 | 숫자 | - | N | N | N | N | - | 증강 원본 추적(V82/V95, 구 PARENT_RAW_SN 대체) |
| BATCH_STTS_CD | 배치상태코드 | 문자 | 32 | Y | N | N | N | - | DATA_STTS_CD alias(라이브) |
| REVIEW_STTS_CD | 검수상태코드 | 문자 | 32 | Y | N | N | N | - | APPROVED 고정 노출(라이브 게이트) |
| REVIEW_COMPLETED_AT | 검수완료일시 | 날짜 | - | Y | N | N | N | - | RVW_CMPL_DT(스냅샷 동결값, 구 s.UPD_DT 아님) |
| REVIEW_VERSION | 검수버전 | 숫자 | - | Y | N | N | N | - | 낙관적 잠금 VER alias(라이브) |
| CCTV_NM | CCTV명 | 문자 | 100 | N | N | N | N | - | 동결 스냅샷 메타 |
| WGS84_LAT | WGS84위도 | 숫자 | - | N | N | N | N | - | 동결 스냅샷 메타 |
| WGS84_LOT | WGS84경도 | 숫자 | - | N | N | N | N | - | 동결 스냅샷 메타 |
| SIDO_NM | 시도명 | 문자 | 50 | N | N | N | N | - | 동결 스냅샷 메타 |
| SGG_NM | 시군구명 | 문자 | 50 | N | N | N | N | - | 동결 스냅샷 메타 |
| FILE_FMT | 파일포맷 | 문자 | 20 | N | N | N | N | - | 동결 스냅샷 메타 |
| EVNT_NM | 이벤트명 | 문자 | 100 | N | N | N | N | - | 동결 스냅샷 메타 |
| VDO_CDC | 비디오코덱 | 문자 | 20 | N | N | N | N | - | 동결 스냅샷 메타 |
| FPS | 초당프레임수 | 숫자 | - | N | N | N | N | - | 동결 스냅샷 메타 |
| BIT_RT | 비트레이트 | 숫자 | - | N | N | N | N | - | 동결 스냅샷 메타 |
| ASPRT_RT | 화면비 | 문자 | 20 | N | N | N | N | - | 동결 스냅샷 메타 |
| RESL | 해상도 | 문자 | 20 | N | N | N | N | - | 동결 스냅샷 메타 |
| VDO_WDTH | 영상가로 | 숫자 | - | N | N | N | N | - | 동결 스냅샷 메타 |
| VDO_HGT | 영상세로 | 숫자 | - | N | N | N | N | - | 동결 스냅샷 메타 |
| FILE_SZ | 파일크기 | 숫자 | - | N | N | N | N | - | 동결 스냅샷 메타 |
| DAY_NGT_CD | 주야코드 | 문자 | 20 | N | N | N | N | - | 동결 스냅샷 메타 |
| SESN_CD | 계절코드 | 문자 | 20 | N | N | N | N | - | 동결 스냅샷 메타 |
| WTHR_NM | 날씨명 | 문자 | 50 | N | N | N | N | - | 동결 스냅샷 메타 |
| EXPORT_PATH_NM | export폴더경로 | 문자 | 500 | N | N | N | N | - | V114, 최신 SUCCEEDED export 폴더 경로(미export 시 NULL) |
| FRAME_CNT | 프레임수 | 숫자 | - | N | N | N | N | - | V114, export 프레임 수(미export 시 NULL) |

#### 2.42 KLID-AT-EN-042 — 검수완료프레임뷰

| 엔티티 ID | KLID-AT-EN-042 | 엔티티명 | 검수완료프레임뷰 |
|---------|---|-------|---|
| 관련 클래스 ID | - ※ | 관련 클래스명 | - ※ |
| 관련 유스케이스 ID | KLID-AT-UC-009 |
| 엔티티 설명 | 데이터마트 적재용 검수 완료 프레임 페어 View(`V_COMPLETED_FRAME`). 한 행에 원본 프레임 경로와 비식별 프레임 경로를 짝으로 노출한다(익명 영상은 비식별 경로가 비어 있을 수 있다). 검수 승인 영상의 프레임만 노출한다. ※ View(읽기 전용 산출)로 D1 설계 클래스를 두지 않는다. |

| 속성명 | 동의어 | 타입 | 길이 | NOT NULL (Y/N) | PK (Y/N) | FK (Y/N) | INX (Y/N) | 기본값 | 제약조건 |
|-------|--------|------|------|--------|------|------|------|-------|--------|
| SRC_SN | 프레임원천일련번호 | 숫자 | - | Y | N | N | N | - | 프레임 식별자 |
| RAW_SN | 원시영상일련번호 | 숫자 | - | Y | N | N | N | - | 영상 식별자 |
| FRAME_NO | 프레임번호 | 숫자 | - | Y | N | N | N | - | FRM_NO alias |
| ORIGINAL_PATH | 원본프레임경로 | 문자 | 500 | Y | N | N | N | - | SRC_FILE_PATH_NM alias |
| DEIDENTIFIED_PATH | 비식별프레임경로 | 문자 | 1000 | N | N | N | N | - | DE_IDNTF_SRC_FILE_PATH_NM alias |
| CAPTURED_AT | 촬영일시 | 날짜 | - | N | N | N | N | - | SHT_DT alias |
| REG_DT | 등록일시 | 날짜 | - | Y | N | N | N | - | - |
| UPD_DT | 수정일시 | 날짜 | - | N | N | N | N | - | - |

#### 2.43 KLID-AT-EN-043 — 검수완료라벨변경점뷰

| 엔티티 ID | KLID-AT-EN-043 | 엔티티명 | 검수완료라벨변경점뷰 |
|---------|---|-------|---|
| 관련 클래스 ID | - ※ | 관련 클래스명 | - ※ |
| 관련 유스케이스 ID | KLID-AT-UC-009 |
| 엔티티 설명 | 데이터마트 적재용 검수 완료 라벨 변경점 View(`V_COMPLETED_LABEL_CHANGE`, **V114 신설** — 구 검수완료라벨뷰(V_COMPLETED_LABEL)를 재정의). 데이터라벨이력(LS_DATA_LBL_HSTRY)에 데이터원천(LS_DATA_SRC)을 조인해 영상 RAW_SN 을 얻고, 검수 승인(APPROVED) 영상의 라벨 변경점(ADDED/UPDATED/DELETED)만 노출한다. 라벨 좌표·속성 본문은 검수 승인 export 폴더의 COCO 기반 JSON 이 진실원이므로 뷰로 중복 노출하지 않는다(구 V_COMPLETED_LABEL·V_COMPLETED_LABEL_ATTR 제거). ※ View(읽기 전용 산출)로 D1 설계 클래스를 두지 않는다. |

| 속성명 | 동의어 | 타입 | 길이 | NOT NULL (Y/N) | PK (Y/N) | FK (Y/N) | INX (Y/N) | 기본값 | 제약조건 |
|-------|--------|------|------|--------|------|------|------|-------|--------|
| LBL_HSTRY_SN | 라벨이력일련번호 | 숫자 | - | Y | N | N | N | - | 라벨 이력 식별자 |
| RAW_SN | 원시영상일련번호 | 숫자 | - | Y | N | N | N | - | 영상 식별자(LS_DATA_SRC 조인으로 도출) |
| SRC_SN | 프레임원천일련번호 | 숫자 | - | Y | N | N | N | - | 프레임 식별자 |
| LBL_SN | 라벨일련번호 | 숫자 | - | N | N | N | N | - | 변경 대상 라벨 식별자(삭제 이력은 소멸 라벨 참조) |
| CHG_KIND_CD | 변경종류코드 | 문자 | 20 | N | N | N | N | - | ADDED/UPDATED/DELETED |
| REG_ID | 등록자아이디 | 문자 | 30 | N | N | N | N | - | 등록자(삭제 이력 경로는 NULL — PII 미저장) |
| REG_DT | 등록일시 | 날짜 | - | Y | N | N | N | - | 변경 기록 시점 |

#### 2.44 KLID-AT-EN-044 — 결번(V114에서 검수완료라벨속성뷰 제거)

> V114 데이터마트 뷰 재편으로 라벨 속성 본문 뷰 `V_COMPLETED_LABEL_ATTR`(구 검수완료라벨속성뷰)를 제거했다 — 라벨 속성값을 포함한 라벨 본문은 검수 승인 export 폴더 JSON 이 진실원이므로 뷰로 중복 노출하지 않는다. 엔티티 ID **KLID-AT-EN-044 는 채번 공백(결번)** 으로 유지한다(D9 KLID-AT-VW-004 결번과 정합, 재사용 금지).

#### 2.45 KLID-AT-EN-045 — 검수완료메타뷰

| 엔티티 ID | KLID-AT-EN-045 | 엔티티명 | 검수완료메타뷰 |
|---------|---|-------|---|
| 관련 클래스 ID | - ※ | 관련 클래스명 | - ※ |
| 관련 유스케이스 ID | KLID-AT-UC-009, KLID-AT-UC-022 |
| 엔티티 설명 | 데이터마트 적재용 검수 완료 시계열 메타 View(`V_COMPLETED_META`). 데이터메타에 데이터메타검토를 조인해, 시계열/외부 인계 메타 중 검수자가 승인(RVW_STTS_CD='APPROVED')하고 영상이 검수 완료된 것만 노출한다. ※ View(읽기 전용 산출)로 D1 설계 클래스를 두지 않는다. |

| 속성명 | 동의어 | 타입 | 길이 | NOT NULL (Y/N) | PK (Y/N) | FK (Y/N) | INX (Y/N) | 기본값 | 제약조건 |
|-------|--------|------|------|--------|------|------|------|-------|--------|
| META_SN | 메타일련번호 | 숫자 | - | Y | N | N | N | - | 메타 식별자 |
| RAW_SN | 원시일련번호 | 숫자 | - | Y | N | N | N | - | 영상 식별자 |
| META_KEY | 메타키 | 문자 | 64 | Y | N | N | N | - | - |
| META_VL | 메타값 | 문자 | 2000 | N | N | N | N | - | - |
| EXTERNAL_JOB_ID | 외부작업아이디 | 문자 | 128 | N | N | N | N | - | OTSD_JOB_ID alias |
| DATA_META_REVIEW_SN | 데이터메타검토일련번호 | 숫자 | - | Y | N | N | N | - | 검토 식별자 |
| META_TYPE_CD | 메타유형코드 | 문자 | 20 | Y | N | N | N | - | VLM/EXTERNAL |
| SRC_SYS_CD | 생성시스템코드 | 문자 | 20 | N | N | N | N | - | AI_SERVER/CONTROL_SERVER/PORTAL |
| RVW_STTS_CD | 검토상태코드 | 문자 | 20 | Y | N | N | N | - | APPROVED 고정 노출 |
| RVW_ID | 검토아이디 | 문자 | 30 | N | N | N | N | - | 검토자 |
| REVIEWED_AT | 검토일시 | 날짜 | - | N | N | N | N | - | RVW_DT alias |

#### 2.46 KLID-AT-EN-046 — 데이터라벨이력

| 엔티티 ID | KLID-AT-EN-046 | 엔티티명 | 데이터라벨이력 |
|---------|---|-------|---|
| 관련 클래스 ID | KLID-AT-DC-093 | 관련 클래스명 | LsDataLblHstry |
| 관련 유스케이스 ID | KLID-AT-UC-007, KLID-AT-UC-016 |
| 엔티티 설명 | 라벨 **저장 이벤트** 이력 — **저장 클릭 1회 = 1행**(프레임 단위). **재구조화(V114, 2026-07-21)**: 기존 '라벨 1건=1행'(라벨단위 라벨일련번호·변경종류코드)에서 '저장 이벤트=1행 + diff 페이로드'로 전환했다. 한 번의 저장 행위를 1행으로 묶어 종류별 건수(추가수/수정수/삭제수)와 변경 상세 diff(변경상세내용, 항목별 {lblSn, changeKind, labelName, before, after} JSON)를 기록한다. 직전 저장 대비 이전값→새값 diff이며 첫 저장은 전부 추가(ADDED), 저장은 프레임 전체 교체라 요청에서 빠진 라벨은 실제 삭제(DELETED), 무변경 저장은 미기록. 라벨 저장(bulkUpsert)·트랙 삭제·비식별 누락 신고 삭제 모두 같은 저장 이벤트 모델로 프레임당 기록한다. 구 라벨일련번호·변경종류코드 속성은 V114에서 제거했다. ※ 기존 R1 핵심 범위본(`docs/design`)에는 KLID-AT-EN-040으로 반영돼 있었으나, 본 전체범위본에서는 §2 번호가 이미 EN-040(영상업로드세션)·EN-041~045(데이터마트 View — V114 재편으로 현재 4종·EN-044 결번)에 배정돼 있어 신규 엔티티로 EN-046 채번했다(누락 보완). |
...
> - ERD는 도메인별로 분리하여 작성한다: 영상관리·라벨·마킹·비식별·검수·작업배정·증강·배치·관제통지·포털·게시판·시스템설정·영상업로드세션·데이터마트적재View 등.

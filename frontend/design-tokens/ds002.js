...
const RADIUS = {
  ...
  /*
   * ★관제 축의 <b>일반 단(sm·md·lg)도 이 사다리 위로 올린다</b> — 포털 채널에서만.
   *
   * 왜 필요한가: 포털 채널 산출물에는 **두 채널이 함께 쓰는 화면**이 있다(라벨링 도구가 그렇다 —
   * 그 본문 115개 파일 중 107개가 관제 소유이고 포털은 43줄짜리 껍데기로 그것을 그대로 쓴다).
   * 그 파일들은 역할 이름(`rounded-surface` 등)을 모르고 일반 단만 쓰므로, 일반 단을 그대로 두면
   * **포털 산출물에서 그 화면만 KRDS 모서리(4/6/8)로 남는다.** 색은 이미 갈리는데 모양만 안 갈린다.
   *
   * 값은 <b>DS-002 자기 사다리에서 가져온다</b> — sm→tag(6) · md→input(8) · lg→tile(12).
   * 지어낸 수가 없고 오름차순도 보존된다.
   *
   * ⚠ <b>음영에는 같은 일을 하지 않는다 — 아래 SHADOW 주석이 그 이유를 갖는다.</b> 두 조치가
   *   달라 보이는 것은 일관성 결여가 아니라 **대응이 서는가**의 차이다: 모서리는 양쪽 다
   *   «작다→크다» 한 축의 사다리라 단이 1:1 로 얹히지만, 음영은 관제가 크기 3단인 반면 DS-002 는
   *   쓰임 2종(카드·드롭다운)이라 얹을 자리가 없다. 거기서 매핑하면 값을 지어내는 것이 된다.
   *
   * ⚠ <b>관제 채널은 한 값도 바뀌지 않는다</b> — 이 표는 포털 투영에서만 병합된다.
   */
  sm: '6px',
  md: '8px',
  lg: '12px',

...
/**
 * SAM2 클릭 프롬프트로 쓸 화면 좌표 후보 — **화면에서 크게 보이는 것**을 겨냥한다.
 *
 * ── 왜 라벨 자리를 뒤지지 않는가 (2026-09-10 사용자 지시)
 *
 * 처음에는 오토라벨 상자가 있는 자리를 후보로 삼았다. 대상이 실재하는 자리라는 점은 맞았지만
 * 이 영상에서 라벨이 붙는 것은 **멀리 있는 작은 차**이고, SAM2 는 거기서 임계(0.3)를 넘지
 * 못했다. 라벨이 붙은 가장 넓은 덩어리는 하필 **비식별 가림막**이라, 면적으로 고르면 개인정보
 * 가림막의 외곽선을 딴 그림이 증적으로 남는다 — 그건 곤란하다.
 *
 * 이 시험이 증명할 것은 「클릭한 대상의 외곽선을 자동으로 딴다」이지 「작은 차를 잡는다」가
 * 아니다. 그래서 화면에서 크고 또렷한 것 — **건물·도로면·보도** — 를 찍는다. 같은 서버에
 * 직접 물어 확인한 값이 그 판단을 뒷받침한다(작은 차 한 점 0.15~0.53 / 넓은 영역 0.91).
 *
 * 좌표는 캔버스 **비율**로 준다. 절대 좌표를 박으면 프레임·해상도가 바뀔 때 빈 곳을 찍는다
 * (그 실패가 이 케이스의 원래 원인이었다).
 */
async function sam2ClickTargets(page) {
  return page.evaluate(() => {
    // 가장 큰 canvas = 프레임이 그려지는 자리.
    let best = null;
    ...
    if (!best) return [];
    /**
     * 캔버스 안의 상대 위치(0~1). 위에서 아래로 = 먼 곳에서 가까운 곳으로.
     * 좌우 가장자리는 건물·구조물, 가운데 아래는 도로면이라 어느 영상에서든 「큰 무언가」다.
     */
    const spots = [
      // ⚠ 화면 **가장자리에 걸친** 대상은 뒤로 — 외곽선이 프레임에 잘려 좁고 길게 나온다
      //   (실측 2026-09-10: 0.12 의 좌측 건물이 그렇게 나왔다). 안쪽부터 시도한다.
      [0.55, 0.52], // 도로면(중앙) — 화면에서 가장 넓고 경계가 또렷한 «바닥»
      [0.62, 0.72], // 도로면(아래쪽)
      [0.22, 0.20], // 좌측 건물군
      [0.70, 0.35], // 우측 사면·구조물
      [0.35, 0.30], // 좌측 상단 구조물
    ...
    return spots.map(([fx, fy]) => [best.left + best.width * fx, best.top + best.height * fy]);
  });
}

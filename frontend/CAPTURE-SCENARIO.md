...
떠서 클립 이름으로는 찾을 수 없다(2026-09-09 실측: `018-01-cut2` 가 이것 때문에 실패).
...
⚠ 이걸 건너뛰면 `019-01` 이 **403(본인에게 배정되지 않은 영상)** 으로 막힌다. 마킹은
작업권한이 **WORKER** 라(D11 019-01) 검수자로 대신 찍으면 안 된다.
...
| `CAPTURE_INGEST_CLIP` | **아직 적재 안 된** 클립 이름 | `018-01-cut1`(적재 전)이 실패 |
...
```
prep-marking-assign  →  019-01(마킹·프레임 생성)
  ↓
021-01 · 022-01 · 034-01 · 004-* · 005-01 · 039-*       (읽기·편집 계열)
  ↓
023-02(반려)  →  023-01(제출→승인)                      ← 승인 버전을 만든다
  ↓
008-01 · 008-02(버전 diff)                              ← 승인 버전이 있어야 한다
  ↓
016-01(신고) · 016-02(해소)                             ★영상을 잠그므로 맨 끝
  ↓
001-01(증강 요청) → 010-01(채택) / 001-01 → 010-02(반려) ← 결정 대상은 요청마다 1개
  ↓
018-01-cut1(적재 전) → 인입 INSERT → 018-01-cut2(적재 후)
  ↓
포털 024-* · 027-*                                      (별도 채널 — §CAPTURE-README §3)
```
...
| 케이스 | 소비하는 것 | 다시 만들려면 |
|---|---|---|
| `019-01` | 마킹을 완료해 영상이 다음 단계로 넘어간다 | 새 영상을 §1~§3 으로 준비 |
| `018-01` | 그 클립이 적재돼 목록에 남는다 | 새 클립 이름 + 인입 INSERT |
| `010-01`·`010-02` | 활용 결정 대기 1건을 확정한다 | `001-01` 을 한 번 더 돌린다 |
| `023-01` | 검수 건을 승인해 대기에서 뺀다 | 작업자가 다시 제출 |
| `016-01` | 영상을 신고 상태로 잠근다 | `016-02` 로 해소 |
| `003-02` | 그 영상의 해상도 프리셋을 소비 | 다른 검수완료 영상 |

⚠ **해상도 파생(`RESL_*`)은 채택·반려 대상이 아니다** — 「활용 결정 대기」로 보여도 버튼이 없다.
`010-*` 은 **외부 증강 결과**를 골라야 한다.
...
PY
```
의도된 공유는 둘뿐이다 — `003-01`↔`003-02-cut2`, `007-01`↔`023-01-cut2`.
...
없다 — KLID-AT-UT-024-01, ... 그대로 두면 그 캡처가 조용히 빠진다
```

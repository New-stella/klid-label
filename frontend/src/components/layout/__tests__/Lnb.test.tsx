// Lnb — 좌측 메뉴 키 유일성 + 「업로드」 그룹 구성 검증. [@design NAV-001]
//
// 배경: Lnb 모듈은 모듈 스코프 가변 배열 MENU 의 '업로드' 그룹에 파일 업로드 항목을 등록한다.
...
//
// 「업로드」 그룹은 데이터를 들여오는 화면만 모은 자리이며 게시판과 관리 **사이**에 온다.
//   두 항목 모두 REVIEWER 전용이라 WORKER 에게는 그룹째 사라진다.
...
describe('Lnb 메뉴 키 유일성', () => {
  ...
  it('업로드_그룹이_없는_메뉴에는_아무것도_등록하지_않는다', () => {
    // fail-closed — 엉뚱한 자리에 '업로드' 그룹을 새로 만들지 않는다.
  ...
  it('업로드_그룹에_산출물_가져오기와_파일_업로드가_이_순서로_놓인다', () => {
    ...
    expect(labels).toEqual(['산출물 가져오기', '파일 업로드']);
  });

  it('업로드_그룹이_게시판과_관리_사이에_놓인다', () => {
    // 그룹 순서는 NAV-001 이 정한다 — 데이터를 들여오는 자리를 설정을 다루는 「관리」 앞에 둔다.
    renderWithProviders(<Lnb />);
  ...
  it('WORKER_에게는_업로드_그룹이_통째로_보이지_않는다', () => {
    // 두 항목 모두 REVIEWER 전용이라 visible.length === 0 으로 그룹 헤더까지 사라진다.
    useAuthStore.setState({
      token: 'tok',
  ...
  it('registerManualUploadMenu_는_파일_업로드만_넣는다', () => {
    // 산출물 가져오기는 MENU 배열에 직접 들어 있어 이 함수의 소관이 아니다 — 그래서 dev 업로드
    // 토글이 꺼져도 「업로드」 그룹이 통째로 사라지지 않는다(그 렌더 검증은 LnbDevUploadOff).
    const menu: MenuGroupLike[] = [{ group: '업로드', items: [] }];
    registerManualUploadMenu(menu as never);
  ...
  it('이벤트유형_관리가_관리_그룹의_비식별_신고_다음에_놓인다', () => {
    ...
    // 「이벤트유형 관리」가 관리 그룹의 마지막이다 — 산출물 가져오기는 「업로드」로 옮겨갔다.

...
//   배열에 직접 들어 있어 그룹과 함께 남는다. 그룹이 사라지면 검수자가 사용자 관리·연동 주소·
//   패스워드 교체로 갈 진입점을 통째로 잃는다.
//   「업로드」 그룹(산출물 가져오기)은 이 토글과 무관하므로 함께 확인한다.
...
describe('LNB — dev 업로드 토글 OFF', () => {
  ...
  it('업로드_그룹은_이_토글과_무관하게_그대로다', async () => {
    // 「산출물 가져오기」는 MENU 배열에 직접 들어 있어 dev 업로드 토글의 영향을 받지 않는다.
    const { Lnb } = await import('@/components/layout/Lnb');
    renderWithProviders(<Lnb />);

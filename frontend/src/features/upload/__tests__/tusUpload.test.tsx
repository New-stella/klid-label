...
describe('TUS 업로드 클라이언트', () => {
  ...
  /**
   * ★출처유형에는 필수 표기를 달지 않는다 — 같은 자리에서 정반대 두 가지를 말하게 되기 때문이다.
   *
   * 이 입력칸 바로 아래에 「지금 고른 경로에서는 이 항목이 전송되지 않습니다」 안내가 붙는데,
   * 라벨에 별표가 함께 있으면 «전송되지 않는데 반드시 채워야 하는 값» 이 된다. 서버도 이 값을
   * 요구하지 않는다(즉시 실행은 계약에 없고, 인입 재현은 비우면 기본값이 붙는다).
   * 입력칸과 인입 재현에서의 전송은 그대로이므로, 사라진 것은 강제뿐이다.
   */
  it('★출처유형에_필수_표기가_없다 — 전송되지_않는다는_안내와_같은_자리다', () => {
    // given/when — 기본 경로(파이프라인 즉시 실행)
    render(<TusUploadPanel />);

    // then — 입력칸은 그대로 있고 별표만 없다
    expect(screen.getByLabelText('출처유형')).toBeInTheDocument();
    expect(screen.queryByLabelText('출처유형 *')).toBeNull();

    // then — 그리고 그 자리에 「전송되지 않습니다」 안내가 실제로 함께 떠 있다

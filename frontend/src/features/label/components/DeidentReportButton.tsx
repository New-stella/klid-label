...
export interface DeidentReportButtonProps {
  ...
  disabled?: boolean;
  /**
   * 이 영상에서는 신고 자체가 불가능할 때의 <b>사유</b>(예: 파생영상 · 검수 승인 영상). 지정하면
   * 버튼을 비활성화하고 툴팁(title)으로 사유를 보여준다.
   *
   * 사유 판정은 이 컴포넌트가 하지 않는다 — `utils/deidentReportEligibility` 단일 지점에 있다.
   *
   * 왜 필요한가: 이런 영상은 BE 가 412 로 거부하는데, 그 사실을 제출 시점에야 알리면 사용자는 사유를
...
export function DeidentReportButton({
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      ...
      if (status === 412) {
        // 파생영상(증강·해상도 변환본)·검수 승인 영상처럼 이 영상에서는 신고를 받지 않는 경우 —
        // 서버 안내문이 "왜 안 되는지"를 담고 있으므로 그대로 노출한다(일반 문구로 덮으면 아무 반응
        // 없이 실패하는 것과 같다). 두 경우 모두 위 unsupportedReason 으로 버튼 단계에서 이미 막히며,
        // 이 분기는 화면이 그 속성을 모르는 경우(구 응답 등)의 안전망이다.

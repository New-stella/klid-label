 ...
 *
 * ★★ **접수 창구(API-231)는 여기 없다.** 요청을 거는 자리는 이 화면이 아니라 포털 업로드 화면의
 *   자산별 액션이고(사양 SCREEN-044·SCREEN-033), 증강 화면에는 대상 영상을 고르러 가는 링크만
 *   둔다. 접수 함수를 여기 두면 같은 행위의 진입이 둘이 되는 첫 단추가 된다.
 *

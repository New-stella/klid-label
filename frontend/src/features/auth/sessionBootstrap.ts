 ...
 *   화면을 다시 불러오는 행위(새 부팅)로 일어난다.
 */
let attempt: { token: string; promise: Promise<ServerRoleOutcome> } | null = null;
...
export function ensureServerRole(): Promise<ServerRoleOutcome> {
  ...
  if (attempt !== null && attempt.token === token) return attempt.promise;

  useAuthStore.getState().setServerRoleStatus('pending');
  const promise = resolveServerRole();
  attempt = { token, promise };
  return promise;
}

async function resolveServerRole(): Promise<ServerRoleOutcome> {
  try {
    ...
    useAuthStore.getState().setServerRole(me.role, me.name);
    useAuthStore.getState().setServerRoleStatus('ready');
    return { kind: 'resolved', role: me.role };
  } catch (err) {
    useAuthStore.getState().setServerRoleStatus('failed');

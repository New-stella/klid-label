 ...
 *
 * 「생성형 AI API 연동명세서 v1.3」 §4.1 정합. 이 모듈이 판정하는 것은 둘이다:
 * 1. **생성 조건**(`mtdt`) — 다섯 항목 전부 필수이며 값은 허용 코드다.
 * 2. **자유 지시문**(`prompt`) — 선택. 정규화 후 비면 키를 싣지 않고, 1000자를 넘으면 오류다.
 *
 * ⚠ **구 축 폐기(2026-09-02 · ADR-059)**: 구 버전은 **이벤트 유형**(FLOOD/WILDFIRE, 필수)과
 * **침수 세부 유형**도 함께 검증했다. 그 값은 벤더가 배경 이미지에 무슨 장면을 만들어 넣을지
 * 정하는 축인데 우리 증강은 이미 이벤트가 담긴 프레임을 변환할 뿐이라 요청자가 고를 자리가
 * 없었다. 이제 요청 본문에 그 필드 자체가 없고 서버가 중립값을 고정 송신한다.
 * **되살리지 말 것** — BE 는 모르는 필드를 400 이 아니라 조용히 무시하므로 신호가 오지 않는다.
 *
 ...
 *
 * `normalizeVisibleText`(BE `VisibleTextNormalizer` 미러)는 **자유 지시문에만** 쓰인다.
 * BE 도 그 값만 정규화하며, 드롭다운 코드에는 보이지 않는 문자가 섞일 자리가 없다.
 *
 * [@design API-060] [@design INT-008] [@design ADR-059]
 */
...
export function validateAugmentConditions(
  ...
  // 2. 자유 지시문 — 선택. 원문 길이와 정규화 결과를 모두 확인한다(BE 와 같은 2단 기준).
  ...
  const ok = Object.keys(errors.mtdt).length === 0 && errors.prompt === undefined;

...
// (잠정 `PORTAL_MOUNT_BASENAME`) 아래에 우리를 마운트하므로 포털 채널 산출물만 그 경로를
// 라우터 basename 으로 가져야 한다.
//
// ⚠ 이 값이 내부(관제) 채널로 새면 **전 라우트가 하위 경로로 밀려 모든 화면이 404** 가 된다.
//   그래서 기본값(미설정)·`internal` 쪽을 함께 못 박는다.
...
describe('라우터 basename 배선 (포털 Host 임베드)', () => {
  ...
  it('채널이_internal이면_라우터_basename이_없다', async () => {
    // given
    vi.resetModules();
    vi.stubEnv('VITE_BUILD_CHANNEL', 'internal');

    // when: 라우터는 모듈 평가 시점에 생성되므로 env 를 바꾼 뒤 다시 import 해야 한다.
    const { router: internalRouter } = await import('@/router');

    // then
    expect(internalRouter.basename).toBe(ROUTER_DEFAULT_BASENAME);
  });

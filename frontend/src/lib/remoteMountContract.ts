// [@design INT-013]
/**
 * 포털 Module Federation 임베딩 계약값 — Host 와 맞춰야 하는 문자열의 **단일 지점**.
 *
 ...
 *
 * ## ★ 왜 `remoteMount.ts` 에서 갈라져 나왔나 — 파일을 합치지 말 것
 * 이 값들을 **빌드 설정(`vite.config.ts`)이 읽어야** 한다. 그 파일은 브라우저가 아니라
 * **Node 에서** 평가되는데, `remoteMount.ts` 는 `./buildChannel` 을 import 하고 그 모듈은
 * 최상위에서 `import.meta.env` 를 **평가**한다(`IS_PORTAL_CHANNEL_BUILD`). Node 에서는 그
 * 자리가 성립하지 않아 설정 로드 자체가 깨진다.
 *
 * 그래서 이 파일은 **아무것도 import 하지 않는다.** 순수 문자열 상수만 둔다.
 * ⚠ 여기에 import 를 하나라도 추가하면 그 순간 빌드 설정이 깨질 수 있다.
 *   판정이 필요한 것(`resolveRouterBasename`)은 종전대로 `remoteMount.ts` 에 있다.
 *
 * 앱 코드는 종전처럼 `@/lib/remoteMount` 에서 가져오면 된다 — 그쪽이 이 값들을 재수출한다.
 */

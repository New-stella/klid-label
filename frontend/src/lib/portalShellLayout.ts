...
/**
 * 임베드 — **거터만 갖고 폭은 묶지 않는다.**
 *
 * ## 축이 둘이고 주인이 다르다 (2026-09-16 실측으로 확정)
 *
 * | 축 | 주인 | 근거 |
 * |---|---|---|
 * | **읽기 폭** (얼마나 넓게 펼칠 것인가) | **Host** | Host 가 마운트 슬롯의 폭을 스스로 정한다 |
 * | **안쪽 여백** (모서리에서 얼마나 뗄 것인가) | **우리** | 마운트되면 Host 가 슬롯 여백을 0 으로 만든다 |
 *
 * 부모 포털 시안을 재 보면 Host 는 **슬롯 자체를 `max-width: 1200` 으로 묶어 가운데 세운다**
 * (뷰포트 1920·1600·1440 어디서나 슬롯 폭이 1200 으로 고정 · 좌우 자동 여백은 Host 가 만든다).
 * 그 안에서 Host 의 판은 1152 로 서므로 **꽉 차고, 가운데 정렬이 드러나지 않는다.**
 *
 * ⚠⚠ **그래서 우리가 최대폭을 또 세우면 안 된다 (구 동작 폐기).** 실배포 Host 본문은 시안보다
 *   넓어(뷰포트 1600 에서 약 1432 실측) 우리가 1200 으로 묶는 순간 좌우로 약 115 씩 비어
 *   **화면이 가운데로 몰린다.** 사용자가 그것을 보고 *"왜 가운데로 컨텐츠가 정렬되는거 같지"*
 *   라고 물었다. 폭을 묶는 일은 Host 가 이미 하고 있으므로 **한 번 더 하면 두 겹이 된다.**
 *   `max-w-wrap`·`mx-auto` 를 되살리지 말 것.
 *
 * ⚠ **거터는 남긴다 — 0 으로 되돌리는 것이 아니다.** 여백 자체를 없애면 같은 날 신고된
 *   *"컨테이너에 너무 딱 달라붙었다"* 가 그대로 재발한다. 위 ⚠⚠ 절 참조.
 * ⚠ 값은 토큰 한 칸(`px-4` = 16)이다 — 어중간한 20 같은 값을 쓰지 않는다.
 *   앵커가 `--spacing: 4px` 를 세우므로 `px-4` 는 16px 이다.
 */
export const PORTAL_SHELL_ALIGN_EMBED = 'w-full px-4';

...
describe('포털 업로드 목록 — 만료 예정일', () => {
  ...
  it('만료가_있는_자산과_없는_자산이_한_목록에_섞여도_각각_맞게_그린다', () => {
    // given: 만료가 실린 자산 + 실리지 않은 자산. 화면은 상태가 아니라 **값**으로 가른다.
    //   ⚠ 아래 픽스처의 UPLOADED+null 조합은 이제 서버가 실제로 내리는 조합이 아니다
    //   (마킹 대기에도 등록일 기산 만료가 실린다 — 2026-09-05 확정). 그래도 데이터를 그대로
    //   두는 것은 이 시험의 목적이 「값이 없으면 자리를 비운다」는 **값 구동 렌더링**이기 때문이다.
    renderList([

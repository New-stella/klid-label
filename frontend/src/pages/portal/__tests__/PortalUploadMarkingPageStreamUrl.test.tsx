...
// 이 화면의 재생기는 포털 전용 무대(`PortalMarkingStage`)다. 흉내 대상이 그 자리를 겨눠야
// 한다 — 옛 부품(`VideoPlayer`)을 흉내내면 화면이 그것을 안 쓰므로 **stub 이 아예 안 그려지고**
// 「요소를 못 찾음」으로만 실패해, 무엇을 지키던 시험인지가 사라진다.
// 지키는 단언(배포 접두 · 재시도 상한 · 회복 시 예산 복원)은 그대로다.
vi.mock('@/features/portal/uploads/components/PortalMarkingStage', () => ({
  PortalMarkingStage: ({
    src,
    onSrcError,
    onSrcRecovered,

...
## 프로젝트 개요
- **목적**: AI 기반 지방정부 CCTV 관제지원시스템(2차)의 학습데이터 저작도구 — 영상/이미지 라벨링, 시계열 메타데이터, 검수 워크플로우, 비식별화, 데이터마트 연계
- **주요 도메인**: 사용자/권한, 배치 파이프라인(프레임 추출·오토라벨링·VLM 메타), 라벨링, 검수(REVIEWER 배정), 비식별화, 버전관리(Gitea), 데이터 증강, 내보내기, 데이터마트, 포털
...
- Quartz 트리거로 1건/분 처리: FFmpeg → 조건부 비식별화 → YOLO → SAM2 → VLM

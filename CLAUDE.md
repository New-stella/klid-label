...
- **★Flyway 스쿼시 완료 — 신규 마이그레이션은 `V3` 부터다 (2026-08-13, 구속)**: 누적 180개(V0~V185)를 **`V1__baseline.sql`(스키마 전량 + 시드 19행)** 하나로 접었고 `V2` 는 `CM_CODE`→**`LS_CM_CODE`** 개명이다. 접은 시점 기준: 테이블 76 · 뷰 4 · 시퀀스 49 · `COMMENT ON` 187 · 시드 19행(`ls_system_config` 12 / `ls_cm_code` 5 / `qrtz_locks` 2 — **그 밖의 테이블에는 시드가 없다**).

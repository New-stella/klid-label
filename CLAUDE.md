...
- **해소(resolve) 시 자동 복구**: `'F'→'Y'` 복원으로 위 게이트가 전부 자동 해제되고, 신고 구간에 보류됐던 **검수 승인(APPROVED) 영상의 export 재산출이 재트리거**된다(`DeidentReportResolvedEvent` → `DatasetExportBridge`, AFTER_COMMIT). **APPROVED 자손(파생영상)까지 팬아웃**한다 — 부모 신고 동안 자손 export 도 함께 skip 됐으므로 부모만 재트리거하면 파생 export 폴더가 옛 내용으로 정체되기 때문이다(다른 조상이 아직 신고 중인 자손은 skip).

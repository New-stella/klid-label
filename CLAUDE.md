...
- **★ 동작·정책이 바뀌면 `docs/test-cases/` 카탈로그도 같은 커밋에서 갱신한다 (2026-08-03 사용자 확정, 구속)**: wiki 만 고치고 테스트케이스를 두면 카탈로그가 **폐기된 동작을 계속 검증 대상으로 들고 있어** 다음 전수 검증에서 "결함"으로 재발견된다(이 저장소의 "철회된 정책 재시도" 사고 패턴과 같은 뿌리).
  - **정정**: 기대결과·전제·근거를 실제 코드로 재확인해 고친다.
    - **★근거에 라인번호를 넣지 않는다 (2026-08-05 사용자 확정, 구속)**: `PresetService.java(resolveLabels)` 처럼 **파일명(+심볼·짧은 설명)만** 적고 `:105-143` 을 적지 않는다. 이미 들어간 라인번호는 발견 시 제거한다.
      - **왜**: ①라인은 무관한 리팩토링에도 흔들려 **정정 노동의 대부분이 의미 없는 드리프트 추적**에 소모된다(회차 4 = 정정 699건 중 기대결과·전제가 실제 바뀐 건 24건뿐, 회차 11 = 76건 **전량**이 순수 리팩토링 드리프트) ②**stale 라인은 없느니만 못하다** — 엉뚱한 함수를 가리켜 검증자를 오판으로 이끈다(회차 4 H-16 17건).
      - 근거 정정이 **실제로 필요한 경우는 파일 리네임·삭제·이동, 판정 주체의 클래스 이동**뿐이다.
      - > supersedes: 구 지침 *"라인 드리프트만 바뀐 것도 고친다 — 근거 정확도가 이 카탈로그의 존재 이유다"* → **폐기**. 근거 정확도는 **파일·심볼 단위로 충분**하다.

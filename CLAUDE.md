...
> - ⚠ **착지처가 아직 없는 컬럼 4** — MIME 유형 · 원본 파일명 · 업로드 상태 · 실패 사유. 「업로드 상태」는
>   영상 원장 상태 컬럼과 **값역이 달라** 단순 이관이 안 되고 「실패 사유」는 대응이 아예 없다.
>   (프레임률·파일 크기는 `video.fps`·`video.filesize` 로 이미 있고, 프레임 수는 프레임 원장을 세면 된다.)

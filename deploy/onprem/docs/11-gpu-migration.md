...
### ★ 조달은 이미 끝났다 — 별도 델타 패키지가 있다 (2026-08-31)

**아래 §2·§3-①②의 조달 절차를 다시 밟을 필요가 없다.** 판을 실측으로 확정하고
교체 휠 22개를 모은 **GPU 전환 델타**(`klid-onprem-gpu-delta-*.tgz`)를 별도로 만들어 뒀다.
그 패키지의 `README.md` 와 `scripts/apply-gpu.sh` 가 확정된 절차다.

| 항목 | 확정값 | 근거 |
|---|---|---|
| 그래픽 드라이버 | **570.86.10** | 사업 담당 확인 (2026-08-31) |
| PyTorch 인덱스 | **`cu126`** | 실측 — 아래 ⚠ |
| `torch` / `torchvision` | `2.12.0+cu126` / `0.27.0+cu126` | 잠긴 판 그대로 |
| 추론 런타임 | **`onnxruntime-gpu 1.26.0`** | 실측 — 아래 ⚠ |
| 델타 크기 | **약 3.57 GiB** (휠 22개) | 실측 |

⚠⚠ **`cu128` 을 고르면 안 된다.** 드라이버 570 계열이 CUDA 12.8 을 지원하는 것은 맞지만,
**PyTorch 의 `cu128` 인덱스에는 우리가 잠근 `torch 2.12.0` 이 없다**(그 인덱스 최대가 2.11.0).
"드라이버가 지원하는 최고 판"과 "그 인덱스에 우리 판이 있는가"는 **다른 축**이다.

⚠⚠ **`onnxruntime-gpu` 를 CPU 판과 같은 `1.27.0` 으로 고르면 안 된다.**
1.27.0 은 `nvidia-cuda-runtime-cu13~=13.0` 을 요구한다 — **CUDA 13 이고 드라이버 580 이상**이라
이 장비에서 돌지 않는다. CPU 판 판번호를 그대로 따라가는 것이 이 함정의 입구다.

⚠ **아직 못 받은 값**: GPU 모델 · VRAM. VRAM 은 SAM2 의 입력 해상도 한계를 정하므로
운영 한도를 잡을 때 필요하다. 조달에는 영향이 없다(위 판이 이미 확정됐다).

### (참고) 판을 처음부터 다시 확정해야 한다면 — 그때의 절차
...
| 항목 | 내용 |
|---|---|
| 매체 용량 | **+3.57 GiB (실측)** — `torch+cu126` 808MB · `cudnn` 679MB · `cublas` 375MB 가 대부분이다. ⚠ **더 줄일 수 없다** — `nccl`·`nvshmem`·`cusparselt` 를 "다중 GPU 전용이니 지연 로드"로 보고 빼 봤으나 셋 다 `import torch` 가 즉시 요구해 실패했다(실측). CUDA 판 torch 는 import 시점에 CUDA 라이브러리를 **전부 preload** 한다 |

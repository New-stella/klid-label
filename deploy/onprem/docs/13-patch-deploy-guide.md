...
├── db/incremental/      ← 이번 회차 증분 + 적용 규칙 + 증분-대조-결과.txt
├── db/schema.sql        ← 통합 정의(이번 회차엔 쓰지 않는다 · 새 장비용)
├── scripts/             ← 배포·적용 수단 전체
├── docs/                ← 운영 문서 전체
└── src/                 ← ★반입 소스 {backend,frontend,ai-server} + SOURCE-INFO.txt
gpu/                     ← GPU 준비 상태 점검(읽기 전용) · 이 회차와 무관
VERSION.txt              ← 기준선·이번 판·증분 목록
SHA256SUMS.patch         ← 매체 전체 무결성
```

### 소스는 무엇이 담기나 — `onprem/src/`

**저장소가 추적하는 파일만** 담는다(`.gitignore` 가 정본). 산출물과 **같은 판**의 소스이며
`SOURCE-INFO.txt` 에 그 판의 커밋이 적혀 있다.

소스는 **선택**이다 — 조립 시 `--no-src` 를 주면 `onprem/src/` 가 없고 `VERSION.txt` 의 `source=` 가 `none` 으로 적힌다.

★ **제외 목록으로 거르지 않는 이유** — 목록은 반드시 빠뜨린다. 실제로 구 방식에는
`backend/storage/` 가 빠져 있어, 소스를 담는 순간 **운영 영상·프레임(원본 포함)이 매체로 함께
나갈** 상태였다. 추적 목록으로 담으면 그 부류가 **구조적으로** 들어오지 못한다.

조립기가 매번 **실제로 열어** 확인한다 — 런타임 산출물(`storage`·`node_modules`·`build`…)이
하나라도 있거나 시험 픽스처가 아닌 미디어가 있으면 **조립을 세운다.**

⚠ **커밋하지 않은 변경은 담기지 않는다.** 반입물은 재현 가능한 판이어야 하기 때문이다.
산출물을 미커밋 변경으로 빌드했다면 소스와 어긋나므로, 조립기가 그 경우 경고한다.

⚠ **빌드 도구는 이 매체에 없다**(JDK·Node·Gradle·의존 캐시). 오프라인 재빌드가 필요하면
전체 설치 매체의 오프라인 빌드 키트를 쓴다.

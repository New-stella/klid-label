...
폐쇄망 대상 서버에서 root 로 실행한다. **외부 네트워크 호출이 전혀 없다**(모든 의존성은 번들).
...
| 1 | `install/11-install-runtimes.sh` | JRE/Python/Caddy 설치 + `.deb` 오프라인 설치 |
...
## 비데비안(RHEL/Rocky 등)에서 시스템 패키지

`syspkgs/deb/` 에 `.deb` 가 없으면 install 은 경고만 낸다. 대상 OS 에 다음을 사전 설치해야 한다
(폐쇄망이면 사내 미러/RPM 번들 필요):

```bash
# RHEL 계열 예시(미러 가정)
sudo dnf install -y ffmpeg mesa-libGL glib2 curl
```

ffmpeg/ffprobe, libGL(opencv), glib2 가 없으면 backend ffmpeg 처리·ai-server opencv 가 실패한다.

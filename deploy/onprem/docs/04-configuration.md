...
| `WEBHOOK_VLM_ALLOWED_IP_CIDRS` | ★(prd·stg) | VLM 무서명 콜백 허용 출처 CIDR(CSV). **미적용을 의도하면 `none` 명시**. 미설정 시 부팅 차단. 형식 오류도 부팅 차단. ★ **템플릿 기본값은 전면 허용 `0.0.0.0/0`**(2026-09-14 사용자 확정 — 외부 유입은 인프라 방화벽이 막는다). 요청 제한·`request_id` 발급 게이트는 남는다 |
| `WEBHOOK_GENAI_ALLOWED_IP_CIDRS` | · | 증강 결과 무서명 콜백 허용 출처 CIDR(CSV). **비우거나 `none` 이면 전면 차단**(fail-closed) — 기동은 정상이고 콜백만 전건 403. 전면 허용은 `0.0.0.0/0` 을 **명시**한다(요청 제한·`request_id` 발급 게이트는 남는다). 형식 오류는 부팅 차단. ★ **템플릿 기본값은 전면 허용 `0.0.0.0/0`**(2026-09-14 사용자 확정 — 외부 유입은 인프라 방화벽이 막는다). ⚠ 2026-09-11 전 템플릿에는 이 줄이 없었다 — 기존 현장 파일에는 손으로 추가해야 한다 |

 ...
 * 잘못된 기대를 준다.
 *
 * <p>⚠ 이 정합이 없으면 <b>운영에서 {@code AUGMENT_EXTERNAL_MODE=http} 로 위탁을 켜는 순간 평문 벤더
 * 주소에서 기동이 막힌다</b>(지금은 운영 기본값이 {@code noop} 이라 설정 클래스 자체가 뜨지 않아
 * 드러나지 않을 뿐이다). 그 벽을 없애는 것이 이 클래스의 목적이다.
 *

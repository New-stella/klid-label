 ...
 *       이력)은 이 콜백이 쓰는 것과 겹치지 않으므로 문제가 없다. ★질문 문구는 <b>위탁 시점에 보낸 값</b>을
 *       수신부가 인자로 넘겨 주므로 협력자가 질문 목록도 마킹도 <b>읽지 않는다</b>(구 판은 둘 다 읽었다).
 *       협력자가 이 콜백이 방금 쓴 값을 읽어야 하게 되면 이 격리를 다시 검토해야 한다.</li>

 ...
 * 수천 건을 손으로 해제해야 하므로 <b>해제·재수행도 함께</b> 일괄로 둔다.
 *
...
public class BatchStageBulkController {
     ...
     *
     * <p>해제는 표식을 지우는 것이 아니라 <b>해제 표식을 덧붙이는 것</b>이라 누가 언제 해제했는지가 함께
    ...
    @Operation(
            ...
            description = "여러 영상의 시계열(VLM) 묶음 스킵을 한 번에 해제한다. 해제는 표식을 지우는 것이 아니라 "
                    + "해제 표식을 덧붙이는 것이라 누가 언제 해제했는지가 함께 남는다(단건과 같다). 해제만으로는 "
    ...
    /**
     * 건너뛰기를 해제한 작업 묶음 일괄 재수행. [@design API-214]
     *
     * <p>건별 결과는 <b>접수 여부</b>이지 파이프라인 완료가 아니다(실행은 비동기). 수락 조건은 단건과
     * 같으며 — 그 영상에서 실제로 건너뛰기를 해제한 묶음인가 하나다 — 검수 승인 이력이 있는 영상도 시계열 묶음은
     * 대상이 된다.
     */
    @Operation(
            summary = "건너뛰기를 해제한 배치 작업 묶음 일괄 재수행 (REVIEWER)",
            description = "건너뛰기를 해제한 시계열(VLM) 묶음을 여러 영상에 대해 한 번에 다시 수행한다. 수락 조건은 그 영상에서 "
                    + "실제로 건너뛰기를 해제한 묶음인가 하나이며 단건 재수행과 같은 판정을 쓴다. 검수 승인 이력이 있는 영상도 "

 ...
 *
 * <p>과거 {@code NoopExternalAugmentClient} 가 {@code @Primary} + {@code matchIfMissing=true} 로
 * 전 환경 기본이었고, 그 사실이 어디에도 드러나지 않아 "증강 요청이 HTTP 로 나간 적이 없다"는
 * 사실을 아무도 눈치채지 못했다. 오설정이 침묵하지 않도록 활성 구현체를 명시적으로 알린다.
 */
@Slf4j
@Component

...
public final class PinnedTarget {
    ...
    /**
     * <b>「호출자가 이미 판단했고, 목적지를 정하지 못했다」</b>는 표식 — 주소가 아니다.
     * [@design ADR-057] [@design AC-1100]
     *
     * <h3>★ 왜 {@code null} 로는 안 되나 (프레임마다 다시 고르게 된다)</h3>
     * <p>배치 경로는 <b>영상 단위로 한 번</b> 목적지를 정하고 그 값을 프레임마다 그대로 넘긴다.
     * 그런데 원장을 읽지 못해 정하지 못했을 때 {@code null} 을 넘기면, 클라이언트는 그것을
     * <b>「아직 안 정했으니 내가 고르라」</b>는 뜻으로 읽어 <b>호출할 때마다 다시 고른다</b>.
     * 그러면 한 영상의 프레임이 여러 장비로 흩어지고, 추적 상태는 장비마다 따로 있으므로 객체
     * 식별자가 <b>프레임 경계에서 끊긴 채 그대로 적재</b>된다 — 오류로 드러나지 않아 더 나쁘고,
     * 그 구간에는 배정 기록도 없어 사후에 어느 프레임이 어디로 갔는지 알아낼 수도 없다.
     *
     * <p>그래서 「정하지 못했다」를 <b>정해진 답</b>으로 만든다: 이 표식을 받은 호출은 다시 고르지
     * 않고 <b>배포 기본 주소</b>로 나간다. 분산만 포기하고 <b>고정은 지킨다</b>.
     *
     * <p>⚠ 이것은 <b>「쓸 수 있는 후보가 0」과 다른 축</b>이다. 후보 0 은 판정을 내린 <b>결과</b>라
     * 종전대로 <b>거부</b>하고, 이 표식은 판정 근거 자체를 얻지 못한 <b>조회 실패</b>다. 두 축을
     * 뭉치면 정상 장비가 있는데 거부하거나, 하나도 없는데 내보내게 된다.
     *
     * <p>⚠ 값 자체는 <b>절대 URI 가 될 수 없는 문자열</b>이라 {@link #canPin} 이 거짓을 돌려준다 —
     * 실수로 주소 자리에 흘러들어도 목적지가 만들어지지 않는다.
     */
    public static final String DEPLOY_DEFAULT_TARGET = "__deploy-default__";

    /** 위 {@link #DEPLOY_DEFAULT_TARGET} 인가 — 판정을 문자열 비교로 흩뿌리지 않는다. */
    public static boolean isDeployDefault(String srvrAddr) {
        return DEPLOY_DEFAULT_TARGET.equals(srvrAddr);
    }

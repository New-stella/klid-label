 ...
 *
 * <p>그래서 {@link ProfileGatedUrlPolicy} 와 <b>동일한 강도·구조</b>로
 * 기동 자체를 실패시킨다. 해제 수단은 명시적이다 — 배포 환경이면 클러스터링을 켜고, 단일 노드
 * 개발 환경(local/dev)이면 그대로 둔다.
 *
 * <p>⚠ <b>구 서술 폐기</b> — 위 문장이 {@link GenAiIntegrationWiringGuard} 도 「동일한 강도」의 사례로
 * 들고 있었으나, 그 가드는 2026-09-03([@design ADR-062])에 <b>기동 차단을 위탁 시점 거부로</b> 옮겼다.
 * <b>이 가드는 그 결정의 대상이 아니며 기동 차단을 유지한다</b> — 연동 오설정은 그 기능 하나만
 * 실패시키고 요청자에게 즉시 드러나지만, 트리거 중복 발화는 <b>조용히 데이터를 어긋나게</b> 만들고
 * 한참 뒤 결과로만 드러나 사용 시점 거부로는 되돌릴 수 없기 때문이다. 「일관성」을 이유로 함께
 * 옮기지 말 것.
 *

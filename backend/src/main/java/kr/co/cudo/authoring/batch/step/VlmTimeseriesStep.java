...
public class VlmTimeseriesStep implements BatchStep {
    ...
    private VlmTimeseriesResponse doSubmit(Long rawSn, LsMarking marking) {
        ...
        //
        //  ★★ 쓸 수 있는 후보가 0이면 선택기가 <사유를 가리지 않고 거부>한다(이 호출이 던진다).
        //    형식 위반이든, 보낼 수 없는 주소든, 주소 정책 탈락이든, 상태점검 실패로 그 유형이 전부
        //    이용불가든, <그 유형의 행이 0건>이든 같다. [design: AC-1093]
        //
        //  ⚠구 주석 폐기(2026-09-07) — 여기 두 문단이 이렇게 적혀 있었고 <둘 다 사실이 아니다>:
        //    ①"고르지 못하면(가용 시계열 노드 0건) 예외가 아니라 <배포 기본 주소로 그대로> 나간다"
        //    ②"단 예외가 하나 있다 — <식별자 형식을 어겨> 전부 후보에서 빠진 경우만 거부한다"
        //    ②의 <사유 한정 게이트>가 바로 이번 변경이 걷어낸 것이다. 그 게이트가 있으면 두 장비가
        //    함께 죽어 전부 이용불가가 된 실제 장애에서 형식 위반이 0건이라 위탁이 조용히 배포 기본
        //    주소로 나가 장애를 감춘다(2026-09-07 현장). 배포 설정값은 그 유형의 첫 행을 심는 <씨앗>
        //    일 뿐이며, 원장에 행이 없을 때 그 값으로 대신 부르면 진실원이 둘이 된다. [design: AC-1092]
        //    <이 두 문단을 되살리지 말 것.>
        //
        //  ★ 그래서 아래에서 node 가 null 이 되는 경우는 <둘뿐>이다 — (1)원장·부하 <조회가 실패>해
        //    선택기가 「분산만 포기」로 낮춘 경우 (2)바로 아래 PinnedTarget 재검사에서 걸린 경우.
        //    「그 유형에 쓸 장비가 없다」는 여기 도달하지 못한다(위에서 예외로 끝난다).
        //    (1)의 폴백은 <의도적으로 유지>한다 — DB 순단이 정상 위탁을 전량 죽이지 않게 하는 축이다.
        //

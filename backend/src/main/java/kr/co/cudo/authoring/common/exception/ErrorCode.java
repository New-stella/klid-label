...
public enum ErrorCode {
    ...
    /**
     * <b>검수에 들어간 배정을 해제하려 한 경우</b> — 배정 해제 창구 전용 (@design API-259 · AC-1123).
     *
     * <p>왜 {@link #ASSIGNMENT_ALREADY_COMPLETED} 를 재사용하지 않는가 — 그 값의 문구가
     * 「완료된 작업은 재배정할 수 없습니다」라 <b>두 겹으로 어긋난다</b>. ①거부되는 상태는 검수 대기·
     * 검수 중·승인이라 「완료」가 아니고 ②이 창구는 재배정이 아니라 <b>해제</b>다. 그 값은 다른 창구가
     * 계속 쓰므로 지우지 않고, 이 조건이 쓰는 값만 전용으로 둔다.
     *
     * <p>이름이 「제출됨」인 이유는 거부되는 세 상태가 <b>전부 검수 제출 이후</b>라 이 한 낱말로 정확히
     * 덮이기 때문이다. ★<b>반려는 이 거부에 들지 않는다</b> — 워크플로가 작업자에게 되돌아온 상태라
     * 그 작업 자체를 접을 수 있어야 한다(「반려도 검수 축」이라는 이유로 함께 막지 말 것).
     */
    ASSIGNMENT_SUBMITTED(HttpStatus.CONFLICT, "검수에 들어간 배정은 해제할 수 없습니다."),
    /**

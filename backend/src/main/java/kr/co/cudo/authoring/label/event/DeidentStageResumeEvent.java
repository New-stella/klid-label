 ...
 * (V171 — 단계 구분 + 재개 지점 분기).
 *
 * <h3>단계와 무관한 2종과 무엇이 다른가 — 셋 다 필요하다</h3>
 * <ul>
 ...
 *       <b>검수 승인(APPROVED) 노드에만</b> 발행. 소비자는 <b>재검토 표시</b>({@code REVLT_YN='Y'})와
 *       변경분 축적이며, export 재산출·관제 재통지는 <b>재승인 시점</b>에 나간다. 역시 <b>무변경</b>이다.
 *       <p>⚠ {@link DeidentReportResolvedEvent} 가 그 자리를 맡던 구 배선은 폐기됐고, 그 이벤트는
 *       <b>발행처가 없는 휴면 확장점</b>으로만 존치한다.</li>
 ...
 * 발행하지 않으면 단계와 무관한 위 2종만 도는 현행 동작이 그대로 유지된다.
 *

 ...
 * async 커밋에서만 관측된다.
 *
 * <h3>Phase 8-B — PII 게이트는 "보류" 이지 "성공 종결" 이 아니다 (E-ISSUE-11)</h3>
 * <p>부모 안전 판정을 <b>상태 전이 이전</b>으로 당겼다. 구 순서(전이 먼저 → 영상 생성 단계에서 게이트)
 * 는 게이트에 걸려도 증강 행이 이미 {@code ACCEPTED} 로 종결돼 ①화면·집계상 정상 완료로 보이고
 * ②신규 영상은 0건이며 ③non-PENDING 멱등 앵커 때문에 재콜백도 스킵돼 그 증강이 <b>영구 유실</b>됐다.
 * 지금은 부모가 비식별 완료가 아니면 상태를 그대로 두고
 * {@link AugmentApplyResult#WITHHELD_PARENT_NOT_DEIDENTIFIED} 로 회신한다 — 신고 해소
 * ({@code DeidentReportResolvedEvent}) 시점에 {@code AugmentRequestBridge} 가 다시 인계한다.
 *
...
public class AugmentResultService {
    ...
    public AugmentApplyResult handle(AugmentOutcome outcome) {
        ...
        //      멱등 스킵돼 그 증강이 영구 유실된다. 판정을 앞으로 당겨 보류는 PENDING 을 유지시킨다.
        ParentGate gate = outcome.success() ? evaluateParentGate(aug) : ParentGate.notApplicable();
        if (gate.decision() == ParentGate.Decision.WITHHOLD) {
            log.warn("[Webhook][Augment] result withheld — parent not deidentified (PENDING 유지, 재개 대기) "
                            + "dataAugSn={} parentRawSn={}",
                    outcome.dataAugSn(), gate.parentRawSn());
            return AugmentApplyResult.WITHHELD_PARENT_NOT_DEIDENTIFIED;
        }
        if (gate.decision() == ParentGate.Decision.FAIL) {
            // 재개 트리거가 존재하지 않는 데이터 이상(부모/프레임 부재)이다. 보류로 두면 아무도 깨우지
            // 못하는 PENDING 고착이 되므로, 실패로 확정해 집계(FAILED)에 드러낸다.
    ...
    /**
     * 비식별 누락 신고 해소 후 <b>보류분 재개 시도</b>를 재시도 횟수에 누적한다 (E-ISSUE-06).
     *
     * <p>{@code AugmentRequestBridge} 의 재개 경로가 시도 직전에 호출한다. 재개는 실패가 아니므로
     * dead-letter 를 찍지 않는다 — 시도 횟수만 쌓아 운영이 "몇 번이나 다시 시도했는지" 를 본다.
     *
     * <p><b>{@code REQUIRES_NEW} 인 이유 (Phase 8 DEV_FIX HIGH-1)</b>: 호출자가 커밋 후 콜백
     * (AFTER_COMMIT) 문맥일 수 있는데, 그때 {@code REQUIRED} 로 두면 <b>이미 커밋된</b> 트랜잭션에
     * 참여해 ①뒤따르는 커밋이 없어 증가분이 사라지고 ②아래 {@code FOR UPDATE} 잠금 조회가
     * {@code TransactionRequiredException} 으로 튄다. 이 메서드는 카운터 1건만 올리는 독립 단위라
     * 호출자 트랜잭션과 운명을 같이할 이유가 없다 — {@code AugmentJobRecorder}(전 메서드
     * {@code REQUIRES_NEW})와 동일 규약이다. 결과 인계({@link #handle})는 웹훅 트랜잭션 조인 의미론이
     * 계약이므로 <b>바꾸지 않는다</b>(클래스 javadoc "E-ISSUE-05" 참조).
     */
    @Transactional(value = "controlTransactionManager", propagation = Propagation.REQUIRES_NEW)
    public void recordResumeAttempt(Long dataAugSn) {
        augRepository.findByDataAugSnForUpdate(dataAugSn).ifPresent(LsDataAug::incrementRetryCount);
    }
     ...
     *
     * @param decision   판정
     * @param parentRaw  PASS/WITHHOLD 시 잠금 조회된 부모 RAW (그 외 null)
    ...
    private record ParentGate(Decision decision, LsDataRaw parentRaw, String reason) {
        ...
        static ParentGate withhold(LsDataRaw parentRaw) {
            return new ParentGate(Decision.WITHHOLD, parentRaw, null);
        }
    ...
    /**
     * 부모 안전 판정 — <b>동기 트랜잭션에서만</b> 수행한다(절대 async 로 이동 금지).
     *

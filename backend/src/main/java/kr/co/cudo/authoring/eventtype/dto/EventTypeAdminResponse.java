 ...
 */
public record EventTypeAdminResponse(String evntTypeCd, String dsplNm, String optrIndctNm,
                                     String evntNm, String evntCtgryNm, String evntClsfCd,
                                     String evntCtgryCd, String clctYn, LocalDateTime regDt) {

    /**
     * 엔티티(+카테고리명) → 응답. 표시명은 {@link EventTypeDisplayNamePolicy} 로만 해석한다
     * (판정 복제 금지). 여부 컬럼은 CHAR(1) 패딩 방어를 위해 trim 한다.
     */
    public static EventTypeAdminResponse from(LsEvntType type, String evntCtgryNm) {
        return new EventTypeAdminResponse(
                type.getEvntTypeCd(),
                EventTypeDisplayNamePolicy.resolve(type.getOptrIndctNm(), type.getEvntNm(),
                        evntCtgryNm, type.getEvntTypeCd()),
                type.getOptrIndctNm(),
                type.getEvntNm(),
                evntCtgryNm,
                type.getEvntClsfCd(),

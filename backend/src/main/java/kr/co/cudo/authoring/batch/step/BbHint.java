 ...
 *
 * <p>Phase 3: trackId 필드 추가 — ultralytics 트래커가 부여한 객체 ID. {@code null} 허용
 * (트래커 미사용 경로 / 저신뢰 detection fallback). Java record 는 하나의 canonical
 * constructor 만 가질 수 있으므로 본 5-arg 가 정식 시그니처이고, 기존 4-arg 호출처는
 * 모두 {@code trackId=null} 명시로 전환되었다.
 *
 * <p>스레드 안전:
 *  - 본 record 는 불변. 정적 필드/싱글톤에 누적 금지 — 메서드 파라미터/지역 변수로만 전달.
 *  - srcSn, label, points, score, trackId 는 입력 검증 후 호출자가 채움.
 *
 ...
 * @param trackId 트래커 부여 객체 ID (null 허용 — Phase 3)
 */
public record BbHint(long srcSn, String label, List<Double> points, double score, Integer trackId) {
}

 ...
 *
 * <p>검수 완료(APPROVED) 원본 영상의 프레임 이미지셋을 표준 하위 해상도로 다운스케일한 결과
 * 1건당 1행을 기록한다. 영상(비디오) 파일·라벨/메타/프레임 행 복사는 하지 않으며, 신규
 * LS_DATA_RAW 행도 만들지 않는다. 동일 영상+해상도 중복 산출은 UNIQUE(DATA_RAW_SN, TARGET_RES_CD)
 * 로 방어한다.
 */
@Entity
@Table(name = "LS_RESOLUTION_EXPORT",
        uniqueConstraints = @UniqueConstraint(
                ...
                columnNames = {"DATA_RAW_SN", "TARGET_RES_CD"}))
@Getter
...
public class LsResolutionExport {
    ...
    @Column(name = "TARGET_RES_CD", nullable = false, length = 16)

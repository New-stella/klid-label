 ...
 * JSON 의 <b>video 블록</b> 개인정보 3필드로 나간다({@link ExportPrivacyPolicy} — 비식별 산출물만 판정,
 * 원천 산출물은 판정하지 않고 null).
 *
 * <p>조회는 <b>프리필</b>이다 — 수동 저장값이 있으면 그 값({@code MANUAL}), 없으면 비식별 기본상수
 * ({@link ExportPrivacyPolicy#DEID_DEFAULT_ANONYMITY} 등)를 {@code DERIVED} 출처로 채워 반환한다.
 * 상수를 화면이 하드코딩하지 않게 하려는 것이며, 상수의 단일 원천은 {@code ExportPrivacyPolicy} 다.
 * 저장은 <b>PUT 전체 교체</b>이며 3필드를 함께 덮어쓴다.
 *
 * <p><b>수동값 승격 주의(FE 계약)</b>: 본 API 는 전송된 값을 그대로 수동값(MANUAL)으로 저장한다.
 * 조회 응답의 프리필({@code DERIVED})을 화면이 그대로 되돌려 보내면 <b>기본상수가 사람의 판정으로
 * 승격</b>되어 export JSON 에 사실처럼 실린다. 따라서 <b>FE 는 사용자가 직접 고르지 않은 필드를 null 로
 * 전송</b>해야 한다. <b>한계(R5 self-fill 금지가 클라이언트 규율에 의존)</b>: BE 는 전송값의 출처를 알 수
 * 없어 승격을 막지 못한다 — {@code EnvironmentMetaService} 와 동일한 알려진 한계이며, BE 강제(요청에
 * source 축 추가 또는 프리필과 동일값 거부)는 요청 계약 변경이라 범위 밖으로 둔다.
 *
 ...
 *
 * <p><b>비식별 신고 게이트(412)</b>: 저장(PUT)은 신고 구간({@code DE_IDNTF_YN='F'})에서 차단한다 —
 * 신고 접수가 이 3필드를 "재판정 대상"으로 리셋하는데({@code DeidentReportService}), 같은 구간에
 * 배정자가 PUT 으로 옛 판정을 즉시 되돌리면 resolve 후 재산출 때 그 값이 그대로 관제로 나간다.
 * 라벨은 작업락으로 409 차단되는데 개인정보 선언만 열려 있는 <b>비대칭</b>을 없앤다.

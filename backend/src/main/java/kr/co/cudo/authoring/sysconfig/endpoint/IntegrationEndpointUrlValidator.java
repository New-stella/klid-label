 ...
 * 즉 자격증명이 평문으로 남는다(CWE-532). 게다가 이 4개 연동은 어느 것도 URL 삽입 자격증명을 쓰지
 * 않으므로 정당한 사용처가 없다. <b>이것은 주소 대역 판정이 아니라 자격증명 삽입 차단이다</b> —
 * 폐지된 대역 차단 정책과 무관하며, 되돌리지 말 것.
 *
 * <h3>★ IP 대역으로는 막지 않는다 (2026-08-10 사용자 확정, 구속)</h3>
 * <p>사설·링크로컬·루프백 대역 차단은 <b>폐지됐다</b>. 근거: 연동 4종(비식별·AI 추론·시계열 분석·관제
 * 통지)은 <b>실제로 내부망의 별도 서버에 있을 가능성이 높아</b>, 대역으로 막으면 정당한 대상을 막는다.

 ...
 *   <li><b>기동 assert</b>({@link #verifyRelaxationScope()}) — allowlist 밖에서 플래그가 켜져 있으면
 *       <b>기동 자체를 실패</b>시킨다. 운영에 완화 설정이 들어오는 순간 배포가 죽으므로 조용히 새지 않는다.</li>
 * </ol>
...
public abstract class ProfileGatedUrlPolicy {
    ...
    public void verifyRelaxationScope() {
        ...
        throw new IllegalStateException(
                flagProperty + "=true 는 " + INSECURE_ALLOWED_PROFILES + " 프로파일에서만 허용됩니다"
                        ...
                        + "). 운영 환경의 평문/사설 외부 URL 을 원천 차단합니다.");
    }
    ...
    /**
     * 활성 프로파일이 <b>전부</b> allowlist 안일 때만 완화를 인정한다.
     *

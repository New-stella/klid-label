 ...
 *
 * <p>⚠ <b>추론 축의 도달성은 지금과 앞으로가 다르다 — 지우지 말 것.</b> 현재 추론 요청은 원장에서
 * 장비를 고르지 않고 <b>배포 기본 주소</b>로 나가므로, 형식을 어긴 추론 노드가 있어도 「관측 가능한
 * 거짓」이 즉시 성립하지는 않는다. 그러나 이 인디케이터가 말하는 {@code usable} 의 <b>선언된 의미</b>
 * (「배정을 받을 수 있는 노드」)는 <b>이미</b> 틀린 상태이고, 추론 축이 선택기를 쓰기 시작하는 순간
 * 그대로 거짓이 된다. 「아직 안 쓰는데 왜 여기까지 봤나」로 되돌리지 말 것 — 그때 고치면 늦다.
 *
 * <p>★★ <b>운영자 오판 방지 — {@code no-selectable-node-id-format} 은 「추론이 멈췄다」는 뜻이 아니다.</b>
 * 그 사유가 뜬 상태에서도 <b>추론은 실제로 동작한다</b>(요청이 배포 기본 주소로 나가기 때문이다).
 * 이 사유가 말하는 것은 <b>「원장으로 장비를 고를 수 없다」</b> 하나뿐이다 — 즉 <b>이중화·분산이
 * 성립하지 않는 상태</b>이지 위탁 전건 거부가 아니다. ⚠ 같은 형상이라도 <b>시계열 축은 다르다</b>
 * (그 축은 실제로 원장에서 고르므로 위탁이 전건 거부된다) — 두 축을 같은 뜻으로 읽지 말 것.
 *
 * <p>그래서 이 사유는 {@code UNKNOWN} 이고 {@code DOWN} 이 아니다 — 인스턴스를 회전에서 빼도
 * 원장은 고쳐지지 않고, 실제로 되던 추론까지 함께 멈춘다.
 *

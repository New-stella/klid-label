 ...
 *
 * <p>대상은 비식별 서버 · AI 추론 서버 · 외부 시계열 분석 벤더 · 관제 통지 수신처 · 관제 계정 창구 · 외부 증강 벤더다.
 *
 ...
 * 사람이 화면에서 고치는 칸은 그보다 <b>좁다</b>(비식별 · 외부 증강 벤더 · 관제 통지 · 관제 계정 창구).
 *
...
public enum IntegrationEndpoint {
     ...
     * [@design ADR-046] [@design API-069] [@design SCREEN-042]
     *
     * <p>★ {@link #CONTROL_NOTIFY} 와 <b>별개 축</b>이다 — 관제가 계정 창구와 데이터셋 창구를 서로 다른
     * 서버에 둘 수 있어 주소를 나눠 쓰면 한쪽이 404 가 된다. 한쪽을 저장해도 다른 쪽 값은 바뀌지 않고,
     * 이 값이 비어 있어도 통지 수신처 주소로 대신하지 않는다(폴백 없음 · 배포 기본값 비어 있음).
     *
     * <p>이 열거에 두는 이유는 다른 연동 주소와 같다 — 빠지면 <b>주소 형식 검증도 관리자 유효창도</b>
     * 걸리지 않은 채 저장된다.
     */
    CONTROL_ACCOUNT(ConfigKeys.CONTROL_ACCOUNT_URL, "관제 계정 창구"),

 ...
 *       {@code READY}(등록일·라벨 최종 저장일 중 늦은 쪽 기준) · {@code FAILED}({@code MDFCN_DT}
 *       기준) 셋을 각자의 기산점으로 판정해 <b>파일 → DB</b> 순으로 삭제한다. 앞의 둘은 보존일수
 *       설정을 공유하고 기산점만 다르다(AC-1070).</li>
 * </ul>
 *
 ...
 *   <li><b>{@code PROCESSING} 은 절대 지우지 않는다</b>(AC-1070). 후보 쿼리에 상태 리터럴이 박혀
 *       있어 구조적으로 후보가 되지 못한다 — 프레임 추출 러너와 경쟁하면 파일·DB 불일치가 난다.
 *       그 상태는 이 잡이 아니라 <b>방치 판정</b>({@link PortalUploadSweepJob})이 회수한다.
 *       ⚠ 반대로 <b>마킹 대기는 2026-09-05 확정으로 이 잡의 대상이 됐다</b> — 방치 판정으로 실패
 *       마감하던 구 동작을 되살린 것이 <b>아니라</b> 일 단위 정상 만료다(타이머 길이도 종착점도
 *       다르다). 그 축이 없으면 마킹하지 않은 자산이 파일째 영구히 남는다.</li>
...
public class PortalRetentionSweepJob {
     ...
     * @design DFEAT-055, AC-1070, AC-036, AC-037
     *
     * <p>★ 마킹 대기 자산에는 <b>프레임이 없어 원본 파일 1건만</b> 후보 경로에 담긴다 — 프레임 목록이
     * 비어도 「파일 먼저, DB 나중」 순서는 그대로 성립한다(지울 파일이 하나뿐일 뿐이다).
     *

 ...
 *   <li>대안 ① 호출부가 절대 URI 를 넘긴다 — 클라이언트를 모두 고쳐야 하고, 새 호출부가 생길 때마다
 ...
 *       대개 메모리 조회지만 <b>미스 시 블로킹</b>이다. 이 연동들은 배치·통지 계열이라 호출 빈도가
...
public final class IntegrationEndpointExchangeFilter {
     ...
     * 지금 배포된 기본값이 모두 경로 없는 형태라 도달하지 않을 뿐, 기본값에 경로가 붙는 순간
     * 활성화되는 결함이다.
     */

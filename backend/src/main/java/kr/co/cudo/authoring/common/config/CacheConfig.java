 ...
 *       <b>정합은 "무효화 전파"가 아니라 "파일 유예 삭제"로 확보</b>한다 —
 *       {@code ResolutionBackfillService} 는 이관 후 구 파일을 즉시 지우지 않고 TTL 보다 긴 유예
 *       (기본 15분) 후에 정리한다. 유예 동안 옛 경로도 <b>바이트 동일 사본</b>이 살아 있어 stale 을 보는
 *       노드도 정상 재생되며(500 없음), TTL 경과 후 자연히 새 경로로 수렴한다. 따라서 이 TTL 을 늘리려면
 *       {@code authoring.resolution-backfill.stale-grace-minutes} 도 함께 늘려야 한다(기동 시 검증).</li>
...
public class CacheConfig {
     ...
     * ({@code ResolutionBackfillService} 가 기동 시 검증).
     */

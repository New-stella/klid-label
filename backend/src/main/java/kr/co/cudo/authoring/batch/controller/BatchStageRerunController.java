...
/**
 * 건너뛰기를 해제한 <b>작업 묶음</b>의 지목 재수행 API — REVIEWER 전용. [@design API-201]
 *
 * <p>「문제가 생긴 곳부터 재시도한다」는 원칙에 따라 전체 재기동({@code /batch/retry})과 분리된
 * 요청이다. 해제한 그 묶음을 지목해 <b>그 묶음만</b> 실행한다 — 묶음이 곧 범위라 요청이 범위를 고르지
 * 않는다(구 {@code scope} 요청 본문 폐기).
 *
 * <p>별도 sub-resource 로 둔 이유는 이 저장소의 「같은 URL 에 쿼리 파라미터로 행위 분기 금지」 규약이다
 * (전례: 프레임 비식별 이미지 전용 엔드포인트).
 */
@Tag(name = "BatchStageRerun", description = "건너뛰기를 해제한 작업 묶음 재수행 — REVIEWER 전용 (VLM/AUTOLABEL)")
@RestController
...
public class BatchStageRerunController {
    ...
    @Operation(
            summary = "건너뛰기를 해제한 작업 묶음 재수행 (REVIEWER)",
            description = "건너뛰기를 해제한 작업 묶음(VLM / AUTOLABEL)을 다시 수행한다. 묶음이 곧 범위라 "
                    + "요청은 범위를 고르지 않으며, 해제한 그 묶음만 수행하고 다른 묶음은 건드리지 않는다. "
                    + "오토라벨을 재수행하면 트랙 보간도 함께 다시 만들어져 사람이 손댄 보간 라벨이 새로 계산된 "
                    + "것으로 바뀐다 — 오토라벨을 통째로 다시 만드는 선택이므로 예상되는 결과이며 화면이 고르는 "
                    + "시점에 알린다. 대상 묶음은 그 영상에서 실제로 건너뛰기를 해제한 묶음만 수락하며 그 외에는 400 이다. "
    ...
    @ApiResponses({
            ...
            @io.swagger.v3.oas.annotations.responses.ApiResponse(responseCode = "400", description = "건너뛰기를 해제한 묶음이 아님 / 지원하지 않는 묶음 / 한번이라도 검수가 완료된 영상"),

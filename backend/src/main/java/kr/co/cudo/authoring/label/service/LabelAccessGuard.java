...
public class LabelAccessGuard {
     ...
     *
     * <p><b>판정 범위 = 해당 영상 + 조상(ORGNL_RAW_SN) 체인</b>: 파생영상(해상도·증강) 프레임은 부모의
     * 비식별 프레임 사본이라 부모가 신고되면 파생본도 같은 PII 를 노출한다. 신고는 부모 행만 {@code 'F'}
     * 로 바꾸므로 자기 행만 보면 파생본이 게이트를 통과한다(적대검증 반증 — 실제 PII 이미지 200 응답).
     * 체인 판정은 {@link DeidentReportGate} 가 수행하며, 이 게이트를 공유하는 모든 호출부
     * (라벨 조회·이력·버전 diff/rollback·프레임 이미지·포털·관제 조회)가 함께 강화된다.
     *

-- =============================================================================
-- V151: LS_DATA_AUG_DSCD 보강 — ①폐기 감사 스냅샷(증강 종류·프롬프트) ②파일 정리 수렴(재시도 상한·포기)
--
-- ★ 스키마 변경은 V150 을 고치지 않고 신규 버전으로 올린다 — 적용된 마이그레이션의 <본문(DDL)>을
--   바꾸면 checksum 불일치로 기동이 깨지기 때문이다.
--   (사실 정정: V150 의 <주석>은 이후 DEV_FIX 에서 실제로 손봤다. V150·V151 이 아직 어떤 배포
--    환경에도 적용되지 않은 상태여서 가능했던 것이고, 한 번이라도 적용된 뒤라면 주석 수정도
--    checksum 을 깨므로 금지다. "V150 을 고치지 않았다" 는 구 서술은 사실과 달라 바로잡는다.)
--
-- ─────────────────────────────────────────────────────────────────────────────
...
--   특히 중복 증강 요청이 허용된 뒤(V147)로는 <b>같은 (영상 × 종류) 파생이 여러 건 공존</b>하고
...
-- 술어가 바뀌므로 기존 partial index 를 버리고 다시 만든다(V150 의 IX_..._FILE_RTY 대체).

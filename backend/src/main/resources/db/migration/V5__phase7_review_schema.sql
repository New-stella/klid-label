...
--   적용 전 관제서버팀 / DBA 와 컬럼 정의 (특히 ISSUE_RSN 길이 1000) · INDEX 범위 합의 필요.
--
...
--   - 좌표 컬럼 없음 (V1 §5A.6 명시)
--   - DATA_RAW_SN 은 LS_DATA_RAW.RAW_SN 을 참조 (영상 단위 반려)
-- ============================================================

CREATE TABLE IF NOT EXISTS LS_DATA_ISSUE (
    DATA_ISSUE_SN       BIGINT          NOT NULL GENERATED BY DEFAULT AS IDENTITY,
    UP_DATA_ISSUE_SN    BIGINT,
    DATA_RAW_SN         BIGINT          NOT NULL,
    ISSUE_RSN           VARCHAR(1000),
    REPORTED_USER_NO    VARCHAR(50),
    REG_DT              TIMESTAMP       NOT NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY (DATA_ISSUE_SN)
);

CREATE INDEX IF NOT EXISTS IX_LS_DATA_ISSUE_VIDEO ON LS_DATA_ISSUE (DATA_RAW_SN);

-- ============================================================================
-- V2: CM_CODE → LS_CM_CODE 개명 (테이블명 소유 접두 LS_ 통일).
--
-- 배경: 이 테이블은 구 V3__cm_code_seed_data_stts.sql 이 만든 <저작도구 자체 소유> 코드 테이블인데
--   혼자만 LS_ 접두가 없어 소유 구분이 흐렸다. 저작도구 전용 테이블은 LS_* 라는 규칙(CLAUDE.md
--   「DB 정책」)에 맞춘다. 관제 공유(MNG_*) 와의 소유 경계를 이름만으로 읽을 수 있게 하는 것이 목적이다.
--
-- ★ 이 파일이 <조건부>인 이유 — 두 경로가 수동 개입 없이 같은 결과로 수렴해야 한다
--   · 신규 설치(빈 DB): V1__baseline.sql 이 이미 LS_CM_CODE 로 만든다 → 여기서는 아무것도 하지 않는다.
...
--     ALTER TABLE LS_CM_CODE RENAME TO CM_CODE;
--     ALTER TABLE CM_CODE RENAME CONSTRAINT ls_cm_code_pkey TO cm_code_pkey;
...
BEGIN
    ...
    IF to_regclass('cm_code') IS NOT NULL AND to_regclass('ls_cm_code') IS NULL THEN
        ALTER TABLE cm_code RENAME TO ls_cm_code;
    END IF;

    -- PK 제약명도 함께 옮긴다. 테이블만 바꾸면 제약·인덱스 이름이 cm_code_pkey 로 남아
    -- 신규 설치본(ls_cm_code_pkey)과 <같은 스키마인데 이름만 다른> 상태가 되어,
    ...
    IF EXISTS (SELECT 1 FROM pg_constraint
                WHERE conname = 'cm_code_pkey'
                  AND conrelid = to_regclass('ls_cm_code')) THEN
        ALTER TABLE ls_cm_code RENAME CONSTRAINT cm_code_pkey TO ls_cm_code_pkey;
    END IF;
END $$;

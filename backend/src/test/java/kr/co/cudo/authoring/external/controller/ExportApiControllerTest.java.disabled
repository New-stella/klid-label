...
/**
 * Phase 4 — 외부 학습데이터 API Controller 통합 테스트.
...
class ExportApiControllerTest {
    ...
    void validTokenMergeFalse() throws Exception {
        ...
        metaRepository.save(LsDataMeta.createWithType(raw.getRawSn(), "weather", "rain", LsDataMeta.META_TYPE_DEID));
    ...
    void validTokenMergeTrue() throws Exception {
        ...
        metaRepository.save(LsDataMeta.createWithType(raw.getRawSn(), "scene", "RAW-V", LsDataMeta.META_TYPE_RAW));
    ...
    @Test
    @DisplayName("좌표_라벨_RAW_프레임_기준_1벌만_응답")
    void labelsBuiltFromRawFramesOnly() throws Exception {
        ...
        // DEID 프레임 row 가 별도로 있어도 응답에 들어가면 안 됨

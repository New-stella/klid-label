 ...
 * 반환(예외 미전파)하는지 검증한다. VideoProbe 는 stub 으로 주입한다.
 */
...
class VideoDurationResolverTest {
    ...
    private LsDataRaw rawWithDuration(Integer durationSec, String path) {
        return LsDataRaw.createFromIngest(
                "CLIP", "CCTV-001", "EVT-A", "11680",
                LsDataRaw.PRVC_TYPE_ANONY, path, LocalDateTime.now(), durationSec);
    }

    @Test
    @DisplayName("durationSec_존재하면_그값을_그대로_사용하고_메타조회_프로브_안함")
    void durationSecPresent_returnsIt_noMetaNoProbe() {
        // given — VDO_LEN_SEC=42 인 영상
        LsDataRaw raw = rawWithDuration(42, "/var/raw/clip.mp4");

        // when
        Integer sec = resolver.resolveDurationSec(1L, raw);

        // then — 그 값 사용 + 폴백 경로(메타/프로브) 미호출
        assertThat(sec).isEqualTo(42);
        verify(metaRepository, never()).findByRawSnAndMetaKey(any(), any());
        verify(videoProbe, never()).probe(any());
    }

    @Test
    @DisplayName("durationSec_null이면_video_duration_ms_메타를_초로_환산해_반환")
    void durationSecNull_resolvesFromDurationMsMeta() {
        // given — durationSec null, 메타 video.duration_ms=30500(ms) 적재됨
        LsDataRaw raw = rawWithDuration(null, "/var/raw/clip.mp4");
        LsDataMeta meta = LsDataMeta.create(2L, "video.duration_ms", "30500");
        when(metaRepository.findByRawSnAndMetaKey(2L, "video.duration_ms"))
                .thenReturn(Optional.of(meta));

        // when
        Integer sec = resolver.resolveDurationSec(2L, raw);
    ...
    void durationSecNull_metaAbsent_resolvesFromProbe() {
        ...
        LsDataRaw raw = rawWithDuration(null, "/var/raw/clip.mp4");
        when(metaRepository.findByRawSnAndMetaKey(3L, "video.duration_ms"))
        ...
        // when
        Integer sec = resolver.resolveDurationSec(3L, raw);
    ...
    void allFail_returnsNull_swallowsProbeException() {
        ...
        LsDataRaw raw = rawWithDuration(null, "/var/raw/clip.mp4");
        when(metaRepository.findByRawSnAndMetaKey(4L, "video.duration_ms"))
                .thenReturn(Optional.empty());
        ...
        assertThat(resolver.resolveDurationSec(4L, raw)).isNull();
    }
    ...
    void subSecondMeta_fallsThroughToProbe() {
        ...
        LsDataRaw raw = rawWithDuration(null, "/var/raw/clip.mp4");
        LsDataMeta meta = LsDataMeta.create(5L, "video.duration_ms", "400");
        when(metaRepository.findByRawSnAndMetaKey(5L, "video.duration_ms"))
                .thenReturn(Optional.of(meta));
        VideoMeta probed = new VideoMeta(1280, 720, "h264", 25.0, null, 45_000L, null);
        when(videoProbe.probe(Path.of("/var/raw/clip.mp4"))).thenReturn(probed);

        // when
        Integer sec = resolver.resolveDurationSec(5L, raw);
    ...
    void unparsableMeta_and_probeNullDuration_returnsNull() {
        ...
        LsDataRaw raw = rawWithDuration(null, "/var/raw/clip.mp4");
        LsDataMeta meta = LsDataMeta.create(6L, "video.duration_ms", "N/A");
        lenient().when(metaRepository.findByRawSnAndMetaKey(6L, "video.duration_ms"))
                .thenReturn(Optional.of(meta));
        VideoMeta probed = new VideoMeta(0, 0, null, null, null, null, null);
        when(videoProbe.probe(Path.of("/var/raw/clip.mp4"))).thenReturn(probed);

        // when / then
        assertThat(resolver.resolveDurationSec(6L, raw)).isNull();
    }
    ...
    void blankPath_noProbe_returnsNull() {
        ...
        LsDataRaw raw = rawWithDuration(null, "   ");
        when(metaRepository.findByRawSnAndMetaKey(7L, "video.duration_ms"))
                .thenReturn(Optional.empty());

        // when / then — 프로브 미호출 + null
        assertThat(resolver.resolveDurationSec(7L, raw)).isNull();
        verify(videoProbe, never()).probe(any());
    }
}

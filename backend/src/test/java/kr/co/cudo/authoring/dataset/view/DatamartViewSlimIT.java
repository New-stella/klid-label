 ...
 *   <li>신설 {@code V_COMPLETED_LABEL_CHANGE} 가 APPROVED 영상의 변경점(ADDED/UPDATED/DELETED)만 반환.</li>
...
class DatamartViewSlimIT {
    ...
    private void seedChangeHistory(long srcSn, Long lblSn, String chgKindCd, String regId) {
        jdbc.update(
                "INSERT INTO LS_DATA_LBL_HSTRY (LBL_SN, SRC_SN, REG_DT, CHG_KIND_CD, REG_ID) "
                        + "VALUES (?, ?, ?, ?, ?)",
                lblSn, srcSn, LocalDateTime.now(), chgKindCd, regId);
    }
    ...
    @Test
    @DisplayName("V_COMPLETED_LABEL_CHANGE가_APPROVED영상의_변경점을_반환한다")
    void labelChange_returnsChangesForApprovedVideo() {
        // given — APPROVED 영상 + 프레임 + 변경 이력 3종(ADDED/UPDATED/DELETED)
        long rawSn = seedRawAndStatus("APPROVED");
        long srcSn = seedFrame(rawSn, 10);
        seedChangeHistory(srcSn, 1001L, "ADDED", "worker1");
        seedChangeHistory(srcSn, 1001L, "UPDATED", "worker1");
        seedChangeHistory(srcSn, 1001L, "DELETED", "worker2");

        // when
        List<Map<String, Object>> rows = jdbc.queryForList(
                "SELECT RAW_SN, SRC_SN, CHG_KIND_CD, REG_ID, REG_DT "
                        + "FROM V_COMPLETED_LABEL_CHANGE WHERE SRC_SN = ? ORDER BY CHG_KIND_CD", srcSn);

        // then — 3건 반환 + RAW_SN 조인 정확 + CHG_KIND_CD/REG_ID/시각 노출
        ...
        assertThat(rows).extracting(r -> r.get("chg_kind_cd"))
                .containsExactly("ADDED", "DELETED", "UPDATED");
        assertThat(rows).extracting(r -> r.get("reg_id"))
                .containsExactlyInAnyOrder("worker1", "worker1", "worker2");
    }

    @Test
    @DisplayName("비APPROVED영상의_변경점은_LABEL_CHANGE에_안나온다")
    void labelChange_hiddenWhenNotApproved() {
        // given — PENDING(미승인) 영상의 변경 이력
        long rawSn = seedRawAndStatus("PENDING");
        long srcSn = seedFrame(rawSn, 1);
        seedChangeHistory(srcSn, 2001L, "ADDED", "worker1");
